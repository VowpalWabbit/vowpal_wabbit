...
JNIEXPORT jlong JNICALL Java_vw_VWBase_initialize(JNIEnv *env, jobject obj, jstring command) {
    jlong vwPtr = 0;
    try {
...
JNIEXPORT void JNICALL Java_vw_VWBase_closeInstance(JNIEnv *env, jobject obj, jlong vwPtr) {
    try {
        VW::finish(*((vw*)vwPtr));
    }
    catch(...) {
