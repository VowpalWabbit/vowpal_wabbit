# Format data.table to Vowpal wabbit format
...
# @param namespaces[list / yaml file]: name of each namespace and each variable for each namespace can be a R list, or a YAML file
...
# @param variables like '_', or same variables perceived differently life "_var" and "var"
# examples
# library(ggplot2)
# library(handy)
# library(data.table)
...
# dt[, weight:=ifelse(y == 1, 1.0, 3.0345)]
# 
# # Use the function directly if the file is small
# dt2vw(dt, '../sample_diamonds2.vw', namespaces, target='y', weight='weight')
# system('head ../sample_diamonds2.vw')
# 
# inxs = c(0, seq(1000, 50000, 1000), nrow(dt))
# i = 0
# while(i < (length(inxs) - 1) ){
#   i = i + 1
#   start = inxs[i] + 1
#   end = inxs[(i+1)]
#   print(start)
#   print(end)
#   dt2vw(dt=train[start:end], fileName = '../out_in_chunks.vw',
#         namespaces = nmp, target = 'target', weight=NULL, append=TRUE)
# }
dt2vw <- function(data, fileName, namespaces = NULL, target, weight = NULL,
...
sprintf2 <- function (fmt, ...) 
{
  ...
  spec_at <- setdiff(gregexpr("%", fmt, fixed = TRUE)[[1L]], 
                     not_a_spec_at)
  ...
  fmt_chunks <- substr(rep.int(fmt, length(fmt_break_at) + 
                                 1L), c(1L, fmt_break_at), c(fmt_break_at - 1L, nchar(fmt)))
  ans_chunks <- mapply(function(fmt_chunk, from, to) do.call(sprintf, 
                                                             c(list(fmt_chunk), args[from:to])), fmt_chunks, break_from, 
                       break_to)
  paste(apply(ans_chunks, 1, paste, collapse = ""), collapse = "\n")
}

# Function used to select the namespace - for the example
