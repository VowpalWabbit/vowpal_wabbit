...
namespace cs_test
{
    ...
    //      RunLDAPredict();
        ...
        private static void RunFeaturesTest()
        {
            ...
            IntPtr vw = VowpalWabbitNative.Initialize("-q st --noconstant --quiet");
            IntPtr example = VowpalWabbitNative.ReadExample(vw, "1 |s p^the_man w^the w^man |t p^un_homme w^un w^homme");
            float score = VowpalWabbitNative.Learn(vw, example);
            VowpalWabbitNative.FinishExample(vw, example);

            VowpalWabbitNative.FEATURE_SPACE[] featureSpace = new VowpalWabbitNative.FEATURE_SPACE[2];//maximum number of index spaces

            VowpalWabbitNative.FEATURE[] sfeatures = new VowpalWabbitNative.FEATURE[3];// the maximum number of features
            VowpalWabbitNative.FEATURE[] tfeatures = new VowpalWabbitNative.FEATURE[3];// the maximum number of features
            ...
            uint snum = VowpalWabbitNative.HashSpace("s");
            featureSpace[0].name = (byte)'s';
            sfeatures[0].weight_index = VowpalWabbitNative.HashFeature("p^the_man", snum);
            sfeatures[0].x = 1;
            ...
            sfeatures[1].weight_index = VowpalWabbitNative.HashFeature(s, snum);
            sfeatures[1].x = 1;
            sfeatures[2].weight_index = VowpalWabbitNative.HashFeature("w^man", snum);
            sfeatures[2].x = 1;
            featureSpace[0].len = 3;

            uint tnum = VowpalWabbitNative.HashSpace("t");
            featureSpace[1].name = (byte)'t';
            tfeatures[0].weight_index = VowpalWabbitNative.HashFeature("p^un_homme", tnum);
            tfeatures[0].x = 1;
            tfeatures[1].weight_index = VowpalWabbitNative.HashFeature("w^un", tnum);
            tfeatures[1].x = 1;
            tfeatures[2].weight_index = VowpalWabbitNative.HashFeature("w^homme", tnum);
            tfeatures[2].x = 1;
            featureSpace[1].len = 3;

            IntPtr importedExample = VowpalWabbitNative.ImportExample(vw, featureSpacePtr, (IntPtr)featureSpace.Length);

            VowpalWabbitNative.AddLabel(importedExample, 1);

            score = VowpalWabbitNative.Learn(vw, importedExample);

            Console.Error.WriteLine("p2 = {0}", score);

            VowpalWabbitNative.Finish(vw);
        ...
        {
            IntPtr vw = VowpalWabbitNative.Initialize("-q st -d ..\\..\\..\\test\\train-sets\\0002.dat -f out");

            VowpalWabbitNative.StartParser(vw, false);

            int count = 0;
            IntPtr example = IntPtr.Zero;
            while (IntPtr.Zero != (example = VowpalWabbitNative.GetExample(vw)))
            {
                float label = VowpalWabbitNative.GetLabel(example);

                count++;
                IntPtr featureSpaceLen = (IntPtr)0;
                IntPtr featureSpacePtr = VowpalWabbitNative.ExportExample(vw, example, ref featureSpaceLen);

                VowpalWabbitNative.FEATURE_SPACE[] featureSpace = new VowpalWabbitNative.FEATURE_SPACE[(int)featureSpaceLen];
                int featureSpace_size = Marshal.SizeOf(typeof(VowpalWabbitNative.FEATURE_SPACE));

                for (int i = 0; i < (int)featureSpaceLen; i++)
                {
                    IntPtr curfeatureSpacePos = new IntPtr(featureSpacePtr.ToInt32() + i * featureSpace_size);
                    featureSpace[i] = (VowpalWabbitNative.FEATURE_SPACE)Marshal.PtrToStructure(curfeatureSpacePos, typeof(VowpalWabbitNative.FEATURE_SPACE));

                    VowpalWabbitNative.FEATURE[] feature = new VowpalWabbitNative.FEATURE[featureSpace[i].len];
                    int feature_size = Marshal.SizeOf(typeof(VowpalWabbitNative.FEATURE));
                    for (int j = 0; j < featureSpace[i].len; j++)
                    {
                        IntPtr curfeaturePos = new IntPtr((featureSpace[i].features.ToInt32() + j * feature_size));
                        feature[j] = (VowpalWabbitNative.FEATURE)Marshal.PtrToStructure(curfeaturePos, typeof(VowpalWabbitNative.FEATURE));
                    }
                }
                VowpalWabbitNative.ReleaseFeatureSpace(featureSpacePtr, featureSpaceLen);

                float score = VowpalWabbitNative.Learn(vw, example);
                VowpalWabbitNative.FinishExample(vw, example);
            }

            VowpalWabbitNative.EndParser(vw);
            VowpalWabbitNative.Finish(vw);
        }
        ...
        private static void RunSpeedTest()
        {
            ...
            //IntPtr vw = VowpalWabbitNative.Initialize("--ngram 2 --skips 4 -l 0.25 -b 22 -d rcv1.train.raw.txt -f out");
            //IntPtr vw = VowpalWabbitNative.Initialize("-d rcv1.train.raw.txt -b 22 --ngram 2 --skips 4 -l 0.25 -c");
            //IntPtr vw = VowpalWabbitNative.Initialize("-d rcv1.train.raw.txt -c");
            IntPtr vw = VowpalWabbitNative.Initialize("-d ..\\..\\..\\test\\train-sets\\0002.dat");
        ...
        {
            //IntPtr vw = VowpalWabbitNative.Initialize("-q st -d rcv1.train.raw.txt -f out");
            IntPtr vw = VowpalWabbitNative.Initialize("-q st -d ..\\..\\..\\test\\train-sets\\0002.dat -f out");

            VowpalWabbitNative.StartParser(vw, false);

            uint stride = (uint)VowpalWabbitNative.Get_Stride(vw);
            ...
            while (IntPtr.Zero != (example = VowpalWabbitNative.GetExample(vw)))
            {
                ...
                float prediction = VowpalWabbitNative.GetPrediction(example);
                float importance = VowpalWabbitNative.GetImportance(example);
                float initial = VowpalWabbitNative.GetInitial(example);
                float label = VowpalWabbitNative.GetLabel(example);

                UInt32 tag_len = (UInt32)VowpalWabbitNative.GetTagLength(example);
                byte[] tag = new byte[tag_len];
                if (tag_len > 0)
                    Marshal.Copy(VowpalWabbitNative.GetTag(example), tag, 0, (int)tag_len); 

                UInt32 num_features = (UInt32)VowpalWabbitNative.GetFeatureNumber(example);
                VowpalWabbitNative.FEATURE[] f;
                if (num_features > 0)
                {
                    f = new VowpalWabbitNative.FEATURE[num_features];

                    IntPtr feature_count = (IntPtr)0;
                    IntPtr ret = VowpalWabbitNative.GetFeatures(vw, example, ref feature_count);

                    int feature_size = Marshal.SizeOf(typeof(VowpalWabbitNative.FEATURE));
                    for (int i = 0; i < (int)feature_count; i++)
                    {
                        IntPtr curfeaturePos = new IntPtr(ret.ToInt32() + i * feature_size);
                        f[i] = (VowpalWabbitNative.FEATURE)Marshal.PtrToStructure(curfeaturePos, typeof(VowpalWabbitNative.FEATURE));
                    }
                }
        ...
        {
            public VowpalWabbitNative.FEATURE_SPACE[] featureSpace;
            ...
            public VWInstanceEx(IntPtr vw, IntPtr ex)
            {
                ...
                IntPtr featureSpaceLen = (IntPtr)0;
                IntPtr featureSpacePtr = VowpalWabbitNative.ExportExample(vw, ex, ref featureSpaceLen);

                this.featureSpace = new VowpalWabbitNative.FEATURE_SPACE[(int)featureSpaceLen];
                int featureSpace_size = Marshal.SizeOf(typeof(VowpalWabbitNative.FEATURE_SPACE));

                for (int i = 0; i < (int)featureSpaceLen; i++)
                {
                    IntPtr curfeatureSpacePos = new IntPtr(featureSpacePtr.ToInt32() + i * featureSpace_size);
                    this.featureSpace[i] = (VowpalWabbitNative.FEATURE_SPACE)Marshal.PtrToStructure(curfeatureSpacePos, typeof(VowpalWabbitNative.FEATURE_SPACE));

                    VowpalWabbitNative.FEATURE[] feature = new VowpalWabbitNative.FEATURE[this.featureSpace[i].len];
                    int feature_size = Marshal.SizeOf(typeof(VowpalWabbitNative.FEATURE));
        ...
        {
            // parse and cache
            IntPtr vw0 = VowpalWabbitNative.Initialize(@"-d 0002.dat -c");
            VowpalWabbitNative.StartParser(vw0, false);
            ...
            {
                IntPtr example = VowpalWabbitNative.GetExample(vw0);

                if (IntPtr.Zero == example)
                    break;

                vwInstanceExs[instanceCount] = new VWInstanceEx(vw0, example);
                VowpalWabbitNative.FinishExample(vw0, example);
                instanceCount++;
            }
            VowpalWabbitNative.EndParser(vw0);
            VowpalWabbitNative.Finish(vw0);

            // learn
            instanceCount = 0;
            IntPtr vw = VowpalWabbitNative.Initialize(@"--quiet --random_seed 276518665 -f save_file.reg --readable_model reable.reg");
            foreach (VWInstanceEx vwInstanceEx in vwInstanceExs)
            {
                VowpalWabbitNative.FEATURE_SPACE[] featureSpace = new VowpalWabbitNative.FEATURE_SPACE[vwInstanceEx.featureSpace.Length];
                ...
                IntPtr importedExample = VowpalWabbitNative.ImportExample(vw, featureSpacePtr, (IntPtr)vwInstanceEx.featureSpace.Length);
                VowpalWabbitNative.Learn(vw, importedExample);
                VowpalWabbitNative.FinishExample(vw, importedExample);
