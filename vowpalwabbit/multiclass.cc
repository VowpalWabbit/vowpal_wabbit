...
void print_label_pred(vw &all, example &ec, uint32_t prediction) {
  ...
  all.sd->print_update(
      ...
      !ss_label.begin ? "unknown" : std::string(ss_label.begin,
                                                ss_label.end - ss_label.begin),
...
void print_probability(vw &all, example &ec, uint32_t prediction) {
  ...
  pred_ss << prediction << "(" << std::setw(2) << std::setprecision(0)
          << std::fixed << 100 * ec.pred.scalars[prediction - 1] << "%)";

  std::stringstream label_ss;
  label_ss << ec.l.multi.label;

  all.sd->print_update(all.holdout_set_off, all.current_pass, label_ss.str(),
                       pred_ss.str(), ec.num_features, all.progress_add,
                       all.progress_arg);
}
