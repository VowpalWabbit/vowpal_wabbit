...
void print_label_pred(vw& all, example& ec, uint32_t prediction)
{
  ...
  all.sd->print_update(all.holdout_set_off, all.gs.current_pass,
...
void print_probability(vw& all, example& ec, uint32_t prediction)
{
  ...
  all.sd->print_update(all.holdout_set_off, all.gs.current_pass, label_ss.str(), pred_ss.str(), ec.num_features,
      all.progress_add, all.progress_arg);
}
...
{
  all.sd->print_update(all.holdout_set_off, all.gs.current_pass, ec.l.multi.label, prediction, ec.num_features,
      all.progress_add, all.progress_arg);
}
