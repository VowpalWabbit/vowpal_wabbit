...
template <class T> uint32_t find_min(std::vector<T> arr) {
  T min_val = FLT_MAX;
  uint32_t argmin = 0;
...
void finish(warm_cb &data) {
  ...
  if (!data.all->quiet) {
    std::cerr << "average variance estimate = "
              << data.cumu_var / data.inter_iter << std::endl;
    std::cerr << "theoretical average variance = "
              << data.num_actions / data.epsilon << std::endl;
    std::cerr << "last lambda chosen = " << data.lambdas[argmin]
              << " among lambdas ranging from " << data.lambdas[0] << " to "
              << data.lambdas[data.choices_lambda - 1] << std::endl;
  }
}
...
base_learner *warm_cb_setup(options_i &options, vw &all) {
  ...
  if (options.was_supplied("baseline")) {
    std::stringstream ss;
    ss << std::max(std::abs(data->loss0), std::abs(data->loss1)) /
