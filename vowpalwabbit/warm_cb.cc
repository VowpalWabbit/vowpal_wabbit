...
float loss_cs(warm_cb &data, v_array<COST_SENSITIVE::wclass> &costs,
...
template <class T> uint32_t find_min(vector<T> arr) {
  T min_val = FLT_MAX;
  uint32_t argmin = 0;
...
void finish(warm_cb &data) {
  ...
  if (!data.all->quiet) {
    cerr << "average variance estimate = " << data.cumu_var / data.inter_iter
         << endl;
    cerr << "theoretical average variance = " << data.num_actions / data.epsilon
         << endl;
    cerr << "last lambda chosen = " << data.lambdas[argmin]
         << " among lambdas ranging from " << data.lambdas[0] << " to "
         << data.lambdas[data.choices_lambda - 1] << endl;
  }
  ...
  for (size_t i = 0; i < data.ws_vali.size(); ++i) {
    if (data.use_cs)
      VW::dealloc_example(COST_SENSITIVE::cs_label.delete_label,
                          *data.ws_vali[i]);
    else
...
void copy_example_to_adf(warm_cb &data, example &ec) {
  ...
  for (size_t a = 0; a < data.num_actions; ++a) {
    ...
    if (CB_ALGS::example_is_newline_not_header(eca) &&
        CB::cb_label.test_label(&eca.l)) {
      eca.tag.push_back('n');
    }
  }
}
...
void setup_lambdas(warm_cb &data) {
  ...
  if (data.lambda_scheme == ABS_CENTRAL ||
      data.lambda_scheme == ABS_CENTRAL_ZEROONE)
    lambdas[mid] = 0.5;
  else
...
float compute_weight_multiplier(warm_cb &data, size_t i, int ec_type) {
  ...
  float total_weight = (1 - data.lambdas[i]) * ws_train_size +
                       data.lambdas[i] * inter_train_size;

  if (ec_type == WARM_START)
    weight_multiplier =
        (1 - data.lambdas[i]) * total_train_size / (total_weight + FLT_MIN);
  else
    weight_multiplier =
        data.lambdas[i] * total_train_size / (total_weight + FLT_MIN);

  return weight_multiplier;
}

uint32_t predict_sublearner_adf(warm_cb &data, multi_learner &base, example &ec,
                                uint32_t i) {
...
template <bool use_cs> void add_to_vali(warm_cb &data, example &ec) {
  // TODO: set the first parameter properly
  example *ec_copy = VW::alloc_examples(sizeof(polylabel), 1);

  if (use_cs)
    VW::copy_example_data(false, ec_copy, &ec, 0,
                          COST_SENSITIVE::cs_label.copy_label);
  else
    VW::copy_example_data(false, ec_copy, &ec, 0,
                          MULTICLASS::mc_label.copy_label);
...
template <bool use_cs>
void predict_or_learn_sup_adf(warm_cb &data, multi_learner &base, example &ec,
                              int ec_type) {
...
uint32_t predict_bandit_adf(warm_cb &data, multi_learner &base, example &ec) {
  ...
  if (sample_after_normalizing(data.app_seed + data.example_counter++,
                               begin_scores(out_ec.pred.a_s),
...
void learn_bandit_adf(warm_cb &data, multi_learner &base, example &ec,
                      int ec_type) {
  copy_example_to_adf(data, ec);
...
template <bool use_cs>
void predict_or_learn_bandit_adf(warm_cb &data, multi_learner &base,
                                 example &ec, int ec_type) {
...
base_learner *warm_cb_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("warm_cb", num_actions).keep().help("Convert multiclass "
                                                           "on <k> classes "
                                                           "into a contextual "
                                                           "bandit problem"))
      .add(make_option("warm_cb_cs", use_cs).help("consume cost-sensitive "
  ...
  if (options.was_supplied("baseline")) {
    stringstream ss;
    ss << max<float>(abs(data->loss0), abs(data->loss1)) /
              (data->loss1 - data->loss0);
  ...
  // Note: the current version of warm start CB can only support epsilon-greedy
  // exploration
  ...
  if (use_cs)
    l = &init_cost_sensitive_learner(
        ...
        predict_or_learn_adf<false, true>, all.p, data->choices_lambda);
  else
    l = &init_multiclass_learner(data, base, predict_or_learn_adf<true, false>,
