...
base_learner* cb_algs_setup(options_i& options, vw& all)
{
  ...
  {
    l = &init_learner(
        data, base, learn_eval, predict_eval, problem_multiplier, prediction_type_t::multiclass, all.get_setup_name(cb_algs_setup)+"-eval");
  ...
  else
  {
    l = &init_learner(data, base, predict_or_learn<true>, predict_or_learn<false>, problem_multiplier,
        prediction_type_t::multiclass, all.get_setup_name(cb_algs_setup));
