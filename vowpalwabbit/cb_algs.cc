...
void predict_or_learn(cb &data, single_learner &base, example &ec) {
  ...
  if (c.known_cost != nullptr &&
      (c.known_cost->action < 1 || c.known_cost->action > c.num_actions))
...
void learn_eval(cb &data, single_learner &, example &ec) {
  ...
  for (size_t i = 0; i < ld.event.costs.size(); i++)
    ld.event.costs[i].partial_prediction =
        data.cb_cs_ld.costs[i].partial_prediction;
...
void output_example(vw &all, cb &data, example &ec, CB::label &ld) {
  ...
  all.sd->update(ec.test_only, !CB::cb_label.test_label(&ld), loss, 1.f,
                 ec.num_features);
...
base_learner *cb_algs_setup(options_i &options, vw &all) {
  ...
  if (eval) {
    ...
    l = &init_learner(data, base, predict_or_learn<true>,
                      predict_or_learn<false>, problem_multiplier,
                      prediction_type::multiclass);
