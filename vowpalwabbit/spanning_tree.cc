...
namespace VW {
SpanningTree::SpanningTree(uint16_t port)
    : m_stop(false), m_port(port), m_future(nullptr) {
#ifdef _WIN32
  WSAData wsaData;
...
void SpanningTree::Run() {
  ...
  while (!m_stop) {
    ...
    cerr << "inbound connection from " << dotted_quad << "(" << hostname << ':'
         << ntohs(m_port) << ") serv=" << servInfo << endl;

    size_t nonce = 0;
    if (recv(f, (char *)&nonce, sizeof(nonce), 0) != sizeof(nonce)) {
      cerr << dotted_quad << "(" << hostname << ':' << ntohs(m_port)
    ...
    int ok = true;
    if (id >= total) {
      cout << dotted_quad << "(" << hostname << ':' << ntohs(m_port)
    ...
    if (partial_nodeset.filled != total) // Need to wait for more connections
    {
      ...
      for (size_t i = 0; i < total; i++) {
        if (partial_nodeset.nodes[i].client_ip == (uint32_t)-1) {
          cout << "nonce " << nonce << " still waiting for "
               << (total - partial_nodeset.filled) << " nodes out of " << total
               << " for example node " << i << endl;
          break;
        }
      }
    } else {
      ...
      for (size_t i = 0; i < total; i++) {
        ...
        if (recv(partial_nodeset.nodes[i].socket, (char *)&(client_ports[i]),
          ...
          cerr << " Port read failed for node " << i << " read " << done
               << endl;
