...
int build_tree(int *parent, uint16_t *kid_count, size_t source_count,
               int offset) {
  if (source_count == 1) {
    kid_count[offset] = 0;
    return offset;
...
namespace VW {
SpanningTree::SpanningTree(uint16_t port, bool quiet)
    : m_stop(false), m_port(port), m_future(nullptr), m_quiet(quiet) {
#ifdef _WIN32
  WSAData wsaData;
  ...
  // which port did we bind too (if m_port is 0 this will give us the actual
  // port)
...
void SpanningTree::Run() {
  ...
  while (!m_stop) {
    ...
    if (NULL == inet_ntop(AF_INET, &(client_address.sin_addr), dotted_quad,
                          INET_ADDRSTRLEN))
    ...
    if (!m_quiet)
      std::cerr << "inbound connection from " << dotted_quad << "(" << hostname
                << ':' << ntohs(m_port) << ") serv=" << servInfo << std::endl;

    size_t nonce = 0;
    if (recv(f, (char *)&nonce, sizeof(nonce), 0) != sizeof(nonce)) {
      THROW(dotted_quad << "(" << hostname << ':' << ntohs(m_port)
                        << "): nonce read failed, exiting");
    } else {
      if (!m_quiet)
        std::cerr << dotted_quad << "(" << hostname << ':' << ntohs(m_port)
    ...
    if (id >= total) {
      if (!m_quiet)
        std::cout << dotted_quad << "(" << hostname << ':' << ntohs(m_port)
                  << "): invalid id=" << id << " >=  " << total << " !"
                  << std::endl;
      ok = false;
    }
    ...
    if (partial_nodeset.filled != total) // Need to wait for more connections
    {
      ...
      for (size_t i = 0; i < total; i++) {
        if (partial_nodeset.nodes[i].client_ip == (uint32_t)-1) {
          if (!m_quiet)
            std::cout << "nonce " << nonce << " still waiting for "
                      << (total - partial_nodeset.filled) << " nodes out of "
      ...
      for (size_t i = 0; i < total; i++) {
        fail_send(partial_nodeset.nodes[i].socket, &kid_count[i],
                  sizeof(kid_count[i]));
      }
      ...
      for (size_t i = 0; i < total; i++) {
        int done = 0;
        if (recv(partial_nodeset.nodes[i].socket, (char *)&(client_ports[i]),
                 sizeof(client_ports[i]), 0) < (int)sizeof(client_ports[i]))

          if (!m_quiet)
            std::cerr << " Port read failed for node " << i << " read " << done
                      << std::endl;
      } // all clients have bound to their ports.

      for (size_t i = 0; i < total; i++) {
        if (parent[i] >= 0) {
          fail_send(partial_nodeset.nodes[i].socket,
                    &partial_nodeset.nodes[parent[i]].client_ip,
                    sizeof(partial_nodeset.nodes[parent[i]].client_ip));
          fail_send(partial_nodeset.nodes[i].socket, &client_ports[parent[i]],
