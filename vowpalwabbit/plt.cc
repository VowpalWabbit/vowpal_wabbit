...
{ MULTILABEL::labels multilabels = ec.l.multilabels;
  ...
  { for (uint32_t i = 0; i < multilabels.label_v.size(); ++i)
    ...
    if (multilabels.label_v[multilabels.label_v.size() - 1] >= p.k)
      std::cout << "label " << multilabels.label_v[multilabels.label_v.size() - 1] << " is not in {0," << p.k - 1 << "} This won't work right." << std::endl;
...
{ auto tree = scoped_calloc_or_throw<plt>();
  ...
  if (tree->top_k > 0)
    l = &init_learner(tree, as_singleline(setup_base(options, all)), learn, predict<false>, tree->t, prediction_type_t::multilabels);
  else
    l = &init_learner(tree, as_singleline(setup_base(options, all)), learn, predict<true>, tree->t, prediction_type_t::multilabels);
