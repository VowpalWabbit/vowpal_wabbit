...
namespace SearnUtil
{
  ...
  void add_history_to_example(vw&all, history_info &hinfo, example* ec, history h, size_t additional_offset=0)
  {
    ...
    for (uint32_t t=1; t<=hinfo.length; t++) {
      v0 = (h[hinfo.length-t] * quadratic_constant * (additional_offset+1) + t) * history_constant;
      ...
      if ((t > 1) && hinfo.bigrams) {
        v1 = (v0 * cubic_constant + h[hinfo.length-t+1] * (additional_offset+1)) * history_constant;
...
namespace Searn {
  ...
  uint32_t searn_predict_without_loss(vw& all, learner& base, example* ecs, size_t num_ec, v_array<uint32_t> *yallowed, v_array<uint32_t> *ystar, bool ystar_is_uint32t)  // num_ec == 0 means normal example, >0 means ldf, yallowed==NULL means all allowed, ystar==NULL means don't know; ystar_is_uint32t means that the ystar ref is really just a uint32_t
  {
    ...
    if (srn->state == INIT_TRAIN) {
      ...
      if (srn->auto_history) srn->rollout_action.push_back(a); // TODO: for ldf make this weight_index
      srn->t++;
      return a;
    }
    if (srn->state == LEARN) {
      if (srn->t < srn->learn_t) {
        ...
        srn->t++;
        if (srn->auto_history) srn->rollout_action.push_back(srn->learn_a); // TODO: for ldf make this weight_index
        return srn->learn_a;
      } else { // t > learn_t
        size_t this_a = 0;
        ...
        if (srn->auto_history) srn->rollout_action.push_back((uint32_t)this_a); // TODO: for ldf make this weight_index
...
void print_update(vw& all, searn* srn)
  ...
  void searn_initialize(vw& all, searn& srn)
  {
    ...
    srn.empty_example = alloc_example(sizeof(CSOAA::label));
