...
{
  std::cout << "otree_c: predict() base.predict() " << simple_label_to_string(ec) << features_to_string(ec) << std::endl;
  ...
  while (!(cur_node.is_leaf))
  {
    ...
    std::cout << "otree_c: predict() base.predict() " << scalar_pred_to_string(ec) << ", nodeid = " << cur_node.id << std::endl;
...
void offset_tree::learn(LEARNER::single_learner& base, example& ec)
{
  ...
  std::cout << "otree_c: learn() -- tree_traversal -- " << std::endl;
  for (uint32_t i = 0; i < ac.size(); i++)
  {
    uint32_t node_id = ac[i].action + binary_tree.internal_node_count();
    std::cout << "otree_c: learn() ac[" << i << "].action  = " << ac[i].action << ", node_id  = " << node_id << std::endl;
  ...
  while (!node_costs.empty())
  {
    ...
    while (!node_costs.empty())
    {
      ...
      std::cout << "otree_c: learn() v.id = " << v.id << ", cost_v = " << cost_v << std::endl;
      if (!node_costs.empty() && node_costs.back().node_id == w.id)
      {
        std::cout << "otree_c: learn() found the sibling" << std::endl;
        cost_w = node_costs.back().cost;
        if (cost_v != cost_w)
        {
          std::cout << "otree_c: learn() cost_w = " << cost_w << ", cost_v != cost_w" << std::endl;
      ...
      {
        std::cout << "otree_c: learn() no sibling" << std::endl;
        if (v.id == v_parent.right_id) ////
          local_action = -1;
      }
      float cost_parent = cost_v;
      std::cout << "otree_c: learn() cost_w = " << cost_w << std::endl;
      if (cost_v != cost_w)  // learn and update the cost of the parent
      {
        ...
        std::cout << "otree_c: learn() binary learning the node " << v.parent_id << std::endl;
        base.learn(ec, v.parent_id);
        base.predict(ec, v.parent_id);
        std::cout << "otree_c: learn() after binary predict:" << scalar_pred_to_string(ec) << ", local_action = " << (local_action) << std::endl;
        if (ec.pred.scalar == local_action)
        {
          cost_parent = min(cost_v, cost_w);
          std::cout << "otree_c: learn() ec.pred.scalar == local_action" << std::endl;
        }
        else
        {
