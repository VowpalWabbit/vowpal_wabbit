...
void expand_namespaces_with_recursion(std::string const &ns,
                                      std::vector<std::string> &res,
                                      std::string &val, size_t pos) {
  assert(pos <= ns.size());
...
expand_interactions(const std::vector<std::string> &vec,
                    const size_t required_length, const std::string &err_msg) {
  std::vector<std::string> res;
  ...
  for (std::string const &i : vec) {
    ...
    // got strict requirement of interaction length and it was failed.
    {
      ...
      THROW("error, feature interactions must involve at least two namespaces"
            << err_msg);
...
void sort_and_filter_duplicate_interactions(std::vector<std::string> &vec,
...
// function which counts generated
// features and sums their squared values it's replaced with more fast (?)
// analytic solution but keeps just in case and
// for doublecheck.
...
constexpr int64_t fast_factorial[] = {1,
                                      1,
                                      2,
                                      6,
                                      24,
...
constexpr size_t size_fast_factorial =
    sizeof(fast_factorial) / sizeof(*fast_factorial);
...
inline size_t factor(const size_t n, const size_t start_from = 1) {
  ...
  size_t res = 1;
  for (size_t i = start_from + 1; i <= n; ++i)
    res *= i;
  return res;
}
...
void eval_count_of_generated_ft(vw &all, example &ec, size_t &new_features_cnt,
                                float &new_features_value) {
  new_features_cnt = 0;
  ...
  } else // case of simple combinations
  {
    ...
    for (std::string &inter : *ec.interactions) {
      ...
      for (auto ns = inter.begin(); ns != inter.end(); ++ns) {
        if ((ns == inter.end() - 1) ||
            (*ns != *(ns + 1))) // neighbour namespaces are different
        {
                   ...
                   // (interaction is preliminary sorted)
        {
          ...
          for (size_t i = 0; i < results.size(); ++i)
            results[i] = 0.;
          while (results.size() < order_of_inter)
            results.push_back(0.);
          ...
          sum_feat_sq_in_inter *=
              results[order_of_inter -
                      1]; // will be explained in http://bit.ly/1Hk9JX1
          ...
          size_t n;
          if (cnt_ft_value_non_1 ==
              0) // number of generated simple combinations is C(n,k)
          {
            ...
            for (size_t l = 0; l <= order_of_inter; ++l) {
              // C(l+m-1, l) * C(n-m, k-l)
              size_t num =
                  (l == 0) ? 1 : (size_t)choose(l + cnt_ft_value_non_1 - 1, l);

              if (ft_size - cnt_ft_value_non_1 >= order_of_inter - l)
                num *= (size_t)choose(ft_size - cnt_ft_value_non_1,
                                      order_of_inter - l);
              else
                num = 0;
    ...
    if (correct_features_cnt != new_features_cnt)
      all.trace_message << "Incorrect new features count " << new_features_cnt
                        << " must be " << correct_features_cnt << std::endl;
    if (fabs(correct_features_value - new_features_value) > 1e-5)
      all.trace_message << "Incorrect new features value " << new_features_value
                        << " must be " << correct_features_value << std::endl;
#endif
  }
