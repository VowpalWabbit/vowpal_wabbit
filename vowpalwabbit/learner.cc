...
void save(example &ec, vw &all) {
  ...
  if (!all.quiet)
    all.trace_message << "saving regressor to " << final_regressor_name
                      << std::endl;
...
inline bool example_is_newline_not_header(example &ec, vw &all) {
  // If we are using CCB, test against CCB implementation otherwise fallback to
  // previous behavior.
...
bool inline is_save_cmd(example *ec) {
  return (ec->tag.size() >= 4) &&
         (0 == strncmp((const char *)ec->tag.begin(), "save", 4));
}
...
// single_instance_context / multi_instance_context - classes incapsulating
// single/multiinstance example processing
// get_master - returns main vw instance for owner's example manipulations (i.e.
// finish)
...
public:
  ...
  template <class T, void (*process_impl)(T &, vw &)> void process(T &ec) {
...
public:
  ...
  template <class T, void (*process_impl)(T &, vw &)> void process(T &ec) {
    // start with last as the first instance will free the example as it is the
    // owner
    for (auto it = _all.rbegin(); it != _all.rend(); ++it)
      process_impl(ec, **it);
  }
...
// single_example_handler / multi_example_handler - consumer classes with
// on_example handle method, incapsulating
// creation of example / multi_ex and passing it to context.process
template <typename context_type> class single_example_handler {
public:
  single_example_handler(const context_type &context) : _context(context) {}

  void on_example(example *ec) {
    if (ec->indices.size() >
        1) // 1+ nonconstant feature. (most common case first)
...
template <typename context_type> class multi_example_handler {
private:
  bool complete_multi_ex(example *ec) {
    ...
    const bool is_newline =
        (example_is_newline_not_header(*ec, master) && is_test_ec);
    if (!is_newline) {
      ec_seq.push_back(ec);
    } else {
...
// ready_examples_queue / custom_examples_queue - adapters for connecting
// example handler to parser produce-consume loop
...
template <typename handler_type> void generic_driver_onethread(vw &all) {
  single_instance_context context(all);
  handler_type handler(context);
...
void generic_driver_onethread(vw &all) {
  if (all.l->is_multiline)
    generic_driver_onethread<multi_example_handler<single_instance_context>>(
        all);
  else
    generic_driver_onethread<single_example_handler<single_instance_context>>(
        all);
}
