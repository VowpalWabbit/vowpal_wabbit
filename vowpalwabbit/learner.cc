...
bool complete_multi_ex(example *ec, multi_ex &ec_seq, vw &all) {
  ...
  if (example_is_newline_not_header(*ec) && is_test_ec) {
    VW::finish_example(all, *ec);
    if (ec_seq.size() == 0) {
      cout << "Something is wrong---an example with no choice.  Do you have "
              "all 0 features? Or multiple empty lines?"
           << endl;
      return false;
    }
...
template <void (*f)(vw &, multi_ex &)> void multi_ex_generic_driver(vw &all) {
  multi_ex ec_seq;
  example *ec = nullptr;
  ...
  if (all.l->is_multiline) {
    ...
    auto multi_ex_fptr = [&ctxt](vw &all, v_array<example *> examples) {
      all.p->end_parsed_examples +=
          examples.size(); // divergence: lock & signal
      for (size_t i = 0; i < examples.size(); ++i)
