...
struct lda {
  ...
  // if it makes it into VS 2015 change the next ifdef to check Visual Studio
  // Release
...
inline float fastlog2(float x) {
  ...
  return y - 124.22544637f - 1.498030302f * mx_f -
...
inline float fastpow2(float p) {
  ...
  uint32_t approx = (uint32_t)((1 << 23) * (clipp + 121.2740838f +
...
inline float fastdigamma(float x) {
  ...
  return -(1.0f + 2.0f * x) / (x * (1.0f + x)) -
         (13.0f + 6.0f * x) / (12.0f * twopx * twopx) + logterm;
}
...
inline v4sf vfastpow2(const v4sf p) {
  ...
  v4sf v =
      v4sfl(1 << 23) * (clipp + c_121_2740838 +
                        c_27_7280233 / (c_4_84252568 - z) - c_1_49012907 * z);
...
inline v4sf vfastlog2(v4sf x) {
  ...
  v4sf mx_f = _mm_castsi128_ps(
      _mm_or_si128(_mm_and_si128(vx_i, v4sil(0x007FFFFF)), v4sil(0x3f000000)));
  ...
  return y - c_124_22551499 - c_1_498030302 * mx_f -
...
inline v4sf vfastdigamma(v4sf x) {
  ...
  return (v4sfl(-48.0f) +
          x * (v4sfl(-157.0f) + x * (v4sfl(-127.0f) - v4sfl(30.0f) * x))) /
...
#else
  extra_sum += v4sf_index<0>(sum) + v4sf_index<1>(sum) + v4sf_index<2>(sum) +
               v4sf_index<3>(sum);
#endif
...
// Templates for common code shared between the three math modes (SIMD, fast
// approximations
// and accurate).
//
...
template <typename T, const lda_math_mode mtype> inline T lgamma(T /* x */) {
  BOOST_STATIC_ASSERT_MSG(
      true, "ldamath::lgamma is not defined for this type and math mode.");
}

// Digamma:
template <typename T, const lda_math_mode mtype> inline T digamma(T /* x */) {
  BOOST_STATIC_ASSERT_MSG(
      true, "ldamath::digamma is not defined for this type and math mode.");
}

// Exponential
template <typename T, lda_math_mode mtype> inline T exponential(T /* x */) {
  BOOST_STATIC_ASSERT_MSG(
      true, "ldamath::exponential is not defined for this type and math mode.");
}

// Powf
template <typename T, lda_math_mode mtype> inline T powf(T /* x */, T /* p */) {
  BOOST_STATIC_ASSERT_MSG(
      true, "ldamath::powf is not defined for this type and math mode.");
}
...
template <>
inline void expdigammify<float, USE_SIMD>(vw &all, float *gamma,
...
inline void expdigammify_2(vw &all, float *gamma, T *norm, const T threshold) {
  std::transform(
      gamma, gamma + all.lda, norm, gamma, [threshold](float g, float n) {
        return fmax(threshold, exponential<T, mtype>(digamma<T, mtype>(g) - n));
      });
}
template <>
inline void expdigammify_2<float, USE_SIMD>(vw &all, float *gamma, float *norm,
...
float lda::digamma(float x) {
  ...
  default:
    // Should not happen.
    std::cerr << "lda::digamma: Trampled or invalid math mode, aborting"
              << std::endl;
    abort();
    return 0.0f;
  }
...
float lda::lgamma(float x) {
  ...
  default:
    std::cerr << "lda::lgamma: Trampled or invalid math mode, aborting"
              << std::endl;
    abort();
    return 0.0f;
  }
...
float lda::powf(float x, float p) {
  ...
  default:
    std::cerr << "lda::powf: Trampled or invalid math mode, aborting"
              << std::endl;
    abort();
    return 0.0f;
  }
...
void lda::expdigammify(vw &all, float *gamma) {
  switch (mmode) {
  case USE_FAST_APPROX:
    ldamath::expdigammify<float, USE_FAST_APPROX>(all, gamma,
                                                  underflow_threshold(), 0.0f);
    break;
  case USE_PRECISE:
    ldamath::expdigammify<float, USE_PRECISE>(all, gamma, underflow_threshold(),
                                              0.0f);
    break;
  case USE_SIMD:
    ldamath::expdigammify<float, USE_SIMD>(all, gamma, underflow_threshold(),
                                           0.0f);
    break;
  default:
    std::cerr << "lda::expdigammify: Trampled or invalid math mode, aborting"
              << std::endl;
    abort();
  }
}
...
void lda::expdigammify_2(vw &all, float *gamma, float *norm) {
  switch (mmode) {
  case USE_FAST_APPROX:
    ldamath::expdigammify_2<float, USE_FAST_APPROX>(all, gamma, norm,
                                                    underflow_threshold());
    break;
  case USE_PRECISE:
    ldamath::expdigammify_2<float, USE_PRECISE>(all, gamma, norm,
                                                underflow_threshold());
    break;
  case USE_SIMD:
    ldamath::expdigammify_2<float, USE_SIMD>(all, gamma, norm,
                                             underflow_threshold());
    break;
  default:
    std::cerr << "lda::expdigammify_2: Trampled or invalid math mode, aborting"
              << std::endl;
    abort();
  }
}
...
static inline float average_diff(vw &all, float *oldgamma, float *newgamma) {
  ...
  // This warps the normal sense of "inner product", but it accomplishes the
  // same
  // thing as the "plain old" for loop. clang does a good job of reducing the
  // common subexpressions.
  sum = std::inner_product(
      oldgamma, oldgamma + all.lda, newgamma, 0.0f,
...
float lda_loop(lda &l, v_array<float> &Elogtheta, float *v, example *ec,
               float) {
  ...
  } while (average_diff(*l.all, old_gamma.begin(), new_gamma.begin()) >
           l.lda_epsilon);
...
struct initial_weights {
  ...
  uint32_t _lda;
  uint32_t _stride;
  initial_weights(weight initial, weight initial_random, bool random,
                  uint32_t lda, uint32_t stride)
      : _initial(initial), _initial_random(initial_random), _random(random),
        _lda(lda), _stride(stride) {}
};
...
void save_load(lda &l, io_buf &model_file, bool read, bool text) {
  ...
  if (read) {
    ...
    initial_weights init(all.initial_t,
                         (float)(l.lda_D / all.lda / all.length() * 200),
                         all.random_weights, all.lda, all.weights.stride());
    if (all.weights.sparse)
      all.weights.sparse_weights.set_default<
          initial_weights, set_initial_lda_wrapper<sparse_parameters>>(init);
    else
      all.weights.dense_weights.set_default<
          initial_weights, set_initial_lda_wrapper<dense_parameters>>(init);
  }
  if (model_file.files.size() > 0) {
    ...
    do {
      ...
      if (!read || all.model_file_ver >= VERSION_FILE_WITH_HEADER_ID)
        ...
        uint32_t j;
        brw += bin_text_read_write_fixed(model_file, (char *)&j, sizeof(j), "",
                                         read, msg, text);
        i = j;
      }
...
void learn_batch(lda &l) {
  ...
  if (l.sorted_features.empty()) // FAST-PASS for real "true"
  {
    ...
    for (size_t d = 0; d < l.examples.size(); d++) {
      ...
      l.examples[d]->pred.scalars.end() =
          l.examples[d]->pred.scalars.begin() + l.topics;
  ...
  for (index_feature *s = &l.sorted_features[0]; s <= &l.sorted_features.back();
    ...
    float decay_component =
        l.decay_levels.end()[-2] -
        l.decay_levels
            .end()[(int)(-1 - l.example_t + *(weights_for_w + l.all->lda))];
  ...
  for (size_t d = 0; d < batch_size; d++) {
    float score = lda_loop(l, l.Elogtheta, &(l.v[d * l.all->lda]),
                           l.examples[d], l.all->power_t);
    if (l.all->audit)
  ...
  if (eta != 0) {
    for (index_feature *s = &l.sorted_features[0];
         s <= &l.sorted_features.back();) {
      index_feature *next = s + 1;
      while (next <= &l.sorted_features.back() &&
             next->f.weight_index == s->f.weight_index)
        next++;
...
void get_top_weights(vw *all, int top_words_count, int topic,
                     std::vector<feature> &output, T &weights) {
  uint64_t length = (uint64_t)1 << all->num_bits;

  // get top features for this topic
  auto cmp = [](feature left, feature right) { return left.x > right.x; };
  std::priority_queue<feature, std::vector<feature>, decltype(cmp)>
      top_features(cmp);
...
void get_top_weights(vw *all, int top_words_count, int topic,
  ...
  if (all->weights.sparse)
    get_top_weights(all, top_words_count, topic, output,
...
template <class T> void compute_coherence_metrics(lda &l, T &weights) {
  ...
  for (size_t topic = 0; topic < l.topics; topic++) {
    ...
    for (size_t i = 0; i < top_features_idx.size(); i++)
      for (size_t j = i + 1; j < top_features_idx.size(); j++)
        word_pairs.push_back(
            feature_pair(top_features_idx[i], top_features_idx[j]));
  }
  ...
  for (size_t topic = 0; topic < l.topics; topic++) {
    ...
    for (auto &pairs : topics_word_pairs[topic]) {
      ...
      auto co_feature_df = std::find_if(
          co_feature.begin(), co_feature.end(),
          [&f2](const word_doc_frequency &v) { return v.idx == f2; });

      if (co_feature_df != co_feature.end()) {
        // printf("(%d:%d + eps)/(%d:%d)\n", f2, co_feature_df->count, f1,
        // l.feature_counts[f1]);
        coherence +=
            logf((co_feature_df->count + epsilon) / l.feature_counts[f1]);
      }
    }
...
void end_pass(lda &l) {
  ...
  if (l.compute_coherence_metrics &&
      l.all->passes_complete == l.all->numpasses) {
    compute_coherence_metrics(l);
    // FASTPASS return;
  }
}

template <class T> void end_examples(lda &l, T &weights) {
  for (typename T::iterator iter = weights.begin(); iter != weights.end();
       ++iter) {
    float decay_component =
        l.decay_levels.last() -
        l.decay_levels.end()[(int)(-1 - l.example_t + (&(*iter))[l.all->lda])];
...
LEARNER::base_learner *lda_setup(options_i &options, vw &all) {
  ...
  LEARNER::learner<lda, example> &l = init_learner(
      ld, ld->compute_coherence_metrics ? learn_with_metrics : learn,
      ld->compute_coherence_metrics ? predict_with_metrics : predict,
      UINT64_ONE << all.weights.stride_shift(), prediction_type::scalars);
