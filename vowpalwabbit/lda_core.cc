  ...
  { memcpy(v, new_gamma.begin(), sizeof(float) * l.topics);
    ...
    for (features& fs : *ec)
      { for (features::iterator& f : fs)
	     {  weight_parameters::iterator u_for_w = weights.begin() + f.index() + l.topics + 1;
  ...
  if (eta != 0)
  {
	  for (index_feature *s = &l.sorted_features[0]; s <= &l.sorted_features.back();)
	  {
		  ...
		  for (; s != next; s++)
		  {
			  ...
			  for (size_t k = 0; k < l.all->lda; k++, ++word_weights, ++ u_for_w)
			  {
...
void get_top_weights(vw* all, int top_words_count, int topic, v_array<tuple<weight, uint64_t>>& output)
{
	...
	weight_parameters::iterator v = weights.begin(topic);
	for (uint64_t i = 0; i < min(top_words_count, length); i++, ++v)
		top_features.push(std::make_tuple(*v, i));

	v = weights.begin(topic) + top_words_count;
	for (uint64_t i = top_words_count; i < length; i++, ++v)
