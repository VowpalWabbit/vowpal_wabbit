...
void predict_or_learn_with_confidence(confidence & /* c */,
                                      single_learner &base, example &ec) {
  float threshold = 0.f;
...
void confidence_print_result(int f, float res, float confidence,
                             v_array<char> tag) {
  if (f >= 0) {
    std::stringstream ss;
...
base_learner *confidence_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("confidence", confidence_arg)
               .keep()
               .help("Get confidence for binary predictions"))
      .add(make_option("confidence_after_training", confidence_after_training)
  ...
  if (!all.training) {
    std::cout << "Confidence does not work in test mode because learning "
                 "algorithm state is needed.  Use --save_resume when "
  ...
  void (*learn_with_confidence_ptr)(confidence &, single_learner &, example &) =
      nullptr;
  void (*predict_with_confidence_ptr)(confidence &, single_learner &,
                                      example &) = nullptr;
  ...
  if (confidence_after_training) {
    ...
    predict_with_confidence_ptr =
        predict_or_learn_with_confidence<false, false>;
  }

  // Create new learner
  learner<confidence, example> &l =
      init_learner(data, as_singleline(setup_base(options, all)),
                   learn_with_confidence_ptr, predict_with_confidence_ptr);
