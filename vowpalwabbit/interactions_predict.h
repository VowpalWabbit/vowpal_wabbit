...
struct feature_gen_data
{
  ...
  size_t loop_end;          // last feature id. May be less than number of features if
                            // namespace involved in interaction more than once
...
template <class R,
    ...
    class W>
inline void inner_kernel(R &dat,
  ...
  {  // current list of namespaces to interact.
    ...
    else if (len == 3)  // special case for triples
    {
      ...
      if (first.nonempty())
      {
        ...
        if (second.nonempty())
        {
          ...
          if (third.nonempty())
          {  // don't compare 1 and 3 as interaction is
             // sorted
...
#endif
    {
      ...
      {                   // if permutations mode is disabeled then namespaces in ns are already
        ...
        {
          fgd2 = fgd - 1;
          fgd->self_interaction = (fgd->ft_arr == fgd2->ft_arr);  // state_data.begin().self_interaction is always
                                                                  // false
      ...
      while (do_it)
      {
        ...
        {                     // last namespace - iterate its features and go back
          ...
          // 'cur_data.loop_idx > cur_data.loop_end' -> exit the while loop
        }  // if last namespace
      }    // while do_it
    }
