...
std::vector<std::tuple<std::pair<features::const_audit_iterator, features::const_audit_iterator>,
    std::pair<features::const_audit_iterator,
        features::const_audit_iterator>>> inline generate_quadratic_combinations(const std::array<features,
                                                                                     NUM_NAMESPACES>& feature_groups,
    namespace_index ns_idx1, namespace_index ns_idx2)
{
  if (!feature_groups[ns_idx1].empty() && !feature_groups[ns_idx2].empty())
  {
...
std::vector<std::tuple<std::pair<features::const_audit_iterator, features::const_audit_iterator>,
    ...
    std::pair<features::const_audit_iterator,
        features::const_audit_iterator>>> inline generate_cubic_combinations(const std::array<features, NUM_NAMESPACES>&
                                                                                 feature_groups,
    namespace_index ns_idx1, namespace_index ns_idx2, namespace_index ns_idx3)
{
  if (!feature_groups[ns_idx1].empty() && !feature_groups[ns_idx2].empty() && !feature_groups[ns_idx3].empty())
  {
    return {std::make_tuple(std::make_pair(feature_groups[ns_idx1].audit_begin(), feature_groups[ns_idx1].audit_end()),
        std::make_pair(feature_groups[ns_idx2].audit_begin(), feature_groups[ns_idx2].audit_end()),
...
std::vector<std::vector<std::pair<features::const_audit_iterator,
    features::const_audit_iterator>>> inline generate_generic_combinations(const std::array<features, NUM_NAMESPACES>&
                                                                               feature_groups,
    ...
    size_t& num_features)  // default value removed to eliminate ambiguity in old complers
{
  ...
  {  // current list of namespaces to interact.
    ...
    {
      for (auto& range : generate_quadratic_combinations(ec.feature_space, ns[0], ns[1]))
      {
        auto& first_begin = std::get<0>(range).first;
        auto& first_end = std::get<0>(range).second;
        auto& second_begin = std::get<1>(range).first;
        ...
        for (; first_begin != first_end; ++first_begin)
        {
          ...
          if (audit) { audit_func(dat, first_begin.audit() != nullptr ? first_begin.audit() : &EMPTY_AUDIT_STRINGS); }
          ...
          inner_kernel<DataT, WeightOrIndexT, FuncT, audit, audit_func>(
              dat, begin, second_end, offset, weights, first_begin.value(), halfhash);
    ...
    else if (len == 3)  // special case for triples
    {
      for (auto& range : generate_cubic_combinations(ec.feature_space, ns[0], ns[1], ns[3]))
      {
        ...
        auto& second_begin = std::get<1>(range).first;
        auto& second_end = std::get<1>(range).second;
        auto& third_begin = std::get<2>(range).first;
...
#endif
    {
      for (auto& range : generate_generic_combinations(ec.feature_space, ns))
      {
        state_data.clear();
        // preparing state data
        for (auto r : range) { state_data.emplace_back(r.first, r.second); }
        ...
        while (do_it)
        {
          ...
          else
          {
            ...
            inner_kernel<DataT, WeightOrIndexT, FuncT, audit, audit_func, WeightsT>(
                dat, begin, cur_data->end_it, offset, weights, ft_value, halfhash);
