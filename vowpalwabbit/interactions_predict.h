...
template <class DataT, class FeatureIndexOrValueT, void (*FuncT)(DataT&, float, FeatureIndexOrValueT), bool audit, void (*audit_func)(DataT&, const audit_strings*), class WeightsT>
    ...
    const uint64_t offset, WeightsT& weights, feature_value ft_value, feature_index halfhash)
{
  if (audit)
  {
    for (; begin != end; ++begin)
    {
      ...
      call_FuncT<DataT, FuncT>(dat, weights, INTERACTION_VALUE(ft_value, begin.value()), (begin.index() ^ halfhash) + offset);
...
template <class DataT, class FeatureIndexOrValueT, void (*FuncT)(DataT&, float, FeatureIndexOrValueT), bool audit, void (*audit_func)(DataT&, const audit_strings*),
      ...
      if (first.nonempty())
      {
        ...
        if (second.nonempty())
        {
          ...
          for (size_t i = 0; i < first.indicies.size(); ++i)
          {
            ...
            inner_kernel<DataT, FeatureIndexOrValueT, FuncT, audit, audit_func>(dat, begin, end, offset, weights, ft_value, halfhash);
...
#endif
    {
      ...
      while (do_it)
      {
        ...
        else
        {
          ...
          inner_kernel<DataT, FeatureIndexOrValueT, FuncT, audit, audit_func, WeightsT>(dat, begin, end, offset, weights, ft_value, halfhash);
