 ...
 * Previous behaviour was: include interactions of feature with itself only if
 * its value != value^2.
 *
 */
...
// 3 template functions to pass T() proper argument (feature idx in regressor,
// or its coefficient)

template <class R, void (*T)(R &, const float, float &), class W>
inline void call_T(R &dat, W &weights, const float ft_value,
                   const uint64_t ft_idx) {
  T(dat, ft_value, weights[ft_idx]);
}

template <class R, void (*T)(R &, const float, const float &), class W>
inline void call_T(R &dat, const W &weights, const float ft_value,
                   const uint64_t ft_idx) {
  T(dat, ft_value, weights[ft_idx]);
}
...
struct feature_gen_data {
  ...
  uint64_t
      hash; // hash of feature interactions of previous namespaces in the list
  float x;  // value of feature interactions of previous namespaces in the list
  size_t loop_end; // last feature id. May be less than number of features if
                   // namespace involved in interaction more
  // than once calculated at preprocessing together with same_ns
  size_t self_interaction; // namespace interacting with itself
  features *ft_arr;
  //    feature_gen_data(): loop_idx(0), x(1.), loop_end(0),
...
// triple interactions
// end switch to usage of non-recursive feature generation algorithm for
// interactions of any length

// #define GEN_INTER_LOOP

template <class R, class S, void (*T)(R &, float, S), bool audit,
          void (*audit_func)(R &, const audit_strings *), class W>
inline void inner_kernel(R &dat, features::iterator_all &begin,
                         features::iterator_all &end, const uint64_t offset,
                         W &weights, feature_value ft_value,
                         feature_index halfhash) {
  if (audit) {
    for (; begin != end; ++begin) {
      audit_func(dat, begin.audit().get());
      call_T<R, T>(dat, weights, INTERACTION_VALUE(ft_value, begin.value()),
                   (begin.index() ^ halfhash) + offset);
...
// this templated function generates new features for given example and set of
// interactions
// and passes each of them to given function T()
// it must be in header file to avoid compilation problems
template <
    class R, class S, void (*T)(R &, float, S), bool audit,
    void (*audit_func)(R &, const audit_strings *),
    class W> // nullptr func can't be used as template param in old compilers
inline void
generate_interactions(
    std::vector<std::string> &interactions, bool permutations,
    ...
    W &weights) // default value removed to eliminate ambiguity in old complers
{
  ...
  //    const uint64_t stride_shift = all.stride_shift; // it seems we don't
  //    need stride shift in FTRL-like hash

  // statedata for generic non-recursive iteration
  v_array<feature_gen_data> state_data = v_init<feature_gen_data>();

  feature_gen_data empty_ns_data; // micro-optimization. don't want to call its
                                  // constructor each time in loop.
  ...
  for (auto &ns : interactions) { // current list of namespaces to interact.

#ifndef GEN_INTER_LOOP

    // unless GEN_INTER_LOOP is defined we use nested 'for' loops for
    // interactions length 2 (pairs) and 3 (triples)
    // and generic non-recursive algorythm for all other cases.
    // nested 'for' loops approach is faster, but can't be used for interation
    // of any length.
    ...
    } else if (len == 3) // special case for triples
    {
      ...
      if (first.nonempty()) {
        ...
        if (second.nonempty()) {
          ...
          if (third.nonempty()) { // don't compare 1 and 3 as interaction is
                                  // sorted
            ...
            for (size_t i = 0; i < first.indicies.size(); ++i) {
              if (audit)
                audit_func(dat, first.space_names[i].get());
              const uint64_t halfhash1 =
                  FNV_prime * (uint64_t)first.indicies[i];
              const float &first_ft_value = first.values[i];
              size_t j = 0;
              if (same_namespace1) // next index differs for permutations and
                                   // simple combinations
                j = (PROCESS_SELF_INTERACTIONS(first_ft_value)) ? i : i + 1;

              for (; j < second.indicies.size(); ++j) { // f3 x k*(f2 x k*f1)
                if (audit)
                  audit_func(dat, second.space_names[j].get());
                feature_index halfhash =
                    FNV_prime * (halfhash1 ^ (uint64_t)second.indicies[j]);
                feature_value ft_value =
                    INTERACTION_VALUE(first_ft_value, second.values[j]);

                features::features_value_index_audit_range range =
                    third.values_indices_audit();
                features::iterator_all begin = range.begin();
                if (same_namespace2) // next index differs for permutations and
                                     // simple combinations
...
#endif
    {
      ...
      for (namespace_index n : ns) {
        ...
        fgd->loop_end =
            ft_cnt - 1; // saving number of features for each namespace
        fgd->ft_arr = &ft;
        ++fgd;
      }

      // if any of interacting namespace has 0 features - whole interaction is
      // skipped
      if (must_skip_interaction)
        continue; // no_data_to_interact

      if (!permutations) // adjust state_data for simple combinations
      { // if permutations mode is disabeled then namespaces in ns are already
        // sorted and thus grouped
        // (in fact, currently they are sorted even for enabled permutations
        // mode)
        // let's go throw the list and calculate number of features to skip in
        // namespaces which
        // repeated more than once to generate only simple combinations of
        // features

        size_t margin =
            0; // number of features to ignore if namespace has been seen before
        ...
        for (fgd = state_data.end() - 1; fgd > state_data.begin(); --fgd) {
          ...
          fgd->self_interaction =
              (fgd->ft_arr ==
               fgd2->ft_arr); // state_data.begin().self_interaction is always
                              // false
          if (fgd->self_interaction) {
            size_t &loop_end = fgd2->loop_end;

            if (!PROCESS_SELF_INTERACTIONS(
                    (*fgd2->ft_arr).values[loop_end - margin])) {
        ...
        // if impossible_without_permutations == true then we faced with case
        // like interaction 'aaaa'
        // where namespace 'a' contains less than 4 unique features. It's
        // impossible to make simple
      ...
      fgd->loop_idx = 0; // loop_idx contains current feature id for curently
                         // processed namespace.

      // beware: micro-optimization.
      /* start & end are always point to features in last namespace of
      interaction.
      ...
      while (do_it) {
        if (cur_data <
            fgd2) // can go further threw the list of namespaces in interaction
        {
          feature_gen_data *next_data = cur_data + 1;
          size_t feature = cur_data->loop_idx;
          features &fs = *(cur_data->ft_arr);

          if (next_data->self_interaction) { // if next namespace is same, we
                                             // should start with loop_idx + 1
                                             // to avoid feature interaction
                                             // with itself
            // unless feature has value x and x != x*x. E.g. x != 0 and x != 1.
            // Features with x == 0 are already
            // filtered out in parce_args.cc::maybeFeature().

            next_data->loop_idx =
                (PROCESS_SELF_INTERACTIONS(fs.values[feature]))
                    ? cur_data->loop_idx
                    : cur_data->loop_idx + 1;
          } else
          ...
          } else { // feature2 xor (16777619*feature1)
            next_data->hash =
                FNV_prime * (cur_data->hash ^ (uint64_t)fs.indicies[feature]);
            next_data->x = INTERACTION_VALUE(fs.values[feature], cur_data->x);
          }

          ++cur_data;
        } else { // last namespace - iterate its features and go back
          ...
          features::features_value_index_audit_range range =
              fs.values_indices_audit();
          features::iterator_all begin = range.begin();
          begin += start_i;
          features::iterator_all end = range.begin();
          end += fgd2->loop_end + 1;
          inner_kernel<R, S, T, audit, audit_func, W>(
              dat, begin, end, offset, weights, ft_value, halfhash);
          ...
          do {
            --cur_data;
            go_further = (++cur_data->loop_idx >
                          cur_data->loop_end); // increment loop_idx
            if (audit)
              audit_func(dat, nullptr);
          } while (go_further && cur_data != fgd);

          do_it = !(cur_data == fgd && go_further);
          // if do_it==false - we've reached 0 namespace but its
          // 'cur_data.loop_idx > cur_data.loop_end' -> exit the
          // while loop
        } // if last namespace
      }   // while do_it
    }
