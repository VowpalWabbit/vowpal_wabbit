...
{
  // current feature id in namespace
  size_t loop_idx = 0;

  // hash of feature interactions of previous namespaces in the list
  uint64_t hash;
  // value of feature interactions of previous namespaces in the list
  float x = 1.f;

  // last feature id. May be less than number of features if namespace involved in interaction more
  // than once calculated at preprocessing together with same_ns
  size_t loop_end = 0;

  // namespace interacting with itself
  bool self_interaction = false;
  features* ft_arr = nullptr;
};
    ...
    size_t& num_features)  // default value removed to eliminate ambiguity in old complers
{
  ...
  {  // current list of namespaces to interact.
...
#endif
    {
      ...
      if (!permutations)
      {
        // adjust state_data for simple combinations
