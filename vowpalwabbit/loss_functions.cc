...
public:
    ...
    if (prediction <= sd->max_label && prediction >= sd->min_label) {
      ...
      if (label == sd->min_label)
        ...
        return (float)((label - sd->min_label) * (label - sd->min_label) +
                       2. * (label - sd->min_label) *
                           (sd->min_label - prediction));
  ...
  float getUpdate(float prediction, float label, float update_scale,
    ...
    return (label - prediction) *
           (1.f - correctedExp(-2.f * update_scale * pred_per_update)) /
           pred_per_update;
  }
...
public:
  ...
  float getLoss(shared_data *, float prediction, float label) {
    if (label != -1.f && label != 1.f)
      cout << "You are using label " << label
           << " not -1 or 1 as loss function expects!" << endl;
  ...
  float getUpdate(float prediction, float label, float update_scale,
    ...
    return label * (update_scale * pred_per_update < err
                        ? update_scale
                        : err / pred_per_update);
  }
...
public:
  ...
  inline float wexpmx(float x) {
     ...
     */
    double w = x >= 1. ? 0.86 * x + 0.01
                       : correctedExp(0.8 * x - 0.65);        // initial guess
...
public:
  ...
  float getLoss(shared_data *, float prediction, float label) {
    if (label < 0.f)
      cout << "You are using label " << label
           << " but loss function expects label >= 0!" << endl;
    float exp_prediction = expf(prediction);
    // deviance is used instead of log-likelihood
    return 2 * (label * (logf(label + 1e-6f) - prediction) -
                (label - exp_prediction));
  }
  ...
  float getUpdate(float prediction, float label, float update_scale,
    ...
    if (label > 0) {
      return label * update_scale -
             log1p(exp_prediction *
                   expm1(label * update_scale * pred_per_update) / label) /
                 pred_per_update;
    } else {
      return -log1p(exp_prediction * update_scale * pred_per_update) /
             pred_per_update;
    }
  }
  ...
  float getRevertingWeight(shared_data * /* sd */, float /* prediction */,
                           float /* eta_t */) {
...
loss_function *getLossFunction(vw &all, string funcName,
  ...
  if (funcName.compare("squared") == 0 || funcName.compare("Huber") == 0)
    ...
    return new logloss();
  } else if (funcName.compare("quantile") == 0 ||
             funcName.compare("pinball") == 0 ||
             funcName.compare("absolute") == 0) {
