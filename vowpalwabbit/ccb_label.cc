  ...
  size_t size =
      ...
      + (ld->outcome == nullptr
             ...
             : sizeof(ld->outcome->cost) // cost
                   ...
                   +
                   sizeof(ACTION_SCORE::action_score) *
                       ld->outcome->probabilities.size()) // probabilities
...
void default_label(void *v) {
  ...
  // This is tested against nullptr, so unfortunately as things are this must be
...
void copy_label(void *dst, void *src) {
  ...
  copy_array(ldDst->explicit_included_actions,
             ldSrc->explicit_included_actions);
  ldDst->type = ldSrc->type;
  ldDst->weight = ldSrc->weight;
}

ACTION_SCORE::action_score convert_to_score(const substring &action_id_str,
  ...
  if (probability > 1.0) {
    std::cerr << "invalid probability > 1 specified for an outcome, resetting "
                 "to 1.\n";
    probability = 1.0;
  }
  if (probability < 0.0) {
    std::cerr << "invalid probability < 0 specified for an outcome, resetting "
                 "to 0.\n";
    probability = .0;
  }
...
CCB::conditional_contextual_bandit_outcome *parse_outcome(substring &outcome) {
  ...
  ccb_outcome.probabilities.push_back(
...
void parse_explicit_inclusions(CCB::label *ld,
...
void parse_label(parser *p, shared_data *, void *v, v_array<substring> &words) {
  ...
  if (substring_equal(type, "shared")) {
    ...
    // If a full distribution has been given, check if it sums to 1, otherwise
    // throw.
    if (ld->outcome && ld->outcome->probabilities.size() > 1) {
      float total_pred = std::accumulate(
          ld->outcome->probabilities.begin(), ld->outcome->probabilities.end(),
          0.f, [](float result_so_far, ACTION_SCORE::action_score action_pred) {
            return result_so_far + action_pred.score;
          });

      // TODO do a proper comparison here.
      if (total_pred > 1.1f || total_pred < 0.9f) {
        THROW("When providing all predicition probabilties they must add up to "
              "1.f");
      }
    }
  } else {
...
label_parser ccb_label_parser = {
    default_label, parse_label, cache_label, read_cached_label, delete_label,
    ccb_weight,    copy_label,  test_label,  sizeof(CCB::label)};
} // namespace CCB
