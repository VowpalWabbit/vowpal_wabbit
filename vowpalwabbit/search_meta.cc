...
void run(Search::search &sch, multi_ex &ec) {
  sch.base_task(ec)
      .foreach_action([](Search::search & /*sch*/, size_t t, float min_cost,
        ...
        cerr << "==DebugMT== foreach_action(t=" << t
      ...
      .post_prediction([](Search::search & /*sch*/, size_t t, action a,
                          float a_cost) -> void {
        cerr << "==DebugMT== post_prediction(t=" << t << ", a=" << a
             << ", a_cost=" << a_cost << ")" << endl;
      })

      .maybe_override_prediction([](Search::search & /*sch*/, size_t t,
                                    action &a, float &a_cost) -> bool {
        cerr << "==DebugMT== maybe_override_prediction(t=" << t << ", a=" << a
             << ", a_cost=" << a_cost << ")" << endl;
        return false;
      })
...
void run(Search::search &sch, multi_ex &ec) {
...

  {
    ...
    d.final.push_back(
        make_pair(make_pair(d.total_cost, original_final), d.output_string));
  }
  ...
  for (size_t i = 0; i < min(d.max_branches, d.branches.size()); i++) {
...

    {
      ...
      d.final.push_back(
          make_pair(make_pair(d.total_cost, this_final), d.output_string));
    }
  }

  // sort the finals by cost
  stable_sort(d.final.begin(), d.final.end(),
              [](const pair<branch, string *> &a,
                 const pair<branch, string *> &b) -> bool {
