...
Search::search_metatask metatask = {"debug", run,     nullptr,
                                    nullptr, nullptr, nullptr};

void run(Search::search &sch, multi_ex &ec) {
  sch.base_task(ec)
      .foreach_action([](Search::search & /*sch*/, size_t t, float min_cost,
                         action a, bool taken, float a_cost) -> void {
        std::cerr << "==DebugMT== foreach_action(t=" << t
                  << ", min_cost=" << min_cost << ", a=" << a
                  << ", taken=" << taken << ", a_cost=" << a_cost << ")"
                  << std::endl;
      })

      .post_prediction([](Search::search & /*sch*/, size_t t, action a,
                          float a_cost) -> void {
        std::cerr << "==DebugMT== post_prediction(t=" << t << ", a=" << a
                  << ", a_cost=" << a_cost << ")" << std::endl;
      })

      .maybe_override_prediction([](Search::search & /*sch*/, size_t t,
                                    action &a, float &a_cost) -> bool {
        std::cerr << "==DebugMT== maybe_override_prediction(t=" << t
...
Search::search_metatask metatask = {
    "selective_branching", run, initialize, finish, nullptr, nullptr};
...
void initialize(Search::search &sch, size_t & /*num_actions*/,
  ...
  new_options
      ...
      .add(make_option("search_kbest", kbest)
               .default_value(0)
               .help("number of best items to output (0=just like "
                     "non-selectional-branching, default)"));
...
void run(Search::search &sch, multi_ex &ec) {
  ...
  sch.base_task(ec)
      .foreach_action([](Search::search &sch, size_t t, float min_cost,
                         action a, bool taken, float a_cost) -> void {
        cdbg << "==DebugMT== foreach_action(t=" << t
             << ", min_cost=" << min_cost << ", a=" << a << ", taken=" << taken
             << ", a_cost=" << a_cost << ")" << std::endl;
        if (taken)
      ...
      .with_output_string(
          [](Search::search &sch, std::stringstream &output) -> void {
            sch.get_metatask_data<task_data>()->output_string =
                new std::string(output.str());
          })
      .Run();
...

  {
    ...
    d.final.push_back(std::make_pair(
        std::make_pair(d.total_cost, original_final), d.output_string));
  }

  // sort the branches by cost
  stable_sort(d.branches.begin(), d.branches.end(),
              [](const branch &a, const branch &b) -> bool {
  ...
  for (size_t i = 0; i < std::min(d.max_branches, d.branches.size()); i++) {
    ...
    sch.base_task(ec)
        .foreach_action([](Search::search & /*sch*/, size_t /*t*/,
                           float /*min_cost*/, action /*a*/, bool /*taken*/,
                           float /*a_cost*/) -> void {})
        .maybe_override_prediction([](Search::search &sch, size_t t, action &a,
                                      float &a_cost) -> bool {
          task_data &d = *sch.get_metatask_data<task_data>();
...

    {
      ...
      d.final.push_back(std::make_pair(std::make_pair(d.total_cost, this_final),
                                       d.output_string));
    }
  }
  ...
  stable_sort(d.final.begin(), d.final.end(),
              [](const std::pair<branch, std::string *> &a,
                 const std::pair<branch, std::string *> &b) -> bool {
  ...
  if (d.output_string && (d.kbest > 0)) {
    ...
    for (size_t i = 0; i < std::min(d.final.size(), d.kbest); i++)
      (*d.kbest_out) << *d.final[i].second << "\t" << d.final[i].first.first
                     << std::endl;
  }
