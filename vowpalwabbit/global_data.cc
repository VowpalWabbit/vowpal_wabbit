...
void compile_gram(std::vector<std::string> grams,
  ...
  for (size_t i = 0; i < grams.size(); i++) {
    ...
    if (isdigit(ngram[0])) {
      ...
      if (!quiet)
        std::cerr << "Generating " << n << "-" << descriptor
                  << " for all namespaces." << std::endl;
      for (size_t j = 0; j < 256; j++)
        dest[j] = n;
    } else if (ngram.size() == 1)
      std::cout << "You must specify the namespace index before the n"
                << std::endl;
    else {
      int n = atoi(ngram.c_str() + 1);
      dest[(uint32_t)(unsigned char)*ngram.c_str()] = n;
      if (!quiet)
        std::cerr << "Generating " << n << "-" << descriptor << " for "
                  << ngram[0] << " namespaces." << std::endl;
    }
  }
}
...
void compile_limits(std::vector<std::string> limits,
  ...
  for (size_t i = 0; i < limits.size(); i++) {
    ...
    if (isdigit(limit[0])) {
      ...
      if (!quiet)
        std::cerr << "limiting to " << n << "features for each namespace."
                  << std::endl;
      for (size_t j = 0; j < 256; j++)
        dest[j] = n;
      ...
      if (!quiet)
        std::cerr << "limiting to " << n << " for namespaces " << limit[0]
                  << std::endl;
    }
  }
}
