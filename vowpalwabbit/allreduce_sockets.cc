...
socket_t AllReduceSockets::sock_connect(const uint32_t ip, const int port) {
...

  {
    ...
    if (nullptr ==
        inet_ntop(AF_INET, &(far_end.sin_addr), dotted_quad, INET_ADDRSTRLEN))
      THROWERRNO("inet_ntop");

    char hostname[NI_MAXHOST];
    char servInfo[NI_MAXSERV];
    if (getnameinfo((sockaddr *)&far_end, sizeof(sockaddr), hostname,
                    NI_MAXHOST, servInfo, NI_MAXSERV, NI_NUMERICSERV))
      THROWERRNO("getnameinfo(" << dotted_quad << ")");

    if (!quiet)
      cerr << "connecting to " << dotted_quad << " = " << hostname << ':'
           << ntohs(port) << endl;
  }

  size_t count = 0;
  int ret;
  while ((ret = connect(sock, (sockaddr *)&far_end, sizeof(far_end))) == -1 &&
         count < 100) {
    count++;
...
// of VW on the same machine to potentially contact the wrong tree node.
  ...
  int enableTKA = 1;
  if (setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (char *)&enableTKA,
                 sizeof(enableTKA)) < 0) {
    if (!quiet)
...
void AllReduceSockets::all_reduce_init() {
  ...
  if (send(master_sock, (const char *)&unique_id, sizeof(unique_id), 0) <
      (int)sizeof(unique_id)) {
    THROW("write unique_id=" << unique_id << " to span server failed");
  } else {
    if (!quiet)
      cerr << "wrote unique_id=" << unique_id << endl;
  }
  if (send(master_sock, (const char *)&total, sizeof(total), 0) <
      (int)sizeof(total)) {
  ...
  if (recv(master_sock, (char *)&kid_count, sizeof(kid_count), 0) <
      (int)sizeof(kid_count)) {
  ...
  if (send(master_sock, (const char *)&netport, sizeof(netport), 0) <
      (int)sizeof(netport))
    THROW("write netport failed!");

  if (recv(master_sock, (char *)&parent_ip, sizeof(parent_ip), 0) <
      (int)sizeof(parent_ip)) {
    THROW("read parent_ip failed!");
  } else {
    char dotted_quad[INET_ADDRSTRLEN];
    if (nullptr ==
        inet_ntop(AF_INET, (char *)&parent_ip, dotted_quad, INET_ADDRSTRLEN)) {
      if (!quiet)
        cerr << "read parent_ip=" << parent_ip
             << "(inet_ntop: " << strerror(errno) << ")" << endl;
    } else {
      if (!quiet)
        cerr << "read parent_ip=" << dotted_quad << endl;
    }
  }
  if (recv(master_sock, (char *)&parent_port, sizeof(parent_port), 0) <
      (int)sizeof(parent_port)) {
  ...
  for (int i = 0; i < kid_count; i++) {
    ...
    // char hostname[NI_MAXHOST];
    // char servInfo[NI_MAXSERV];
    // getnameinfo((sockaddr *) &child_address, sizeof(sockaddr), hostname,
    // NI_MAXHOST, servInfo, NI_MAXSERV,
    // NI_NUMERICSERV); cerr << "connected to " << hostname << ':' <<
    // ntohs(port) << endl;
    socks.children[i] = f;
  }
...
void AllReduceSockets::pass_down(char *buffer, const size_t parent_read_pos,
                                 size_t &children_sent_pos) {
  size_t my_bufsize =
      std::min(ar_buf_size, (parent_read_pos - children_sent_pos));

  if (my_bufsize > 0) {
    // going to pass up this chunk of data to the children
    if (socks.children[0] != -1 &&
        send(socks.children[0], buffer + children_sent_pos, (int)my_bufsize,
             0) < (int)my_bufsize) {
      THROW("Write to left child failed");
    }
    if (socks.children[1] != -1 &&
        send(socks.children[1], buffer + children_sent_pos, (int)my_bufsize,
             0) < (int)my_bufsize) {
...
void AllReduceSockets::broadcast(char *buffer, const size_t n) {
  ...
  while (parent_read_pos < n || children_sent_pos < n) {
    ...
    if (socks.parent != -1) {
      ...
      int read_size =
          recv(socks.parent, buffer + parent_read_pos, (int)count, 0);
      if (read_size == -1) {
