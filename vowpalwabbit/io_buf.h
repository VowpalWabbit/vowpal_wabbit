...
public:
  v_array<char> space; // space.begin = beginning of loaded values.  space.end =
                       // end of read or written values from/to
                       // the buffer.
  v_array<int> files;
  ...
  virtual int open_file(const char *name, bool stdin_off, int flag = READ) {
    ...
    case READ:
      if (*name != '\0') {
#ifdef _WIN32
        // _O_SEQUENTIAL hints to OS that we'll be reading sequentially, so
        // cache aggressively.
        _sopen_s(&ret, name, _O_RDONLY | _O_BINARY | _O_SEQUENTIAL, _SH_DENYWR,
                 0);
#else
    ...
    case WRITE:
#ifdef _WIN32
      _sopen_s(&ret, name, _O_CREAT | _O_WRONLY | _O_BINARY | _O_TRUNC,
  ...
  ssize_t fill(int f) { // if the loaded values have reached the allocated space
    if (space.end_array - space.end() ==
        0) { // reallocate to twice as much space
    ...
    if (num_read >= 0) { // if some bytes were actually loaded, update the end
                         // of loaded values
  ...
  virtual void flush() {
    if (!files.empty()) {
      if (write_file(files[0], space.begin(), head - space.begin()) !=
          (int)(head - space.begin()))
  ...
  // if read_message is null, just read it in.  Otherwise do a comparison and
  // barf on read_message.
  size_t bin_read_fixed(char *data, size_t len, const char *read_message) {
    if (len > 0) {
      char *p;
      // if the model is corrupt the number of bytes can be less then specified
      // (as there isn't enought data available
      // in the file)
      len = buf_read(p, len);
...
inline size_t
bin_text_read_write_fixed_validated(io_buf &io, char *data, size_t len,
                                    const char *read_message, bool read,
                                    std::stringstream &msg, bool text) {
  size_t nbytes =
      bin_text_read_write_fixed(io, data, len, read_message, read, msg, text);
...
#define writeit(what, str)                                                     \
  do {                                                                         \
    msg << str << " = " << what << " ";                                        \
    bin_text_read_write_fixed(model_file, (char *)&what, sizeof(what), "",     \
                              read, msg, text);                                \
  } while (0);
...
#define writeitvar(what, str, mywhat)                                          \
  ...
  do {                                                                         \
    msg << str << " = " << mywhat << " ";                                      \
    bin_text_read_write_fixed(model_file, (char *)&mywhat, sizeof(mywhat), "", \
                              read, msg, text);                                \
  } while (0);
