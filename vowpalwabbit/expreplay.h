...
namespace ExpReplay {
template <label_parser &lp> struct expreplay {
  vw *all;
  std::shared_ptr<rand_state> _random_state;
  size_t N;            // how big is the buffer?
  example *buf;        // the deep copies of examples (N of them)
  bool *filled;        // which of buf[] is filled
  size_t replay_count; // each time er.learn() is called, how many times do we
                       // call base.learn()? default=1 (in which
...
void predict_or_learn(
    expreplay<lp> &er, LEARNER::single_learner &base,
    example &ec) { // regardless of what happens, we must predict
  base.predict(ec);
  ...
  for (size_t replay = 1; replay < er.replay_count; replay++) {
    size_t n =
        (size_t)(er._random_state->get_and_update_random() * (float)er.N);
    if (er.filled[n])
      base.learn(er.buf[n]);
  }
...
void multipredict(expreplay<lp> &, LEARNER::single_learner &base, example &ec,
...
template <label_parser &lp>
void end_pass(expreplay<lp> &er) { // we need to go through and learn on
                                   // everyone who remains
  // also need to clean up remaining examples
  for (size_t n = 0; n < er.N; n++)
    if (er.filled[n]) { // TODO: if er.replay_count > 1 do we need to play these
                        // more?
...
LEARNER::base_learner *expreplay_setup(VW::config::options_i &options,
                                       vw &all) {
  ...
  new_options
      .add(VW::config::make_option(replay_string, er->N)
               .keep()
               .help("use experience replay at a specified level "
      ...
      .add(VW::config::make_option(replay_count_string, er->replay_count)
               .default_value(1)
               .help("how many times (in expectation) should each example be "
  ...
  if (!all.quiet)
    std::cerr << "experience replay level=" << er_level << ", buffer=" << er->N
              << ", replay count=" << er->replay_count << std::endl;

  er->base = LEARNER::as_singleline(setup_base(options, all));
  LEARNER::learner<expreplay<lp>, example> *l = &init_learner(
      er, er->base, predict_or_learn<true, lp>, predict_or_learn<false, lp>);
