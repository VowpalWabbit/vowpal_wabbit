...
void predict_or_learn(
    expreplay &er, LEARNER::single_learner &base,
...
void multipredict(expreplay &, LEARNER::single_learner &base, example &ec,
...
void end_pass(
    expreplay &er) { // we need to go through and learn on everyone who remains
...
template <label_parser &lp> void finish(expreplay &er) {
  for (size_t n = 0; n < er.N; n++) {
...
LEARNER::base_learner *expreplay_setup(VW::config::options_i &options,
  ...
  LEARNER::learner<expreplay, example> *l = &init_learner(
