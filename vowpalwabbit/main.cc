...
vw *setup(options_i &options) {
  ...
  if (!all->quiet && !all->bfgs && !all->searchstr &&
    ...
    all->trace_message << std::left << std::setw(shared_data::col_avg_loss)
                       << std::left << "average"
                       << " " << std::setw(shared_data::col_since_last)
                       << std::left << "since"
                       << " " << std::right
                       ...
                       << "current"
                       << " " << std::setw(shared_data::col_current_predict)
                       << "current"
                       << " " << std::setw(shared_data::col_current_features)
                       << "current" << std::endl;
    all->trace_message << std::left << std::setw(shared_data::col_avg_loss)
                       << std::left << "loss"
                       << " " << std::setw(shared_data::col_since_last)
  ...
  try {
    ...
    if (argc == 3 && !strcmp(argv[1], "--args")) {
      ...
      while (std::getline(arg_file, line)) {
        ...
        std::unique_ptr<options_boost_po> ptr(
            new options_boost_po(l_argc, l_argv));
    ...
    std::cerr << "vw (" << e.Filename() << ":" << e.LineNumber()
              << "): " << e.what() << std::endl;
    exit(1);
