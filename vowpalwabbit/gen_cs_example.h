...
void gen_cs_example_ips(cb_to_cs &c, CB::label &ld,
                        COST_SENSITIVE::label &cs_ld, float clip_p = 0.f);

template <bool is_learn>
void gen_cs_example_dm(
    cb_to_cs &c, example &ec,
    COST_SENSITIVE::label &cs_ld) { // this implements the direct estimation
  ...
  if (ld.costs.size() == 0 ||
      (ld.costs.size() == 1 && ld.costs[0].cost != FLT_MAX)) // this is a
                                                             // typical example
                                                             // where we can
                                                             // perform all
                                                             // actions
  ...
  { // in this case generate cost-sensitive example with only allowed actions
    for (auto &cl : ld.costs) {
      ...
      // get cost prediction for this action
      wc.x = CB_ALGS::get_cost_pred<is_learn>(c.scorer, c.known_cost, ec,
                                              cl.action, 0);
...
template <bool is_learn>
void gen_cs_label(cb_to_cs &c, example &ec, COST_SENSITIVE::label &cs_ld,
                  uint32_t action, float clip_p = 0.f) {
  COST_SENSITIVE::wclass wc = {0., action, 0., 0.};

  // get cost prediction for this action
  wc.x = CB_ALGS::get_cost_pred<is_learn>(c.scorer, c.known_cost, ec, action,
                                          c.num_actions);
  ...
  if (c.known_cost != nullptr && c.known_cost->action == action) {
    ...
    c.last_pred_reg = wc.x;
    c.last_correct_cost = c.known_cost->cost;
    wc.x += (c.known_cost->cost - wc.x) /
            std::max(c.known_cost->probability, clip_p);
  }
...
void gen_cs_example_dr(
    cb_to_cs &c, example &ec, CB::label &ld, COST_SENSITIVE::label &cs_ld,
    float /*clip_p*/ = 0.f) { // this implements the doubly robust method
  cs_ld.costs.clear();
  c.pred_scores.costs.clear();
  if (ld.costs.size() == 0) // a test example
    for (uint32_t i = 1; i <= c.num_actions;
         i++) { // Explicit declaration for a weak compiler.
      COST_SENSITIVE::wclass temp = {FLT_MAX, i, 0., 0.};
      cs_ld.costs.push_back(temp);
    }
  else if (ld.costs.size() == 0 ||
           (ld.costs.size() == 1 && ld.costs[0].cost != FLT_MAX))
...
template <bool is_learn>
void gen_cs_example(cb_to_cs &c, example &ec, CB::label &ld,
...
void gen_cs_example_mtr(cb_to_cs_adf &c, multi_ex &ec_seq,
                        COST_SENSITIVE::label &cs_labels);

void gen_cs_example_sm(multi_ex &examples, uint32_t chosen_action,
                       float sign_offset,
                       ACTION_SCORE::action_scores action_vals,
                       COST_SENSITIVE::label &cs_labels);

template <bool is_learn>
void gen_cs_example_dr(cb_to_cs_adf &c, multi_ex &examples,
                       COST_SENSITIVE::label &cs_labels,
                       float clip_p = 0.f) { // size_t mysize = examples.size();
  c.pred_scores.costs.clear();
  ...
  for (size_t i = 0; i < examples.size(); i++) {
    ...
    if (c.known_cost.action == i) {
      ...
      wc.x = CB_ALGS::get_cost_pred<is_learn>(c.scorer, &(c.known_cost),
                                              *(examples[i]), 0, 2);
      c.known_cost.action = known_index;
    } else
      wc.x = CB_ALGS::get_cost_pred<is_learn>(c.scorer, nullptr, *(examples[i]),
                                              0, 2);
...
template <bool is_learn>
void gen_cs_example(cb_to_cs_adf &c, multi_ex &ec_seq,
                    COST_SENSITIVE::label &cs_labels) {
  switch (c.cb_type) {
  case CB_TYPE_IPS:
...
template <bool is_learn>
void call_cs_ldf(LEARNER::multi_learner &base, multi_ex &examples,
