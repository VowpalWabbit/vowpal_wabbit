...
//-----------------------------------------------------------------
template <bool is_learn>
void predict_or_learn_logistic(boosting &o, LEARNER::single_learner &base,
                               example &ec) {
  label_data &ld = ec.l.simple;
...
template <bool is_learn>
void predict_or_learn_adaptive(boosting &o, LEARNER::single_learner &base,
                               example &ec) {
  label_data &ld = ec.l.simple;
...
void save_load_sampling(boosting &o, io_buf &model_file, bool read, bool text) {
  ...
  stringstream os;
  os << "boosts " << o.N << endl;
  bin_text_read_write_fixed(model_file, (char *)&(o.N), sizeof(o.N), "", read,
                            os, text);
  ...
  for (int i = 0; i < o.N; i++)
    if (read) {
      ...
      bin_text_write_fixed(model_file, (char *)&(o.alpha[i]),
                           sizeof(o.alpha[i]), os2, text);
    }
  ...
  for (int i = 0; i < o.N; i++)
    if (read) {
      ...
      stringstream os2;
      os2 << "v " << o.v[i] << endl;
      bin_text_write_fixed(model_file, (char *)&(o.v[i]), sizeof(o.v[i]), os2,
                           text);
    }
  ...
  if (read) {
    ...
    cerr << "Saving alpha and v, current weighted_examples = "
         << o.all->sd->weighted_labeled_examples +
                o.all->sd->weighted_unlabeled_examples
         << endl;
  }
  for (int i = 0; i < o.N; i++) {
...
void save_load(boosting &o, io_buf &model_file, bool read, bool text) {
  ...
  if (!o.all->quiet) {
    if (read)
      ...
      cerr << "Saving alpha, current weighted_examples = "
           << o.all->sd->weighted_examples() << endl;
    for (int i = 0; i < o.N; i++)
      cerr << o.alpha[i] << " " << endl;
...
LEARNER::base_learner *boosting_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("boosting", data->N)
               .keep()
  ...
  data->C = std::vector<std::vector<int64_t>>(
      data->N, std::vector<int64_t>(data->N, -1));
  data->t = 0;
  data->all = &all;
  ...
  if (data->alg == "BBM")
    l = &init_learner<boosting, example>(
        data, as_singleline(setup_base(options, all)), predict_or_learn<true>,
        predict_or_learn<false>, data->N);
  else if (data->alg == "logistic") {
    l = &init_learner<boosting, example>(
        data, as_singleline(setup_base(options, all)),
        predict_or_learn_logistic<true>, predict_or_learn_logistic<false>,
        data->N);
    l->set_save_load(save_load);
  } else if (data->alg == "adaptive") {
    l = &init_learner<boosting, example>(
        data, as_singleline(setup_base(options, all)),
        predict_or_learn_adaptive<true>, predict_or_learn_adaptive<false>,
        data->N);
