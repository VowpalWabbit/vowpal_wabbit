...
// in order of scores. The probabilities
// are the probability with which each action should be replaced to the top of
// the list.
...
void predict_or_learn_first(cb_explore_adf &data, multi_learner &base,
                            multi_ex &examples) {
  // Explore tau times, then act according to optimal.
  if (is_learn && data.gen_cs.known_cost.probability < 1 &&
      test_adf_sequence(examples) != nullptr)
  ...
  enforce_minimum_probability(data.epsilon, true, begin_scores(preds),
                              end_scores(preds));
}

template <bool is_learn>
void predict_or_learn_greedy(cb_explore_adf &data, multi_learner &base,
                             multi_ex &examples) {
...
template <bool is_learn>
void predict_or_learn_regcb(cb_explore_adf &data, multi_learner &base,
                            multi_ex &examples) {
  if (is_learn && test_adf_sequence(examples) != nullptr) {
...
template <bool is_learn>
void predict_or_learn_bag(cb_explore_adf &data, multi_learner &base,
                          multi_ex &examples) {
  ...
  for (uint32_t i = 0; i < data.bag_size; i++) {
    ...
    uint32_t count =
        is_learn ? ((data.greedify && i == 0) ? 1 : BS::weight_gen(*data.all))
                 : 0;
  ...
  generate_bag(begin(data.top_actions), end(data.top_actions),
               begin_scores(data.action_probs), end_scores(data.action_probs));

  enforce_minimum_probability(data.epsilon, true,
                              begin_scores(data.action_probs),
                              end_scores(data.action_probs));
...
template <bool is_learn>
void predict_or_learn_cover(cb_explore_adf &data, multi_learner &base,
                            multi_ex &examples) {
  // Randomize over predictions from a base set of predictors
  ...
  const float min_prob =
      min(1.f / num_actions, 1.f / (float)sqrt(data.counter * num_actions));
  ...
  for (size_t i = 1; i < data.cover_size; i++) {
    ...
    if (is_learn) {
      ...
      for (uint32_t j = 0; j < num_actions; j++) {
        float pseudo_cost =
            data.cs_labels.costs[j].x -
            data.psi * min_prob / (max(probs[j].score, min_prob) / norm);
        data.cs_labels_2.costs.push_back({pseudo_cost, j, 0., 0.});
      }
      GEN_CS::call_cs_ldf<true>(*(data.cs_ldf_learner), examples,
                                data.cb_labels, data.cs_labels_2,
                                data.prepped_cs_labels, data.offset, i + 1);
    } else
      GEN_CS::call_cs_ldf<false>(*(data.cs_ldf_learner), examples,
                                 data.cb_labels, data.cs_labels,
                                 data.prepped_cs_labels, data.offset, i + 1);
  ...
  enforce_minimum_probability(min_prob * num_actions, !data.nounif,
                              begin_scores(probs), end_scores(probs));
...
template <bool is_learn>
void predict_or_learn_softmax(cb_explore_adf &data, multi_learner &base,
                              multi_ex &examples) {
  if (is_learn && test_adf_sequence(examples) != nullptr)
    multiline_learn_or_predict<true>(base, examples, data.offset);
  else
  ...
  v_array<action_score> &preds = examples[0]->pred.a_s;
  generate_softmax(-data.lambda, begin_scores(preds), end_scores(preds),
                   begin_scores(preds), end_scores(preds));
...
void output_example(vw &all, cb_explore_adf &c, multi_ex &ec_seq) {
  ...
  all.sd->update(holdout_example, labeled_example, loss, ec.weight,
                 num_features);
...
template <bool is_learn>
void do_actual_learning(cb_explore_adf &data, multi_learner &base,
                        multi_ex &ec_seq) {
  ...
  if (label_example == nullptr || !is_learn) {
    ...
    case REGCB:
      ...
      break;
    default:
      THROW("Unknown explorer type specified for contextual bandit learning: "
            << data.explore_type);
    }
...
base_learner *cb_explore_adf_setup(options_i &options, vw &all) {
  ...
  new_options
      ...
      .add(make_option("bag", data->bag_size)
               .keep()
               .help("bagging-based exploration"))
      .add(make_option("cover", data->cover_size)
               .keep()
               .help("Online cover based exploration"))
      .add(make_option("psi", data->psi)
               .keep()
               .default_value(1.0f)
               .help("disagreement parameter for cover"))
      .add(make_option("nounif", data->nounif).keep().help("do not explore "
                                                           "uniformly on "
                                                           "zero-probability "
                                                           "actions in cover"))
      ...
      .add(make_option("regcbopt", data->regcbopt)
               .keep()
      ...
      .add(make_option("greedify", data->greedify)
               .keep()
               .help("always update first policy once in bagging"))
      .add(make_option("cb_min_cost", data->min_cb_cost)
               .keep()
               .default_value(0.f)
               .help("lower bound on cost"))
      .add(make_option("cb_max_cost", data->max_cb_cost)
               .keep()
               .default_value(1.f)
      ...
      .add(make_option("lambda", data->lambda)
               .keep()
               .default_value(1.f)
               .help("parameter for softmax"))
      .add(make_option("cb_type", type_string).keep().help("contextual bandit "
                                                           "method to use in "
                                                           "{ips,dr,mtr}. "
                                                           "Default: mtr"));
  ...
  data->all = &all;
  if (data->lambda <
      0) // Lambda should always be negative because we are using a cost basis.
  ...
  if (options.was_supplied("cb_type")) {
    ...
    if (data->explore_type == REGCB && data->gen_cs.cb_type != CB_TYPE_MTR) {
      all.trace_message
          << "warning: bad cb_type, RegCB only supports mtr; resetting to mtr."
          << std::endl;
      options.replace("cb_type", "mtr");
    }
  }
  ...
  learner<cb_explore_adf, multi_ex> &l =
      init_learner(data, base, CB_EXPLORE_ADF::do_actual_learning<true>,
                   CB_EXPLORE_ADF::do_actual_learning<false>,
                   problem_multiplier, prediction_type::action_probs);
