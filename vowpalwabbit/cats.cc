...
namespace continuous_action
{
  ...
  void reduction_output::output_predictions(v_array<int>& predict_file_descriptors, actions_pdf::action_pdf_value& prediction)
  {
  ...
  LEARNER::base_learner* cats_setup(options_i& options, vw& all)
  {
    option_group_definition new_options("Continuous action tree with smoothing");
    int num_actions = 0, pdf_num_actions = 0;
    new_options
      .add(make_option("cats", num_actions)
        .keep()
        .help("Continuous action tree with smoothing"))
      .add(make_option("cats_pdf", pdf_num_actions)
        .keep()
        .help("Continuous action tree with smoothing (pdf)")
      );
