...
template <typename T> class dense_iterator {
private:
  T *_current;
  T *_begin;
  uint32_t _stride;
...
public:
  ...
  dense_parameters(size_t length, uint32_t stride_shift = 0)
      : _begin(calloc_mergable_or_throw<weight>(length << stride_shift)),
        _weight_mask((length << stride_shift) - 1), _stride_shift(stride_shift),
        _seeded(false) {}
  ...
  void share(size_t length) {
    float *shared_weights = (float *)mmap(
        0, (length << _stride_shift) * sizeof(float), PROT_READ | PROT_WRITE,
  ...
  ~dense_parameters() {
    if (_begin != nullptr && !_seeded) // don't free weight vector if it is
