...
std::string find_in_path(std::vector<std::string> paths, std::string fname) {
  ...
  for (std::string path : paths) {
    std::string full = ends_with(path, delimiter) ? (path + fname)
                                                  : (path + delimiter + fname);
...
void parse_dictionary_argument(vw &all, std::string str) {
  ...
  if (fname == "")
    THROW("error: cannot find dictionary '"
          << s << "' in path; try adding --dictionary_path");
  ...
  if (!all.quiet)
    all.trace_message << "scanned dictionary '" << s << "' from '" << fname
  ...
  for (size_t id = 0; id < all.loaded_dictionaries.size(); id++)
    if (all.loaded_dictionaries[id].file_hash == fd_hash) {
      all.namespace_dictionaries[(size_t)ns].push_back(
          all.loaded_dictionaries[id].dict);
      io->close_file();
      delete io;
      return;
    }
  ...
  do {
    ...
    uint64_t hash =
        uniform_hash(ss.begin, ss.end - ss.begin, quadratic_constant);
  ...
  if (!all.quiet)
    all.trace_message << "dictionary " << s << " contains " << map->size()
                      << " item" << (map->size() == 1 ? "" : "s") << endl;
...
void parse_diagnostics(options_i &options, vw &all) {
  ...
  diagnostic_group
      ...
      .add(make_option("audit", all.audit)
               .short_name("a")
               .help("print weights of features"))
      .add(make_option("progress", progress_arg)
               .short_name("P")
               .help("Progress update frequency. int: additive, float: "
                     "multiplicative"))
      .add(make_option("quiet", all.quiet)
               .help("Don't output disgnostics and progress updates"))
      .add(
          make_option("help", help)
              .short_name("h")
              .help("Look here: http://hunch.net/~vw/ and click on Tutorial."));
    ...
    if (progress_arg.find_first_of(".") == std::string::npos) {
      ...
      if (all.progress_arg <= 1.0) {
        all.trace_message << "warning: multiplicative --progress <float>: "
                          << progress_arg << " is <= 1.0: adding 1.0" << endl;
...
input_options parse_source(vw &all, options_i &options) {
  ...
  input_options
      .add(make_option("data", all.data_filename)
               .short_name("d")
               .help("Example set"))
      .add(make_option("daemon", parsed_options.daemon)
      ...
      .add(make_option("port", parsed_options.port)
               .help("port to listen on; use 0 to pick unused port"))
      .add(make_option("num_children", all.num_children)
               .help("number of children for persistent daemon mode"))
      .add(make_option("pid_file", parsed_options.pid_file)
               .help("Write pid file in persistent daemon mode"))
      .add(make_option("port_file", parsed_options.port_file)
               .help("Write port used in persistent daemon mode"))
      .add(make_option("cache", parsed_options.cache)
               .short_name("c")
               .help("Use a cache.  The default is <data>.cache"))
      .add(make_option("cache_file", parsed_options.cache_files)
               .help("The location(s) of cache_file."))
      .add(
          make_option("json", parsed_options.json).help("Enable JSON parsing."))
      .add(make_option("dsjson", parsed_options.dsjson)
      ...
      .add(make_option("compressed", parsed_options.compressed)
               .help("use gzip format whenever possible. If a cache file is "
                     "being created, this option creates a "
                     "compressed cache file. A mixture of raw-text & "
                     "compressed inputs are supported with autodetection."))
      .add(make_option("no_stdin", all.stdin_off)
               .help("do not default to reading from stdin"));

  options.add_and_parse(input_options);

  // If the option provider is program_options try and retrieve data as a
  ...
  if (parsed_options.daemon || options.was_supplied("pid_file") ||
      (options.was_supplied("port") && !all.active)) {
    all.daemon = true;
  ...
  if ((parsed_options.cache || options.was_supplied("cache_file")) &&
      options.was_supplied("invert_hash"))
    THROW("invert_hash is incompatible with a cache file.  Use it in single "
          "pass mode only.");
...
bool interactions_settings_doubled =
    false; // local setting setted in parse_modules()
namespace VW {
const char *are_features_compatible(vw &vw1, vw &vw2) {
  if (vw1.p->hasher != vw2.p->hasher)
    return "hasher";

  if (!std::equal(vw1.spelling_features.begin(), vw1.spelling_features.end(),
                  vw2.spelling_features.begin()))
    return "spelling_features";

  if (!std::equal(vw1.affix_features.begin(), vw1.affix_features.end(),
                  vw2.affix_features.begin()))
  ...
  if (vw1.ignore_some &&
      !std::equal(vw1.ignore.begin(), vw1.ignore.end(), vw2.ignore.begin()))
    return "ignore";
  ...
  if (vw1.ignore_some_linear &&
      !std::equal(vw1.ignore_linear.begin(), vw1.ignore_linear.end(),
                  vw2.ignore_linear.begin()))
  ...
  if (vw1.redefine_some &&
      !std::equal(vw1.redefine.begin(), vw1.redefine.end(),
                  vw2.redefine.begin()))
    return "redefine";
  ...
  if (!std::equal(vw1.dictionary_path.begin(), vw1.dictionary_path.end(),
                  vw2.dictionary_path.begin()))
  ...
  while (pos < (int)arg.size() - 3) {
    if (arg[pos] == '\\' && arg[pos + 1] == 'x') {
      ...
      if (*p == '\0') {
        ...
        cerr << "Possibly malformed hex representation of a namespace: '\\x"
             << substr << "'\n";
...
void parse_feature_tweaks(options_i &options, vw &all,
  ...
  feature_options
      ...
      .add(make_option("hash_seed", all.hash_seed)
               .keep()
               .default_value(0)
               .help("seed for hash function"))
      .add(make_option("ignore", ignores)
               .keep()
               .help("ignore namespaces beginning with character <arg>"))
      .add(make_option("ignore_linear", ignore_linears)
               .keep()
               .help("ignore namespaces beginning with character <arg> for "
                     "linear terms only"))
      .add(make_option("keep", keeps)
               .keep()
               .help("keep namespaces beginning with character <arg>"))
      .add(make_option("redefine", redefines)
               .keep()
               .help("redefine namespaces beginning with characters of "
                     "std::string S as namespace N. <arg> shall be in form "
                     "'N:=S' where := is operator. Empty N or S are treated as "
                     "default namespace. Use ':' as a "
                     "wildcard in S.")
               .keep())
      .add(make_option("bit_precision", new_bits)
               .short_name("b")
               .help("number of bits in the feature table"))
      .add(make_option("noconstant", noconstant)
      ...
      .add(make_option("ngram", all.ngram_strings)
               .help("Generate N grams. To generate N grams for a single "
                     "namespace 'foo', arg should be fN."))
      .add(make_option("skips", all.skip_strings)
               .help("Generate skips in N grams. This in conjunction with the "
                     "ngram tag can be used to generate "
                     "generalized n-skip-k-gram. To generate n-skips for a "
                     "single namespace 'foo', arg should be fN."))
      .add(make_option("feature_limit", all.limit_strings)
               .help("limit to N features. To apply to a single namespace "
                     "'foo', arg should be fN"))
      .add(make_option("affix", affix)
               .keep()
               .help("generate prefixes/suffixes of features; argument "
      ...
      .add(make_option("spelling", spelling_ns)
               .keep()
               .help("compute spelling features for a give namespace (use '_' "
                     "for default namespace)"))
      .add(make_option("dictionary", dictionary_nses)
               .keep()
               .help("read a dictionary for additional features (arg either "
                     "'x:file' or just 'file')"))
      .add(make_option("dictionary_path", dictionary_path)
               .help("look in this directory for dictionaries; defaults to "
                     "current directory or env{PATH}"))
      .add(make_option("interactions", interactions)
               .keep()
               .help("Create feature interactions of any level between "
                     "namespaces."))
      .add(make_option("permutations", all.permutations)
               .help("Use permutations instead of combinations for feature "
                     "interactions of same namespace."))
      .add(make_option("leave_duplicate_interactions",
                       leave_duplicate_interactions)
               .help("Don't remove interactions with duplicate combinations of "
                     "namespaces. For ex. this is a "
                     "duplicate: '-q ab -q ba' and a lot more in '-q ::'."))
      .add(make_option("quadratic", quadratics)
               .short_name("q")
               .keep()
               .help("Create and use quadratic features"))
      // TODO this option is unused - remove?
      .add(
          make_option("q:", q_colon)
              .help(": corresponds to a wildcard for all printable characters"))
      .add(make_option("cubic", cubics)
               .keep()
  ...
  if (options.was_supplied("bit_precision")) {
    if (all.default_bits == false && new_bits != all.num_bits)
      THROW("Number of bits is set to "
            << new_bits << " and " << all.num_bits
  ...
  if ((((!all.pairs.empty() || !all.triples.empty() ||
         !all.interactions.empty()) && /*data was restored from old model file
                                          directly to v_array and will be
                                          overriden automatically*/
        (options.was_supplied("quadratic") || options.was_supplied("cubic") ||
    ...
    all.trace_message << "WARNING: model file has set of {-q, --cubic, "
                         "--interactions} settings stored, but they'll be "
                         "OVERRIDEN by set of {-q, --cubic, --interactions} "
                         "settings from command line."
                      << endl;

    // in case arrays were already filled in with values from old model file -
    // reset them
    if (!all.pairs.empty())
      all.pairs.clear();
  ...
  if (options.was_supplied("quadratic")) {
    ...
    for (std::vector<std::string>::iterator i = quadratics.begin();
         i != quadratics.end(); ++i) {
    ...
    expanded_interactions = INTERACTIONS::expand_interactions(
        quadratics, 2, "error, quadratic features must involve two sets.");
  ...
  if (options.was_supplied("cubic")) {
    ...
    for (std::vector<std::string>::iterator i = cubics.begin();
         i != cubics.end(); ++i) {
      *i = spoof_hex_encoded_namespaces(*i);
      if (!all.quiet)
        all.trace_message << *i << " ";
    ...
    std::vector<std::string> exp_cubic = INTERACTIONS::expand_interactions(
        cubics, 3, "error, cubic features must involve three sets.");
    expanded_interactions.insert(std::begin(expanded_interactions),
                                 std::begin(exp_cubic), std::end(exp_cubic));
  ...
  if (options.was_supplied("interactions")) {
    ...
    for (std::vector<std::string>::iterator i = interactions.begin();
    ...
    std::vector<std::string> exp_inter =
        INTERACTIONS::expand_interactions(interactions, 0, "");
    expanded_interactions.insert(std::begin(expanded_interactions),
                                 std::begin(exp_inter), std::end(exp_inter));
  ...
  if (expanded_interactions.size() > 0) {
    ...
    INTERACTIONS::sort_and_filter_duplicate_interactions(
        expanded_interactions, !leave_duplicate_interactions, removed_cnt,
        sorted_cnt);
    ...
    if (removed_cnt > 0)
      all.trace_message
          ...
          << removed_cnt << '.' << endl
          << "You can use --leave_duplicate_interactions to disable this "
             "behaviour."
          << endl;
    if (sorted_cnt > 0)
      all.trace_message << "WARNING: some interactions contain duplicate "
                           "characters and their characters order has "
    ...
    // copy interactions of size 2 and 3 to old vectors for backward
    // compatibility
  ...
  if (options.was_supplied("ignore")) {
    ...
    for (std::vector<std::string>::iterator i = ignores.begin();
         i != ignores.end(); i++) {
      *i = spoof_hex_encoded_namespaces(*i);
  ...
  if (options.was_supplied("ignore_linear")) {
    ...
    for (std::vector<std::string>::iterator i = ignore_linears.begin();
         i != ignore_linears.end(); i++) {
      *i = spoof_hex_encoded_namespaces(*i);
      for (std::string::const_iterator j = i->begin(); j != i->end(); j++)
  ...
  if (options.was_supplied("keep")) {
    ...
    for (std::vector<std::string>::iterator i = keeps.begin(); i != keeps.end();
         i++) {
      *i = spoof_hex_encoded_namespaces(*i);
      for (std::string::const_iterator j = i->begin(); j != i->end(); j++)
  ...
  if (options.was_supplied("redefine")) {
    ...
    // so --redefine :=L --redefine ab:=M  --ignore L  will ignore all except a
    // and b under new M namspace

    for (std::vector<std::string>::iterator arg_iter = redefines.begin();
         arg_iter != redefines.end(); arg_iter++) {
      ...
      if (!operator_found)
        THROW("argument of --redefine is malformed. Valid format is N:=S, :=S "
              "or N:=");

      if (++operator_pos > 3) // seek operator end
        all.trace_message << "WARNING: multiple namespaces are used in target "
                             "part of --redefine argument. Only first one ('"
                          << new_namespace
                          << "') will be used as target namespace." << endl;

      all.redefine_some = true;

      // case ':=S' doesn't require any additional code as new_namespace = ' '
      // by default

      if (operator_pos ==
          arg_len) // S is empty, default namespace shall be used
...
void parse_example_tweaks(options_i &options, vw &all) {
  ...
  example_options
      .add(make_option("testonly", test_only)
               .short_name("t")
               .help("Ignore label information and just test"))
      .add(make_option("holdout_off", all.holdout_set_off)
               .help("no holdout data in multiple passes"))
      .add(make_option("holdout_period", all.holdout_period)
               .default_value(10)
               .help("holdout period for test only"))
      .add(make_option("holdout_after", all.holdout_after)
               .help("holdout after n training examples, default off (disables "
                     "holdout_period)"))
      .add(make_option("early_terminate", early_terminate_passes)
               .default_value(3)
               .help("Specify the number of passes tolerated when holdout loss "
                     "doesn't decrease before early termination"))
      .add(make_option("passes", all.numpasses)
      ...
      .add(make_option("min_prediction", all.sd->min_label)
               .help("Smallest prediction to output"))
      .add(make_option("max_prediction", all.sd->max_label)
               .help("Largest prediction to output"))
      .add(make_option("sort_features", all.p->sort_features)
               .help("turn this on to disregard order in which features have "
                     "been defined. This will lead to smaller "
                     "cache sizes"))
      .add(
          make_option("loss_function", loss_function)
              .default_value("squared")
              .help("Specify the loss function to be used, uses squared by "
                    "default. Currently available ones are "
                    "squared, classic, hinge, logistic, quantile and poisson."))
      .add(make_option("quantile_tau", loss_parameter)
               .default_value(0.5f)
               .help("Parameter \\tau associated with Quantile loss. Defaults "
                     "to 0.5"))
      .add(make_option("l1", all.l1_lambda).help("l_1 lambda"))
      .add(make_option("l2", all.l2_lambda).help("l_2 lambda"))
      .add(make_option("no_bias_regularization", all.no_bias)
               .help("no bias in regularization"))
      .add(make_option("named_labels", named_labels)
               .keep()
               .help("use names for labels (multiclass, etc.) rather than "
                     "integers, argument specified all possible "
  ...
  if (options.was_supplied("min_prediction") ||
      options.was_supplied("max_prediction") || test_only)
  ...
  if (options.was_supplied("named_labels")) {
    ...
    if (!all.quiet)
      all.trace_message << "parsed " << all.sd->ldict->getK() << " named labels"
                        << endl;
  }
  ...
  if (all.l1_lambda < 0.) {
    all.trace_message << "l1_lambda should be nonnegative: resetting from "
                      << all.l1_lambda << " to 0" << endl;
    all.l1_lambda = 0.;
  }
  if (all.l2_lambda < 0.) {
    all.trace_message << "l2_lambda should be nonnegative: resetting from "
                      << all.l2_lambda << " to 0" << endl;
    all.l2_lambda = 0.;
  }
  ...
  if (!all.quiet) {
    if (all.reg_mode % 2 && !options.was_supplied("bfgs"))
      all.trace_message << "using l1 regularization = " << all.l1_lambda
                        << endl;
    if (all.reg_mode > 1)
      all.trace_message << "using l2 regularization = " << all.l2_lambda
                        << endl;
  }
}
...
void parse_output_preds(options_i &options, vw &all) {
  ...
  output_options
      .add(make_option("predictions", predictions)
               .short_name("p")
...
#ifdef _WIN32
      _sopen_s(&f, fstr, _O_CREAT | _O_WRONLY | _O_BINARY | _O_TRUNC,
               _SH_DENYWR, _S_IREAD | _S_IWRITE);
#else
      f = open(fstr, O_CREAT | O_WRONLY | O_LARGEFILE | O_TRUNC, 0666);
#endif
      if (f < 0)
        all.trace_message << "Error opening the predictions file: " << fstr
                          << endl;
  ...
  if (options.was_supplied("raw_predictions")) {
    if (!all.quiet) {
      ...
      if (options.was_supplied("binary"))
        all.trace_message << "Warning: --raw_predictions has no defined value "
                             "when --binary specified, expect no output"
                          << endl;
    }
    if (raw_predictions == "stdout")
      ...
      int f;
#ifdef _WIN32
      _sopen_s(&f, t, _O_CREAT | _O_WRONLY | _O_BINARY | _O_TRUNC, _SH_DENYWR,
               _S_IREAD | _S_IWRITE);
#else
...
void parse_output_model(options_i &options, vw &all) {
  ...
  output_model_options
      .add(make_option("final_regressor", all.final_regressor_name)
               .short_name("f")
               .help("Final regressor"))
      .add(make_option("readable_model", all.text_regressor_name)
               .help("Output human-readable final regressor with numeric "
                     "features"))
      .add(make_option("invert_hash", all.inv_hash_regressor_name)
               .help("Output human-readable final regressor with feature "
                     "names.  Computationally expensive."))
      .add(make_option("save_resume", all.save_resume).help("save extra state "
                                                            "so learning can "
                                                            "be resumed later "
                                                            "with new data"))
      .add(make_option("preserve_performance_counters",
                       all.preserve_performance_counters)
               .help("reset performance counters when warmstarting"))
      .add(make_option("save_per_pass", all.save_per_pass)
               .help("Save the model after every pass over data"))
      .add(make_option("output_feature_regularizer_binary",
                       all.per_feature_regularizer_output)
               .help("Per feature regularization output file"))
      .add(make_option("output_feature_regularizer_text",
                       all.per_feature_regularizer_text)
               .help("Per feature regularization output file, in text"))
      .add(make_option("id", all.id)
  ...
  if (all.final_regressor_name.compare("") && !all.quiet)
    all.trace_message << "final_regressor = " << all.final_regressor_name
                      << endl;
  ...
  // if (options.was_supplied("id") && find(arg.args.begin(), arg.args.end(),
  // "--id") == arg.args.end())
  // {
...
void load_input_model(vw &all, io_buf &io_temp) {
  ...
  if (!all.feature_mask.empty() && all.initial_regressors.size() > 0 &&
      all.feature_mask == all.initial_regressors[0]) {
    // load rest of regressor
    all.l->save_load(io_temp, true, false);
    io_temp.close_file();

    parse_mask_regressor_args(all, all.feature_mask, all.initial_regressors);
  } else { // load mask first
    parse_mask_regressor_args(all, all.feature_mask, all.initial_regressors);
...
void parse_reductions(options_i &options, vw &all) {
  ...
  all.reduction_stack.push(
      ExpReplay::expreplay_setup<'m', MULTICLASS::mc_label>);
  ...
  all.reduction_stack.push(
      ExpReplay::expreplay_setup<'c', COST_SENSITIVE::cs_label>);
...
vw &parse_args(options_i &options, trace_message_t trace_listener,
               void *trace_context) {
  vw &all = *(new vw());
  all.options = &options;
  ...
  try {
    ...
    vw_args
        .add(make_option("ring_size", ring_size)
                 .default_value(256)
                 .help("size of example ring"))
        .add(make_option("strict_parse", strict_parse)
    ...
    update_args
        ...
        .add(make_option("feature_mask", all.feature_mask)
                 .help("Use existing regressor to determine which parameters "
                       "may be updated.  If no initial_regressor "
                       "given, also used for initial weights."));
    options.add_and_parse(update_args);

    option_group_definition weight_args("Weight options");
    weight_args
        .add(make_option("initial_regressor", all.initial_regressors)
                 .help("Initial regressor(s)")
                 .short_name("i"))
        .add(make_option("initial_weight", all.initial_weight)
                 .help("Set all weights to an initial value of arg."))
        .add(make_option("random_weights", all.random_weights)
                 .help("make initial weights random"))
        .add(make_option("normal_weights", all.normal_weights)
                 .help("make initial weights normal"))
        .add(make_option("truncated_normal_weights", all.tnormal_weights)
                 .help("make initial weights truncated normal"))
        .add(make_option("sparse_weights", all.weights.sparse)
                 .help("Use a sparse datastructure for weights"))
        .add(make_option("input_feature_regularizer",
                         all.per_feature_regularizer_input)
    ...
    parallelization_args
        .add(make_option("span_server", span_server_arg)
                 .help("Location of server for setting up spanning tree"))
        //(make_option("threads", threads_arg).help("Enable multi-threading"))
        //Unused option?
        .add(make_option("unique_id", unique_id_arg)
                 .default_value(0)
                 .help("unique id used for cluster parallel jobs"))
        .add(make_option("total", total_arg)
                 .default_value(1)
                 .help("total number of nodes used in cluster parallel job"))
        .add(make_option("node", node_arg)
                 .default_value(0)
    ...
    if ((options.was_supplied("total") || options.was_supplied("node") ||
         options.was_supplied("unique_id")) &&
        !(options.was_supplied("total") && options.was_supplied("node") &&
          options.was_supplied("unique_id"))) {
    ...
    if (options.was_supplied("span_server")) {
      ...
      all.all_reduce =
          new AllReduceSockets(span_server_arg, span_server_port_arg,
                               unique_id_arg, total_arg, node_arg, all.quiet);
    }
...
bool check_interaction_settings_collision(options_i &options,
  ...
  bool command_line_has_interaction =
      options.was_supplied("q") || options.was_supplied("quadratic") ||
      options.was_supplied("cubic") || options.was_supplied("interactions");

  if (!command_line_has_interaction)
    return false;

  // we don't use -q to save pairs in all.file_options, so only 3 options
  // checked
  bool file_options_has_interaction =
      file_options.find("--quadratic") != std::string::npos;
  file_options_has_interaction =
      file_options_has_interaction ||
      (file_options.find("--cubic") != std::string::npos);
  file_options_has_interaction =
      file_options_has_interaction ||
      (file_options.find("--interactions") != std::string::npos);

  return file_options_has_interaction;
}

options_i &load_header_merge_options(options_i &options, vw &all,
                                     io_buf &model) {
  std::string file_options;
  save_load_header(all, model, true, false, file_options, options);

  interactions_settings_doubled =
      check_interaction_settings_collision(options, file_options);

  // Convert file_options into  vector.
  std::istringstream ss{file_options};
  std::vector<std::string> container{std::istream_iterator<std::string>{ss},
  ...
  for (auto opt : pos.options) {
    // If we previously encountered an option we want to skip, ignore tokens
    // without --.
    if (skipping) {
    ...
    // If the key starts with a digit, this is a mis-interpretation of a value
    // as a key. Pull it into the previous
    // option. This was found in the case of --lambda -1, misinterpreting -1 as
    // an option key. The easy way to fix this
    // requires introducing "identifier-like" semantics for options keys, e.g.
    // "does not begin with a digit". That does
    // not seem like an unreasonable restriction. The logical check here is: is
    // "string_key" of the form {'-', <digit>,
    // <etc.>}.
    if (opt.string_key.length() > 1 && opt.string_key[0] == '-' &&
        opt.string_key[1] >= '0' && opt.string_key[1] <= '9') {
      treat_as_value = true;
    }

    // If the interaction settings are doubled, the copy in the model file is
    // ignored.
    if (interactions_settings_doubled &&
        (opt.string_key == "quadratic" || opt.string_key == "cubic" ||
         opt.string_key == "interactions")) {
      // skip this option.
      skipping = true;
      continue;
    }
    ...
    if (!treat_as_value && opt.string_key != "") {
      // If the new token is a new option and there were no values previously it
      // was a bool option. Add it as a switch.
      ...
      // If treat_as_value is set, boost incorrectly interpreted the token as
      // containing an option key
      // In this case, what should have happened is all original_tokens items
      // should be in value.
...
void parse_modules(options_i &options, vw &all,
  ...
  rand_options.add(make_option("random_seed", all.random_seed)
  ...
  if (!all.quiet) {
    ...
    if (all.numpasses > 1)
      all.trace_message << "decay_learning_rate = " << all.eta_decay_rate
                        << endl;
  }
}

void parse_sources(options_i &options, vw &all, io_buf &model,
                   bool skipModelLoad) {
  if (!skipModelLoad)
...
namespace VW {
void cmd_string_replace_value(std::stringstream *&ss,
                              std::string flag_to_replace,
                              std::string new_value) {
  flag_to_replace.append(" "); // add a space to make sure we obtain the right
                               // flag in case 2 flags start with the same set
                               // of characters
  std::string cmd = ss->str();
  size_t pos = cmd.find(flag_to_replace);
  if (pos == std::string::npos)
    // flag currently not present in command std::string, so just append it to
    // command std::string
    *ss << " " << flag_to_replace << new_value;
    ...
    if (pos_after_value == std::string::npos) {
      // we reach the end of the std::string, so replace the all characters
      // after pos by new_value
...
char **to_argv_escaped(std::string const &s, int &argc) {
  substring ss = {const_cast<char *>(s.c_str()),
...
vw *initialize(options_i &options, io_buf *model, bool skipModelLoad,
  ...
  try {
    ...
    io_buf localModel;
    if (!model) {
      std::vector<std::string> all_initial_regressor_files(
          all.initial_regressors);
      if (options.was_supplied("input_feature_regularizer")) {
        all_initial_regressor_files.push_back(
            all.per_feature_regularizer_input);
      }
    ...
    // Loads header of model files and loads the command line options into the
    // options object.
...
vw *initialize(std::string s, io_buf *model, bool skipModelLoad,
               trace_message_t trace_listener, void *trace_context) {
  int argc = 0;
  ...
  try {
    ret = initialize(argc, argv, model, skipModelLoad, trace_listener,
                     trace_context);
  } catch (...) {
    free_args(argc, argv);
    throw;
...
vw *initialize_escaped(std::string const &s, io_buf *model, bool skipModelLoad,
                       trace_message_t trace_listener, void *trace_context) {
  int argc = 0;
...
vw *seed_vw_model(vw *vw_model, const std::string extra_args,
                  trace_message_t trace_listener, void *trace_context) {
  ...
  for (auto const &option : vw_model->options->get_all_options()) {
    if (vw_model->options->was_supplied(option->m_name)) {
      // ignore no_stdin since it will be added by vw::initialize, and ignore -i
      // since we don't want to reload the
      // model.
      if (option->m_name == "no_stdin" ||
          option->m_name == "initial_regressor") {
        continue;
      }
  ...
  vw *new_model =
      VW::initialize(serialized_options.c_str(), nullptr,
                     true /* skipModelLoad */, trace_listener, trace_context);
...
void sync_stats(vw &all) {
  if (all.all_reduce != nullptr) {
    ...
    all.sd->weighted_labeled_examples =
        (double)accumulate_scalar(all, weighted_labeled_examples);
    ...
    float weighted_unlabeled_examples =
        (float)all.sd->weighted_unlabeled_examples;
    all.sd->weighted_unlabeled_examples =
        (double)accumulate_scalar(all, weighted_unlabeled_examples);
...
void finish(vw &all, bool delete_all) {
  ...
  if (!all.quiet && !all.options->was_supplied("audit_regressor")) {
    ...
    if (all.current_pass == 0 || all.current_pass == 1)
      all.trace_message << endl
                        << "number of examples = " << all.sd->example_number;
    else {
      all.trace_message << endl
                        << "number of examples per pass = "
                        << all.sd->example_number / all.current_pass;
      all.trace_message << endl << "passes used = " << all.current_pass;
    }
    all.trace_message << endl
                      << "weighted example sum = "
                      << all.sd->weighted_examples();
    all.trace_message << endl
                      << "weighted label sum = " << all.sd->weighted_labels;
    all.trace_message << endl << "average loss = ";
    if (all.holdout_set_off)
      if (all.sd->weighted_labeled_examples > 0)
        all.trace_message << all.sd->sum_loss /
                                 all.sd->weighted_labeled_examples;
      else
        all.trace_message << "n.a.";
    else if ((all.sd->holdout_best_loss == FLT_MAX) ||
             (all.sd->holdout_best_loss == FLT_MAX * 0.5))
    ...
    if (all.sd->report_multiclass_log_loss) {
      if (all.holdout_set_off)
        all.trace_message << endl
                          ...
                          << all.sd->multiclass_log_loss /
                                 all.sd->weighted_labeled_examples;
      else
        all.trace_message << endl
                          << "average multiclass log loss = "
                          << all.sd->holdout_multiclass_log_loss /
                                 all.sd->weighted_labeled_examples
                          << " h";
    }
    ...
    all.trace_message << endl
                      << "total feature number = " << all.sd->total_features;
    if (all.sd->queries > 0)
  ...
  for (size_t i = 0; i < all.loaded_dictionaries.size(); i++) {
    ...
    // (a) dictionary_info.name is allocated using
    // 'calloc_or_throw<char>(strlen(s)+1)' and (b) freed using
    // 'free(all.loaded_dictionaries[i].name)'
    //
    // When the call to allocation is replaced by (a) 'new char[strlen(s)+1]'
    // and deallocated using (b) 'delete []', the
