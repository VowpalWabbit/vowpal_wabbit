...
string find_in_path(vector<string> paths, string fname) {
  ...
  for (string path : paths) {
    string full = ends_with(path, delimiter) ? (path + fname)
...
const char *are_features_compatible(vw &vw1, vw &vw2) {
  ...
  if (!equal(vw1.spelling_features,
             vw1.spelling_features +
                 (sizeof(vw1.spelling_features) / sizeof(bool)),
             vw2.spelling_features))
    return "spelling_features";

  if (!equal(vw1.affix_features,
             vw1.affix_features +
                 (sizeof(vw1.affix_features) / sizeof(uint32_t)),
             vw2.affix_features))
    return "affix_features";

  if (!equal(vw1.ngram, vw1.ngram + (sizeof(vw1.ngram) / sizeof(uint32_t)),
             vw2.ngram))
    return "ngram";

  if (!equal(vw1.skips, vw1.skips + (sizeof(vw1.skips) / sizeof(uint32_t)),
             vw2.skips))
    return "skips";

  if (!equal(vw1.limit, vw1.limit + (sizeof(vw1.limit) / sizeof(uint32_t)),
             vw2.limit))
    return "limit";
  ...
  if (vw1.ignore_some &&
      !equal(vw1.ignore, vw1.ignore + (sizeof(vw1.ignore) / sizeof(bool)),
             vw2.ignore))
    return "ignore";
  ...
  if (vw1.ignore_some_linear &&
      !equal(vw1.ignore_linear,
             vw1.ignore_linear + (sizeof(vw1.ignore_linear) / sizeof(bool)),
             vw2.ignore_linear))
  ...
  if (vw1.redefine_some &&
      !equal(vw1.redefine,
             vw1.redefine + (sizeof(vw1.redefine) / sizeof(unsigned char)),
             vw2.redefine))
    return "redefine";
  ...
  if (!equal(vw1.dictionary_path.begin(), vw1.dictionary_path.end(),
...
void parse_feature_tweaks(options_i &options, vw &all,
  ...
  feature_options
      ...
      .add(make_option("redefine", redefines)
               .keep()
               .help("redefine namespaces beginning with characters of string "
                     "S as namespace N. <arg> shall be in form "
  ...
  if (options.was_supplied("quadratic")) {
    ...
    for (vector<string>::iterator i = quadratics.begin(); i != quadratics.end();
         ++i) {
  ...
  if (options.was_supplied("interactions")) {
    ...
    for (vector<string>::iterator i = interactions.begin();
  ...
  if (options.was_supplied("ignore")) {
    ...
    for (vector<string>::iterator i = ignores.begin(); i != ignores.end();
         i++) {
      *i = spoof_hex_encoded_namespaces(*i);
  ...
  if (options.was_supplied("ignore_linear")) {
    ...
    for (vector<string>::iterator i = ignore_linears.begin();
  ...
  if (options.was_supplied("redefine")) {
    ...
    for (vector<string>::iterator arg_iter = redefines.begin();
    ...
    if (options.was_supplied("span_server")) {
      ...
      all.all_reduce =
          new AllReduceSockets(span_server_arg, span_server_port_arg,
                               unique_id_arg, total_arg, node_arg);
    }
...
namespace VW {
void cmd_string_replace_value(std::stringstream *&ss, string flag_to_replace,
                              string new_value) {
  ...
  if (pos == string::npos)
    // flag currently not present in command string, so just append it to
    // command string
    *ss << " " << flag_to_replace << new_value;
    ...
    if (pos_after_value == string::npos) {
      // we reach the end of the string, so replace the all characters after pos
      // by new_value
