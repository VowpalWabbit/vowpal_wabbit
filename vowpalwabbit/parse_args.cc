...
void parse_feature_tweaks(arguments& arg)
{
  ...
  if (arg.new_options("Feature options")
      ...
      .keep_vector<string>("spelling", po::value(&spelling_ns), "compute spelling features for a give namespace (use '_' for default namespace)")
      .keep_vector<string>("dictionary", po::value(&dictionary_nses), "read a dictionary for additional features (arg either 'x:file' or just 'file')")
      ("dictionary_path", po::value< vector<string> >(), "look in this directory for dictionaries; defaults to current directory or env{PATH}")
      .keep_vector<string>("interactions", po::value(&interactions), "Create feature interactions of any level between namespaces.")
      ...
      .keep_vector<string>("quadratic,q", po::value(&quadratics), "Create and use quadratic features")
