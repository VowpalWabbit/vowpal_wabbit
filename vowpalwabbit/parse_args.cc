...
void parse_example_tweaks(options_i& options, VW::workspace& all)
{
  ...
  example_options.add(make_option("testonly", test_only).short_name("t").help("Ignore label information and just test"))
      ...
      .add(make_option("initial_pass_length", all.pass_length).default_value(std::numeric_limits<size_t>::max()).help("Initial number of examples per pass"))
      .add(make_option("examples", all.max_examples).default_value(std::numeric_limits<size_t>::max()).help("Number of examples to parse"))
