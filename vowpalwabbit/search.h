...
extern uint32_t AUTO_CONDITION_FEATURES, AUTO_HAMMING_LOSS,
    EXAMPLES_DONT_CHANGE, IS_LDF, NO_CACHING, ACTION_COSTS;
...
public:
  ...
  inline BaseTask &foreach_action(void (*f)(search &, size_t, float, action,
                                            bool, float)) {
    _foreach_action = f;
    return *this;
  }
  ...
  inline BaseTask &maybe_override_prediction(bool (*f)(search &, size_t,
                                                       action &, float &)) {
    _maybe_override_prediction = f;
    return *this;
  }
  inline BaseTask &with_output_string(void (*f)(search &,
                                                std::stringstream &)) {
...
struct search { // INTERFACE
  ...
  // arguments:
  //   ec                    the example (features) on which to make a
  //   prediction
  //   my_tag                a tag for this prediction, so that you can
  //   explicitly
  ...
  //   oracle_actions_cnt    the length of the previous array, or 0 if it's
  //   nullptr
  //   condition_on          an array of previous (or future) predictions on
  //   which
  //                           this prediction depends. the semantics of
  //                           conditioning
  //                           is that IF the predictions for all the tags in
  //                           condition_on were the same, then the prediction
  //                           for
  //                           _this_ example will also be the same. i.e., same
  //                           features, etc. (also assuming same policy). if
  //                           AUTO_CONDITION_FEATURES is on, then we will
  //                           automatically
  //                           add features to ec based on what you're
  //                           conditioning on.
  //                           nullptr => independent prediction
  //   condition_on_names    a string containing the list of names of features
  //   you're
  //                           conditioning on. used explicitly for auditing,
  //                           implicitly
  //                           for keeping tags separated. also,
  //                           strlen(condition_on_names)
  //                           tells us how long condition_on is
  //   allowed_actions       an array of actions that are allowed at this step,
  //   or
  //                           nullptr if everything is allowed
  //   allowed_actions_cnt   the length of allowed_actions (0 if allowed_actions
  //   is null)
  //   allowed_actions_cost  if you can precompute the cost-under-rollout-by-ref
  //   for each
  //                           allowed action, and the underlying algorithm can
  //                           use this
  //                           (i.e., rollout=none or rollout=mix_per_roll and
  //                           we're on
  //                           a rollout-by-ref), then fill this in and rollouts
  //                           will be
  //                           avoided. note: if you provide
  //                           allowed_actions_cost,
  //                           then oracle_actions will be ignored (might as
  //                           well pass
  //                           nullptr). if allowed_actions
  //                           is a nullptr, then allowed_actions_cost should be
  //                           a vector
  //                           of length equal to the total number of actions
  //                           ("A"); otherwise
  //                           it should be of length allowed_actions_cnt. only
  //                           valid
  //                           if ACTION_COSTS is specified as an option.
  //   learner_id            the id for the underlying learner to use (via
  //   set_num_learners)
  action
  predict(example &ec, ptag my_tag, const action *oracle_actions,
          size_t oracle_actions_cnt = 1, const ptag *condition_on = nullptr,
          const char *condition_on_names =
              nullptr // strlen(condition_on_names) should == |condition_on|
          ,
          const action *allowed_actions = nullptr,
          size_t allowed_actions_cnt = 0,
          const float *allowed_actions_cost = nullptr, size_t learner_id = 0,
          float weight = 0.);

  // make an LDF prediction on a list of examples. arguments are identical to
  // predict(...)
  ...
  action predictLDF(example *ecs, size_t ec_cnt, ptag my_tag,
                    const action *oracle_actions, size_t oracle_actions_cnt = 1,
  ...
  // get the action sequence from the test run (only run if test_only or -t
  // or...)
  ...
  vw &get_vw_pointer_unsafe(); // although you should rarely need this, some
                               // times you need a poiter to the vw data
                               // structure :(
  void set_force_oracle(bool force); // if the library wants to force search to
                                     // use the oracle, set this to true
  search();
  ~search();
};

// for defining new tasks, you must fill out a search_task
struct search_task { // required
  const char *task_name;
...
struct search_metatask { // required
  const char *metatask_name;
  void (*run)(search &, multi_ex &);
...
public:
  ...
  predictor &set_input(
      ...
      size_t input_length); // if you're lucky and have an array of examples

  // the following is mostly to make life manageable for the Python interface
  void set_input_length(size_t input_length); // declare that we have an
                                              // input_length-long LDF example
  void set_input_at(size_t posn, example &input_example); // set the
                                                          // corresponding input
                                                          // (*after*
                                                          // set_input_length)
  ...
  predictor &add_condition_range(
      ptag hi, ptag count,
      char name0); // add (hi,name0), (hi-1,name0+1), ..., (h-count,name0+count)
  predictor &set_condition_range(
      ptag hi, ptag count,
      char name0); // set (hi,name0), (hi-1,name0+1), ..., (h-count,name0+count)
...
private:
  ...
  bool oracle_is_pointer; // if we're pointing to your memory TRUE; if it's our
                          // own memory FALSE
  v_array<ptag> condition_on_tags;
  v_array<char> condition_on_names;
  v_array<action> allowed_actions;
  bool allowed_is_pointer; // if we're pointing to your memory TRUE; if it's our
                           // own memory FALSE
  v_array<float> allowed_actions_cost;
  bool allowed_cost_is_pointer; // if we're pointing to your memory TRUE; if
                                // it's our own memory FALSE
  size_t learner_id;
  search &sch;
  ...
  template <class T>
  predictor &add_to(v_array<T> &A, bool &A_is_ptr, T *a, size_t count,
                    bool clear_first);
  void free_ec();

  // prevent the user from doing something stupid :) ... ugh needed to turn this
  // off for python :(
...
/*template<class T> void check_option(T& ret, vw&all, po::variables_map& vm,
const char* opt_name, bool
default_to_cmdline, bool(*equal)(T,T), const char* mismatch_error_string, const
char* required_error_string) { if
(vm.count(opt_name)) { ret = vm[opt_name].as<T>(); *all.args_n_opts.file_options
<< " --" << opt_name << " " << ret;
  }
...
// void check_option(bool& ret, vw&all, po::variables_map& vm, const char*
// opt_name, bool default_to_cmdline, const
