...
public:
  vw_exception(const char *file, int lineNumber,
...
public:
  strict_parse_exception(const char *file, int lineNumber,
                         const std::string &message)
      : vw_exception(file, lineNumber, message) {}
...
// UNUSED is used to silence the warning: warning: ISO C++11 requires at least
// one argument for the "..." in a variadic macro
#define THROW_OR_RETURN(...)                                                   \
  EXPAND(GET_MACRO(__VA_ARGS__, THROW_OR_RETURN_NORMAL, THROW_OR_RETURN_VOID,  \
                   UNUSED)(__VA_ARGS__))
