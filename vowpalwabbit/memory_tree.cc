...
inline int random_sample_example_pop(memory_tree &b, uint64_t &cn) {
  ...
  while (b.nodes[cn].internal == 1) {
    ...
    if (b.nodes[cn].nl < 1)      // no examples routed to left ever:
      ...
      pred = merand48(b.all->random_state) <
  ...
  if (b.nodes[cn].examples_index.size() >= 1) {
    int loc_at_leaf =
        int(merand48(b.all->random_state) * b.nodes[cn].examples_index.size());
...
void split_leaf(memory_tree &b, single_learner &base, const uint64_t cn) {
  ...
  b.nodes[cn].nr =
      (std::max)(double(b.nodes[right_child].examples_index.size()),
...
void learn_at_leaf_random(memory_tree &b, single_learner &base,
  ...
  if (b.nodes[leaf_id].examples_index.size() > 0) {
    uint32_t pos = uint32_t(merand48(b.all->random_state) *
...
void single_query_and_learn(memory_tree &b, single_learner &base,
  ...
  if (path_to_leaf.size() > 1) {
    // uint32_t random_pos =
    // merand48(b.all->random_state)*(path_to_leaf.size()-1);
    uint32_t random_pos = (uint32_t)(merand48(b.all->random_state) *
...
void learn(memory_tree &b, single_learner &base, example &ec) {
  if (b.test_mode == false) {
    ...
    if (b.iter % 5000 == 0) {
      if (b.oas == false)
        cout << "at iter " << b.iter << ", top(" << b.top_K
        ...
        cout << "at iter " << b.iter
             << ", avg hamming loss: " << b.hamming_loss * 1. / b.iter << endl;
    }
    ...
    if (b.iter % 5000 == 0) {
      if (b.oas == false)
        cout << "at iter " << b.iter
             << ", pred error: " << b.num_mistakes * 1. / b.iter << endl;
      else
        cout << "at iter " << b.iter
             << ", avg hamming loss: " << b.hamming_loss * 1. / b.iter << endl;
    }
...
void save_load_example(example *ec, io_buf &model_file, bool &read, bool &text,
                       stringstream &msg, bool &oas) { // deal with tag
                                                       // deal with labels:
