...
///////////////////////Helper//////////////////////////////
//////////////////////////////////////////////////////////
template <typename T> void remove_at_index(v_array<T> &array, uint32_t index) {
  if (index >= array.size()) {
...
void copy_example_data(example *dst, example *src,
                       bool oas = false) // copy example data.
{
  if (oas == false) {
    dst->l = src->l;
...
void diag_kronecker_prod_fs_test(features &f1, features &f2, features &prod_f,
                                 float &total_sum_feat_sq, float norm_sq1,
                                 float norm_sq2) {
  prod_f.delete_v();
  ...
  while (idx1 < f1.size() && idx2 < f2.size()) {
    ...
    if (ec1pos < ec2pos)
      ...
      total_sum_feat_sq += f1.values[idx1] * f2.values[idx2] /
                           denominator; // make this out of loop
      idx1++;
      idx2++;
    }
  }
...
void diag_kronecker_product_test(example &ec1, example &ec2, example &ec,
  ...
  ec.total_sum_feat_sq = 0.0; // sort namespaces.  pass indices array into
                              // sort...template (leave this to the end)

  qsort(ec1.indices.begin(), ec1.indices.size(), sizeof(namespace_index),
        cmpfunc);
  qsort(ec2.indices.begin(), ec2.indices.size(), sizeof(namespace_index),
        cmpfunc);

  size_t idx1 = 0;
  size_t idx2 = 0;
  while (idx1 < ec1.indices.size() && idx2 < ec2.indices.size())
  // for (size_t idx1 = 0, idx2 = 0; idx1 < ec1.indices.size() && idx2 <
  // ec2.indices.size(); idx1++)
  {
    ...
    if (c1 < c2)
      ...
      diag_kronecker_prod_fs_test(ec1.feature_space[c1], ec2.feature_space[c2],
                                  ec.feature_space[c1], ec.total_sum_feat_sq,
...
struct memory_tree {
  ...
  bool learn_at_leaf; // indicator for turning on learning the scorer function
                      // at the leaf level
...
float linear_kernel(const flat_example *fec1, const flat_example *fec2) {
  ...
  for (size_t idx1 = 0, idx2 = 0; idx1 < fs_1.size() && idx2 < fs_2.size();
       idx1++) {
...
float normalized_linear_prod(memory_tree &b, example *ec1, example *ec2) {
  ...
  float norm_sqrt =
      pow(fec1->total_sum_feat_sq * fec2->total_sum_feat_sq, 0.5f);
...
void init_tree(memory_tree &b) {
  ...
  b.kprod_ec = &calloc_or_throw<example>(); // allocate space for kronecker
                                            // product example
  ...
  std::cout << "tree initiazliation is done...." << std::endl
            ...
            << "num of dream operations per example: " << b.dream_repeats
            << std::endl
...
inline int random_sample_example_pop(memory_tree &b, uint64_t &cn) {
  ...
  while (b.nodes[cn].internal == 1) {
    ...
    if (b.nodes[cn].nl < 1)      // no examples routed to left ever:
      ...
      pred = b._random_state->get_and_update_random() <
                     (b.nodes[cn].nl * 1. / (b.nodes[cn].nr + b.nodes[cn].nl))
                 ? -1.f
                 : 1.f;
    else {
      std::cout << cn << " " << b.nodes[cn].nl << " " << b.nodes[cn].nr
                << std::endl;
  ...
  if (b.nodes[cn].examples_index.size() >= 1) {
    int loc_at_leaf = int(b._random_state->get_and_update_random() *
                          b.nodes[cn].examples_index.size());
...
float train_node(memory_tree &b, single_learner &base, example &ec,
                 const uint64_t cn) {
  ...
  float weighted_value =
      (float)((1. - b.alpha) * log(b.nodes[cn].nl / (b.nodes[cn].nr + 1e-1)) /
                  log(2.) +
              b.alpha * prediction);
  ...
  base.learn(
      ec, b.nodes[cn]
              .base_router); // update the router according to the new example.
...
void split_leaf(memory_tree &b, single_learner &base, const uint64_t cn) {
  ...
  for (size_t ec_id = 0; ec_id < b.nodes[cn].examples_index.size();
       ec_id++) // scan all examples stored in the cn
  {
  ...
  b.nodes[cn].nl = std::max(double(b.nodes[left_child].examples_index.size()),
...
inline uint32_t over_lap(v_array<uint32_t> &array_1,
...
// template<typename T>
inline uint32_t hamming_loss(v_array<uint32_t> &array_1,
                             v_array<uint32_t> &array_2) {
  uint32_t overlap = over_lap(array_1, array_2);
  return (uint32_t)(array_1.size() + array_2.size() - 2 * overlap);
}

void collect_labels_from_leaf(memory_tree &b, const uint64_t cn,
                              v_array<uint32_t> &leaf_labs) {
  if (b.nodes[cn].internal != -1)
    std::cout << "something is wrong, it should be a leaf node" << std::endl;

  leaf_labs.clear();
  for (size_t i = 0; i < b.nodes[cn].examples_index.size();
       i++) { // scan through each memory in the leaf
    uint32_t loc = b.nodes[cn].examples_index[i];
    for (uint32_t lab :
         b.examples[loc]->l.multilabels.label_v) { // scan through each label:
...
inline void train_one_against_some_at_leaf(memory_tree &b, single_learner &base,
                                           const uint64_t cn, example &ec) {
...
inline uint32_t compute_hamming_loss_via_oas(memory_tree &b,
                                             single_learner &base,
                                             const uint64_t cn, example &ec,
                                             v_array<uint32_t> &selected_labs) {
  selected_labs.delete_v();
  v_array<uint32_t> leaf_labs = v_init<uint32_t>();
  collect_labels_from_leaf(b, cn,
                           leaf_labs); // unique labels stored in the leaf.
  MULTILABEL::labels multilabels = ec.l.multilabels;
  MULTILABEL::labels preds = ec.pred.multilabels;
  ec.l.simple = {FLT_MAX, 1.f, 0.f};
  for (size_t i = 0; i < leaf_labs.size(); i++) {
...
int64_t pick_nearest(memory_tree &b, single_learner &base, const uint64_t cn,
                     example &ec) {
...
// for any two examples, use number of overlap labels to indicate the similarity
...
float F1_score_for_two_examples(example &ec1, example &ec2) {
  ...
  float v1 =
      (float)(num_overlaps / (1e-7 + ec1.l.multilabels.label_v.size() * 1.));
  float v2 =
      (float)(num_overlaps / (1e-7 + ec2.l.multilabels.label_v.size() * 1.));
  if (num_overlaps == 0.f)
    return 0.f;
  else
...
void predict(memory_tree &b, single_learner &base, example &ec) {
  ...
  while (b.nodes[cn].internal == 1) { // if it's internal{
    ...
    uint64_t newcn =
        ec.pred.scalar < 0
            ? b.nodes[cn].left
            : b.nodes[cn].right; // do not need to increment nl and nr.
    cn = newcn;
  }
  ...
  if (b.oas == false) {
    ...
    ec.loss =
        (float)compute_hamming_loss_via_oas(b, base, cn, ec, selected_labs);
    b.hamming_loss += ec.loss;
  }
}

float return_reward_from_node(memory_tree &b, single_learner &base, uint64_t cn,
                              example &ec, float weight = 1.f) {
  ...
  float reward = 0.f;
  closest_ec =
      pick_nearest(b, base, cn, ec); // no randomness for picking example.
  if (b.oas == false) {
    if ((closest_ec != -1) &&
        (b.examples[closest_ec]->l.multi.label == ec.l.multi.label))
      reward = 1.f;
  } else {
    if (closest_ec != -1)
      reward = F1_score_for_two_examples(ec, *b.examples[closest_ec]);
  ...
  if (b.oas == true)
    train_one_against_some_at_leaf(b, base, cn,
                                   ec); /// learn the inference procedure anyway

  return reward;
}

void learn_at_leaf_random(memory_tree &b, single_learner &base,
                          const uint64_t &leaf_id, example &ec,
                          const float &weight) {
  b.total_num_queries++;
  int32_t ec_id = -1;
  float reward = 0.f;
  if (b.nodes[leaf_id].examples_index.size() > 0) {
    uint32_t pos = uint32_t(b._random_state->get_and_update_random() *
                            b.nodes[leaf_id].examples_index.size());
...
void route_to_leaf(memory_tree &b, single_learner &base,
                   const uint32_t &ec_array_index, uint64_t cn,
  ...
  if (insertion == true) {
    ...
    if ((b.nodes[cn].examples_index.size() >= b.max_leaf_examples) &&
        (b.nodes.size() + 2 < b.max_nodes))
      split_leaf(b, base, cn);
  }
}

// we roll in, then stop at a random step, do exploration. //no real insertion
// happens in the function.
void single_query_and_learn(memory_tree &b, single_learner &base,
                            const uint32_t &ec_array_index, example &ec) {
  v_array<uint64_t> path_to_leaf = v_init<uint64_t>();
  route_to_leaf(b, base, ec_array_index, 0, path_to_leaf,
                false); // no insertion happens here.

  if (path_to_leaf.size() > 1) {
    // uint32_t random_pos =
    // merand48(b._random_state->get_current_state())*(path_to_leaf.size()-1);
    uint32_t random_pos = (uint32_t)(b._random_state->get_and_update_random() *
                                     (path_to_leaf.size())); // include leaf
    ...
    if (b.nodes[cn].internal != -1) { // if it's an internal node:'
      ...
      float coin =
          b._random_state->get_and_update_random() < prob_right ? 1.f : -1.f;
      float weight = path_to_leaf.size() * 1.f / (path_to_leaf.size() - 1.f);
      if (coin == -1.f) { // go left
        float reward_left_subtree =
            return_reward_from_node(b, base, b.nodes[cn].left, ec, weight);
        objective =
            (float)((1. - b.alpha) * log(b.nodes[cn].nl / b.nodes[cn].nr) +
                    b.alpha * (-reward_left_subtree / (1. - prob_right)) / 2.);
      } else { // go right:
        float reward_right_subtree =
            return_reward_from_node(b, base, b.nodes[cn].right, ec, weight);
        objective =
            (float)((1. - b.alpha) * log(b.nodes[cn].nl / b.nodes[cn].nr) +
      ...
      if (ec.weight >=
          100.f) // crop the weight, otherwise sometimes cause NAN outputs.
        ec.weight = 100.f;
      ...
      if (b.learn_at_leaf == true)
        learn_at_leaf_random(b, base, cn, ec, weight); // randomly sample one
                                                       // example, query reward,
                                                       // and update leaf
                                                       // learner
...
void update_rew(memory_tree &b, single_learner &base,
                const uint32_t &ec_array_index, example &ec) {
  single_query_and_learn(b, base, ec_array_index, ec);
}

// node here the ec is already stored in the b.examples, the task here is to
// rout it to the leaf,
// and insert the ec_array_index to the leaf.
void insert_example(memory_tree &b, single_learner &base,
                    const uint32_t &ec_array_index, bool fake_insert = false) {
  ...
  while (b.nodes[cn].internal == 1) // if it's internal node:
  {
    ...
    uint64_t newcn =
        insert_descent(b.nodes[cn], router_pred); // updated nr or nl
    cn = newcn;
  }

  if (b.oas == true) // if useing oas as inference procedure, we just train oas
                     // here, as it's independent of the memory
                     // unit anyway'
  ...
  if ((b.nodes[cn].internal == -1) && (fake_insert == false)) // get to leaf:
  {
    ...
    float leaf_pred = train_node(b, base, *b.examples[ec_array_index],
                                 cn);       // tain the leaf as well.
    insert_descent(b.nodes[cn], leaf_pred); // this is a faked descent, the
                                            // purpose is only to update nl and
                                            // nr of cn

    // if the number of examples exceeds the max_leaf_examples, and not reach
    // the max_nodes - 2 yet, we split:
    if ((b.nodes[cn].examples_index.size() >= b.max_leaf_examples) &&
        (b.nodes.size() + 2 <= b.max_nodes)) {
      split_leaf(b, base, cn);
    }
...
void learn(memory_tree &b, single_learner &base, example &ec) {
  if (b.test_mode == false) {
    ...
    if (b.iter % 5000 == 0) {
      if (b.oas == false)
        std::cout << "at iter " << b.iter << ", top(" << b.top_K
                  << ") pred error: " << b.num_mistakes * 1. / b.iter
                  << ", total num queires so far: " << b.total_num_queries
                  << ", max depth: " << b.max_depth
                  << ", max exp in leaf: " << b.max_ex_in_leaf << std::endl;
      else
        std::cout << "at iter " << b.iter
                  << ", avg hamming loss: " << b.hamming_loss * 1. / b.iter
                  << std::endl;
    }
    ...
    if (b.current_pass < 1) { // in the first pass, we need to store the memory:
      ...
      if (b.online == true)
        update_rew(b, base, (uint32_t)(b.examples.size() - 1),
                   *b.examples[b.examples.size() - 1]); // query and learn

      insert_example(
          b, base, (uint32_t)(b.examples.size() - 1)); // unsupervised learning.
      for (uint32_t i = 0; i < b.dream_repeats; i++)
        experience_replay(b, base);
    } else { // starting from the current pass, we just learn using
             // reinforcement signal, no insertion needed:
      size_t ec_id = (b.iter) % b.examples.size();
      update_rew(b, base, (uint32_t)ec_id,
                 *b.examples[ec_id]); // no insertion will happen in this call
      for (uint32_t i = 0; i < b.dream_repeats; i++)
        experience_replay(b, base);
    }
...
void end_pass(memory_tree &b) {
  ...
  std::cout << "######### Current Pass: " << b.current_pass
            << ", with number of memories strored so far: " << b.examples.size()
            << std::endl;
}
...
void save_load_example(example *ec, io_buf &model_file, bool &read, bool &text,
                       std::stringstream &msg, bool &oas) { // deal with tag
                                                            // deal with labels:
...
void save_load_node(node &cn, io_buf &model_file, bool &read, bool &text,
...
void save_load_memory_tree(memory_tree &b, io_buf &model_file, bool read,
                           bool text) {
  std::stringstream msg;
...
base_learner *memory_tree_setup(options_i &options, vw &all) {
  ...
  new_options
      ...
      .add(make_option("learn_at_leaf", tree->learn_at_leaf)
               .help("whether or not learn at leaf (defualt = True)"))
  ...
  tree->max_leaf_examples =
      (size_t)(tree->leaf_example_multiplier * (log(tree->max_nodes) / log(2)));
  ...
  if (tree->oas == false) {
    ...
    learner<memory_tree, example> &l =
        init_multiclass_learner(tree, as_singleline(setup_base(options, all)),
                                learn, predict, all.p, num_learners);
    // srand(time(0));
    ...
    learner<memory_tree, example> &l =
        init_learner(tree, as_singleline(setup_base(options, all)), learn,
                     predict, num_learners, prediction_type::multilabels);
