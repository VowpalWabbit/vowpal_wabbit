...
inline int random_sample_example_pop(memory_tree& b, uint64_t& cn)
{
  ...
  while (b.nodes[cn].internal == 1)
  {
    ...
    if (b.nodes[cn].nl < 1)       // no examples routed to left ever:
      ...
      pred = b.m_random_state->get_and_update_random() < (b.nodes[cn].nl * 1. / (b.nodes[cn].nr + b.nodes[cn].nl)) ? -1.f
                                                                                                                  : 1.f;
    else
    {
  ...
  {
    int loc_at_leaf = int(b.m_random_state->get_and_update_random() * b.nodes[cn].examples_index.size());
    ...
    memory_tree& b, single_learner& base, const uint64_t& leaf_id, example& ec, const float& weight)
{
  ...
  {
    uint32_t pos = uint32_t(b.m_random_state->get_and_update_random() * b.nodes[leaf_id].examples_index.size());
...
void single_query_and_learn(memory_tree& b, single_learner& base, const uint32_t& ec_array_index, example& ec)
{
  ...
  {
    // uint32_t random_pos = merand48(b.m_random_state->get_current_state())*(path_to_leaf.size()-1);
    uint32_t random_pos = (uint32_t)(b.m_random_state->get_and_update_random() * (path_to_leaf.size()));  // include leaf
    ...
    {  // if it's an internal node:'
      ...
      float coin = b.m_random_state->get_and_update_random() < prob_right ? 1.f : -1.f;
