...
typedef union {
  ...
  ACTION_SCORE::action_scores
      a_s; // a sequence of classes with scores.  Also used for probabilities.
  uint32_t multiclass;
  MULTILABEL::labels multilabels;
...
inline int example_is_newline(
    example &ec) { // if only index is constant namespace or no index
  if (ec.tag.size() > 0)
    return false;
  return (
      (ec.indices.size() == 0) ||
      ((ec.indices.size() == 1) && (ec.indices.last() == constant_namespace)));
}

inline bool valid_ns(char c) { return !(c == '|' || c == ':'); }

inline void add_passthrough_feature_magic(example &ec, uint64_t magic,
                                          uint64_t i, float x) {
  if (ec.passthrough)
...
#define add_passthrough_feature(ec, i, x)                                      \
  add_passthrough_feature_magic(                                               \
      ec, __FILE__[0] * 483901 + __FILE__[1] * 3417 + __FILE__[2] * 8490177,   \
      i, x);
