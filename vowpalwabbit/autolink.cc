...
void predict_or_learn(VW::autolink &b, LEARNER::single_learner &base,
                      example &ec) {
  if (is_learn)
    b.learn(base, ec);
  else
...
LEARNER::base_learner *autolink_setup(options_i &options, vw &all) {
  ...
  auto autolink_reduction =
      scoped_calloc_or_throw<VW::autolink>(d, all.weights.stride_shift());
  return make_base(
      init_learner(autolink_reduction, as_singleline(setup_base(options, all)),
