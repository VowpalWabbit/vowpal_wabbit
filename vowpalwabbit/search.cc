...
search_metatask *all_metatasks[] = {&DebugMT::metatask,
...
uint32_t AUTO_CONDITION_FEATURES = 1, AUTO_HAMMING_LOSS = 2,
         EXAMPLES_DONT_CHANGE = 4, IS_LDF = 8, NO_CACHING = 16,
         ACTION_COSTS = 32;
...
struct auto_condition_settings {
  size_t max_bias_ngram_length; // add a "bias" feature for each ngram up to and
                                // including this length. eg., if it's 1,
  // then you get a single feature for each conditional
  size_t max_quad_ngram_length; // add bias *times* input features for each
                                // ngram up to and including this length
  float feature_value; // how much weight should the conditional features get?
  bool use_passthrough_repr; // should we ask lower-level reductions for their
                             // internal state?
};
...
struct scored_action {
  ...
  scored_action(action _a = (action)-1, float _s = 0)
      : a(_a), s(_s) {} // , repr(v_init<feature>()) {}
  // scored_action(action _a, float _s, v_array<feature>& _repr) : a(_a), s(_s),
  // repr(_repr) {}
...
struct search_private {
  ...
  uint64_t offset;
  bool auto_condition_features; // do you want us to automatically add
                                // conditioning features?
  bool auto_hamming_loss; // if you're just optimizing hamming loss, we can do
                          // it for you!
  bool examples_dont_change; // set to true if you don't do any internal example
                             // munging
  bool is_ldf;               // user declared ldf
  bool use_action_costs; // task promises to define per-action rollout-by-ref
                         // costs

  v_array<int32_t>
      neighbor_features; // ugly encoding of neighbor feature requirements
  auto_condition_settings acset; // settings for auto-conditioning
  size_t history_length; // value of --search_history_length, used by some
                         // tasks, default 1

  size_t A;                 // total number of actions, [1..A]; 0 means ldf
  size_t num_learners;      // total number of learners;
  bool cb_learner;          // do contextual bandit learning on action (was "!
                            // rollout_all_actions" which was confusing)
  SearchState state;        // current state of learning
  size_t learn_learner_id;  // we allow user to use different learners for
                            // different states
  int mix_per_roll_policy;  // for MIX_PER_ROLL, we need to choose a policy to
                            // use; this is where it's stored (-2 means
                            // "not selected yet")
  bool no_caching;          // turn off caching
  size_t rollout_num_steps; // how many calls of "loss" before we stop really
                            // predicting on rollouts and switch to
                            // oracle (0 means "infinite")
  bool linear_ordering; // insist that examples are generated in linear order
                        // (rather that the default hoopla
                        // permutation)
  bool (*label_is_test)(
      polylabel &); // tell me if the label data from an example is test

  size_t t;                       // current search step
  size_t T;                       // length of root trajectory
  v_array<example> learn_ec_copy; // copy of example(s) at learn_t
  example *learn_ec_ref; // reference to example at learn_t, when there's no
                         // example munging
  size_t
      learn_ec_ref_cnt; // how many are there (for LDF mode only; otherwise 1)
  v_array<ptag> learn_condition_on; // a copy of the tags used for conditioning
  ...
  v_array<action>
      learn_allowed_actions; // which actions were allowed at training time?
  v_array<action_repr> ptag_to_action; // tag to action mapping for conditioning
  std::vector<action> test_action_sequence; // if test-mode was run, what was
  ...
  size_t loss_declared_cnt; // how many times did run declare any loss
  ...
  bool done_with_all_actions; // set to true when there are no more learn_a_idx
                              // to go
  ...
  float perturb_oracle; // with this probability, choose a random action instead
                        // of oracle action

  size_t num_calls_to_run, num_calls_to_run_previous, save_every_k_runs;

  // if we're printing to stderr we need to remember if we've printed the header
  // yet
  ...
  float alpha; // parameter used to adapt beta for dagger (see above comment),
               // should be in (0,1)

  RollMethod rollout_method;
  RollMethod rollin_method;
  float subsample_timesteps; // train at every time step or just a (random)
                             // subset?
  bool xv; // train three separate policies -- two for providing examples to the
           // other and a third training on the
           // union (which will be used at test time -- TODO)

  bool allow_current_policy; // should the current policy be used for training?
                             // true for dagger
  bool adaptive_beta; // used to implement dagger-like algorithms. if true, beta
                      // = 1-(1-alpha)^n after n updates, and
  // policy is mixed with oracle as \pi' = (1-beta)\pi^* + beta \pi
  size_t passes_per_policy; // if we're not in dagger-mode, then we need to know
                            // how many passes to train a policy
  ...
  size_t meta_t; // the metatask has it's own notion of time. meta_t+t, during a
                 // single run, is the way to think about
  // the "real" decision step but this really only matters for caching purposes
  v_array<v_array<action_cache> *> memo_foreach_action; // when foreach_action
                                                        // is on, we need to
                                                        // cache TRAIN
                                                        // trajectory actions
                                                        // for LEARN
  ...
  if (this->priv && this->priv->all) {
    ...
    if (!priv.examples_dont_change) {
      void (*delete_label)(void *) =
          priv.is_ldf ? CS::cs_label.delete_label : MC::mc_label.delete_label;
...
inline bool need_memo_foreach_action(search_private &priv) {
  return (priv.state == INIT_TRAIN) && (priv.metatask) &&
         (priv.metaoverride); // &&
  //        (priv.metaoverride->_foreach_action ||
  //        priv.metaoverride->_post_prediction);
}
...
int random_policy(search_private &priv, bool allow_current, bool allow_optimal,
  ...
  if (priv.beta >= 1) {
    ...
    if (allow_optimal)
      return -1;
    std::cerr << "internal error (bug): no valid policies to choose from!  "
  ...
  int num_valid_policies =
      (int)priv.current_policy + allow_optimal + allow_current;
  int pid = -1;
  ...
  if (num_valid_policies == 0) {
    ...
    pid = (advance_prng ? priv._random_state->get_and_update_random()
                        : priv._random_state->get_random()) >= priv.beta;
  else {
    // SPEEDUP this up in the case that beta is small!
    float r = (advance_prng ? priv._random_state->get_and_update_random()
                            : priv._random_state->get_random());
    pid = 0;
...
int select_learner(search_private &priv, int policy, size_t learner_id,
                   bool is_training, bool is_local) {
  if (policy < 0)
    ...
    if (priv.xv) {
      ...
      if (!is_local)
        learner_id +=
            1 + (size_t)(is_training ^ (priv.all->sd->example_number % 2 == 1));
    }
...
bool should_print_update(vw &all, bool hit_new_pass = false) {
  ...
  if (PRINT_UPDATE_EVERY_PASS && hit_new_pass)
    return true;
  return (all.sd->weighted_examples() >= all.sd->dump_interval) && !all.quiet &&
         !all.bfgs;
}
...
bool might_print_update(vw &all) {
  ...
  return (all.sd->weighted_examples() + 1. >= all.sd->dump_interval) &&
         !all.quiet && !all.bfgs;
}

bool must_run_test(vw &all, multi_ex &ec, bool is_test_ex) {
  return (all.final_prediction_sink.size() >
          0) || // if we have to produce output, we need to run this
         ...
         ((!all.quiet ||
           !all.vw_is_main) && // had to disable this because of library mode!
          (!is_test_ex) &&
...
void print_update(search_private &priv) {
  ...
  if (!priv.printed_output_header && !all.quiet) {
    ...
    fprintf(stderr, header_fmt, "average", "since", "instance", "current true",
            "current predicted", "cur", "cur", "predic", "cache", "examples",
            "");
    if (priv.active_csoaa)
      fprintf(stderr, header_fmt, "loss", "last", "counter", "output prefix",
              "output prefix", "pass", "pol", "made", "hits", "gener", "#run");
    else
      fprintf(stderr, header_fmt, "loss", "last", "counter", "output prefix",
              "output prefix", "pass", "pol", "made", "hits", "gener", "beta");
  ...
  bool use_heldout_loss = (!all.holdout_set_off && all.current_pass >= 1) &&
                          (all.sd->weighted_holdout_examples > 0);
  if (use_heldout_loss) {
    avg_loss = safediv((float)all.sd->holdout_sum_loss,
                       (float)all.sd->weighted_holdout_examples);
    avg_loss_since =
        safediv((float)all.sd->holdout_sum_loss_since_last_dump,
                (float)all.sd->weighted_holdout_examples_since_last_dump);

    all.sd->weighted_holdout_examples_since_last_dump = 0;
    all.sd->holdout_sum_loss_since_last_dump = 0.0;
  } else {
    avg_loss = safediv((float)all.sd->sum_loss,
                       (float)all.sd->weighted_labeled_examples);
  ...
  fprintf(stderr, "%-10.6f %-10.6f %8s  [%s] [%s] %5d %5d  %7s  %7s  %7s  %-8f",
          avg_loss, avg_loss_since, inst_cntr.c_str(), true_label, pred_label,
  ...
  if (PRINT_CLOCK_TIME) {
    size_t num_sec =
        (size_t)(((float)(clock() - priv.start_clock_time)) / CLOCKS_PER_SEC);
...
void add_new_feature(search_private &priv, float val, uint64_t idx) {
  ...
  features &fs =
      priv.dat_new_feature_ec->feature_space[priv.dat_new_feature_namespace];
  fs.push_back(val * priv.dat_new_feature_value,
  ...
  if (priv.all->audit) {
    ...
    fs.space_names.push_back(audit_strings_ptr(
        new audit_strings(*priv.dat_new_feature_feature_space, temp.str())));
  }
}

void del_features_in_top_namespace(search_private & /* priv */, example &ec,
                                   size_t ns) {
  if ((ec.indices.size() == 0) || (ec.indices.last() != ns)) {
    return;
    // if (ec.indices.size() == 0)
    //{ THROW("internal error (bug): expecting top namespace to be '" << ns <<
    //"' but it was empty"); }
    // else
    //{ THROW("internal error (bug): expecting top namespace to be '" << ns <<
    //"' but it was " <<
...
void add_neighbor_features(search_private &priv, multi_ex &ec_seq) {
  ...
  for (size_t n = 0; n < ec_seq.size();
       n++) // iterate over every example in the sequence
  {
    example &me = *ec_seq[n];
    for (size_t n_id = 0; n_id < priv.neighbor_features.size(); n_id++) {
      ...
      if (priv.all->audit) {
        ...
        priv.dat_new_feature_audit_ss.str("");
        priv.dat_new_feature_audit_ss << '@' << ((offset > 0) ? '+' : '-')
                                      << (char)(abs(offset) + '0');
        if (ns != ' ')
      ...
      else // this is actually a neighbor
      {
        ...
        GD::foreach_feature<search_private, add_new_feature>(
            priv.all, other.feature_space[ns], priv, me.ft_offset);
      }
    }
...
void reset_search_structure(search_private &priv) {
  ...
  if (priv.adaptive_beta) {
    ...
    priv.beta =
        (x <= log_of_2) ? -expm1f(-x) : (1 - expf(-x)); // numerical stability
    // float priv_beta = 1.f - powf(1.f - priv.alpha,
    // (float)priv.total_examples_generated);
  ...
  if (!priv.cb_learner) // was: if rollout_all_actions
  {
    priv._random_state->set_random_state(
        (uint32_t)(priv.read_example_last_id * 147483 + 4831921) * 2147483647);
  }
}
...
void search_declare_loss(search_private &priv, float loss) {
  ...
  case INIT_TRAIN:
    ...
    break;
  case LEARN:
    if ((priv.rollout_num_steps == 0) ||
        (priv.loss_declared_cnt <= priv.rollout_num_steps)) {
      priv.learn_loss += loss;
      cdbg << "priv.learn_loss += " << loss << " (now = " << priv.learn_loss
           << ")" << endl;
    }
    break;
  default:
...
template <class T> void cdbg_print_array(std::string str, v_array<T> &A) {
  cdbg << str << " = [";
  for (size_t i = 0; i < A.size(); i++)
    cdbg << " " << A[i];
  cdbg << " ]" << endl;
}
template <class T> void cerr_print_array(std::string str, v_array<T> &A) {
  std::cerr << str << " = [";
  for (size_t i = 0; i < A.size(); i++)
    std::cerr << " " << A[i];
  std::cerr << " ]" << endl;
...
void add_example_conditioning(search_private &priv, example &ec,
  ...
  size_t N = std::max(priv.acset.max_bias_ngram_length,
                      priv.acset.max_quad_ngram_length);
  for (size_t i = 0; i < I; i++) // position in conditioning
  {
    ...
    for (size_t n = 0; n < N; n++) // length of ngram
    {
      ...
      // we're going to add features for the ngram condition_on_actions[i ..
      // i+N]
      uint64_t name = condition_on_names[i + n];
      fid = fid * 328901 +
            71933 *
                ((condition_on_actions[i + n].a + 349101) * (name + 38490137));
      ...
      if (n < priv.acset.max_bias_ngram_length)
        add_new_feature(priv, 1., (uint64_t)4398201
                                      << priv.all->weights.stride_shift());
      // add the quadratic features
      if (n < priv.acset.max_quad_ngram_length)
        GD::foreach_feature<search_private, uint64_t, add_new_feature>(
            *priv.all, ec, priv);
    }
  }
    ...
    for (size_t i = 0; i < I; i++) {
      ...
      for (size_t k = 0; k < fs.size(); k++)
        if ((fs.values[k] > 1e-10) || (fs.values[k] < -1e-10)) {
          uint64_t fid = 84913 + 48371803 * (extra_offset + 8392817 * name) +
                         840137 * (4891 + fs.indicies[k]);
          if (priv.all->audit) {
            priv.dat_new_feature_audit_ss.str("");
            priv.dat_new_feature_audit_ss.clear();
            priv.dat_new_feature_audit_ss << "passthrough_repr_" << i << '_'
                                          << k;
          }
...
inline float cs_get_cost_partial_prediction(bool isCB, polylabel &ld,
                                            size_t k) {
...
inline void cs_cost_push_back(bool isCB, polylabel &ld, uint32_t index,
                              float value) {
  if (isCB) {
...
polylabel &allowed_actions_to_ld(search_private &priv, size_t ec_cnt,
  ...
  if (priv.is_ldf) // LDF version easier
  {
    ...
    } else // manually specified actions
    {
      ...
      for (action k = 0; k < allowed_actions_cnt; k++)
        cs_cost_push_back(isCB, ld, allowed_actions[k],
                          allowed_actions_cost[k]);
    }
  } else // non-LDF version, no action costs
  {
    if ((allowed_actions == nullptr) ||
        (allowed_actions_cnt == 0)) // any action is allowed
    {
      if (num_costs != priv.A) // if there are already A-many actions, they must
                               // be the right ones, unless the user did
      // something stupid like putting duplicate allowed_actions...
      {
        cs_costs_erase(isCB, ld);
        for (action k = 0; k < priv.A; k++)
          cs_cost_push_back(isCB, ld, k + 1,
                            FLT_MAX); //+1 because MC is 1-based
      }
...
void allowed_actions_to_label(search_private &priv, size_t ec_cnt,
                              const action *allowed_actions,
                              size_t allowed_actions_cnt,
                              const float *allowed_actions_cost,
  ...
  if (priv.is_ldf) // LDF version easier
  {
    ...
    for (action k = 0; k < ec_cnt; k++)
      cs_cost_push_back(isCB, lab, k, array_contains<action>(k, oracle_actions,
                                                             oracle_actions_cnt)
                                          ? 0.f
                                          : 1.f);
    // std::cerr << "lab = ["; for (size_t i=0; i<lab.cs.costs.size(); i++) cdbg
    // << ' ' << lab.cs.costs[i].class_index << ':'
    ...
    } else // manually specified actions
    {
      cs_costs_erase(isCB, lab);
      for (action k = 0; k < allowed_actions_cnt; k++)
        cs_cost_push_back(isCB, lab, allowed_actions[k],
                          allowed_actions_cost[k]);
    }
  } else // non-LDF, no action costs
  {
        ...
        (allowed_actions_cnt == 0)) // any action is allowed
    {
      ...
      // std::cerr << "lab = ["; for (size_t i=0; i<lab.cs.costs.size(); i++)
      // cdbg << ' ' << lab.cs.costs[i].class_index <<
      // ':' << lab.cs.costs[i].x; cdbg << " ]" << endl;
      if (oracle_actions_cnt <= 1) // common case to speed up
      {
        ...
        for (action k = 0; k < priv.A; k++)
          cs_set_cost_loss(
              isCB, lab, k,
              array_contains<action>(k + 1, oracle_actions, oracle_actions_cnt)
                  ? 0.f
                  : 1.f);
      }
    ...
    {
      cs_costs_erase(isCB, lab);
      float w = 1.; // array_contains<action>(3, oracle_actions,
                    // oracle_actions_cnt) ? 5.f : 1.f;
      for (size_t i = 0; i < allowed_actions_cnt; i++) {
        action k = allowed_actions[i];
        cs_cost_push_back(
            isCB, lab, k,
            (array_contains<action>(k, oracle_actions, oracle_actions_cnt))
                ? 0.f
                : w); // 1.f );
      }
    }
...
template <class T> void ensure_size(v_array<T> &A, size_t sz) {
...
action choose_oracle_action(search_private &priv, size_t ec_cnt,
                            const action *oracle_actions,
  ...
  if (priv.use_action_costs) {
    ...
    for (size_t k = 0; k < K; k++)
      cdbg << ' ' << allowed_actions_cost[k];
    cdbg << " ]" << endl;
    float min_cost = FLT_MAX;
    for (size_t k = 0; k < K; k++)
      min_cost = std::min(min_cost, allowed_actions_cost[k]);
    ...
    if (min_cost < FLT_MAX) {
      ...
      for (size_t k = 0; k < K; k++)
        if (allowed_actions_cost[k] <= min_cost) {
          cdbg << ", hit @ " << k;
          count++;
          if ((count == 1) || (priv._random_state->get_and_update_random() <
                               1. / (float)count)) {
            a = (allowed_actions == nullptr) ? (uint32_t)(k + 1)
                                             : allowed_actions[k];
            cdbg << "***";
          }
        }
  ...
  if (a == (action)-1) {
    ...
    a = (oracle_actions_cnt > 0)
            ...
            : (allowed_actions_cnt > 0)
                  ? allowed_actions[random(priv._random_state,
                                           allowed_actions_cnt)]
                  : priv.is_ldf
                        ? (action)random(priv._random_state, ec_cnt)
  ...
  if (need_memo_foreach_action(priv) && (priv.state == INIT_TRAIN)) {
    ...
    *this_cache = v_init<action_cache>();
    ...
    polylabel l = allowed_actions_to_ld(
        priv, 1, allowed_actions, allowed_actions_cnt, allowed_actions_cost);
    ...
    cdbg << "memo_foreach_action[" << priv.meta_t + priv.t - 1
         << "] = " << this_cache << " from oracle" << endl;
  }
  return a;
}

action single_prediction_notLDF(search_private &priv, example &ec, int policy,
                                const action *allowed_actions,
                                size_t allowed_actions_cnt,
                                const float *allowed_actions_cost,
                                float &a_cost,
                                action override_action) // if override_action !=
                                                        // -1, then we return it
                                                        // as the action and
                                                        // a_cost is set to the
// appropriate cost for that action
{
  vw &all = *priv.all;
  polylabel old_label = ec.l;
  bool need_partial_predictions =
      need_memo_foreach_action(priv) ||
      (priv.metaoverride && priv.metaoverride->_foreach_action) ||
      (override_action != (action)-1) || priv.active_csoaa;
  ...
  cdbg << "a=" << act << " from";
  if (allowed_actions) {
    for (size_t ii = 0; ii < allowed_actions_cnt; ii++)
      cdbg << ' ' << allowed_actions[ii];
  }
  cdbg << endl;
  ...
  if (need_partial_predictions) {
    ...
    for (size_t k = 0; k < K; k++) {
      ...
      if (priv.metaoverride && priv.metaoverride->_foreach_action)
        priv.metaoverride->_foreach_action(*priv.metaoverride->sch, priv.t - 1,
                                           min_cost, cl, cl == act, cost);
  ...
  if ((priv.state == INIT_TRAIN) &&
      (priv.subsample_timesteps <= -1)) // active learning
  {
    size_t K = cs_get_costs_size(priv.cb_learner, ec.l);
    ...
    if (min_cost2 < FLT_MAX)
      priv.active_uncertainty.push_back(
          std::make_pair(min_cost2 - min_cost, priv.t + priv.meta_t));
  }
  if ((priv.state == INIT_TRAIN) && priv.active_csoaa) {
    ...
    while (priv.active_known.size() <= cur_t) {
      ...
      priv.active_known[priv.active_known.size() - 1] =
          v_init<std::pair<CS::wclass &, bool>>();
    ...
    for (size_t k = 0; k < ec.l.cs.costs.size(); k++) {
      /* priv.active_known[cur_t].push_back( ec.l.cs.costs[k].pred_is_certain
                                          ...
                                          : FLT_MAX );
                                          cdbg << "active_known[" << cur_t <<
         "][" << (priv.active_known[cur_t].size() -
      ...
      bool query_needed =
          v_array_contains(ec.pred.multilabels.label_v, wc.class_index);
      std::pair<CS::wclass &, bool> p = {wc, query_needed};
      // Push into active_known[cur_t] with wc
      priv.active_known[cur_t].push_back(p);
      // cdbg << "active_known[" << cur_t << "][" <<
      // (priv.active_known[cur_t].size() - 1) << "] = " << wc.class_index
      // << ':' << wc.x << " pp=" << wc.partial_prediction << " query_needed="
      // << wc.query_needed << " max_pred=" <<
      // wc.max_pred << " min_pred=" << wc.min_pred << " is_range_overlapped="
      // << wc.is_range_overlapped << "
      // is_range_large=" << wc.is_range_large << endl;
      // query_needed=" << ec.l.cs.costs[k].query_needed << ", cost=" <<
      // ec.l.cs.costs[k].partial_prediction << "}" <<
      // endl;
    }
  }
  ...
  if ((priv.state == INIT_TEST) && (all.raw_prediction > 0)) {
    ...
    for (size_t k = 0; k < cs_get_costs_size(priv.cb_learner, ec.l); k++) {
      ...
      (*priv.rawOutputStringStream)
          << cs_get_cost_index(priv.cb_learner, ec.l, k) << ':'
          << cs_get_cost_partial_prediction(priv.cb_learner, ec.l, k);
    }
    all.print_text(all.raw_prediction, priv.rawOutputStringStream->str(),
                   ec.tag);
  }
...
action single_prediction_LDF(search_private &priv, example *ecs, size_t ec_cnt,
                             int policy, float &a_cost,
                             action override_action) // if override_action !=
                                                     // -1, then we return it as
                                                     // the action and a_cost is
                                                     // set to the
// appropriate cost for that action
{
  bool need_partial_predictions =
  ...
  size_t start_K =
      (priv.is_ldf && COST_SENSITIVE::ec_is_example_header(ecs[0])) ? 1 : 0;
  ...
  for (action a = (uint32_t)start_K; a < ec_cnt; a++) {
    ...
    cdbg << "partial_prediction[" << a << "] = " << ecs[a].partial_prediction
         << endl;
    ...
    if (override_action != (action)-1) {
      if (a == override_action)
        a_cost = ecs[a].partial_prediction;
               ...
               (ecs[a].partial_prediction < best_prediction)) {
      best_prediction = ecs[a].partial_prediction;
      best_action = a;
      a_cost = best_prediction;
    }
    if (this_cache)
      this_cache->push_back(
          action_cache(0., a, false, ecs[a].partial_prediction));
...
int choose_policy(search_private &priv, bool advance_prng = true) {
  RollMethod method = (priv.state == INIT_TEST)
                          ? POLICY
                          : (priv.state == LEARN)
                                ? priv.rollout_method
  ...
  switch (method) {
  case POLICY:
    return random_policy(priv,
                         priv.allow_current_policy || (priv.state == INIT_TEST),
                         false, advance_prng);
  ...
  case MIX_PER_ROLL:
    if (priv.mix_per_roll_policy == -2) // then we have to choose one!
      priv.mix_per_roll_policy =
          random_policy(priv, priv.allow_current_policy, true, advance_prng);
...
// returns true if found and do_store is false. if do_store is true, always
// returns true.
bool cached_action_store_or_find(search_private &priv, ptag mytag,
                                 const ptag *condition_on,
                                 const char *condition_on_names,
                                 action_repr *condition_on_actions,
                                 size_t condition_on_cnt, int policy,
                                 size_t learner_id, action &a, bool do_store,
                                 float &a_cost) {
  if (priv.no_caching)
    return do_store;
  if (mytag == 0)
  ...
  size_t sz = sizeof(size_t) + sizeof(ptag) + sizeof(int) + sizeof(size_t) +
              sizeof(size_t) +
              condition_on_cnt * (sizeof(ptag) + sizeof(action) + sizeof(char));
  if (sz % 4 != 0)
    sz += 4 - (sz % 4); // make sure sz aligns to 4 so that uniform_hash does
                        // the right thing
...
void generate_training_example(search_private &priv, polylabel &losses,
                               float weight, bool add_conditioning = true,
                               float min_loss = FLT_MAX) // min_loss = FLT_MAX
                                                         // means "please
                                                         // compute it for me as
                                                         // the actual min"; any
                                                         // other value
                                                         // means to use this
{
  ...
  if (priv.cb_learner) {
    if (min_loss == FLT_MAX)
      for (size_t i = 0; i < losses.cb.costs.size(); i++)
        min_loss = std::min(min_loss, losses.cb.costs[i].cost);
    for (size_t i = 0; i < losses.cb.costs.size(); i++)
      losses.cb.costs[i].cost = losses.cb.costs[i].cost - min_loss;
  } else {
    if (min_loss == FLT_MAX)
      for (size_t i = 0; i < losses.cs.costs.size(); i++)
        min_loss = std::min(min_loss, losses.cs.costs[i].x);
    for (size_t i = 0; i < losses.cs.costs.size(); i++)
      losses.cs.costs[i].x = (losses.cs.costs[i].x - min_loss) * weight;
  }
  // std::cerr << "losses = ["; for (size_t i=0; i<losses.cs.costs.size(); i++)
  // std::cerr << ' ' << losses.cs.costs[i].class_index
  ...
  if (!priv.is_ldf) // not LDF
  {
    ...
    ec.l = losses; // labels;
    if (add_conditioning)
      add_example_conditioning(priv, ec, priv.learn_condition_on.size(),
                               priv.learn_condition_on_names.begin(),
                               priv.learn_condition_on_act.begin());
    for (size_t is_local = 0; is_local <= (size_t)priv.xv; is_local++) {
      int learner = select_learner(priv, priv.current_policy,
                                   priv.learn_learner_id, true, is_local > 0);
      ec.in_use = true;
  ...
  } else // is  LDF
  {
    ...
    size_t start_K = (priv.is_ldf && COST_SENSITIVE::ec_is_example_header(
                                         priv.learn_ec_ref[0]))
                         ? 1
                         : 0;
    ...
    for (size_t is_local = 0; is_local <= (size_t)priv.xv; is_local++) {
      ...
      for (action a = (uint32_t)start_K; a < priv.learn_ec_ref_cnt; a++) {
        ...
        tmp.push_back(&ec);
        cdbg << "generate_training_example called learn on action a=" << a
             << ", costs.size=" << lab.costs.size() << " ec=" << &ec << endl;
        priv.total_examples_generated++;
      }
...
bool search_predictNeedsExample(search_private &priv) {
  ...
  case LEARN:
    if (priv.t + priv.meta_t < priv.learn_t)
      return false; // TODO: in meta search mode with foreach feature we'll need
                    // it even here
    if (priv.t + priv.meta_t == priv.learn_t)
      return true; // SPEEDUP: we really only need it on the last learn_a, but
  ...
  int pol =
      choose_policy(priv, false); // choose a policy but don't advance prng
  return (pol != -1);
}

void foreach_action_from_cache(search_private &priv, size_t t,
                               action override_a = (action)-1) {
  cdbg << "foreach_action_from_cache: t=" << t
       << ", memo_foreach_action.size()=" << priv.memo_foreach_action.size()
       << ", override_a=" << override_a << endl;
  assert(t < priv.memo_foreach_action.size());
  v_array<action_cache> *cached = priv.memo_foreach_action[t];
  if (!cached)
    return; // the only way this can happen is if the metatask overrode this
            // action
  cdbg << "memo_foreach_action size = " << cached->size() << endl;
  for (size_t id = 0; id < cached->size(); id++) {
    action_cache &ac = (*cached)[id];
    priv.metaoverride->_foreach_action(
        *priv.metaoverride->sch, t - priv.meta_t, ac.min_cost, ac.k,
        (override_a == (action)-1) ? ac.is_opt : (ac.k == override_a), ac.cost);
  }
}

// note: ec_cnt should be 1 if we are not LDF
action search_predict(search_private &priv, example *ecs, size_t ec_cnt,
                      ptag mytag, const action *oracle_actions,
                      size_t oracle_actions_cnt, const ptag *condition_on,
                      const char *condition_on_names,
                      const action *allowed_actions, size_t allowed_actions_cnt,
                      const float *allowed_actions_cost, size_t learner_id,
                      float &a_cost, float /* weight */) {
  ...
  assert(((allowed_actions == nullptr) && (allowed_actions_cost == nullptr)) ==
         (allowed_actions_cnt == 0));
  ...
  if ((priv.state == GET_TRUTH_STRING) || priv.force_oracle) {
    action a = choose_oracle_action(priv, ec_cnt, oracle_actions,
                                    oracle_actions_cnt, allowed_actions,
                                    allowed_actions_cnt, allowed_actions_cost);
    // if (priv.metaoverride && priv.metaoverride->_post_prediction)
    //  priv.metaoverride->_post_prediction(*priv.metaoverride->sch,
    //  t-priv.meta_t, a, 0.);
    a_cost = 0.;
    return a;
  }
  ...
  if ((priv.state == LEARN) && (t < priv.learn_t)) {
    ...
    cdbg << "LEARN " << t << " < priv.learn_t ==> a=" << a
         << ", a_cost=" << a_cost << endl;
    if (priv.metaoverride && priv.metaoverride->_foreach_action)
      foreach_action_from_cache(priv, t);
    if (priv.metaoverride && priv.metaoverride->_post_prediction)
      priv.metaoverride->_post_prediction(*priv.metaoverride->sch,
                                          t - priv.meta_t, a, a_cost);
    return a;
  }

  // for LDF, # of valid actions is ec_cnt; otherwise it's either
  // allowed_actions_cnt or A
  size_t valid_action_cnt = priv.is_ldf ? ec_cnt : (allowed_actions_cnt > 0)
                                                       ? allowed_actions_cnt
                                                       : priv.A;
  ...
  if ((priv.state == LEARN) && (t == priv.learn_t)) {
    ...
    if (priv.learn_a_idx >= valid_action_cnt) {
      ...
      if (priv.examples_dont_change)
        ...
        size_t label_size =
            priv.is_ldf ? sizeof(CS::label) : sizeof(MC::label_t);
        void (*label_copy_fn)(void *, void *) =
            priv.is_ldf ? CS::cs_label.copy_label : nullptr;

        ensure_size(priv.learn_ec_copy, ec_cnt);
        for (size_t i = 0; i < ec_cnt; i++)
          VW::copy_example_data(priv.all->audit, priv.learn_ec_copy.begin() + i,
                                ecs + i, label_size, label_copy_fn);
      ...
      if (priv.auto_condition_features) {
        ...
        priv.learn_condition_on.end() =
            priv.learn_condition_on.begin() +
            condition_on_cnt; // allow .size() to be used in lieu of _cnt

        memcpy(priv.learn_condition_on.begin(), condition_on,
               condition_on_cnt * sizeof(ptag));

        for (size_t i = 0; i < condition_on_cnt; i++)
          push_at(priv.learn_condition_on_act,
                  action_repr(((1 <= condition_on[i]) &&
                               (condition_on[i] < priv.ptag_to_action.size()))
        ...
        if (condition_on_names == nullptr) {
          ...
          ensure_size(priv.learn_condition_on_names,
                      strlen(condition_on_names) + 1);
      ...
      if (allowed_actions && (allowed_actions_cnt > 0)) {
        ...
        memcpy(priv.learn_allowed_actions.begin(), allowed_actions,
               allowed_actions_cnt * sizeof(action));
        cdbg_print_array("in LEARN, learn_allowed_actions",
                         priv.learn_allowed_actions);
      }
    }
    ...
    a_cost = 0.;
    action a_name = (allowed_actions && (allowed_actions_cnt > 0))
                        ? allowed_actions[a]
                        : priv.is_ldf ? a : (a + 1);
    if (priv.metaoverride && priv.metaoverride->_foreach_action) {
      foreach_action_from_cache(priv, t, a_name);
      if (priv.memo_foreach_action[t]) {
        cdbg << "@ memo_foreach_action: t=" << t << ", a=" << a
             << ", cost=" << (*priv.memo_foreach_action[t])[a].cost << endl;
  ...
  if ((priv.state == LEARN) && (t > priv.learn_t) &&
      (priv.rollout_num_steps > 0) &&
      (priv.loss_declared_cnt >= priv.rollout_num_steps)) {
    cdbg << "... skipping" << endl;
    action a = priv.is_ldf ? 0 : ((allowed_actions && (allowed_actions_cnt > 0))
                                      ? allowed_actions[0]
                                      : 1);
    if (priv.metaoverride && priv.metaoverride->_post_prediction)
      priv.metaoverride->_post_prediction(*priv.metaoverride->sch,
  ...
  if ((priv.state == INIT_TRAIN) || (priv.state == INIT_TEST) ||
    ...
    bool gte_here = (priv.state == INIT_TRAIN) &&
    ...
    if (priv.metaoverride && priv.metaoverride->_maybe_override_prediction &&
        (priv.state !=
         LEARN)) // if LEARN and t>learn_t,then we cannot allow overrides!
    {
      skip = priv.metaoverride->_maybe_override_prediction(
          *priv.metaoverride->sch, t - priv.meta_t, a, a_cost);
      cdbg << "maybe_override_prediction --> " << skip << ", a=" << a
           << ", a_cost=" << a_cost << endl;
    ...
    if ((!skip) && (policy == -1))
      a = choose_oracle_action(priv, ec_cnt, oracle_actions, oracle_actions_cnt,
                               allowed_actions, allowed_actions_cnt,
                               allowed_actions_cost); // TODO: we probably want
                                                      // to actually get costs
                                                      // for oracle actions???

    bool need_fea = (policy == -1) && priv.metaoverride &&
                    priv.metaoverride->_foreach_action;

    if ((policy >= 0) || gte_here ||
        need_fea) // the last case is we need to do foreach action
    {
      ...
      if ((!skip) && (!need_fea) && not_test &&
          cached_action_store_or_find(
              ...
              priv.condition_on_actions.begin(), condition_on_cnt, policy,
      ...
      {
        size_t start_K =
            (priv.is_ldf && COST_SENSITIVE::ec_is_example_header(ecs[0])) ? 1
                                                                          : 0;
        priv.last_action_repr.clear();
        if (priv.auto_condition_features)
          for (size_t n = start_K; n < ec_cnt; n++)
            add_example_conditioning(priv, ecs[n], condition_on_cnt,
                                     condition_on_names,
                                     priv.condition_on_actions.begin());

        if (((!skip) && (policy >= 0)) ||
            need_fea) // only make a prediction if we're going to use the output
        {
          ...
          a = priv.is_ldf
                  ? single_prediction_LDF(priv, ecs, ec_cnt, learner, a_cost,
                                          need_fea ? a : (action)-1)
                  : single_prediction_notLDF(
                        priv, *ecs, learner, allowed_actions,
                        allowed_actions_cnt, allowed_actions_cost, a_cost,
                        need_fea ? a : (action)-1);

          cdbg << "passthrough = [";
          for (size_t kk = 0; kk < priv.last_action_repr.size(); kk++)
            cdbg << ' ' << priv.last_action_repr.indicies[kk] << ':'
                 << priv.last_action_repr.values[kk];
          cdbg << " ]" << endl;
        ...
        if (gte_here) {
          cdbg << "INIT_TRAIN, NO_ROLLOUT, at least one oracle_actions, a=" << a
               << endl;
          // we can generate a training example _NOW_ because we're not doing
          // rollouts
          // allowed_actions_to_losses(priv, ec_cnt, allowed_actions,
          // allowed_actions_cnt, oracle_actions,
          // oracle_actions_cnt, losses);
          allowed_actions_to_label(priv, ec_cnt, allowed_actions,
                                   allowed_actions_cnt, allowed_actions_cost,
                                   oracle_actions, oracle_actions_cnt,
                                   priv.gte_label);
          cdbg << "priv.gte_label = [";
          for (size_t i = 0; i < priv.gte_label.cs.costs.size(); i++)
            cdbg << ' ' << priv.gte_label.cs.costs[i].class_index << ':'
                 << priv.gte_label.cs.costs[i].x;
          cdbg << " ]" << endl;

          priv.learn_ec_ref = ecs;
          priv.learn_ec_ref_cnt = ec_cnt;
          if (allowed_actions) {
            ensure_size(priv.learn_allowed_actions,
                        allowed_actions_cnt); // TODO: do we really need this?
            memcpy(priv.learn_allowed_actions.begin(), allowed_actions,
                   allowed_actions_cnt * sizeof(action));
          }
          size_t old_learner_id = priv.learn_learner_id;
          priv.learn_learner_id = learner_id;
          generate_training_example(priv, priv.gte_label, 1.,
                                    false); // this is false because the
        ...
        if (not_test && (!skip))
          cached_action_store_or_find(
              priv, mytag, condition_on, condition_on_names,
              priv.condition_on_actions.begin(), condition_on_cnt, policy,
              learner_id, a, true, a_cost);
      }
    }

    if (priv.state == INIT_TRAIN)
      priv.train_trajectory.push_back(
          scored_action(a, a_cost)); // note the action for future reference
...
inline bool cmp_size_t_pair(const std::pair<size_t, size_t> &a,
...
void hoopla_permute(size_t *B, size_t *end) {
  ...
  size_t i = 0,
         j = N -
             1; // which parts of B have we already covered? [0,i] and [j,N-1]
  while (i + 1 < j) {
...
void get_training_timesteps(search_private &priv, v_array<size_t> &timesteps) {
  ...
  if (priv.subsample_timesteps <= -1) {
    for (size_t i = 0; i < priv.active_uncertainty.size(); i++)
      if (priv._random_state->get_and_update_random() >
          priv.active_uncertainty[i].first)
        timesteps.push_back(priv.active_uncertainty[i].second - 1);
    /*
    float k = (float)priv.total_examples_generated;
    priv.ec_seq[t]->revert_weight =
    priv.all->loss->getRevertingWeight(priv.all->sd, priv.ec_seq[t].pred.scalar,
    priv.all->eta / powf(k, priv.all->power_t)); float importance =
    query_decision(active_str, *priv.ec_seq[t], k); if
  ...
  // if subsample in (0,1) then pick steps with that probability, but ensuring
  // there's at least one!
    ...
    if (timesteps.size() == 0) // ensure at least one
      timesteps.push_back(
          (size_t)(priv._random_state->get_and_update_random() * priv.T));
  }

  // finally, if subsample >= 1, then pick (int) that many uniformly at random
  // without replacement; could use an LFSR
  // but why? :P
  else {
    while ((timesteps.size() < (size_t)priv.subsample_timesteps) &&
           (timesteps.size() < priv.T)) {
      size_t t =
          (size_t)(priv._random_state->get_and_update_random() * (float)priv.T);
...
void verify_active_csoaa(COST_SENSITIVE::label &losses,
  ...
  for (COST_SENSITIVE::wclass &wc : losses.costs) {
    if (!known[i].second) {
      ...
      if (err > threshold) {
        std::cerr << "verify_active_csoaa failed: truth " << wc.class_index
                  << ":" << wc.x << ", known[" << i
                  << "]=" << known[i].first.partial_prediction
                  << ", error=" << err << " vs threshold " << threshold << endl;
      }
    }
    i++;
  }
...
void advance_from_known_actions(search_private &priv) {
  ...
  if (t >= priv.active_known.size())
    return;
  cdbg << "advance_from_known_actions t=" << t
       << " active_known.size()=" << priv.active_known.size()
  ...
  if (priv.learn_a_idx >= priv.active_known[t].size()) {
    cdbg << "advance_from_known_actions setting done_with_all_actions=true "
            "(active_known[t].size()="
  ...
  cs_cost_push_back(priv.cb_learner,
                    priv.learn_losses,
                    priv.is_ldf ? (uint32_t)(priv.learn_a_idx - 1) :
  (uint32_t)priv.learn_a_idx,
                    priv.active_known[t][priv.learn_a_idx],
                    true);
  */
  priv.learn_losses.cs.costs.push_back(
      priv.active_known[t][priv.learn_a_idx].first);
  cdbg << "  --> adding " << priv.learn_a_idx << ":"
       << priv.active_known[t][priv.learn_a_idx].first.x << endl;
  priv.learn_a_idx++;
...
void train_single_example(search &sch, bool is_test_ex, bool is_holdout_ex,
  ...
  search_private &priv = *sch.priv;
  vw &all = *priv.all;
  bool ran_test = false; // we must keep track so that even if we skip test, we
                         // still update # of examples seen

  // if (! priv.no_caching)
  clear_cache_hash_map(priv);

  cdbg << "is_test_ex=" << is_test_ex << " vw_is_main=" << all.vw_is_main
       << endl;
  ...
  if (must_run_test(all, ec_seq, is_test_ex)) {
    cdbg << "======================================== INIT TEST ("
         << priv.current_policy << "," << priv.read_example_last_pass
         << ") ========================================" << endl;
    ...
    priv.state = INIT_TEST;
    priv.should_produce_string = might_print_update(all) ||
                                 (all.final_prediction_sink.size() > 0) ||
                                 (all.raw_prediction > 0);
    priv.pred_string->str("");
    ...
    // accumulate loss
    if (!is_test_ex)
      all.sd->update(ec_seq[0]->test_only, !is_test_ex, priv.test_loss, 1.f,
                     priv.num_features);
  ...
  if ((!is_learn) || is_test_ex || is_holdout_ex || ec_seq[0]->test_only ||
      (!priv.all->training))
    return;
  ...
  priv.train_trajectory
      .clear(); // this is where we'll store the training sequence
  run_task(sch, ec_seq);

  if (!ran_test) // was  && !priv.ec_seq[0]->test_only) { but we know it's not
                 // test_only
    all.sd->update(ec_seq[0]->test_only, true, priv.test_loss, 1.f,
                   priv.num_features);

  // if there's nothing to train on, we're done!
  if ((priv.loss_declared_cnt == 0) || (priv.t + priv.meta_t == 0) ||
      (priv.rollout_method ==
       NO_ROLLOUT)) // TODO: make sure NO_ROLLOUT works with beam!
  {
    return;
  }
  ...
  for (size_t tid = 0; tid < priv.timesteps.size(); tid++) {
    cdbg << "timestep = " << priv.timesteps[tid] << " [" << tid << "/"
         << priv.timesteps.size() << "]" << endl;

    if (priv.metatask && !priv.memo_foreach_action[tid]) {
      cdbg << "skipping because it looks like this was overridden by metatask"
           << endl;
      continue;
    }
    ...
    while (!priv.done_with_all_actions) {
      ...
      cdbg << "----------------------------------------------------------------"
              "---------------------"
           << endl;
      cdbg << "learn_t = " << priv.learn_t
           << ", learn_a_idx = " << priv.learn_a_idx << endl;
      // cdbg_print_array("priv.active_known[learn_t]",
      // priv.active_known[priv.learn_t]);
      run_task(sch, ec_seq);
      // cerr_print_array("in GENER, learn_allowed_actions",
      // priv.learn_allowed_actions);
      ...
      //                          priv.learn_allowed_actions[priv.learn_a_idx-1]
      //                          : priv.is_ldf ? (priv.learn_a_idx-1) :
      //                          (priv.learn_a_idx),
      //                           priv.learn_loss);
    }
    if (priv.active_csoaa_verify > 0.)
      verify_active_csoaa(priv.learn_losses.cs, priv.active_known[priv.learn_t],
                          ec_seq[0]->example_counter, priv.active_csoaa_verify);
    ...
    if (skipped_all_actions) {
      ...
      cdbg << "<<<<<" << endl;
      cdbg << "skipped all actions; learn_t = " << priv.learn_t
           << ", learn_a_idx = " << priv.learn_a_idx << endl;
    ...
    if (priv.learn_allowed_actions.size() > 0) {
      for (size_t i = 0; i < priv.learn_allowed_actions.size(); i++) {
        priv.learn_losses.cs.costs[i].class_index =
            priv.learn_allowed_actions[i];
      }
    }
    // float min_loss = 0.;
    // if (priv.metatask)
    //  for (size_t aid=0; aid<priv.memo_foreach_action[tid]->size(); aid++)
    //    min_loss = std::min(min_loss,
    //    priv.memo_foreach_action[tid]->get(aid).cost);
    cdbg << "priv.learn_losses = [";
    for (auto &wc : priv.learn_losses.cs.costs)
      cdbg << " " << wc.class_index << ":" << wc.x;
    cdbg << " ]" << endl;
    cdbg << "gte" << endl;
    generate_training_example(priv, priv.learn_losses, 1.,
                              true); // , min_loss);  // TODO: weight
    if (!priv.examples_dont_change)
...
void inline adjust_auto_condition(search_private &priv) {
  if (priv.auto_condition_features) {
    ...
    if ((priv.history_length == 0) || (priv.acset.feature_value == 0.f)) {
      std::cerr << "warning: turning off AUTO_CONDITION_FEATURES because "
                   "settings make it useless"
                << endl;
...
void do_actual_learning(search &sch, base_learner &base, multi_ex &ec_seq) {
  ...
  // if we're going to have to print to the screen, generate the "truth"
  // std::string
  cdbg << "======================================== GET TRUTH STRING ("
       << priv.current_policy << "," << priv.read_example_last_pass
       << ") ========================================" << endl;
...
void end_pass(search &sch) {
  ...
  if (priv.passes_since_new_policy >= priv.passes_per_policy) {
    ...
    if (priv.current_policy > priv.total_number_of_policies) {
      std::cerr << "internal error (bug): too many policies; not advancing"
                << endl;
      priv.current_policy = priv.total_number_of_policies;
    }
    // reset search_trained_nb_policies in options_from_file so it is saved to
    // regressor file later
    // TODO work out a better system to update state that will be saved in the
    // model.
    all->options->replace("search_trained_nb_policies",
                          std::to_string(priv.current_policy));
    all->options->get_typed_option<uint32_t>("search_trained_nb_policies")
        .value(priv.current_policy);
  }
}
...
void end_examples(search &sch) {
  ...
  if (all->training) {
    // TODO work out a better system to update state that will be saved in the
    // model.
    // Dig out option and change it in case we already loaded a predictor which
    // had a value stored for
    // --search_trained_nb_policies
    auto val = (priv.passes_since_new_policy == 0) ? priv.current_policy
    ...
    all->options->replace("search_total_nb_policies",
                          std::to_string(priv.total_number_of_policies));
    all->options->get_typed_option<uint32_t>("search_total_nb_policies")
        .value(priv.total_number_of_policies);
  }
}
...
void handle_condition_options(vw &all, auto_condition_settings &acset) {
  ...
  new_options.add(
      make_option("search_max_bias_ngram_length", acset.max_bias_ngram_length)
          .keep()
          .default_value(1)
          .help(
              "add a \"bias\" feature for each ngram up to and including this "
              "length. eg., if it's 1 "
              "(default), then you get a single feature for each conditional"));
  new_options.add(
      make_option("search_max_quad_ngram_length", acset.max_quad_ngram_length)
          .keep()
          .default_value(0)
          .help("add bias *times* input features for each ngram up to and "
                "including this length (def: 0)"));
  new_options.add(
      make_option("search_condition_feature_value", acset.feature_value)
          .keep()
          .default_value(1.f)
          .help("how much weight should the conditional features get? (def: "
                "1.)"));
  new_options.add(
      make_option("search_use_passthrough_repr", acset.use_passthrough_repr)
          .keep()
          .help("should we use lower-level reduction _internal state_ as "
                "additional features? (def: no)"));
...
v_array<CS::label> read_allowed_transitions(action A, const char *filename) {
  ...
  while ((rd = fscanf(f, "%d:%d", &from, &to)) > 0) {
    if ((from < 0) || (from > (int)A)) {
      std::cerr << "warning: ignoring transition from " << from
                << " because it's out of the range [0," << A << "]" << endl;
    }
    if ((to < 0) || (to > (int)A)) {
      std::cerr << "warning: ignoring transition to " << to
                << " because it's out of the range [0," << A << "]" << endl;
    }
    bg[from * (A + 1) + to] = true;
    count++;
  ...
  std::cerr << "read " << count << " allowed transitions from " << filename
            << endl;
...
void parse_neighbor_features(std::string &nf_string, search &sch) {
  ...
  while (p != 0) {
    ...
    if (cmd.size() == 1) {
      ...
      std::cerr << "warning: ignoring malformed neighbor specification: '" << p
                << "'" << endl;
    }
...
base_learner *setup(options_i &options, vw &all) {
  ...
  new_options.add(
      make_option("search", priv.A).keep().help("Use learning to search, "
                                                "argument=maximum action id or "
                                                "0 for LDF"));
  ...
  new_options.add(make_option("search_metatask", metatask_string)
                      .keep()
                      .help("the search metatask (use \"--search_metatask "
                            "list\" to get a list of available metatasks)"));
  new_options.add(
      make_option("search_interpolation", interpolation_string)
          .keep()
  ...
  new_options.add(
      make_option("search_rollout", rollout_string)
          .help("how should rollouts be executed?           "
                "[policy|oracle|*mix_per_state|mix_per_roll|none]"));
  ...
  new_options.add(
      make_option("search_passes_per_policy", priv.passes_per_policy)
          .default_value(1)
          .help("number of passes per policy (only valid for "
                "search_interpolation=policy)"));
  new_options.add(make_option("search_beta", priv.beta)
                      .default_value(0.5f)
                      .help("interpolation rate for policies (only valid for "
                            "search_interpolation=policy)"));
  new_options.add(make_option("search_alpha", priv.alpha)
                      .default_value(1e-10f)
                      .help("annealed beta = 1-(1-alpha)^t (only valid for "
                            "search_interpolation=data)"));
  new_options.add(
      make_option("search_total_nb_policies", priv.total_number_of_policies)
          .help("if we are going to train the policies through multiple "
                "separate calls to vw, we need to "
                "specify this parameter and tell vw how many policies are "
                "eventually going to be trained"));
  new_options.add(
      make_option("search_trained_nb_policies", search_trained_nb_policies)
          .help("the number of trained policies in a file"));
  new_options.add(
      make_option("search_allowed_transitions", search_allowed_transitions)
          .help("read file of allowed transitions [def: all transitions are "
                "allowed]"));
  new_options.add(make_option("search_subsample_time", priv.subsample_timesteps)
                      .help("instead of training at all timesteps, use a "
                            "subset. if value in (0,1), train on a random "
                            "v%. if v>=1, train on precisely v steps per "
                            "example, if v<=-1, use active learning"));
  new_options.add(
      make_option("search_neighbor_features", neighbor_features_string)
          .keep()
          .help("copy features from neighboring lines. argument looks like: "
                "'-1:a,+2' meaning copy previous line "
                "namespace a and next next line from namespace _unnamed_, "
                "where ',' separates them"));
  new_options.add(
      make_option("search_rollout_num_steps", priv.rollout_num_steps)
          .help("how many calls of \"loss\" before we stop really predicting "
                "on rollouts and switch to "
                "oracle (default means \"infinite\")"));
  new_options.add(make_option("search_history_length", priv.history_length)
                      .keep()
                      .default_value(1)
                      .help("some tasks allow you to specify how much history "
                            "their depend on; specify that here"));
  new_options.add(make_option("search_no_caching", priv.no_caching)
                      .help("turn off the built-in caching ability (makes "
                            "things slower, but technically more safe)"));
  new_options.add(
      make_option("search_xv", priv.xv)
          .help(
              "train two separate policies, alternating prediction/learning"));
  new_options.add(
      make_option("search_perturb_oracle", priv.perturb_oracle)
          .default_value(0.f)
  ...
  new_options.add(make_option("search_linear_ordering", priv.linear_ordering)
                      .help("insist on generating examples in linear order "
                            "(def: hoopla permutation)"));
  new_options.add(make_option("search_active_verify", priv.active_csoaa_verify)
                      .help("verify that active learning is doing the right "
                            "thing (arg = multiplier, should be = "
                            "cost_range * range_c)"));
  new_options.add(
      make_option("search_save_every_k_runs", priv.save_every_k_runs)
  ...
  if ((rollout_string.compare("policy") == 0) ||
      (rollout_string.compare("learn") == 0))
    priv.rollout_method = POLICY;
  else if ((rollout_string.compare("oracle") == 0) ||
           (rollout_string.compare("ref") == 0))
    priv.rollout_method = ORACLE;
  else if ((rollout_string.compare("mix_per_state") == 0))
    priv.rollout_method = MIX_PER_STATE;
  else if ((rollout_string.compare("mix_per_roll") == 0) ||
           (rollout_string.compare("mix") == 0))
    priv.rollout_method = MIX_PER_ROLL;
  else if ((rollout_string.compare("none") == 0)) {
    priv.rollout_method = NO_ROLLOUT;
    priv.no_caching = true;
  } else
    THROW("error: --search_rollout must be 'learn', 'ref', 'mix', "
          "'mix_per_state' or 'none'");

  if ((rollin_string.compare("policy") == 0) ||
      (rollin_string.compare("learn") == 0))
    priv.rollin_method = POLICY;
  else if ((rollin_string.compare("oracle") == 0) ||
           (rollin_string.compare("ref") == 0))
    priv.rollin_method = ORACLE;
  else if ((rollin_string.compare("mix_per_state") == 0))
    priv.rollin_method = MIX_PER_STATE;
  else if ((rollin_string.compare("mix_per_roll") == 0) ||
           (rollin_string.compare("mix") == 0))
    priv.rollin_method = MIX_PER_ROLL;
  else
    THROW("error: --search_rollin must be 'learn', 'ref', 'mix' or "
          "'mix_per_state'");

  // check if the base learner is contextual bandit, in which case, we dont
  // rollout all actions.
  ...
  ensure_param(priv.beta, 0.0, 1.0, 0.5,
               "warning: search_beta must be in (0,1); resetting to 0.5");
  ensure_param(priv.alpha, 0.0, 1.0, 1e-10f,
               "warning: search_alpha must be in (0,1); resetting to 1e-10");

  priv.num_calls_to_run = 0;

  // compute total number of policies we will have at end of training
  // we add current_policy for cases where we start from an initial set of
  // policies loaded through -i option
  uint32_t tmp_number_of_policies = priv.current_policy;
  if (all.training)
    tmp_number_of_policies +=
        (int)ceil(((float)all.numpasses) / ((float)priv.passes_per_policy));

  // the user might have specified the number of policies that will eventually
  // be trained through multiple vw calls,
  // so only set total_number_of_policies to computed value if it is larger
  cdbg << "current_policy=" << priv.current_policy
  ...
  if (tmp_number_of_policies > priv.total_number_of_policies) {
    ...
    if (priv.current_policy > 0) // we loaded a file but total number of
                                 // policies didn't match what is needed for
                                 // training
      std::cerr << "warning: you're attempting to train more classifiers than "
                   "was allocated initially. Likely to cause "
                   "bad performance."
                << endl;
  }

  // current policy currently points to a new policy we would train
  // if we are not training and loaded a bunch of policies for testing, we need
  // to subtract 1 from current policy
  // so that we only use those loaded when testing (as run_prediction is called
  // with allow_current to true)
  ...
  cdbg << "search current_policy = " << priv.current_policy
       << " total_number_of_policies = " << priv.total_number_of_policies
       << endl;
  ...
  if (priv.task == nullptr) {
    if (!options.was_supplied("help"))
      THROW("fail: unknown task for --search_task '"
            << task_string << "'; use --search_task list to get a list");
  }
  ...
  if (!options.was_supplied("csoaa") && !options.was_supplied("cs_active") &&
      !options.was_supplied("csoaa_ldf") && !options.was_supplied("wap_ldf") &&
  ...
  cdbg << "active_csoaa = " << priv.active_csoaa
       << ", active_csoaa_verify = " << priv.active_csoaa_verify << endl;

  base_learner *base = setup_base(*all.options, all);

  // default to OAA labels unless the task wants to override this (which they
  // can do in initialize)
  ...
  if (options.was_supplied("search_allowed_transitions"))
    read_allowed_transitions((action)priv.A,
                             search_allowed_transitions.c_str());

  // set up auto-history (used to only do this if AUTO_CONDITION_FEATURES was
  // on, but that doesn't work for hooktask)
  ...
  learner<search, multi_ex> &l =
      init_learner(sch, make_base(*base), do_actual_learning<true>,
...
float action_cost_loss(action a, const action *act, const float *costs,
                       size_t sz) {
  if (act == nullptr)
    return costs[a - 1];
  for (size_t i = 0; i < sz; i++)
...
action search::predict(example &ec, ptag mytag, const action *oracle_actions,
                       ...
                       float weight) {
  float a_cost = 0.;
  action a = search_predict(
      *priv, &ec, 1, mytag, oracle_actions, oracle_actions_cnt, condition_on,
      condition_on_names, allowed_actions, allowed_actions_cnt,
      allowed_actions_cost, learner_id, a_cost, weight);
  ...
  if (mytag != 0) {
    ...
    if (priv->acset.use_passthrough_repr) {
      assert((mytag >= priv->ptag_to_action.size()) ||
             (priv->ptag_to_action[mytag].repr == nullptr));
      push_at(priv->ptag_to_action, action_repr(a, &(priv->last_action_repr)),
              mytag);
    } else
  ...
  if (priv->auto_hamming_loss)
    loss(priv->use_action_costs
             ? action_cost_loss(a, allowed_actions, allowed_actions_cost,
                                allowed_actions_cnt)
...
action search::predictLDF(example *ecs, size_t ec_cnt, ptag mytag,
                          const action *oracle_actions,
                          size_t oracle_actions_cnt, const ptag *condition_on,
                          const char *condition_on_names, size_t learner_id,
                          float weight) {
  float a_cost = 0.;
  // TODO: action costs for ldf
  action a =
      search_predict(*priv, ecs, ec_cnt, mytag, oracle_actions,
                     oracle_actions_cnt, condition_on, condition_on_names,
                     nullptr, 0, nullptr, learner_id, a_cost, weight);
  if (priv->state == INIT_TEST)
    priv->test_action_sequence.push_back(a);

  // If there is a shared example (example header), then action "1" is at index
  // 1, but otherwise
  // action "1" is at index 0. Map action to its appropriate index. In
  // particular, this fixes an
  // issue where the predicted action is the last, and there is no example
  // header, causing an index
  // beyond the end of the array (usually resulting in a segfault at some
  // point.)
  size_t action_index =
  ...
  if ((mytag != 0) && ecs[action_index].l.cs.costs.size() > 0) {
    ...
    push_at(priv->ptag_to_action, action_repr(ecs[a].l.cs.costs[0].class_index,
                                              &(priv->last_action_repr)),
            mytag);
  }
  if (priv->auto_hamming_loss)
    loss(action_hamming_loss(a, oracle_actions,
                             oracle_actions_cnt)); // TODO: action costs
  cdbg << "predict returning " << a << endl;
  return a;
}
...
void search::set_options(uint32_t opts) {
  ...
  if (this->priv->is_ldf && this->priv->use_action_costs)
    THROW("using LDF and actions costs is not yet implemented; turn off action "
          "costs"); // TODO fix

  if (this->priv->use_action_costs &&
      (this->priv->rollout_method != NO_ROLLOUT))
    std::cerr << "warning: task is designed to use rollout costs, but this "
                 "only works when --search_rollout none is specified"
              << endl;
}

void search::set_label_parser(label_parser &lp, bool (*is_test)(polylabel &)) {
  if (this->priv->all->vw_is_main && (this->priv->state != INITIALIZE))
    std::cerr << "warning: task should not set label parser except in "
                 "initialize function!"
              << endl;
...
predictor::predictor(search &sch, ptag my_tag)
    : is_ldf(false), my_tag(my_tag), ec(nullptr), ec_cnt(0), ec_alloced(false),
      weight(1.), oracle_is_pointer(false), allowed_is_pointer(false),
      allowed_cost_is_pointer(false), learner_id(0), sch(sch) {
...
void predictor::set_input_at(size_t posn, example &ex) {
  ...
  if (posn >= ec_cnt)
    THROW("call to set_input_at with too large a position: posn ("
          << posn << ") >= ec_cnt(" << ec_cnt << ")");

  VW::copy_example_data(false, ec + posn, &ex, CS::cs_label.label_size,
                        CS::cs_label.copy_label); // TODO: the false is "audit"
}
...
template <class T>
predictor &predictor::add_to(v_array<T> &A, bool &A_is_ptr, T a,
                             bool clear_first) {
...
template <class T>
predictor &predictor::add_to(v_array<T> &A, bool &A_is_ptr, T *a, size_t count,
                             bool clear_first) {
  size_t old_size = A.size();
  if (old_size > 0) {
    if (A_is_ptr) // we need to make our own memory
...
predictor &predictor::add_allowed(v_array<action> &a) {
  return add_to(allowed_actions, allowed_is_pointer, a.begin(), a.size(),
                false);
}
...
predictor &predictor::add_allowed(action *a, float *costs,
                                  size_t action_count) {
  add_to(allowed_actions_cost, allowed_cost_is_pointer, costs, action_count,
         false);
  return add_to(allowed_actions, allowed_is_pointer, a, action_count, false);
}
...
predictor &predictor::set_allowed(action *a, float *costs,
                                  size_t action_count) {
  add_to(allowed_actions_cost, allowed_cost_is_pointer, costs, action_count,
         true);
  return add_to(allowed_actions, allowed_is_pointer, a, action_count, true);
...
action predictor::predict() {
  const action *orA =
      oracle_actions.size() == 0 ? nullptr : oracle_actions.begin();
  const ptag *cOn =
      condition_on_names.size() == 0 ? nullptr : condition_on_tags.begin();
  ...
  const action *alA =
      (allowed_actions.size() == 0) ? nullptr : allowed_actions.begin();
  const float *alAcosts = (allowed_actions_cost.size() == 0)
                              ? nullptr
                              : allowed_actions_cost.begin();
  size_t numAlA = std::max(allowed_actions.size(), allowed_actions_cost.size());
  action p =
      is_ldf
          ? sch.predictLDF(ec, ec_cnt, my_tag, orA, oracle_actions.size(), cOn,
                           cNa, learner_id, weight)
          : sch.predict(*ec, my_tag, orA, oracle_actions.size(), cOn, cNa, alA,
                        numAlA, alAcosts, learner_id, weight);
...
// TODO: valgrind --leak-check=full ./vw --search 2 -k -c --passes 1
// --search_task sequence -d test_beam --holdout_off
