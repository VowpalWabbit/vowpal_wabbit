...
{ action a;
  v_array<feature> repr;
  action_repr(action _a, v_array<feature>&_repr) : a(_a), repr(v_init<feature>())
  { copy_array(repr, _repr);
  }
    ...
    { int learner = select_learner(priv, policy, learner_id, false, priv.state != INIT_TEST);
      ...
      { size_t start_K = (priv.is_ldf && COST_SENSITIVE::ec_is_example_header(ecs[0])) ? 1 : 0;
        ...
        { if (priv.auto_condition_features && priv.acset.use_passthrough_repr)
          ...
          cdbg << "passthrough = [";
          for (size_t kk=0; kk<priv.last_action_repr.size(); kk++) cdbg << ' ' << priv.last_action_repr[kk].weight_index << ':' << priv.last_action_repr[kk].x;
          cdbg << " ]" << endl;
        ...
        { cdbg << "INIT_TRAIN, NO_ROLLOUT, at least one oracle_actions, a=" << a << endl;
          ...
          cdbg << "priv.gte_label = [";
          for (size_t i=0; i<priv.gte_label.cs.costs.size(); i++) cdbg << ' ' << priv.gte_label.cs.costs[i].class_index << ':' << priv.gte_label.cs.costs[i].x;
          cdbg << " ]" << endl;
...
{ if (allowed_is_pointer) allowed_actions.end = allowed_actions.begin;
  ...
  if (allowed_cost_is_pointer) allowed_actions_cost.end = allowed_actions_cost.begin;
  else allowed_actions_cost.erase();
  return *this;
}
