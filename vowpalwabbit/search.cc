...
struct search_private {
  ...
  vector<action> test_action_sequence; // if test-mode was run, what was the
                                       // corresponding action sequence; it's a
...
int random_policy(search_private &priv, bool allow_current, bool allow_optimal,
  ...
  if (num_valid_policies == 0) {
    ...
    pid = (advance_prng
               ? merand48(priv.all->random_state)
               : merand48_noadvance(priv.all->random_state)) >= priv.beta;
  else {
    // SPEEDUP this up in the case that beta is small!
    float r = (advance_prng ? merand48(priv.all->random_state)
                            : merand48_noadvance(priv.all->random_state));
    pid = 0;
...
void print_update(search_private &priv) {
  ...
  fprintf(stderr, "%-10.6f %-10.6f %8s  [%s] [%s] %5d %5d  %7s  %7s  %7s  %-8f",
          avg_loss, avg_loss_since, inst_cntr, true_label, pred_label,
...
void add_example_conditioning(search_private &priv, example &ec,
  ...
  size_t N =
      max(priv.acset.max_bias_ngram_length, priv.acset.max_quad_ngram_length);
  ...
  if (priv.is_ldf) // LDF version easier
  {
    ...
    for (action k = 0; k < ec_cnt; k++)
      cs_cost_push_back(isCB, lab, k, array_contains<action>(k, oracle_actions,
                                          ...
                                          ? 0.f
                                          : 1.f);
    // cerr << "lab = ["; for (size_t i=0; i<lab.cs.costs.size(); i++) cdbg << '
    ...
    if (min_cost < FLT_MAX) {
      ...
      for (size_t k = 0; k < K; k++)
        if (allowed_actions_cost[k] <= min_cost) {
          ...
          count++;
          if ((count == 1) ||
              (merand48(priv.all->random_state) < 1. / (float)count)) {
  ...
  if (a == (action)-1) {
    ...
    a = (oracle_actions_cnt > 0)
            ...
            : (allowed_actions_cnt > 0)
                  ? allowed_actions[random(priv.all->random_state,
                                           allowed_actions_cnt)]
                  : priv.is_ldf
                        ? (action)random(priv.all->random_state, ec_cnt)
      ...
      (priv.subsample_timesteps <= -1)) // active learning
  {
    ...
    if (min_cost2 < FLT_MAX)
      priv.active_uncertainty.push_back(
          make_pair(min_cost2 - min_cost, priv.t + priv.meta_t));
  }
      ...
      else // we need to predict, and then cache, and maybe run foreach_action
      {
            ...
            need_fea) // only make a prediction if we're going to use the output
        {
          if (priv.auto_condition_features && priv.acset.use_passthrough_repr) {
            if (priv.is_ldf) {
              std::cerr << "search cannot use state representations in ldf mode"
                        << endl;
              throw exception();
            }
...
inline bool cmp_size_t_pair(const pair<size_t, size_t> &a,
...
struct final_item {
  ...
  string str;
  float total_cost;
  final_item(v_array<scored_action> *p, string s, float ic)
...
void verify_active_csoaa(COST_SENSITIVE::label &losses,
                         v_array<pair<CS::wclass &, bool>> &known, size_t t,
                         float multiplier) {
...
predictor &predictor::set_allowed(v_array<pair<action, float>> &a) {
  erase_alloweds();
  return add_allowed(a);
}
predictor &predictor::set_allowed(vector<pair<action, float>> &a) {
  erase_alloweds();
  return add_allowed(a);
}
