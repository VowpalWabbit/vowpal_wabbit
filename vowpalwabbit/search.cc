...
{ uint64_t mask = priv.all->wv.getMask();
  size_t ss   = priv.all->wv.getStride();
  ...
  { example& me = *priv.ec_seq[n];
    ...
    { int32_t offset = priv.neighbor_features[n_id] >> 24;
      ...
      if ((offset < 0) && (n < (uint64_t)(-offset))) // add <s> feature
        add_new_feature(priv, 1., 925871901 << priv.all->wv.getStride());
      else if (n + offset >= priv.ec_seq.size()) // add </s> feature
        add_new_feature(priv, 1., 3824917 << priv.all->wv.getStride());
  ...
  { uint64_t fid = 71933 + 8491087 * extra_offset;
    ...
    { if (i + n >= I) break; // no more ngrams
      ...
      if (n < priv.acset.max_bias_ngram_length)
        add_new_feature(priv, 1., 4398201 << priv.all->wv.getStride());
...
uint64_t search::get_mask() { return this->priv->all->wv.getMask();}
size_t search::get_stride_shift() { return this->priv->all->wv.getStride();}
