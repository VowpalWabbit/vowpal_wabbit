...
inline void parse_dispatch(vw &all, dispatch_fptr dispatch) {
  ...
  try {
    while (!all.p->done) {
      examples.push_back(
          &VW::get_unused_example(&all)); // need at least 1 example
      if (!all.do_reset_source && example_number != all.pass_length &&
        ...
        if (all.passes_complete == all.numpasses &&
            example_number == all.pass_length) {
    ...
    std::cerr << "vw example #" << example_number << "(" << e.Filename() << ":"
