...
public:
  ...
  chained_proxy_iterator& operator++()
  {
    ...
    if (_current == (*_outer_current)._features.audit_end() && (_outer_current != _outer_end))
    {
      ++_outer_current;
      _current = (*_outer_current)._features.audit_begin();
    }
    return *this;
  }
  ...
  chained_proxy_iterator& operator+=(difference_type diff)
  {
    assert(diff >= 0);
    while (true) {
      auto current_group_distance_to_end = std::distance(_current, (*_outer_current)._features.audit_end());
  ...
  friend difference_type operator-(const chained_proxy_iterator& lhs, chained_proxy_iterator rhs)
  {
    ...
    {
      accumulator += std::distance(rhs._current, (*(rhs._outer_current))._features.audit_end());
      ++rhs._outer_current;
      rhs._current = (*rhs._outer_current)._features.audit_begin();
    }
