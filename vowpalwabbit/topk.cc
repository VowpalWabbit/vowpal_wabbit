...
struct topk {
  ...
  priority_queue<scored_example, vector<scored_example>,
                 compare_scored_examples>
      pr_queue;
  vw *all;
};

void print_result(int f, priority_queue<scored_example, vector<scored_example>,
                                        compare_scored_examples> &pr_queue) {
  if (f >= 0) {
    char temp[30];
    std::stringstream ss;
...
void predict_or_learn(topk &d, LEARNER::single_learner &base,
                      multi_ex &ec_seq) {
...
void finish(topk &d) {
  d.pr_queue = priority_queue<scored_example, vector<scored_example>,
                              compare_scored_examples>();
}
...
LEARNER::base_learner *topk_setup(options_i &options, vw &all) {
  ...
  new_options.add(
      make_option("top", data->K).keep().help("top k recommendation"));
