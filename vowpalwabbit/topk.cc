...
std::pair<VW::topk::const_iterator_t, VW::topk::const_iterator_t>
VW::topk::get_container_view() {
...
void print_result(int file_descriptor,
                  std::pair<VW::topk::const_iterator_t,
                            VW::topk::const_iterator_t> const &view) {
...
void predict_or_learn(VW::topk &d, LEARNER::single_learner &base,
                      multi_ex &ec_seq) {
  if (is_learn)
    d.learn(base, ec_seq);
  else
