...
VW::LEARNER::base_learner* topk_setup(options_i& options, vw& all)
{
  ...
  VW::LEARNER::learner<VW::topk, multi_ex>& l = init_learner(
      data, as_singleline(setup_base(options, all)), predict_or_learn<true>, predict_or_learn<false>, all.get_setup_name(topk_setup));
