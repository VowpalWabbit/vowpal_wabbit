...
void finish(explore_eval &data) {
  if (!data.all->quiet) {
    data.all->trace_message << "update count = " << data.update_count
                            << std::endl;
    if (data.violations > 0)
      data.all->trace_message << "violation count = " << data.violations
                              << std::endl;
    if (!data.fixed_multiplier)
      data.all->trace_message << "final multiplier = " << data.multiplier
                              << std::endl;
  }
}
...
void do_actual_learning(explore_eval &data, multi_learner &base,
                        multi_ex &ec_seq) {
...
base_learner *explore_eval_setup(options_i &options, vw &all) {
  ...
  learner<explore_eval, multi_ex> &l =
      init_learner(data, base, do_actual_learning<true>,
                   do_actual_learning<false>, 1, prediction_type::action_probs);
