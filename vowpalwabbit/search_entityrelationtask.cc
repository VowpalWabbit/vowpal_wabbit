...
Search::search_task task = {"entity_relation", run,    initialize, finish,
                            nullptr,           nullptr};
}
...
void update_example_indicies(bool audit, example *ec, uint64_t mult_amount,
...
void initialize(Search::search &sch, size_t & /*num_actions*/,
  ...
  new_options
      .add(make_option("relation_cost", my_task_data->relation_cost)
               .keep()
               .default_value(1.f)
               .help("Relation Cost"))
      .add(make_option("entity_cost", my_task_data->entity_cost)
               .keep()
               .default_value(1.f)
               .help("Entity Cost"))
      .add(make_option("constraints", my_task_data->constraints)
               .keep()
               .help("Use Constraints"))
      ...
      .add(make_option("search_order", my_task_data->search_order)
               .keep()
               .default_value(0)
               .help("Search Order 0: EntityFirst 1: Mix 2: Skip 3: "
                     "EntityFirst(LDF)"));
  options.add_and_parse(new_options);

  // setup entity and relation labels
  // Entity label 1:E_Other 2:E_Peop 3:E_Org 4:E_Loc
  // Relation label 5:R_Live_in 6:R_OrgBased_in 7:R_Located_in 8:R_Work_For
  // 9:R_Kill 10:R_None
  for (int i = 1; i < 5; i++)
...
void finish(Search::search &sch) {
  ...
  if (my_task_data->search_order == 3) {
    for (size_t a = 0; a < 10; a++)
      VW::dealloc_example(CS::cs_label.delete_label,
                          my_task_data->ldf_entity[a]);
...
bool check_constraints(size_t ent1_id, size_t ent2_id, size_t rel_id) {
  size_t valid_ent1_id[] = {2, 3, 4, 2,
                            2}; // encode the valid entity-relation combinations
  size_t valid_ent2_id[] = {4, 4, 4, 3, 2};
  if (rel_id - 5 == 5)
    return true;
  if (valid_ent1_id[rel_id - 5] == ent1_id &&
      valid_ent2_id[rel_id - 5] == ent2_id)
    return true;
  return false;
}
...
size_t predict_entity(Search::search &sch, example *ex,
                      v_array<size_t> & /*predictions*/, ptag my_tag,
                      bool isLdf = false) {
  task_data *my_task_data = sch.get_task_data<task_data>();
  size_t prediction;
  if (my_task_data->allow_skip) {
    ...
    if (isLdf) {
      for (uint32_t a = 0; a < 4; a++) {
        ...
        update_example_indicies(true, &my_task_data->ldf_entity[a], 28904713,
                                4832917 * (uint64_t)(a + 1));
...
size_t predict_relation(Search::search &sch, example *ex,
                        v_array<size_t> &predictions, ptag my_tag,
                        bool isLdf = false) {
  char type;
  int id1, id2;
  ...
  if (my_task_data->constraints && predictions[id1] != 0 &&
      predictions[id2] != 0) {
  ...
  for (size_t j = 0; j < my_task_data->y_allowed_relation.size(); j++) {
    if (!my_task_data->constraints || hist[0] == (size_t)0 ||
        check_constraints(hist[0], hist[1],
                          my_task_data->y_allowed_relation[j]))
      constrained_relation_labels.push_back(
          my_task_data->y_allowed_relation[j]);
  }
  ...
  if (my_task_data->allow_skip) {
    ...
    if (isLdf) {
      int correct_label =
          0; // if correct label is not in the set, use the first one
      for (size_t a = 0; a < constrained_relation_labels.size(); a++) {
        ...
        update_example_indicies(true, &my_task_data->ldf_relation[a], 28904713,
                                4832917 *
                                    (uint64_t)(constrained_relation_labels[a]));
      ...
      size_t pred_pos = Search::predictor(sch, my_tag)
                            .set_input(my_task_data->ldf_relation,
                                       constrained_relation_labels.size())
...
void entity_first_decoding(Search::search &sch, multi_ex &ec,
                           v_array<size_t> &predictions, bool isLdf = false) {
  ...
  for (size_t i = 0; i < ec.size(); i++) {
    if (i < n_ent)
      ...
      predictions[i] =
          predict_relation(sch, ec[i], predictions, (ptag)i, isLdf);
  }
}

void er_mixed_decoding(Search::search &sch, multi_ex &ec,
...
void er_allow_skip_decoding(Search::search &sch, multi_ex &ec,
                            v_array<size_t> &predictions) {
  task_data *my_task_data = sch.get_task_data<task_data>();
  // ec.size = #entity + #entity*(#entity-1)/2
  size_t n_ent = (size_t)(sqrt(ec.size() * 8 + 1) - 1) / 2;
...
void run(Search::search &sch, multi_ex &ec) {
  ...
  case 3:
    ...
    break;
  default:
    cerr << "search order " << my_task_data->search_order << "is undefined."
         << endl;
  }
...
void update_example_indicies(bool /* audit */, example *ec,
                             uint64_t mult_amount, uint64_t plus_amount) {
  for (features &fs : *ec)
