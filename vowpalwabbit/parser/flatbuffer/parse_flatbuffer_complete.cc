...
void parse_flatbuf_example(vw* all, example* ae, const Example* eg)
{
  ...
  ae->l.simple.label = eg->label()->weight();
...
{
  VW::string_view name(ns->name()->c_str());
  ...
  for (int j=0; j<ns->features()->size(); j++){
    parse_flatbuf_features(all, ae, fs, ns->features()->Get(j), _index, _channel_hash);
  }
...
{
  uint64_t word_hash = feature->hash();
