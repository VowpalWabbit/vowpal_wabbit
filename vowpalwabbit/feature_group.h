...
// class proxy_audit_features_iterator final
// {
// private:
//   feature_value* _current_begin_values;
//   feature_index* _current_begin_indices;
//   audit_strings_ptr* _current_begin_audit;
//   std::vector<features*>::iterator _current_feature_group_it;
//   std::vector<features*>::iterator _current_feature_group_end_it;

// public:
//   proxy_audit_features_iterator(
//     feature_value* _current_begin_values,
//     feature_index* _current_begin_indices,
//     audit_strings_ptr* _current_begin_audit,
//     std::vector<features*>::iterator current_feature_group_it
//     std::vector<features*>::iterator current_feature_group_end_it
//     )
//       :
//       _current_begin_values(_current_begin_values),
//       _current_begin_indices(_current_begin_indices),
//       _current_begin_audit(_current_begin_audit),
//       _current_feature_group_it(current_feature_group_it),
//       _current_feature_group_end_it(current_feature_group_end_it)
//   {
//   }

//   proxy_audit_features_iterator(const proxy_audit_features_iterator&) = default;
//   proxy_audit_features_iterator& operator=(const proxy_audit_features_iterator&) = default;
//   proxy_audit_features_iterator(proxy_audit_features_iterator&&) = default;
//   proxy_audit_features_iterator& operator=(proxy_audit_features_iterator&&) = default;

//   inline feature_value& value() { return *_begin_values; }
//   inline const feature_value& value() const { return *_begin_values; }

//   inline feature_index& index() { return *_begin_indices; }
//   inline const feature_index& index() const { return *_begin_indices; }

//   inline audit_strings_ptr* audit() { return _begin_audit; }
//   inline const audit_strings_ptr* audit() const { return _begin_audit; }

//   inline reference operator*() { return *this; }
//   inline const_reference operator*() const { return *this; }

//   // Required for forward_iterator
//   proxy_audit_features_iterator& operator++()
//   {
//     auto& current_fgroup = (*(*_current_feature_group_it))

//     // Reached end of current feature group. Move to next one.
//     if(_current_begin_values == current_fgroup.end())
//     {
//       assert(_current_feature_group_it != _current_feature_group_end_it);
//       _current_feature_group_it++;
//       _current_begin_values = (*_current_feature_group_it)->values.begin();
//       _begin_indices = (*_current_feature_group_it)->indicies.begin();
//     }
//     _begin_values++;
//     _begin_indices++;
//     return *this;
...
//   friend bool operator==(const proxy_audit_features_iterator& lhs, const proxy_audit_features_iterator& rhs)
//   {
//     return (lhs._current_feature_group_it == rhs._current_feature_group_it) && (lhs._current_begin_values == rhs._current_begin_values);
//   }
...
// struct proxy_features
// {
//   using iterator = proxy_audit_features_iterator;

//   std::vector<features*> _inner_feat_groups;
...
//     size_t counter = 0;
//     for (const auto* inner_group : _inner_feat_groups)
//     {
//       counter += inner_group->size();
//     }
...
//     for (const auto* inner_group : _inner_feat_groups)
//     {
//       if (!inner_group->empty()) return false;
//     }
...
//     _inner_feat_groups.front()->values.begin(),
//     _inner_feat_groups.front()->indicies.begin(),
//     _inner_feat_groups.front()->space_names.begin(),
//     _inner_feat_groups.begin(),
//     _inner_feat_groups.end()
//   };
//  }
//   inline iterator end() {  return {
//     _inner_feat_groups.back()->values.end(),
//     _inner_feat_groups.back()->indicies.end(),
//     _inner_feat_groups.back()->space_names.end(),
//     _inner_feat_groups.begin(),
//     _inner_feat_groups.end()
//   };
//   }
