...
struct feature_slice // a helper struct for functions using the set
                     // {v,i,space_name}
{
  feature_value x;
...
public:
  ...
  /// \remark template<typename T> used to avoid warnings or tons of overloads
  /// for int, size_t, ...
  template <typename T> features_value_iterator operator+(T index) {
  ...
  friend void swap(features_value_iterator &lhs, features_value_iterator &rhs) {
    std::swap(lhs._begin, rhs._begin);
  }
...
public:
  features_value_index_iterator(feature_value *begin,
  ...
  template <typename T> features_value_index_iterator &operator+=(T index) {
  ...
  operator=(const features_value_index_iterator &other) {
  ...
  friend void swap(features_value_index_iterator &lhs,
                   features_value_index_iterator &rhs) {
    swap(static_cast<features_value_iterator &>(lhs),
         static_cast<features_value_iterator &>(rhs));
...
class features_value_index_audit_iterator
    : public features_value_index_iterator {
protected:
  audit_strings_ptr *_begin_audit;

public:
  features_value_index_audit_iterator(feature_value *begin,
                                      feature_index *begin_index,
                                      audit_strings_ptr *begin_audit)
      : features_value_index_iterator(begin, begin_index),
        _begin_audit(begin_audit) {}

  features_value_index_audit_iterator(
      const features_value_index_audit_iterator &other)
  ...
  template <typename T> features_value_index_audit_iterator operator+(T index) {
    return features_value_index_audit_iterator(
        _begin + index, _begin_index + index, _begin_audit + index);
  }
  ...
  features_value_index_audit_iterator &
  operator=(const features_value_index_audit_iterator &other) {
  ...
  friend void swap(features_value_index_audit_iterator &lhs,
                   features_value_index_audit_iterator &rhs) {
    swap(static_cast<features_value_index_iterator &>(lhs),
         static_cast<features_value_index_iterator &>(rhs));
...
struct features {
  ...
  public:
    ...
    iterator_all begin() {
      return iterator_all(_outer->values.begin(), _outer->indicies.begin(),
                          _outer->space_names.begin());
    }
    iterator_all end() {
      return iterator_all(_outer->values.end(), _outer->indicies.end(),
                          _outer->space_names.end());
    }
  };
  ...
  // if one wants to add proper destructor for features, make sure to update
  ...
  bool sort(uint64_t parse_mask) {
    ...
    if (!space_names.empty()) {
      ...
      for (size_t i = 0; i < indicies.size(); i++) {
        feature_slice temp = {values[i], indicies[i] & parse_mask,
                              *space_names[i].get()};
        slice.push_back(temp);
      }
      qsort(slice.begin(), slice.size(), sizeof(feature_slice),
      ...
      qsort(slice.begin(), slice.size(), sizeof(feature),
