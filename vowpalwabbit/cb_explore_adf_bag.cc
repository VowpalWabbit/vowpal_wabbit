...
public:
  cb_explore_adf_bag(float epsilon, size_t bag_size, bool greedify,
...
cb_explore_adf_bag::cb_explore_adf_bag(float epsilon, size_t bag_size,
                                       bool greedify, bool first_only,
                                       std::shared_ptr<rand_state> random_state)
    : _epsilon(epsilon), _bag_size(bag_size), _greedify(greedify),
      _first_only(first_only), _random_state(random_state) {}

template <bool is_learn>
void cb_explore_adf_bag::predict_or_learn_impl(LEARNER::multi_learner &base,
                                               multi_ex &examples) {
  ...
  for (uint32_t i = 0; i < _bag_size; i++) {
    ...
    uint32_t count =
        is_learn ? ((_greedify && i == 0) ? 1 : BS::weight_gen(_random_state))
                 : 0;
    ...
    if (!_first_only) {
      ...
      for (size_t i = 0; i < tied_actions; ++i)
        _top_actions[preds[i].action] += 1.f / tied_actions;
    } else
  ...
  _action_probs.clear();
  for (uint32_t i = 0; i < _scores.size(); i++)
    _action_probs.push_back({i, 0.});

  // generate distribution over actions
  exploration::generate_bag(begin(_top_actions), end(_top_actions),
                            begin_scores(_action_probs),
                            end_scores(_action_probs));

  exploration::enforce_minimum_probability(
      _epsilon, true, begin_scores(_action_probs), end_scores(_action_probs));
...
LEARNER::base_learner *setup(VW::config::options_i &options, vw &all) {
  ...
  new_options
      ...
      .add(
          make_option("bag", bag_size).keep().help("bagging-based exploration"))
  ...
  auto data = scoped_calloc_or_throw<explore_type>(
      epsilon, bag_size, greedify, first_only, all.get_random_state());
