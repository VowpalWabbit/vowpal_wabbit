...
void mf_print_offset_features(gdmf &d, example &ec, size_t offset) {
  ...
  for (features &fs : ec) {
    ...
    for (auto &f : fs.values_indices_audit()) {
      ...
      cout << f.index() << "(" << ((f.index() + offset) & mask) << ")" << ':'
           << f.value();
      cout << ':' << (&weights[f.index()])[offset];
    }
  }
  for (string &i : all.pairs)
    if (ec.feature_space[(unsigned char)i[0]].size() > 0 &&
        ec.feature_space[(unsigned char)i[1]].size() > 0) {
      /* print out
       * nsk^feature:hash:value:weight:nsk^feature^:hash:value:weight:prod_weights
       */
      for (size_t k = 1; k <= d.rank; k++) {
        for (features::iterator_all &f1 :
             ec.feature_space[(unsigned char)i[0]].values_indices_audit())
          for (features::iterator_all &f2 :
               ec.feature_space[(unsigned char)i[1]].values_indices_audit()) {
            cout << '\t' << f1.audit().get()->first << k << '^'
                 << f1.audit().get()->second << ':' << ((f1.index() + k) & mask)
                 << "(" << ((f1.index() + offset + k) & mask) << ")" << ':'
                 << f1.value();
            cout << ':' << (&weights[f1.index()])[offset + k];

            cout << ':' << f2.audit().get()->first << k << '^'
                 << f2.audit().get()->second << ':'
                 << ((f2.index() + k + d.rank) & mask) << "("
                 << ((f2.index() + offset + k + d.rank) & mask) << ")" << ':'
                 << f2.value();
            cout << ':' << (&weights[f2.index()])[offset + k + d.rank];

            cout << ':'
                 << (&weights[f1.index()])[offset + k] *
                        (&weights[f2.index()])[offset + k + d.rank];
          }
      }
    }
...
template <class T> float mf_predict(gdmf &d, example &ec, T &weights) {
  ...
  // interaction terms
  for (string &i : d.all->pairs) {
    if (ec.feature_space[(int)i[0]].size() > 0 &&
        ec.feature_space[(int)i[1]].size() > 0) {
      for (uint64_t k = 1; k <= d.rank; k++) {
        // x_l * l^k
        // l^k is from index+1 to index+d.rank
        // float x_dot_l = sd_offset_add(weights,
        // ec.atomics[(int)(*i)[0]].begin(), ec.atomics[(int)(*i)[0]].end(), k);
        pred_offset x_dot_l = {0., k};
        GD::foreach_feature<pred_offset, offset_add, T>(
            weights, ec.feature_space[(int)i[0]], x_dot_l);
        // x_r * r^k
        // r^k is from index+d.rank+1 to index+2*d.rank
        // float x_dot_r = sd_offset_add(weights,
        // ec.atomics[(int)(*i)[1]].begin(), ec.atomics[(int)(*i)[1]].end(),
        // k+d.rank);
        pred_offset x_dot_r = {0., k + d.rank};
        GD::foreach_feature<pred_offset, offset_add, T>(
            weights, ec.feature_space[(int)i[1]], x_dot_r);
...
template <class T>
void sd_offset_update(T &weights, features &fs, uint64_t offset, float update,
                      float regularization) {
  for (size_t i = 0; i < fs.size(); i++)
    (&weights[fs.indicies[i]])[offset] +=
        update * fs.values[i] -
        regularization * (&weights[fs.indicies[i]])[offset];
}

template <class T> void mf_train(gdmf &d, example &ec, T &weights) {
  vw &all = *d.all;
  label_data &ld = ec.l.simple;
  ...
  float eta_t = all.eta /
                powf((float)all.sd->t + ec.weight, (float)all.power_t) / 3.f *
                ec.weight;
  float update = all.loss->getUpdate(ec.pred.scalar, ld.label, eta_t,
                                     1.); // ec.total_sum_feat_sq);
  ...
  for (string &i : all.pairs) {
    if (ec.feature_space[(int)i[0]].size() > 0 &&
      ...
      for (size_t k = 1; k <= d.rank; k++) {
        ...
        sd_offset_update<T>(weights, ec.feature_space[(int)i[0]], k,
                            update * r_dot_x, regularization);
      }
      // update r^k weights
      for (size_t k = 1; k <= d.rank; k++) {
        // l^k \cdot x_l
        ...
        sd_offset_update<T>(weights, ec.feature_space[(int)i[1]], k + d.rank,
...
void save_load(gdmf &d, io_buf &model_file, bool read, bool text) {
    ...
    if (all.random_weights) {
      ...
      if (all.weights.sparse)
        all.weights.sparse_weights
            .set_default<uint32_t, set_rand_wrapper<sparse_parameters>>(stride);
      else
        all.weights.dense_weights
            .set_default<uint32_t, set_rand_wrapper<dense_parameters>>(stride);
    }
  }
  ...
  if (model_file.files.size() > 0) {
    ...
    do {
      ...
      if (text) {
        msg << "\n";
        brw += bin_text_read_write_fixed(model_file, nullptr, 0, "", read, msg,
                                         text);
      }
...
base_learner *gd_mf_setup(options_i &options, vw &all) {
  ...
  gf_md_options.add(make_option("rank", data->rank)
                        .keep()
                        .help("rank for matrix factorization."));

  // Not supported, need to be checked to be false.
  gf_md_options.add(
      make_option("bfgs", bfgs).help("Option not supported by this reduction"));
  gf_md_options.add(make_option("conjugate_gradient", conjugate_gradient)
  ...
  if (options.was_supplied("exact_adaptive_norm"))
    THROW("normalized adaptive updates is not implemented for matrix "
          "factorization");
  ...
  learner<gdmf, example> &l = init_learner(
      data, learn, predict, (UINT64_ONE << all.weights.stride_shift()));
