...
template <int offset> inline void vec_add(float &p, const float x, float &w) {
  float *ws = &w;
  p += x * ws[offset];
}

template <int offset> inline float inline_predict(vw &all, example &ec) {
...
float predict_stable(const svrg &s, example &ec) {
  return GD::finalize_prediction(s.all->sd,
                                 inline_predict<W_STABLE>(*s.all, ec));
}
...
inline void update_inner_feature(update &u, float x, float &w) {
  float *ws = &w;
  w -= u.eta *
       ((u.g_scalar_inner - u.g_scalar_stable) * x + ws[W_STABLEGRAD] / u.norm);
}
...
void learn(svrg &s, single_learner &base, example &ec) {
  ...
  if (pass % (s.stage_size + 1) == 0) // Compute exact gradient
  {
    if (s.prev_pass != pass && !s.all->quiet) {
      std::cout << "svrg pass " << pass << ": committing stable point"
                << std::endl;
      ...
      std::cout << "svrg pass " << pass << ": computing exact gradient"
                << std::endl;
    }
    update_stable(s, ec);
    s.stable_grad_count++;
...
base_learner *svrg_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("svrg", svrg_option)
               .keep()
               .help("Streaming Stochastic Variance Reduced Gradient"))
      .add(make_option("stage_size", s->stage_size)
               .default_value(1)
  ...
  learner<svrg, example> &l =
      init_learner(s, learn, predict, UINT64_ONE << all.weights.stride_shift());
