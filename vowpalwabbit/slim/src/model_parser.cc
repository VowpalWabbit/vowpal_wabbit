...
model_parser::model_parser(const char *model, size_t length)
  ...
  std::cout << "moder_parser("
            << "0x" << std::hex << std::setw(8) << (uint64_t)_model_begin << "-"
...
int model_parser::read(const char *field_name, size_t field_length,
                       const char **ret) { // check if we're inside the buffer
...
int model_parser::skip(size_t bytes) {
  ...
  if (bytes > 0)
    _checksum = (uint32_t)uniform_hash(_model, bytes, _checksum);
