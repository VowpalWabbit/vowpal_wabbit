...
void find_opt(std::string const &command_line_args, std::string arg_name,
  ...
  for (size_t start = 0; start < command_line_args.size();) {
    ...
    for (; idx_after_arg < command_line_args.size() &&
           std::isspace(command_line_args[idx_after_arg]);
         ++idx_after_arg)
      ;
    ...
    if (command_line_args[idx_after_arg] == '-' &&
        ...
        !(idx_after_arg + 1 < command_line_args.size() &&
          (command_line_args[idx_after_arg + 1] >= '0' &&
           command_line_args[idx_after_arg + 1] <= '9'))) {
      start = idx_after_arg;
    ...
    for (; idx_after_value < command_line_args.size() &&
           !std::isspace(command_line_args[idx_after_value]);
         ++idx_after_value)
      ;
...
std::vector<std::string> find_opt(std::string const &command_line_args,
                                  std::string arg_name) {
...
bool find_opt_parse(std::string const &command_line_args, std::string arg_name,
                    T &value) {
...
bool find_opt_float(std::string const &command_line_args, std::string arg_name,
                    float &value) {
  return find_opt_parse<float, double, atof>(command_line_args, arg_name,
                                             value);
}

bool find_opt_int(std::string const &command_line_args, std::string arg_name,
                  int &value) {
