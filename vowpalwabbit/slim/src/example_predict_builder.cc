...
example_predict_builder::example_predict_builder(
  ...
  _namespace_hash = hashstring(
      {namespace_name, namespace_name + (strlen(namespace_name))}, 0);
}

example_predict_builder::example_predict_builder(
    example_predict *ex, namespace_index namespace_idx,
...
void example_predict_builder::push_feature_string(char *feature_name,
                                                  feature_value value) {
  feature_index feature_hash =
      _feature_index_bit_mask &
      hashstring({feature_name, feature_name + (strlen(feature_name))},
                 _namespace_hash);
  _ex->feature_space[_namespace_idx].push_back(value, feature_hash);
}

void example_predict_builder::push_feature(feature_index feature_idx,
                                           feature_value value) {
  _ex->feature_space[_namespace_idx].push_back(value,
                                               _namespace_hash + feature_idx);
}
}; // end-of-namespace
