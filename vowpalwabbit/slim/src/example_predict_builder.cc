...
namespace vw_slim {
	example_predict_builder::example_predict_builder(example_predict* ex, char* namespace_name, uint32_t feature_index_num_bits)
		: _ex(ex)
	{
	...
	example_predict_builder::example_predict_builder(example_predict* ex, namespace_index namespace_idx, uint32_t feature_index_num_bits)
	...
	{
    feature_index feature_hash = _feature_index_bit_mask & hashstring({feature_name, feature_name + (strlen(feature_name))}, _namespace_hash);
