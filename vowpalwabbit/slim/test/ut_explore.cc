...
TEST(ExploreTestSuite, EpsilonGreedy) {
  ...
  EXPECT_THAT(S_EXPLORATION_OK, exploration::generate_epsilon_greedy(
                                    0.4f, 2, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-6f),
                             std::vector<float>{0.1f, 0.1f, 0.7f, 0.1f}));
}

TEST(ExploreTestSuite, EpsilonGreedyTopActionOutOfBounds) {
  std::vector<float> pdf(4);
  EXPECT_THAT(S_EXPLORATION_OK, exploration::generate_epsilon_greedy(
                                    0.4f, 8, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-6f),
                             std::vector<float>{0.1f, 0.1f, 0.1f, 0.7f}));
}

TEST(ExploreTestSuite, EpsilonGreedy_bad_range) {
  std::vector<float> pdf;
  float x;
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, exploration::generate_epsilon_greedy(
                                           0.4f, 0, begin(pdf), end(pdf)));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE,
              exploration::generate_epsilon_greedy(0.4f, 0, &x, &x - 3));
...
TEST(ExploreTestSuite, Softmax) {
  ...
  std::vector<float> pdf(4);
  EXPECT_THAT(S_EXPLORATION_OK,
              exploration::generate_softmax(0.2f, begin(scores), end(scores),
                                            begin(pdf), end(pdf)));
  EXPECT_THAT(pdf,
              Pointwise(FloatNearPointwise(1e-3f),
                        std::vector<float>{0.128f, 0.157f, 0.192f, 0.522f}));
}
...
TEST(ExploreTestSuite, SoftmaxInBalanced) {
  ...
  EXPECT_THAT(S_EXPLORATION_OK,
              exploration::generate_softmax(0.2f, begin(scores), end(scores),
                                            begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f),
                             std::vector<float>{0.269f, 0.328f, 0.401f, 0}));
}
...
TEST(ExploreTestSuite, Bag) {
  ...
  std::vector<float> pdf(4);
  EXPECT_THAT(S_EXPLORATION_OK,
              exploration::generate_bag(begin(top_actions), end(top_actions),
                                        begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f),
                             std::vector<float>{0, 0, 0.5, 0.5f}));
}
...
TEST(ExploreTestSuite, enforce_minimum_probability) {
  ...
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f),
                             std::vector<float>{.8f, .1f, .1f}));
}

TEST(ExploreTestSuite, enforce_minimum_probability_no_zeros) {
  std::vector<float> pdf = {0.9f, 0.1f, 0};
  EXPECT_THAT(S_EXPLORATION_OK, exploration::enforce_minimum_probability(
                                    0.6f, false, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f),
                             std::vector<float>{.8f, .2f, .0f}));
}

TEST(ExploreTestSuite, enforce_minimum_probability_uniform) {
  std::vector<float> pdf = {0.9f, 0.1f, 0, 0};
  EXPECT_THAT(S_EXPLORATION_OK, exploration::enforce_minimum_probability(
                                    1.f, true, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f),
                             std::vector<float>{.25f, .25f, .25f, .25f}));
}

TEST(ExploreTestSuite, enforce_minimum_probability_uniform_no_zeros) {
  std::vector<float> pdf = {0.9f, 0.1f, 0};
  EXPECT_THAT(S_EXPLORATION_OK, exploration::enforce_minimum_probability(
                                    1.f, false, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f),
                             std::vector<float>{.5f, .5f, .0f}));
}

TEST(ExploreTestSuite, enforce_minimum_probability_bad_range) {
  std::vector<float> pdf;
  float x;
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, exploration::enforce_minimum_probability(
                                           1.f, false, begin(pdf), end(pdf)));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE,
              exploration::enforce_minimum_probability(1.f, false, &x, &x - 3));
}
...
TEST(ExploreTestSuite, sampling) {
  ...
  for (size_t i = 0; i < rep; i++) {
    ...
    s << "abcde" << i;
    ASSERT_EQ(
        0, exploration::sample_after_normalizing(
               s.str().c_str(), std::begin(pdf), std::end(pdf), chosen_index));
...
TEST(ExploreTestSuite, sampling_rank) {
  ...
  for (size_t i = 0; i < rep; i++) {
    ...
    ASSERT_EQ(S_EXPLORATION_OK,
              vw_slim::vw_predict<float>::sort_by_scores(
                  std::begin(pdf), std::end(pdf), std::begin(scores),
                  std::end(scores), std::begin(ranking), std::end(ranking)));

    // Sample from the pdf
    uint32_t chosen_action_idx;
    ASSERT_EQ(S_EXPLORATION_OK, exploration::sample_after_normalizing(
                                    s.str().c_str(), std::begin(pdf),
                                    std::end(pdf), chosen_action_idx));
...
TEST(ExploreTestSuite, sampling_rank_bad_range) {
  ...
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE,
              exploration::sample_after_normalizing(
                  "abc", std::begin(pdf), std::end(pdf), chosen_index));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, exploration::sample_after_normalizing(
                                           "abc", &x, &x - 3, chosen_index));
}
