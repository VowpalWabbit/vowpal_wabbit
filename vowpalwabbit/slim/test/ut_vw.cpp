...
#define TEST_DATA(input, filename) { if (!strcmp(input, #filename)) { \
...
TEST_P(PredictTest, Run)
{
	if (GetParam().weight_type == PredictParamWeightType::Sparse)
		run_predict_in_memory<sparse_parameters>(GetParam().model_filename, GetParam().data_filename, GetParam().prediction_reference_filename);
	else
		run_predict_in_memory<dense_parameters>(GetParam().model_filename, GetParam().data_filename, GetParam().prediction_reference_filename);
}
...
std::vector<PredictParam> GenerateTestParams()
{
	...
	{ "regression_data_no_constant", "regression_data_1.txt", "regression_data_no-constant.pred", PredictParamWeightType::All },
	{ "regression_data_ignore_linear", "regression_data_2.txt", "regression_data_ignore_linear.pred", PredictParamWeightType::All },
	...
	for (size_t i = 0; i < sizeof(predict_params) / sizeof(PredictParam); i++)
	{
		...
		{
			for (int weight_type = PredictParamWeightType::Sparse; weight_type <= PredictParamWeightType::Dense; weight_type++)
			{
...
TEST_P(InvalidModelTest, Run)
{
	...
	for (size_t end = 0; end < model_file_size - 1; ++end)
	{
		...
		// type parameterized and value parameterized test cases can't be combined: https://stackoverflow.com/questions/8507385/google-test-is-there-a-way-to-combine-a-test-which-is-both-type-parameterized-a
		if (GetParam().weight_type == PredictParamWeightType::Sparse)
		{
...
InvalidModelParam invalid_model_param[] = {
	{ "regression_data_1", regression_data_1_model, regression_data_1_model_len, { 84, 92 }, PredictParamWeightType::Sparse }, // 2 weights
	{ "regression_data_1", regression_data_1_model, regression_data_1_model_len, { 84, 92 }, PredictParamWeightType::Dense }, // 2 weights
	{ "regression_data_6", regression_data_6_model, regression_data_6_model_len, { 99, 111, 123, 135 }, PredictParamWeightType::Sparse } // 4 weights
};
...
TEST_P(CBPredictTest, CBRunPredict)
{
	...
	for (size_t i = 0; i < rep; i++)
	{
		...
		if (i == 0)
			EXPECT_THAT(pdf, Pointwise(FloatNearPointwise(1e-3f), pdf_expected));
