...
public:
...
#ifdef MODEL_PARSER_DEBUG
    std::fstream log("vwslim-debug.log", std::fstream::app);
    log << std::setw(18) << field_name << " 0x" << std::hex << std::setw(8)
        << (uint64_t)_model << "-" << std::hex << std::setw(8)
        << (uint64_t)_model_end << " " << std::setw(4)
        << (_model - _model_begin) << " field: " << std::setw(8)
        << (uint64_t)&val << std::dec;
#endif
  ...
  template <typename T> int read(const char *field_name, T &val) {
...
#ifdef MODEL_PARSER_DEBUG
      std::cout << "weight. idx: " << idx << ":" << (*weights)[idx]
                << std::endl;
#endif
    }
  ...
  template <typename W>
  int read_weights(std::unique_ptr<W> &weights, uint32_t num_bits,
