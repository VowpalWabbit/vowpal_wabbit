...
namespace vw_slim {
namespace internal {
template <typename It1, typename It2> class location_reference;

template <typename It1, typename It2> class location_value {
public:
...
public:
  ...
  // must support: a) default-construction, b) copy-construction, c)
  // copy-assignment, d) destruction
  collection_pair_iterator() = default;
  collection_pair_iterator(const collection_pair_iterator &rhs) = default;
  collection_pair_iterator &
  operator=(const collection_pair_iterator &rhs) = default;

  // must support: a == b; a != b;
  bool operator==(const Iter &rhs) const {
    return (_ptr1 == rhs._ptr1 && _ptr2 == rhs._ptr2);
  }
  ...
  // must support: *a = t;
  Ref operator*() {
    return Ref(_ptr1, _ptr2);
  } // Non-conforming - normally returns loc&
...
/**
 * @brief Vowpal Wabbit slim predictor. Supports: regression, multi-class
 * classification and contextual bandits.
 */
template <typename W> class vw_predict {
...
public:
  ...
  /**
   * @brief Reads the Vowpal Wabbit model from the supplied buffer (produced
   * using vw -f <modelname>)
   *
   * @param model The binary model.
   * @param length The length of the binary model.
   * @return int Returns 0 (S_VW_PREDICT_OK) if succesful, otherwise one of the
   * error codes (see E_VW_PREDICT_ERR_*).
   */
  int load(const char *model, size_t length) {
    ...
    RETURN_ON_FAIL(
        mp.read_string<true>("file_options", _command_line_arguments));

    // command line arg parsing
    _no_constant =
        _command_line_arguments.find("--noconstant") != std::string::npos;

    // only 0-valued hash_seed supported
    int hash_seed;
    if (find_opt_int(_command_line_arguments, "--hash_seed", hash_seed) &&
        hash_seed)
    ...
    // VW performs the following transformation as a side-effect of looking for
    // duplicates.
    ...
    if (_command_line_arguments.find("--cb_explore_adf") != std::string::npos) {
      ...
      } else if (_command_line_arguments.find("--softmax") !=
                 std::string::npos) {
        if (find_opt_float(_command_line_arguments, "--lambda", _lambda)) {
          if (_lambda > 0) // Lambda should always be negative because we are
                           // using a cost basis.
            _lambda = -_lambda;
  ...
  /**
   * @brief True if the model describes a contextual bandit (cb) model using
   * action dependent features (afd)
   *
  ...
  bool is_cb_explore_adf() {
    return _command_line_arguments.find("--cb_explore_adf") !=
           std::string::npos;
  }

  /**
   * @brief True if the model describes a cost sensitive one-against-all
   * (csoaa). This is also true for cb_explore_adf
   ...
   *
   * Regular regression with support for constant feature (bias term) and
   * interactions
   *
  ...
  int predict(example_predict &ex, float &score) {
    ...
    if (!_no_constant) {
      ...
      ns_copy_guard = std::unique_ptr<namespace_copy_guard>(
          new namespace_copy_guard(ex, constant_namespace));
      ns_copy_guard->feature_push_back(1.f, (constant << _stride_shift) +
                                                ex.ft_offset);
    }

    score = GD::inline_predict<W>(*_weights, false, _ignore_linear,
                                  _interactions, /* permutations */ false, ex);

    return S_VW_PREDICT_OK;
  }

  // multiclass classification
  int predict(example_predict &shared, example_predict *actions,
              size_t num_actions, std::vector<float> &out_scores) {
    if (!_model_loaded)
      return E_VW_PREDICT_ERR_NO_MODEL_LOADED;
    ...
    for (size_t i = 0; i < num_actions; i++, action++) {
      ...
      for (auto ns : shared.indices) {
        // insert namespace
        auto ns_copy_guard = std::unique_ptr<namespace_copy_guard>(
  ...
  int predict(const char *event_id, example_predict &shared,
              example_predict *actions, size_t num_actions,
    ...
    case vw_predict_exploration::epsilon_greedy: {
      ...
      auto top_action_iterator =
          std::min_element(std::begin(scores), std::end(scores));
      uint32_t top_action =
          (uint32_t)(top_action_iterator - std::begin(scores));

      RETURN_EXPLORATION_ON_FAIL(exploration::generate_epsilon_greedy(
          _epsilon, top_action, std::begin(pdf), std::end(pdf)));
      break;
    }
    case vw_predict_exploration::softmax: {
      ...
      RETURN_EXPLORATION_ON_FAIL(exploration::generate_softmax(
          _lambda, std::begin(scores), std::end(scores), std::begin(pdf),
          std::end(pdf)));
      break;
    }
    case vw_predict_exploration::bag: {
      ...
      stride_shift_guards.push_back(std::unique_ptr<stride_shift_guard>(
          new stride_shift_guard(shared, _stride_shift)));
      example_predict *actions_end = actions + num_actions;
      for (example_predict *action = actions; action != actions_end; ++action)
        stride_shift_guards.push_back(std::unique_ptr<stride_shift_guard>(
            new stride_shift_guard(*action, _stride_shift)));

      for (size_t i = 0; i < _bag_size; i++) {
        std::vector<std::unique_ptr<feature_offset_guard>>
            feature_offset_guards;
        for (example_predict *action = actions; action != actions_end; ++action)
          feature_offset_guards.push_back(std::unique_ptr<feature_offset_guard>(
              new feature_offset_guard(*action, i)));
      ...
      // generate exploration distribution
      RETURN_EXPLORATION_ON_FAIL(exploration::generate_bag(
          std::begin(top_actions), std::end(top_actions), std::begin(pdf),
          std::end(pdf)));

      if (_minimum_epsilon > 0)
        RETURN_EXPLORATION_ON_FAIL(exploration::enforce_minimum_probability(
            _minimum_epsilon, true, std::begin(pdf), std::end(pdf)));
    ...
    RETURN_EXPLORATION_ON_FAIL(sort_by_scores(
        std::begin(pdf), std::end(pdf), std::begin(scores), std::end(scores),
        std::begin(ranking), std::end(ranking)));

    // Sample from the pdf
    uint32_t chosen_action_idx;
    RETURN_EXPLORATION_ON_FAIL(exploration::sample_after_normalizing(
        event_id, std::begin(pdf), std::end(pdf), chosen_action_idx));

    // Swap top element with chosen one (unless chosen is the top)
    if (chosen_action_idx != 0) {
      std::iter_swap(std::begin(ranking),
                     std::begin(ranking) + chosen_action_idx);
  ...
  static int sort_by_scores(PdfIt pdf_first, PdfIt pdf_last,
    ...
    // Pdf starts out in the same order as ranking.  Ranking and pdf should been
    // sorted
    ...
    std::sort(
        begin_coll, end_coll, [&scores_first](const Loc &l, const Loc &r) {
