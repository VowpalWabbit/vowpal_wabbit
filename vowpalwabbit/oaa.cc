...
struct oaa {
  ...
  uint64_t
      num_subsample; // for randomized subsampling, how many negatives to draw?
  uint32_t *subsample_order; // for randomized subsampling, in what order should
                             // we touch classes
  size_t
      subsample_id; // for randomized subsampling, where do we live in the list
...
void learn_randomized(oaa &o, LEARNER::single_learner &base, example &ec) {
  ...
  if (ld.label == 0 || (ld.label > o.k && ld.label != (uint32_t)-1))
    std::cout << "label " << ld.label << " is not in {1," << o.k
...
void predict_or_learn(oaa &o, LEARNER::single_learner &base, example &ec) {
  ...
  if (mc_label_data.label == 0 ||
      (mc_label_data.label > o.k && mc_label_data.label != (uint32_t)-1))
    std::cout << "label " << mc_label_data.label << " is not in {1," << o.k
              << "} This won't work right." << std::endl;
...
void finish_example_scores(vw &all, oaa &o, example &ec) {
  ...
  if (probabilities) {
    if (ec.l.multi.label <=
        o.k) // prevent segmentation fault if labeĺ==(uint32_t)-1
  ...
  all.sd->update(ec.test_only, ec.l.multi.label != (uint32_t)-1, zero_one_loss,
...
LEARNER::base_learner *oaa_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("oaa", data->k)
               .keep()
               .help("One-against-all multiclass with <k> labels"))
      .add(
          make_option("oaa_subsample", data->num_subsample)
              .help("subsample this number of negative examples when learning"))
      .add(make_option("probabilities", probabilities)
  ...
  if (data->num_subsample > 0) {
    if (data->num_subsample >= data->k) {
      ...
      for (size_t i = 0; i < data->k; i++)
        data->subsample_order[i] = (uint32_t)i;
      for (size_t i = 0; i < data->k; i++) {
        size_t j = (size_t)(all.get_random_state()->get_and_update_random() *
                            (float)(data->k - i)) +
                   i;
  ...
  if (probabilities || scores) {
    ...
    if (probabilities) {
      ...
      // scores
      l = &LEARNER::init_multiclass_learner(
          data, base, predict_or_learn<true, false, true, true>,
          predict_or_learn<false, false, true, true>, all.p, data->k,
      ...
      l = &LEARNER::init_multiclass_learner(
          ...
          predict_or_learn<false, false, true, false>, all.p, data->k,
          prediction_type::scalars);
      l->set_finish_example(finish_example_scores<false>);
    }
  } else if (all.raw_prediction > 0)
    l = &LEARNER::init_multiclass_learner(
        data, base, predict_or_learn<true, true, false, false>,
        predict_or_learn<false, true, false, false>, all.p, data->k,
        prediction_type::multiclass);
  else
    l = &LEARNER::init_multiclass_learner(
        data, base, predict_or_learn<true, false, false, false>,
        predict_or_learn<false, false, false, false>, all.p, data->k,
        prediction_type::multiclass);
