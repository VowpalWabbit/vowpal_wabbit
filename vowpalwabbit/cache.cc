...
int read_cached_features(vw *all, v_array<example *> &examples) {
  ...
  for (; num_indices > 0; num_indices--) {
    ...
    if ((temp = input->buf_read(c, sizeof(index) + sizeof(size_t))) <
        sizeof(index) + sizeof(size_t)) {
      all->trace_message << "truncated example! " << temp << " "
                         << char_size + sizeof(size_t) << std::endl;
      return 0;
    }
    ...
    if (input->buf_read(c, storage) < storage) {
      all->trace_message << "truncated example! wanted: " << storage << " bytes"
                         << std::endl;
      return 0;
    }
...
void output_features(io_buf &cache, unsigned char index, features &fs,
                     uint64_t mask) {
  char *c;
