    ...
    float& min_pred, float& max_pred, bool& is_range_large)
{
  ...
  if (cs_a.t <= 1 || std::isnan(sens) || std::isinf(sens))
  {
    ...
    if (cs_a.print_debug_stuff)
      logger::errlog_info(" find_cost_rangeA: i={0} pp={1} sens={2} eta={3} [{4}, {5}] = {6}", i, ec.partial_prediction,
          sens, eta, min_pred, max_pred, (max_pred - min_pred));
  }
  else
  {
    ...
    if (cs_a.print_debug_stuff)
      logger::errlog_info(" find_cost_rangeB: i={0} pp={1} sens={2} eta={3} [{4}, {5}] = {6}", i, ec.partial_prediction,
          sens, eta, min_pred, max_pred, (max_pred - min_pred));
  }
}
