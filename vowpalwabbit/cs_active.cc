    ...
    float& min_pred, float& max_pred, bool& is_range_large)
{
  ...
  if (cs_a.t <= 1 || std::isnan(sens) || std::isinf(sens))
  {
    ...
    if (cs_a.print_debug_stuff)
      cs_a.all->logger.info("find_cost_rangeA: i={0} pp={1} sens={2} eta={3} [{4}, {5}] = {6}", i,
  ...
  else
  {
    ...
    if (cs_a.print_debug_stuff)
      cs_a.all->logger.info("find_cost_rangeB: i={0} pp={1} sens={2} eta={3} [{4}, {5}] = {6}", i,
          ec.partial_prediction, sens, eta, min_pred, max_pred, (max_pred - min_pred));
  }
}
...
base_learner* cs_active_setup(VW::setup_base_i& stack_builder)
{
  ...
  if (!options.was_supplied("adax")) { all.logger.warn("--cs_active should be used with --adax"); }
