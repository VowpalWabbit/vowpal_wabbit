...
struct lq_data {
  ...
  float max_pred; // The max cost for this label predicted by the current set of
                  // good regressors
  float min_pred; // The min cost for this label predicted by the current set of
                  // good regressors
  bool is_range_large; // Indicator of whether this label's cost range was large
  bool is_range_overlapped; // Indicator of whether this label's cost range
                            // overlaps with the cost range that has the
                            // minimnum max_pred
  bool query_needed; // Used in reduction mode: tell upper-layer whether a query
                     // is needed for this label
...
inline void inner_loop(cs_active &cs_a, single_learner &base, example &ec,
                       uint32_t i, float cost, uint32_t &prediction,
                       float &score, float &partial_prediction,
                       bool query_this_label, bool &query_needed) {
  base.predict(ec, i - 1);
  // cerr << "base.predict ==> partial_prediction=" << ec.partial_prediction <<
  // endl;
  if (is_learn) {
    vw &all = *cs_a.all;
    ...
    if (is_simulation) {
      ...
      // In reduction mode.
      // If the cost of this label was previously queried, then it should be
      // available for learning now.
      // If the cost of this label was not queried, then skip it.
      if (query_needed) {
        ec.l.simple.label = cost;
        if ((cost < cs_a.cost_min) || (cost > cs_a.cost_max))
          cerr << "warning: cost " << cost << " outside of cost range ["
               << cs_a.cost_min << ", " << cs_a.cost_max << "]!" << endl;
      } else
        ec.l.simple.label = FLT_MAX;
    }
    ...
    // Prediction in reduction mode could be used by upper layer to ask whether
    // this label needs to be queried.
    // So we return that.
...
inline void find_cost_range(cs_active &cs_a, single_learner &base, example &ec,
                            uint32_t i, float delta, float eta, float &min_pred,
                            float &max_pred, bool &is_range_large) {
  float tol = 1e-6f;
  ...
  if (cs_a.t <= 1 || std::isnan(sens) || std::isinf(sens)) {
    ...
    if (cs_a.print_debug_stuff)
      cerr << "  find_cost_rangeA: i=" << i << " pp=" << ec.partial_prediction
           << " sens=" << sens << " eta=" << eta << " [" << min_pred << ", "
           << max_pred << "] = " << (max_pred - min_pred) << endl;
  } else {
    // finding max_pred and min_pred by binary search
    max_pred = std::min(ec.pred.scalar +
                            sens * binarySearch(cs_a.cost_max - ec.pred.scalar,
                                                delta, sens, tol),
                        cs_a.cost_max);
    min_pred = std::max(ec.pred.scalar -
                            sens * binarySearch(ec.pred.scalar - cs_a.cost_min,
                                                delta, sens, tol),
                        cs_a.cost_min);
    is_range_large = (max_pred - min_pred > eta);
    if (cs_a.print_debug_stuff)
      cerr << "  find_cost_rangeB: i=" << i << " pp=" << ec.partial_prediction
           << " sens=" << sens << " eta=" << eta << " [" << min_pred << ", "
           << max_pred << "] = " << (max_pred - min_pred) << endl;
  }
}
...
void predict_or_learn(cs_active &cs_a, single_learner &base, example &ec) {
  ...
  // cerr << "is_learn=" << is_learn << " ld.costs.size()=" << ld.costs.size()
  // << endl;
  if (cs_a.all->sd->queries >= cs_a.min_labels * cs_a.num_classes) {
    // save regressor
    std::stringstream filename;
    filename << cs_a.all->final_regressor_name << "." << ec.example_counter
             << "." << cs_a.all->sd->queries << "." << cs_a.num_any_queries;
    VW::save_predictor(*(cs_a.all), filename.str());
    cerr << endl
         << "Number of examples with at least one query = "
    ...
    for (size_t i = 0; i < cs_a.examples_by_queries.size(); i++) {
      cerr << endl
           << "examples with " << i
           << " labels queried = " << cs_a.examples_by_queries[i];
    }

    cerr << endl
         << "labels outside of cost range = " << cs_a.labels_outside_range;
    cerr << endl
  ...
  float eta = cs_a.c1 * (cs_a.cost_max - cs_a.cost_min) /
              std::sqrt(t); // threshold on cost range
  float delta = cs_a.c0 *
                log((float)(cs_a.num_classes * std::max(t_prev, 1.f))) *
                pow(cs_a.cost_max - cs_a.cost_min,
                    2); // threshold on empirical loss difference
  ...
  if (ld.costs.size() > 0) {
    ...
    for (lq_data &lqd : cs_a.query_data) {
      find_cost_range(cs_a, base, ec, lqd.cl.class_index, delta, eta,
                      lqd.min_pred, lqd.max_pred, lqd.is_range_large);
    ...
    for (lq_data &lqd : cs_a.query_data) {
      ...
      // large_range = large_range || (cl.is_range_overlapped &&
      // cl.is_range_large);
      // if(cl.is_range_overlapped && is_learn)
      //{ std::cout << "label " << cl.class_index << ", min_pred = " <<
      //cl.min_pred << ", max_pred = " << cl.max_pred << ",
      // is_range_large = " << cl.is_range_large << ", eta = " << eta << ",
      // min_max_cost = " << min_max_cost << endl;
      //}
      cs_a.overlapped_and_range_small +=
          (size_t)(lqd.is_range_overlapped && !lqd.is_range_large);
      if (lqd.cl.x > lqd.max_pred || lqd.cl.x < lqd.min_pred) {
        cs_a.labels_outside_range++;
        // cs_a.all->sd->distance_to_range[cl.class_index-1] += std::max(cl.x -
        // cl.max_pred, cl.min_pred - cl.x);
        cs_a.distance_to_range +=
            std::max(lqd.cl.x - lqd.max_pred, lqd.min_pred - lqd.cl.x);
    ...
    for (lq_data &lqd : cs_a.query_data) {
      bool query_label =
          ((query && cs_a.is_baseline) ||
           (!cs_a.use_domination && lqd.is_range_large) ||
      ...
      if (cs_a.print_debug_stuff)
        cerr << "label=" << lqd.cl.class_index << " x=" << lqd.cl.x
             << " prediction=" << prediction << " score=" << score
             << " pp=" << lqd.cl.partial_prediction << " ql=" << query_label
             << " qn=" << lqd.query_needed << " ro=" << lqd.is_range_overlapped
    ...
    for (uint32_t i = 1; i <= cs_a.num_classes; i++) {
      inner_loop<false, is_simulation>(cs_a, base, ec, i, FLT_MAX, prediction,
...
base_learner *cs_active_setup(options_i &options, vw &all) {
  ...
  new_options
      ...
      .add(make_option("baseline", data->is_baseline)
               .help("cost-sensitive active learning baseline"))
      .add(
          make_option("domination", domination)
              .default_value(1)
      ...
      .add(make_option("range_c", data->c1)
               .default_value(0.5f)
               .help("parameter controlling the threshold for per-label cost "
      ...
      .add(make_option("cost_max", data->cost_max)
               .default_value(1.f)
               .help("cost upper bound. Default 1."))
      .add(make_option("cost_min", data->cost_min)
               .default_value(0.f)
               .help("cost lower bound. Default 0."))
      // TODO replace with trace and quiet
      .add(make_option("csa_debug", data->print_debug_stuff)
  ...
  if (!options.was_supplied("adax"))
    all.trace_message << "WARNING: --cs_active should be used with --adax"
                      << endl;
