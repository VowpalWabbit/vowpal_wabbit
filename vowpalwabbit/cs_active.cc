  ...
  if (is_learn)
  {
    ...
    else
    {
      ...
      if (query_needed)
      {
        ...
        {
          cs_a.all->logger.warn("Cost {0} outside of cost range[{1}, {2}]", cost, cs_a.cost_min, cs_a.cost_max);
        }
      }
      else
        ec.l.simple.label = FLT_MAX;
    ...
    float& min_pred, float& max_pred, bool& is_range_large)
{
  ...
  if (cs_a.t <= 1 || std::isnan(sens) || std::isinf(sens))
  {
    ...
    if (cs_a.print_debug_stuff)
      cs_a.all->logger.info("find_cost_rangeA: i={0} pp={1} sens={2} eta={3} [{4}, {5}] = {6}",
  ...
  else
  {
    ...
    if (cs_a.print_debug_stuff)
      cs_a.all->logger.info("find_cost_rangeB: i={0} pp={1} sens={2} eta={3} [{4}, {5}] = {6}",
                          i, ec.partial_prediction, sens, eta, min_pred, max_pred, (max_pred - min_pred));
  }
}
...
void predict_or_learn(cs_active& cs_a, single_learner& base, example& ec)
{
  ...
  if (cs_a.all->sd->queries >= cs_a.min_labels * cs_a.num_classes)
  {
    ...
    *(cs_a.all->driver_output) << endl << "Number of examples with at least one query = " << cs_a.num_any_queries;
    ...
    {
      *(cs_a.all->driver_output) << endl << "examples with " << i << " labels queried = " << cs_a.examples_by_queries[i];
    }
  ...
  if (ld.costs.size() > 0)
  {
    ...
    for (lq_data& lqd : cs_a.query_data)
    {
      ...
      if (cs_a.print_debug_stuff)
        cs_a.all->logger.info("label={0} x={1} prediction={2} score={3} pp={4} ql={5} qn={6} ro={7} rl={8} "
