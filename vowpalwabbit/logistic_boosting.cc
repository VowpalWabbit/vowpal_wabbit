...
template <bool is_learn>
  void predict_or_learn(boosting& o, LEARNER::base_learner& base, example& ec) {
	...
	if (o.discrete) {
	    s += ld.label * sign(ec.pred.scalar);
	} 
	else {
...
#ifdef DEBUG
    cout << "Returning with total prediction " << ec.pred.scalar << endl;
#endif
  }
  ...
  void predict_or_learn_logistic(boosting& o, LEARNER::base_learner& base, example& ec) {
    ...
    for (int i = 0; i < o.N; i++) {
      ...
      if (is_learn) {
        ...
        base.predict(ec, i);
	float z = ld.label * sign(ec.pred.scalar);
        s += z * o.alpha[i];
...
#ifdef DEBUG
    cout << "Returning with total prediction " << ec.pred.scalar << endl;
    if (ec.pred.scalar*ld.label < 0) { e++; cout << "Error " << e << " " << ec.example_counter << endl; }
#endif
  }
...
LEARNER::base_learner* boosting_setup(vw& all)
{
    ...
    new_options(all, "Boosting Options")
	 ...
	 "specify the boosting algorithm: BBM (default), logistic, smooth");
    add_options(all);
    ...
    *temp = all.vm["alg"].as<string>();
    ...
    learner<boosting>* l;
    if (*data.alg == "BBM") {
        l = &init_learner<boosting>(&data, setup_base(all), predict_or_learn<true>, predict_or_learn<false>, data.N);

    } else if (*data.alg == "smooth") {
        l = &init_learner<boosting>(&data, setup_base(all), predict_or_learn_smooth<true>, predict_or_learn_smooth<false>, data.N);

    } else if (*data.alg == "logistic") {
        l = &init_learner<boosting>(&data, setup_base(all), predict_or_learn_logistic<true>, predict_or_learn_logistic<false>, data.N);
