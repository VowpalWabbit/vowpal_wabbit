...
namespace VW {
vw_exception::vw_exception(const char *pfile, int plineNumber,
                           std::string const &pmessage) noexcept
    : file(pfile),
      message(pmessage),
...
void vw_trace(const char *filename, int linenumber, const char *fmt, ...) {
  char buffer[4 * 1024];
  int offset = sprintf_s(buffer, sizeof(buffer), "%s:%d (%d): ", filename,
                         linenumber, GetCurrentThreadId());
...
double StopWatch::MilliSeconds() const {
  ...
  return double(now.QuadPart - data->startTime_.QuadPart) /
         (double(data->frequency_.QuadPart) / 1000);
}
...
bool launchDebugger() {
  ...
  if (!CreateProcessW(NULL, &cmdLine[0], NULL, NULL, FALSE, 0, NULL, NULL, &si,
                      &pi))
    return false;
