...
constexpr inline bool example_is_test(example &ec) {
...
void predict_or_learn(LRQFAstate &lrq, single_learner &base, example &ec) {
  ...
  for (unsigned int iter = 0; iter < maxiter; ++iter, ++which) {
    ...
    for (std::string::const_iterator i1 = lrq.field_name.begin();
         i1 != lrq.field_name.end(); ++i1) {
      for (std::string::const_iterator i2 = i1 + 1; i2 != lrq.field_name.end();
           ++i2) {
        ...
        for (unsigned int lfn = 0; lfn < lrq.orig_size[left]; ++lfn) {
          ...
          for (unsigned int n = 1; n <= k; ++n) {
            uint64_t lwindex =
                (lindex +
                 ((uint64_t)(rfd_id * k + n)
                  << stride_shift)); // a feature has k weights in each field
            ...
            for (unsigned int rfn = 0; rfn < lrq.orig_size[right]; ++rfn) {
              ...
              uint64_t rwindex =
                  (rindex + ((uint64_t)(lfd_id * k + n) << stride_shift));
...
#endif
                rfs.space_names.push_back(audit_strings_ptr(
                    new audit_strings(new_space, new_feature)));
              }
            }
          }
        }
...
LEARNER::base_learner *lrqfa_setup(options_i &options, vw &all) {
  ...
  new (&lrq->field_name) std::string(
      lrqopt.substr(0, last_index + 1)); // make sure there is no duplicates
  ...
  learner<LRQFAstate, example> &l = init_learner(
      lrq, as_singleline(setup_base(options, all)), predict_or_learn<true>,
