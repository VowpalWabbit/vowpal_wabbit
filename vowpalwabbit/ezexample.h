...
private:
  ...
  void setup_for_predict() {
    static example *empty_example =
        is_multiline ? VW::read_example(*vw_par_ref, (char *)"") : nullptr;
...
public:
  ...
  //   do NOT touch the underlying example unless you really know what you're
  //   done)
  ezexample(vw *this_vw, example *this_ec, bool multiline = false,
  ...
  inline fid addf(char to_ns, fid fint, float v) {
    ...
    ec->feature_space[(int)to_ns].push_back(
        v, fint << vw_ref->weights.stride_shift());
  ...
  // copy an entire namespace from this other example, you can even give it a
  // new namespace name if you want!
  ...
  void add_other_example_ns(example &other,
                            char ns) // default to_ns to other_ns
  {
  ...
  void mini_setup_example() {
    ...
    for (std::vector<std::string>::iterator i = vw_ref->pairs.begin();
         i != vw_ref->pairs.end(); i++) {
      quadratic_features_num += ec->feature_space[(int)(*i)[0]].size() *
                                ec->feature_space[(int)(*i)[1]].size();
      quadratic_features_sqr += ec->feature_space[(int)(*i)[0]].sum_feat_sq *
                                ec->feature_space[(int)(*i)[1]].sum_feat_sq;
    }
    ec->num_features += quadratic_features_num;
    ec->total_sum_feat_sq += quadratic_features_sqr;
  }
  ...
  void train() // if multiline, add to stack; otherwise, actually train
  {
    ...
    {      // we need to make a copy
      ...
      assert(ec->in_use);
      VW::copy_example_data(vw_ref->audit, copy, ec,
                            vw_par_ref->p->lp.label_size,
                            vw_par_ref->p->lp.copy_label);
      assert(copy->in_use);
      vw_ref->learn(*copy);
  ...
  inline fid hash(std::string fstr) {
  ...
  inline fid addf(char ns, std::string fstr, float val) {
    return addf(ns, hash(ns, fstr), val);
  }
  inline fid addf(char ns, std::string fstr) {
