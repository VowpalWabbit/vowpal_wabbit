...
public:
  ...
  void mini_setup_example() {
    ...
    for (auto const &pair : vw_ref->pairs) {
      quadratic_features_num += ec->feature_space[(int)pair[0]].size() *
                                ec->feature_space[(int)pair[1]].size();
      quadratic_features_sqr += ec->feature_space[(int)pair[0]].sum_feat_sq *
                                ec->feature_space[(int)pair[1]].sum_feat_sq;
    }
    ec->num_features += quadratic_features_num;
    ec->total_sum_feat_sq += quadratic_features_sqr;
  }
