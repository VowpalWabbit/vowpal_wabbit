...
class ezexample
{
 ...
 public:
  ...
  {
    if (vw_ref->gs.ignore_some && vw_ref->fc.ignore[(int)c])
      return true;
    if (ns_exists[(int)c])
      return false;
  ...
  void mini_setup_example()
  {
    ...
    for (auto const& interaction : vw_ref->gs.interactions)
    {
    ...
    ec->num_features += quadratic_features_num;
    ec->total_sum_feat_sq += quadratic_features_sqr;
    ec->interactions = &vw_ref->gs.interactions;
  }
  ...
  void train()  // if multiline, add to stack; otherwise, actually train
  {
    ...
    {     // we need to make a copy
      ...
      VW::copy_example_data(vw_ref->oc.audit, copy, ec, vw_par_ref->p->lp.label_size, vw_par_ref->p->lp.copy_label);
      vw_ref->learn(*copy);
