...
namespace VW {
template <typename T> struct default_cleanup {
  void operator()(T *) {}
};
...
struct no_lock_object_pool {
  ...
  no_lock_object_pool(size_t initial_chunk_size, TInitializer initializer = {},
                      size_t chunk_size = 8)
      : m_initializer(initializer), m_initial_chunk_size(initial_chunk_size),
...
struct object_pool {
  object_pool() = default;
  object_pool(size_t initial_chunk_size, TInitializer initializer = {},
              size_t chunk_size = 8)
