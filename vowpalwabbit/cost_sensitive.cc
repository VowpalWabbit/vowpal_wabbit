...
void parse_label(parser *p, shared_data *sd, void *v,
    ...
    if (eq_shared || eq_label) {
      if (eq_shared) {
        if (p->parse_name.size() != 1)
          std::cerr << "shared feature vectors should not have costs on: "
                    << words[0] << std::endl;
        else {
      ...
      if (eq_label) {
        if (p->parse_name.size() != 2)
          std::cerr << "label feature vectors should have exactly one cost on: "
                    << words[0] << std::endl;
        else {
  ...
  for (unsigned int i = 0; i < words.size(); i++) {
    ...
    if (p->parse_name.size() == 1 || p->parse_name.size() == 2 ||
        p->parse_name.size() == 3) {
      f.class_index = sd->ldict ? (uint32_t)sd->ldict->get(p->parse_name[0])
                                : (uint32_t)hashstring(p->parse_name[0], 0);
      if (p->parse_name.size() == 1 &&
          f.x >= 0) // test examples are specified just by un-valued class #s
        f.x = FLT_MAX;
    } else
...
label_parser cs_label = {default_label,     parse_label,  cache_label,
                         read_cached_label, delete_label, weight,
                         copy_label,        test_label,   sizeof(label)};
...
void print_update(vw &all, bool is_test, example &ec, multi_ex *ec_seq,
  ...
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.quiet &&
    ...
    if (action_scores || all.sd->ldict) {
      ...
      if (action_scores)
        pred_buf << ".....";
      all.sd->print_update(all.holdout_set_off, all.current_pass, label_buf,
                           pred_buf.str(), num_current_features,
                           all.progress_add, all.progress_arg);
      ;
    } else
      all.sd->print_update(all.holdout_set_off, all.current_pass, label_buf,
                           prediction, num_current_features, all.progress_add,
                           all.progress_arg);
  }
}
...
void output_example(vw &all, example &ec) {
  ...
  if (!test_label(&ld)) {
    ...
    if (chosen_loss == FLT_MAX)
      std::cerr << "warning: csoaa predicted an invalid class. Are all "
                   "multi-class labels in the {1..k} range?"
                << std::endl;
  ...
  all.sd->update(ec.test_only, !test_label(&ld), loss, ec.weight,
                 ec.num_features);
  ...
  for (int sink : all.final_prediction_sink)
    if (!all.sd->ldict)
      ...
      all.print_text(sink,
                     std::string(ss_pred.begin, ss_pred.end - ss_pred.begin),
                     ec.tag);
    }
  ...
  print_update(all, test_label(&ec.l.cs), ec, nullptr, false,
               ec.pred.multiclass);
}
...
bool ec_is_example_header(
    example const &ec) // example headers look like "shared"
{
  v_array<COST_SENSITIVE::wclass> costs = ec.l.cs.costs;
  if (costs.size() != 1)
    return false;
