...
void print_update(vw& all, bool is_test, example& ec, multi_ex* ec_seq, bool action_scores, uint32_t prediction)
{
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.logger.quiet && !all.bfgs)
  {
    ...
    if (action_scores || all.sd->ldict)
    {
      ...
      if (action_scores)
        pred_buf << ".....";
      all.sd->print_update(all.holdout_set_off, all.gs.current_pass, label_buf, pred_buf.str(), num_current_features,
          all.progress_add, all.progress_arg);
      ;
    }
    else
      all.sd->print_update(all.holdout_set_off, all.gs.current_pass, label_buf, prediction, num_current_features,
          all.progress_add, all.progress_arg);
  }
}
