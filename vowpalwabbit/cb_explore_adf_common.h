...
// Most of these includes are required because templated functions are using the
// objects defined in them
// A few options to get rid of them:
// - Use virtual function calls in predict/learn to get rid of the templates
// entirely (con: virtual function calls)
// - Cut out the portions of code that actually use the objects and put them
// into new functions
...
// Free functions
inline void sort_action_probs(v_array<ACTION_SCORE::action_score> &probs,
  ...
  std::sort(probs.begin(), probs.end(),
            [&scores](const ACTION_SCORE::action_score &as1,
                      const ACTION_SCORE::action_score &as2) {
...
public:
  ...
  static void finish_multiline_example(vw &all,
                                       cb_explore_adf_base<ExploreType> &data,
                                       multi_ex &ec_seq);
  static void predict(cb_explore_adf_base<ExploreType> &data,
                      LEARNER::multi_learner &base, multi_ex &examples);
  static void learn(cb_explore_adf_base<ExploreType> &data,
                    LEARNER::multi_learner &base, multi_ex &examples);
...
inline void cb_explore_adf_base<ExploreType>::predict(
    cb_explore_adf_base<ExploreType> &data, LEARNER::multi_learner &base,
    multi_ex &examples) {
...
template <typename ExploreType>
inline void
cb_explore_adf_base<ExploreType>::learn(cb_explore_adf_base<ExploreType> &data,
...
template <typename ExploreType>
void cb_explore_adf_base<ExploreType>::output_example(vw &all,
                                                      multi_ex &ec_seq) {
  if (ec_seq.size() <= 0)
    return;
  ...
  for (auto sink : all.final_prediction_sink)
    ACTION_SCORE::print_action_score(sink, ec.pred.a_s, ec.tag);
...
template <typename ExploreType>
void cb_explore_adf_base<ExploreType>::output_example_seq(vw &all,
                                                          multi_ex &ec_seq) {
  if (ec_seq.size() > 0) {
