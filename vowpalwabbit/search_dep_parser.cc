...
Search::search_task task = {"dep_parser", run,   initialize,
                            finish,       setup, nullptr};
}
...
struct task_data {
  ...
  size_t root_label;
  uint32_t num_label;
  v_array<uint32_t> valid_actions, action_loss, gold_heads, gold_tags, stack,
      heads, tags, temp, valid_action_temp;
...
void initialize(Search::search &sch, size_t & /*num_actions*/,
                options_i &options) {
  ...
  new_options.add(
      make_option("root_label", data->root_label)
          .keep()
          .default_value(8)
          .help("Ensure that there is only one root in each sentence"));
  new_options.add(make_option("num_label", data->num_label)
                      .keep()
                      .default_value(12)
                      .help("Number of arc labels"));
  ...
  new_options.add(make_option("old_style_labels", data->old_style_labels)
                      .keep()
                      .help("Use old hack of label information"));
  ...
  const char *pair[] = {"BC", "BE", "BB", "CC", "DD", "EE", "FF",
                        "GG", "EF", "BH", "BJ", "EL", "dB", "dC",
                        "dD", "dE", "dF", "dG", "dd"};
  const char *triple[] = {"EFG", "BEF", "BCE", "BCD", "BEL", "ELM",
                          "BHI", "BCC", "BEJ", "BEH", "BJK", "BEN"};
  ...
  all.interactions.insert(std::end(all.interactions), std::begin(all.pairs),
                          std::end(all.pairs));
  all.interactions.insert(std::end(all.interactions), std::begin(all.triples),
                          std::end(all.triples));
  if (data->cost_to_go)
  ...
  sch.set_label_parser(COST_SENSITIVE::cs_label, [](polylabel &l) -> bool {
...
void inline add_feature(example &ex, uint64_t idx, unsigned char ns,
                        uint64_t mask, uint64_t multiplier,
                        bool /* audit */ = false) {
  ex.feature_space[(int)ns].push_back(1.0f, (idx * multiplier) & mask);
}

void add_all_features(example &ex, example &src, unsigned char tgt_ns,
                      uint64_t mask, uint64_t multiplier, uint64_t offset,
                      bool /* audit */ = false) {
...
// arc-hybrid System.
size_t transition_hybrid(Search::search &sch, uint64_t a_id, uint32_t idx,
                         uint32_t t_id, uint32_t /* n */) {
  task_data *data = sch.get_task_data<task_data>();
  v_array<uint32_t> &heads = data->heads, &stack = data->stack,
  ...
  if (a_id == SHIFT) {
    ...
    children[1][hd]++;
    tags[last] = t_id;
    sch.loss(gold_heads[last] != heads[last] ? 2 : (gold_tags[last] != t_id)
                                                       ? 1.f
                                                       : 0.f);
    assert(!stack.empty());
    stack.pop();
...
// arc-eager system
size_t transition_eager(Search::search &sch, uint64_t a_id, uint32_t idx,
                        uint32_t t_id, uint32_t n) {
  task_data *data = sch.get_task_data<task_data>();
  v_array<uint32_t> &heads = data->heads, &stack = data->stack,
                    &gold_heads = data->gold_heads,
...
void extract_features(Search::search &sch, uint32_t idx, multi_ex &ec) {
  ...
  v_array<uint32_t> &stack = data->stack, &tags = data->tags,
                    *children = data->children, &temp = data->temp;
  ...
  for (size_t i = 0; i < 3; i++)
    ec_buf[i] = (stack.size() > i && *(stack.end() - (i + 1)) != 0)
                    ? ec[*(stack.end() - (i + 1)) - 1]
                    : 0;
  ...
  // element stack ec_buf[6]: sl1, ec_buf[7]: sl2,
  ...
  for (size_t i = 10; i < 12; i++)
    ec_buf[i] = (idx <= n && children[i - 8][idx] != 0)
                    ? ec[children[i - 8][idx] - 1]
                    : 0;
  ec_buf[12] = (stack.size() > 1 && *(stack.end() - 2) != 0 &&
                children[2][*(stack.end() - 2)] != 0)
  ...
  for (size_t i = 0; i < 13; i++) {
    ...
    if (!ec_buf[i])
      add_feature(ex, (uint64_t)438129041 + additional_offset,
                  (unsigned char)((i + 1) + 'A'), mask, multiplier);
    else
      add_all_features(ex, *ec_buf[i], 'A' + (unsigned char)(i + 1), mask,
                       multiplier, additional_offset, false);
  }

  // Other features
  temp.resize(10);
  temp[0] = empty ? 0 : (idx > n ? 1 : 2 + std::min(static_cast<uint32_t>(5),
                                                    idx - (uint32_t)last));
  temp[1] =
      empty ? 1 : 1 + std::min(static_cast<uint32_t>(5), children[0][last]);
  temp[2] =
      empty ? 1 : 1 + std::min(static_cast<uint32_t>(5), children[1][last]);
  temp[3] =
      idx > n ? 1 : 1 + std::min(static_cast<uint32_t>(5), children[0][idx]);
  for (size_t i = 4; i < 8; i++)
    temp[i] = (!empty && children[i - 2][last] != 0)
                  ? tags[children[i - 2][last]]
                  : 15;
  for (size_t i = 8; i < 10; i++)
    temp[i] = (idx <= n && children[i - 6][idx] != 0)
                  ? tags[children[i - 6][idx]]
                  : 15;

  uint64_t additional_offset = val_namespace * offset_const;
  for (size_t j = 0; j < 10; j++) {
    additional_offset += j * 1023;
    add_feature(ex, temp[j] + additional_offset, val_namespace, mask,
                multiplier);
  }
  size_t count = 0;
  ...
  size_t new_count;
  float new_weight;
  INTERACTIONS::eval_count_of_generated_ft(all, *data->ex, new_count,
                                           new_weight);
...
void get_valid_actions(Search::search &sch, v_array<uint32_t> &valid_action,
                       uint64_t idx, uint64_t n, uint64_t stack_depth,
                       uint64_t state) {
  task_data *data = sch.get_task_data<task_data>();
  uint32_t &sys = data->transition_system;
  v_array<uint32_t> &stack = data->stack, &heads = data->heads,
                    &temp = data->temp;
  valid_action.clear();
  if (sys == arc_hybrid) {
    if (idx <= n) // SHIFT
...
void get_eager_action_cost(Search::search &sch, uint32_t idx, uint64_t n) {
  task_data *data = sch.get_task_data<task_data>();
  v_array<uint32_t> &action_loss = data->action_loss, &stack = data->stack,
                    &gold_heads = data->gold_heads, heads = data->heads;
  ...
  if (gold_heads[idx] > idx ||
      (gold_heads[idx] == 0 && size > 0 && stack[0] != 0))
    action_loss[REDUCE_RIGHT] += 1;
}
...
void get_hybrid_action_cost(Search::search &sch, size_t idx, uint64_t n) {
  ...
  if (!stack.empty())
    for (size_t i = 0; i < size - 1; i++)
      if (idx <= n &&
          (gold_heads[stack[i]] == idx || gold_heads[idx] == stack[i]))
        action_loss[SHIFT] += 1;
...
void get_cost_to_go_losses(
    Search::search &sch, v_array<std::pair<action, float>> &gold_action_losses,
  ...
  if (one_learner) {
    if (is_valid(SHIFT, valid_actions))
      gold_action_losses.push_back(
          std::make_pair(SHIFT, (float)action_loss[SHIFT]));
    for (uint32_t i = 2; i <= 3; i++)
      if (is_valid(i, valid_actions)) {
        for (uint32_t j = 1; j <= num_label; j++)
          if (sys == arc_eager || j != data->root_label)
            gold_action_losses.push_back(std::make_pair(
                (1 + j + (i - 2) * num_label),
                action_loss[i] +
                    (float)(j !=
                            (i == REDUCE_LEFT ? left_label : right_label))));
      }
    if (sys == arc_eager && is_valid(REDUCE, valid_actions))
      gold_action_losses.push_back(
          std::make_pair(2 + num_label * 2, (float)action_loss[REDUCE]));
  } else {
    ...
    if (sys == arc_eager && is_valid(REDUCE, valid_actions))
      gold_action_losses.push_back(
          std::make_pair(REDUCE, (float)action_loss[REDUCE]));
  }
}

void get_gold_actions(Search::search &sch, uint32_t idx, uint64_t /* n */,
  ...
  if (sys == arc_hybrid && is_valid(SHIFT, valid_actions) &&
      (stack.empty() || gold_heads[idx] == last)) {
    gold_actions.push_back(SHIFT);
    return;
  }

  if (sys == arc_hybrid && is_valid(REDUCE_LEFT, valid_actions) &&
      gold_heads[last] == idx) {
...
void setup(Search::search &sch, multi_ex &ec) {
  task_data *data = sch.get_task_data<task_data>();
  v_array<uint32_t> &gold_heads = data->gold_heads, &heads = data->heads,
                    &gold_tags = data->gold_tags, &tags = data->tags;
  size_t n = ec.size();
  heads.resize(n + 1);
  tags.resize(n + 1);
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (data->old_style_labels) {
      ...
      tag = (costs.size() <= 1) ? (uint32_t)data->root_label
                                : costs[1].class_index;
    }
    if (tag > data->num_label)
      THROW("invalid label " << tag
                             << " which is > num actions=" << data->num_label);
...
void run(Search::search &sch, multi_ex &ec) {
  task_data *data = sch.get_task_data<task_data>();
  v_array<uint32_t> &stack = data->stack, &gold_heads = data->gold_heads,
                    &valid_actions = data->valid_actions, &heads = data->heads,
                    &gold_tags = data->gold_tags, &tags = data->tags,
                    &valid_action_temp = data->valid_action_temp;
  v_array<uint32_t> &gold_action_temp = data->gold_action_temp;
  v_array<std::pair<action, float>> &gold_action_losses =
      data->gold_action_losses;
  ...
  while (true) {
    ...
    get_valid_actions(sch, valid_actions, idx, n, (uint64_t)stack.size(),
    ...
    if (one_learner) {
      if (cost_to_go) {
        ...
        convert_to_onelearner_actions(sch, gold_actions, gold_action_temp,
                                      left_label, right_label);
        convert_to_onelearner_actions(sch, valid_actions, valid_action_temp,
                                      my_null, my_null);
        a_id =
            P.set_tag((ptag)count)
                .set_input(*(data->ex))
      ...
      if (a_id != SHIFT && a_id != REDUCE) {
        ...
        if (cost_to_go) {
          ...
          for (size_t i = 1; i <= data->num_label; i++)
            gold_action_losses.push_back(std::make_pair(
                (action)i,
                i != (a_id == REDUCE_LEFT ? left_label : right_label)));
          t_id =
              P.set_tag((ptag)count)
                  .set_input(*(data->ex))
                  .set_allowed(gold_action_losses)
  ...
  if (sch.output().good())
    for (size_t i = 1; i <= n; i++)
      sch.output() << (heads[i]) << ":" << tags[i] << std::endl;
}
} // namespace DepParserTask
