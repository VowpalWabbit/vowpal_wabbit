...
struct parser {
  parser(size_t ring_size, bool strict_parse_)
      : example_pool{ring_size}, ready_parsed_examples{ring_size},
        ring_size{ring_size}, strict_parse{strict_parse_} {
  ...
  uint64_t begin_parsed_examples =
      0; // The index of the beginning parsed example.
  uint64_t end_parsed_examples = 0; // The index of the fully parsed example.
  uint32_t in_pass_counter = 0;
  bool emptylines_separate_examples = false; // true if you want to have holdout
                                             // computed on a per-block basis
                                             // rather
