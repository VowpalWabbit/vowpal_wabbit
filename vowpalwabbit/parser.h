...
struct parser
{
  ...
  example *examples;
  uint64_t used_index;
  bool emptylines_separate_examples;  // true if you want to have holdout
                                      // computed on a per-block basis rather
                                      // than a per-line basis

  // Both example condition_variables use the same mutex.
  // examples_lock protects: p->done, ec.in_use, p->begin_parsed_examples,
  // p->end_parse_examples, and p->used_index.
