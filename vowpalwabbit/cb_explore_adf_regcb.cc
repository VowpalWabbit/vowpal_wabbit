...
public:
  cb_explore_adf_regcb(bool regcbopt, float c0, bool first_only,
                       float min_cb_cost, float max_cb_cost);
...
private:
  ...
  void get_cost_ranges(float delta, LEARNER::multi_learner &base,
                       multi_ex &examples, bool min_only);
  float binary_search(float fhat, float delta, float sens, float tol = 1e-6);
};

cb_explore_adf_regcb::cb_explore_adf_regcb(bool regcbopt, float c0,
                                           bool first_only, float min_cb_cost,
                                           float max_cb_cost)
    : _regcbopt(regcbopt), _c0(c0), _first_only(first_only),
      _min_cb_cost(min_cb_cost), _max_cb_cost(max_cb_cost) {}

// TODO: same as cs_active.cc, move to shared place
float cb_explore_adf_regcb::binary_search(float fhat, float delta, float sens,
                                          float tol) {
...
void cb_explore_adf_regcb::get_cost_ranges(float delta,
...
template <bool is_learn>
void cb_explore_adf_regcb::predict_or_learn_impl(LEARNER::multi_learner &base,
                                                 multi_ex &examples) {
  if (is_learn) {
  ...
  const float delta =
      _c0 * log((float)(num_actions * _counter)) * pow(max_range, 2);
    ...
    if (_regcbopt) // optimistic variant
    {
      ...
      for (size_t i = 0; i < preds.size(); ++i) {
        if (preds[i].action == a_opt ||
            (!_first_only && _min_costs[preds[i].action] == min_cost))
          preds[i].score = 1;
        else
          preds[i].score = 0;
      }
    } else // elimination variant
    {
      ...
      for (size_t i = 0; i < preds.size(); ++i) {
        ...
        exploration::enforce_minimum_probability(
            1.0, /*update_zero_elements=*/false, begin_scores(preds),
            end_scores(preds));
      }
    }
  }
...
LEARNER::base_learner *setup(VW::config::options_i &options, vw &all) {
  ...
  new_options
      ...
      .add(make_option("regcbopt", regcbopt)
               .keep()
  ...
  auto data = scoped_calloc_or_throw<explore_type>(regcbopt, c0, first_only,
                                                   min_cb_cost, max_cb_cost);
  LEARNER::learner<explore_type, multi_ex> &l = LEARNER::init_learner(
      data, base, explore_type::learn, explore_type::predict,
