...
VW::LEARNER::base_learner* setup(config::options_i& options, vw& all)
{
  ...
  {
    all.oc.trace_message << "warning: currently, mtr is only used for the first policy in cover, other policies use dr"
                      << std::endl;
    cb_type_enum = CB_TYPE_MTR;
  }
  else
  {
    all.oc.trace_message << "warning: cb_type must be in {'ips','dr','mtr'}; resetting to mtr." << std::endl;
  ...
  all.gs.label_type = label_type_t::cb;
