...
public:
  cb_explore_adf_cover(size_t cover_size, float psi, bool nounif,
                       bool first_only, LEARNER::multi_learner *cs_ldf_learner,
                       LEARNER::single_learner *scorer, size_t cb_type);
...
cb_explore_adf_cover::cb_explore_adf_cover(
    size_t cover_size, float psi, bool nounif, bool first_only,
    LEARNER::multi_learner *cs_ldf_learner, LEARNER::single_learner *scorer,
    size_t cb_type)
    : _cover_size(cover_size), _psi(psi), _nounif(nounif),
      _first_only(first_only), _cs_ldf_learner(cs_ldf_learner) {
  _gen_cs.cb_type = cb_type;
  _gen_cs.scorer = scorer;
}

template <bool is_learn>
void cb_explore_adf_cover::predict_or_learn_impl(LEARNER::multi_learner &base,
                                                 multi_ex &examples) {
  // Redundant with the call in cb_explore_adf_base, but encapsulation means we
  ...
  if (is_learn) {
    ...
    LEARNER::multiline_learn_or_predict<true>(base, examples,
                                              examples[0]->ft_offset);
  } else {
    GEN_CS::gen_cs_example_ips(examples, _cs_labels);
    LEARNER::multiline_learn_or_predict<false>(base, examples,
                                               examples[0]->ft_offset);
  }
  ...
  const float min_prob = (std::min)(
      1.f / num_actions, 1.f / (float)std::sqrt(_counter * num_actions));
  _action_probs.clear();
  for (uint32_t i = 0; i < num_actions; i++)
    _action_probs.push_back({i, 0.});
  _scores.clear();
  for (uint32_t i = 0; i < num_actions; i++)
  ...
  if (!_first_only) {
    ...
    for (size_t i = 0; i < tied_actions; ++i)
      _action_probs[preds[i].action].score +=
          additive_probability / tied_actions;
  } else
  ...
  for (size_t i = 1; i < _cover_size; i++) {
    ...
    if (is_learn) {
      ...
      /* Cover's learn policy is similar to bag in that we have multiple
       * learners
       * The main difference here is that Cover's learners interact with each
       * other.
       * The following code generates cost = cost + penalty where a penalty is
       * applied
       * to any action a previous policy has already selected
       */
      for (uint32_t j = 0; j < num_actions; j++) {
        float pseudo_cost =
            _cs_labels.costs[j].x -
            _psi * min_prob /
                ((std::max)(_action_probs[j].score, min_prob) / norm);
        _cs_labels_2.costs.push_back({pseudo_cost, j, 0., 0.});
      }
      GEN_CS::call_cs_ldf<true>(*(_cs_ldf_learner), examples, _cb_labels,
                                _cs_labels_2, _prepped_cs_labels,
                                examples[0]->ft_offset, i + 1);
    } else
      GEN_CS::call_cs_ldf<false>(*(_cs_ldf_learner), examples, _cb_labels,
                                 _cs_labels, _prepped_cs_labels,
                                 examples[0]->ft_offset, i + 1);
    ...
    if (!_first_only) {
      ...
      for (size_t i = 0; i < tied_actions; ++i) {
        if (_action_probs[preds[i].action].score < min_prob)
          norm += (std::max)(
              0.f,
              add_prob - (min_prob - _action_probs[preds[i].action].score));
        else
          norm += add_prob;
      ...
      if (_action_probs[action].score < min_prob)
        norm += (std::max)(0.f, additive_probability -
                                    (min_prob - _action_probs[action].score));
      else
  ...
  exploration::enforce_minimum_probability(min_prob * num_actions, !_nounif,
                                           begin_scores(_action_probs),
                                           end_scores(_action_probs));

  sort_action_probs(_action_probs, _scores);
  for (size_t i = 0; i < num_actions; i++)
    preds[i] = _action_probs[i];
...
cb_explore_adf_cover::~cb_explore_adf_cover() {
  _cb_labels.delete_v();
  for (size_t i = 0; i < _prepped_cs_labels.size(); i++)
    _prepped_cs_labels[i].costs.delete_v();
...
LEARNER::base_learner *setup(config::options_i &options, vw &all) {
  ...
  new_options
      ...
      .add(make_option("cover", cover_size)
               .keep()
               .help("Online cover based exploration"))
      .add(make_option("psi", psi)
               .keep()
               .default_value(1.0f)
               .help("disagreement parameter for cover"))
      .add(make_option("nounif", nounif).keep().help("do not explore uniformly "
                                                     "on zero-probability "
                                                     "actions in cover"))
  ...
  if (type_string.compare("dr") == 0)
    ...
    all.trace_message << "warning: currently, mtr is only used for the first "
                         "policy in cover, other policies use dr"
                      << std::endl;
  ...
  LEARNER::multi_learner *base =
      LEARNER::as_multiline(setup_base(options, all));
  all.p->lp = CB::cb_label;
  all.label_type = label_type::cb;

  using explore_type = cb_explore_adf_base<cb_explore_adf_cover>;
  auto data = scoped_calloc_or_throw<explore_type>(
      cover_size, psi, nounif, first_only, as_multiline(all.cost_sensitive),
      all.scorer, cb_type_enum);
