...
public:
                                         ...
                                         // array
  {
    ...
    if ((size == 0) || (_begin[a] > ele) ||
        (_begin[a] == ele)) // pusta tablica, nie wchodzi w while
      return a;
...
template <class T> inline v_array<T> v_init() {
...
template <class T> void push_many(v_array<T> &v, const T *_begin, size_t num) {
  if (v._end + num >= v.end_array)
    v.resize(
        max(2 * (size_t)(v.end_array - v._begin) + 3, v._end - v._begin + num));
...
std::ostream &operator<<(std::ostream &os, const v_array<std::pair<T, U>> &v) {
  os << '[';
  for (std::pair<T, U> *i = v._begin; i != v._end; ++i)
    os << ' ' << i->first << ':' << i->second;
  os << " ]";
  return os;
}
