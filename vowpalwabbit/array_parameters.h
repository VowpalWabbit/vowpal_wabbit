...
template <typename T> class sparse_iterator {
private:
...
public:
  sparse_parameters(size_t length, uint32_t stride_shift = 0)
      : _map(), _weight_mask((length << stride_shift) - 1),
        _stride_shift(stride_shift), _seeded(false), _delete(false),
        default_data(nullptr), fun(nullptr) {
    default_value = calloc_mergable_or_throw<weight>(stride());
  }

  sparse_parameters()
      : _map(), _weight_mask(0), _stride_shift(0), _seeded(false),
        _delete(false), default_data(nullptr), fun(nullptr) {
    default_value = calloc_mergable_or_throw<weight>(stride());
  }
  ...
  weight *first() {
    throw 1;
  } // TODO: Throw better exceptions. Allreduce currently not supported in
    // sparse.
  ...
  inline weight &operator[](size_t i) {
    ...
    if (iter == _map.end()) {
      _map.insert(
          std::make_pair(index, calloc_mergable_or_throw<weight>(stride())));
  ...
  inline weight &strided_index(size_t index) {
    return operator[](index << _stride_shift);
  }
  ...
  ~sparse_parameters() {
    if (!_delete && !_seeded) // don't free weight vector if it is shared with
                              // another instance
    {
      for (auto iter = _map.begin(); iter != _map.end(); ++iter)
        free(iter->second);
