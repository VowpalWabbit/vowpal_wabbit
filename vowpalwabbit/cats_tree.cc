...
namespace VW {
  namespace cats_tree {
        ...
        for (uint32_t i = 0; i < set_d.size(); i++)
        {
          ...
          if (cost_v != cost_w)
          {
            ...
            if (!filter)
            {
              ...
              {
                cost_parent =
                  (std::min)(cost_v, cost_w) * std::abs(ec.pred.scalar) + (std::max)(cost_v, cost_w) * (1 - std::abs(ec.pred.scalar));
                VW_DBG(ec) << "tree_c: learn() ec.pred.scalar == local_action" << std::endl;
              }
              else
              {
                cost_parent =
                  (std::max)(cost_v, cost_w) * std::abs(ec.pred.scalar) + (std::min)(cost_v, cost_w) * (1 - std::abs(ec.pred.scalar));
                VW_DBG(ec) << "tree_c: learn() ec.pred.scalar != local_action" << std::endl;
              }
            }
    ...
    void predict(cats_tree& ot, single_learner& base, example& ec)
    {
      ...
      VW_DBG(ec) << "tree_c: after tree.predict() " << multiclass_pred_to_string(ec) << features_to_string(ec)
        << std::endl;
    }
