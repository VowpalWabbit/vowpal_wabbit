...
void bs_predict_vote(example &ec, vector<double> &pred_vec) {
  ...
  if (multivote_detected &&
      majority_found ==
          false) // then find most frequent element - if tie: smallest tie label
  {
...
void output_example(vw &all, bs &d, example &ec) {
  ...
  if (all.final_prediction_sink.size() !=
      0) // get confidence interval only when printing out predictions
  {
    d.lb = FLT_MAX;
    d.ub = -FLT_MAX;
