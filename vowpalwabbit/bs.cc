...
void bs_predict_mean(vw &all, example &ec, std::vector<double> &pred_vec) {
  ec.pred.scalar = (float)accumulate(pred_vec.cbegin(), pred_vec.cend(), 0.0) /
                   pred_vec.size();
  if (ec.weight > 0 && ec.l.simple.label != FLT_MAX)
    ec.loss = all.loss->getLoss(all.sd, ec.pred.scalar, ec.l.simple.label) *
              ec.weight;
}
...
void bs_predict_vote(example &ec, std::vector<double> &pred_vec) {
  ...
  bool multivote_detected =
      false; // distinct(votes)>2: used to skip part of the algorithm
  auto pred_vec_sz = pred_vec.size();
  int *pred_vec_int = new int[pred_vec_sz];

  for (unsigned int i = 0; i < pred_vec_sz; i++) {
    pred_vec_int[i] = (int)floor(pred_vec[i] + 0.5); // could be added: link(),
                                                     // min_label/max_label,
                                                     // cutoff between
    ...
    if (!multivote_detected) // distinct(votes)>2 detection bloc
    {
      ...
      } else if (init_label != current_label &&
                 pred_vec_int[i] != current_label &&
  ...
  if (counter > 0 && multivote_detected) // remove this condition for: "avg on
                                         // votes" and getLoss()
  {
    counter = 0;
    for (unsigned int i = 0; i < pred_vec.size(); i++)
      if (pred_vec_int[i] == current_label) {
        counter++;
        // sum_labels += pred_vec[i]; // uncomment for: "avg on votes" and
        // getLoss()
      }
  ...
  if (multivote_detected && !majority_found) // then find most frequent element
                                             // - if tie: smallest tie label
  {
  ...
  // ld.prediction = sum_labels/(float)counter; //replace line below for: "avg
  // on votes" and getLoss()
  ec.pred.scalar = (float)current_label;

  // ec.loss = all.loss->getLoss(all.sd, ld.prediction, ld.label) * ec.weight;
  // //replace line below for: "avg on votes"
  // and getLoss()
...
void output_example(vw &all, bs &d, example &ec) {
  ...
  all.sd->update(ec.test_only, ld.label != FLT_MAX, ec.loss, ec.weight,
                 ec.num_features);
  if (ld.label != FLT_MAX && !ec.test_only)
    all.sd->weighted_labels += ((double)ld.label) * ec.weight;

  if (!all.final_prediction_sink.empty()) // get confidence interval only when
...
base_learner *bs_setup(options_i &options, vw &all) {
  ...
  learner<bs, example> &l =
      init_learner(data, as_singleline(setup_base(options, all)),
                   predict_or_learn<true>, predict_or_learn<false>, data->B);
