    ...
    size_t& num_interacted_features)
{
  ...
  if (ignore_some_linear)
  {
    for (auto& bucket : ec)
    {
      for (auto& fs : bucket)
      {
        if (!ignore_linear[fs.index]) { foreach_feature<DataT, FuncT, WeightsT>(weights, fs.features, dat, offset); }
      }
    }
  }
  ...
  else
  {
    for (auto& bucket : ec)
    {
      for (auto& fs : bucket) { foreach_feature<DataT, FuncT, WeightsT>(weights, fs.features, dat, offset); }
    }
  }
