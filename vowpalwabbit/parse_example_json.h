...
struct VWReaderHandler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, VWReaderHandler<audit>>
{
	...
	bool RawNumber(const Ch* str, rapidjson::SizeType length, bool copy) { return false; }
...
namespace VW
{
	...
	void read_line_json(vw& all, v_array<example*>& examples, char* line, example_factory_t example_factory, void* ex_factory_context)
	{
		...
		ParseResult result = parser->reader.Parse<kParseInsituFlag, InsituStringStream, VWReaderHandler<audit>>(ss, handler);
		if (!result.IsError())
			return;
