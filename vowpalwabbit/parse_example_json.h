...
inline std::unique_ptr<VW::json_example_parser> make_json_parser(VW::workspace& all);
...
public:
  ...
  VW::example_factory_i* _example_factory;
...
struct VWReaderHandler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, VWReaderHandler<audit>>
{
      ...
      VW::example_factory_i* example_factory, const std::unordered_map<uint64_t, example*>* dedup_examples = nullptr)
  {
    ...
    ctx._example_factory = example_factory;
...
template <bool audit>
inline void VW::json_example_parser::parse_line(char* line, size_t length,
    const std::unordered_map<uint64_t, example*>* dedup_examples, v_array<example*>& examples)
{
  ...
  handler.init(_label_parser, _hash_func, _hash_seed, _parse_mask, _chain_hash, &_parser_mem, _ldict, &examples, &ss,
      line + length, _example_factory.get(), dedup_examples);
...
inline void VW::json_example_parser::parse_object(
...
inline bool VW::json_example_parser::next(io_buf& input, v_array<example*>& output)
{
...
inline bool VW::dsjson_example_parser::next(io_buf& input, v_array<example*>& output)
{
  DecisionServiceInteraction interaction;
  return next_with_interaction(input, output, interaction);
}

inline bool VW::dsjson_example_parser::next_with_interaction(
...
inline void VW::details::persist_dsjson_metrics(
    VW::metric_sink& sink, VW::label_type_t label_type, const VW::details::dsjson_metrics& metrics)
{
  ...
  if (label_type == VW::label_type_t::ccb)
  {
    ...
    sink.set_uint("dsjson_number_label_equal_baseline_first_slot", metrics.DsjsonNumberOfLabelEqualBaselineFirstSlot);
    sink.set_uint(
        "dsjson_number_label_not_equal_baseline_first_slot", metrics.DsjsonNumberOfLabelNotEqualBaselineFirstSlot);
...
inline void VW::dsjson_example_parser::persist_metrics(VW::metric_sink& sink)
{
  if (!_should_record_metrics) { return; }

  VW::details::persist_dsjson_metrics(sink, _label_parser.label_type, _ds_metrics);
}

inline void reset_example_list_to_default(VW::example_factory_i& factory, v_array<example*>& examples)
{
    ...
    VW::details::dsjson_metrics* metrics, v_array<example*>& examples, DecisionServiceInteraction& interaction)
{
  ...
  if (!result)
  {
    reset_example_list_to_default(*_example_factory, examples);
...
inline void VW::dsjson_example_parser::parse_object(
