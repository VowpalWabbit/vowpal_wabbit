...
struct base_option {
  base_option(std::string name, size_t type_hash)
      : m_name(std::move(name)), m_type_hash(type_hash) {}
...
template <typename T> struct typed_option : base_option {
...
struct options_i {
  ...
  // Will throw if any options were supplied that do not having a matching
...
bool operator==(typed_option<T> &lhs, typed_option<T> &rhs) {
  return lhs.m_name == rhs.m_name && lhs.m_type_hash == rhs.m_type_hash &&
         ...
         lhs.m_keep == rhs.m_keep && lhs.default_value() == rhs.default_value();
}
