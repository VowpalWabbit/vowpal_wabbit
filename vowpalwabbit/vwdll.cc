...
// This interface now provides "wide" functions for compatibility with C programs build with gcc
// based on vwdll.cpp - .NET interop inteface with two changes:
// vwdll2.h and vwdll.cc codecvt is not supported by gcc - so all methods that use it are removed
// vwdll2.h - VW_CALLING_CONV and VW_DLL_MEMBER macrosses blacked out - they use Windows specific __stdcall and __declspec
//
