...
Search::search_task task = {"sequence", run,     initialize,
                            nullptr,    nullptr, nullptr};
}
namespace SequenceSpanTask {
Search::search_task task = {"sequencespan", run,   initialize,
                            finish,         setup, takedown};
}
namespace SequenceTaskCostToGo {
Search::search_task task = {"sequence_ctg", run,     initialize,
                            nullptr,        nullptr, nullptr};
}
namespace ArgmaxTask {
Search::search_task task = {"argmax", run,     initialize,
                            finish,   nullptr, nullptr};
}
namespace SequenceTask_DemoLDF {
Search::search_task task = {
    "sequence_demoldf", run, initialize, finish, nullptr, nullptr};
}
...
void initialize(Search::search &sch, size_t & /*num_actions*/,
                options_i & /*options*/) {
  sch.set_options(
      Search::AUTO_CONDITION_FEATURES | // automatically add history features to
                                        // our examples, please
      Search::AUTO_HAMMING_LOSS | // please just use hamming loss on individual
                                  // predictions -- we won't declare loss
...
//       m % 4 == 3    last-(m div 4)
//   thus, valid transitions are:
//     1     -> 1; 2, 6, 10, ...; 3, 7, 11, ...         out to { out, unit-Y,
//     begin-Y }       1
//     m%4=0 -> 1; 2, 6, 10, ..., 3, 7, 11, ...         unit-X to { out, unit-Y,
//     begin-Y }    2, 6, 10, 14, ...
//     m%4=1 -> m+1, m+2                                begin-X to { in-X,
//     last-X }           3, 7, 11, 15, ...
//     m%4=2 -> m, m+1                                  in-X to { in-X, last-X }
//     4, 8, 12, 16, ...
//     m%4=3 -> 1; 2, 6, 10, ...; 3, 7, 11, ...         last-X to { out, unit-Y,
//     begin-Y }    5, 9, 13, 17, ...
...
void initialize(Search::search &sch, size_t &num_actions, options_i &options) {
  ...
  new_options
      .add(make_option("search_span_bilou", search_span_bilou)
               .help("switch to (internal) BILOU encoding instead of BIO "
                     "encoding"))
      .add(make_option("search_span_multipass", D->multipass)
               .default_value(1)
               .help("do multiple passes"));
  options.add_and_parse(new_options);

  if (search_span_bilou) {
    std::cerr << "switching to BILOU encoding for sequence span labeling"
              << std::endl;
    D->encoding = BILOU;
  ...
  if (D->encoding == BIO) {
    ...
    D->allowed_actions.push_back(
        1); // push back an extra 1 that we can overwrite later if we want
  ...
  for (size_t pass = 1; pass <= D.multipass; pass++) {
    ...
    for (size_t i = 0; i < ec.size(); i++) {
      ...
      if (D.encoding == BIO) {
        ...
        if ((oracle > 1) && (oracle % 2 == 1) && (last_prediction != oracle) &&
            (last_prediction != oracle - 1))
          oracle = 1; // if we are supposed to I-X, but last wasn't B-X or I-X,
                      // then say O
      } else if (D.encoding == BILOU) {
            ...
            ((last_prediction - 2) % 4 == 3)) // O or unit-X or last-X
        {
          ...
          if ((oracle > 1) &&
              (((oracle - 2) % 4 == 2) || ((oracle - 2) % 4 == 3)))
            oracle = 1;
        } else // begin-X or in-X
        {
          action other = ((last_prediction - 2) % 4 == 1)
                             ? (last_prediction + 2)
                             : last_prediction;
      ...
      if (pass > 1)
        P.add_condition_range((ptag)(i + 1 + sch.get_history_length()),
                              sch.get_history_length() + 1, 'a');
      P.set_oracle(oracle);
      last_prediction = P.predict();

      if ((pass == D.multipass) && sch.output().good())
        sch.output() << ((D.encoding == BIO) ? last_prediction
                                             : bilou_to_bio(last_prediction))
                     << ' ';
    }
  }
}
...
void initialize(Search::search &sch, size_t &num_actions,
  ...
  sch.set_options(
      ...
      Search::AUTO_HAMMING_LOSS | // please just use hamming loss on individual
                                  // predictions -- we won't declare loss
      Search::EXAMPLES_DONT_CHANGE | // we don't do any internal example munging
      Search::ACTION_COSTS | // we'll provide cost-per-action (rather than
                             // oracle)
      0);
...
void initialize(Search::search &sch, size_t & /*num_actions*/,
  ...
  new_options
      .add(make_option("cost", D->false_negative_cost)
               .default_value(10.0f)
               .help("False Negative Cost"))
      .add(make_option("negative_weight", D->negative_weight)
               .default_value(1.f)
               .help("Relative weight of negative examples"))
      .add(make_option("max", D->predict_max)
               .help("Disable structure: just predict the max"));
  options.add_and_parse(new_options);

  sch.set_task_data(D);

  if (D->predict_max)
    sch.set_options(Search::EXAMPLES_DONT_CHANGE); // we don't do any internal
                                                   // example munging
  else
    sch.set_options(Search::AUTO_CONDITION_FEATURES | // automatically add
                                                      // history features to our
                                                      // examples, please
                    Search::EXAMPLES_DONT_CHANGE); // we don't do any internal
                                                   // example munging
}
...
void run(Search::search &sch, multi_ex &ec) {
  ...
  for (size_t i = 0; i < ec.size(); i++)
    max_label = std::max(ec[i]->l.multi.label, max_label);
...
void my_update_example_indicies(Search::search &sch, bool /* audit */,
                                example *ec, uint64_t mult_amount,
...
void run(Search::search &sch, multi_ex &ec) {
  ...
  for (ptag i = 0; i < ec.size(); i++) {
    for (uint32_t a = 0; a < data->num_actions; a++) {
      if (sch.predictNeedsExample()) // we can skip this work if `predict` won't
                                     // actually use the example data
      {
        VW::copy_example_data(false, &data->ldf_examples[a],
                              ec[i]); // copy but leave label alone!
        // now, offset it appropriately for the action id
        my_update_example_indicies(sch, true, &data->ldf_examples[a], 28904713,
                                   4832917 * (uint64_t)a);
      }

      // regardless of whether the example is needed or not, the class info is
      // needed
      CS::label &lab = data->ldf_examples[a].l.cs;
      // need to tell search what the action id is, so that it can add history
      // features correctly!
      lab.costs[0].x = 0.;
    ...
    action pred_id = P.set_tag((ptag)(i + 1))
                         ...
                         .predict();
    action prediction =
        pred_id + 1; // or ldf_examples[pred_id]->ld.costs[0].weight_index
