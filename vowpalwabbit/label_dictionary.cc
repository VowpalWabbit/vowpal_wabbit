...
void del_example_namespace(example &ec, namespace_index ns, features &fs) {
  ...
  // so shared features are included in the reported number of "current
  // features"
  // ec.num_features -= numf;
  features &del_target = ec.feature_space[(size_t)ns];
  assert(del_target.size() >= fs.size());
  assert(ec.indices.size() > 0);
  if (ec.indices.last() == ns &&
      ec.feature_space[(size_t)ns].size() == fs.size())
    ec.indices.pop();
...
void del_example_namespaces_from_example(example &target, example &source) {
  if (source.indices.size() ==
      0) // making sure we can deal with empty shared example
    return;
...
void add_example_namespace_from_memory(label_feature_map &lfm, example &ec,
                                       size_t lab) {
...
void del_example_namespace_from_memory(label_feature_map &lfm, example &ec,
                                       size_t lab) {
  size_t lab_hash = hash_lab(lab);
  features &res = lfm.get(lab, lab_hash);
  if (res.size() == 0)
