...
VW::LEARNER::base_learner* scorer_setup(options_i& options, vw& all)
{
  ...
  else if (link == "poisson")
  {
    l = &init_learner(s, base, predict_or_learn<true, expf>, predict_or_learn<false, expf>,
        all.get_setup_name(scorer_setup) + "-poisson");
