...
bool dis_test(vw &all, example &ec, single_learner &base,
              float /* prediction */, float threshold) {
...
float get_threshold(float sum_loss, float t, float c0, float alpha) {
  if (t < 3.f) {
    ...
    float threshold =
        std::sqrt(c0 * avg_loss / t) + fmax(2.f * alpha, 4.f) * c0 * log(t) / t;
    return threshold;
  }
}
...
float query_decision(active_cover &a, single_learner &l, example &ec,
                     float prediction, float pmin, bool in_dis) {
  ...
  for (size_t i = 0; i < a.cover_size; i++) {
    l.predict(ec, i + 1);
    q2 += ((float)(sign(ec.pred.scalar) != sign(prediction))) *
...
void predict_or_learn_active_cover(active_cover &a, single_learner &base,
                                   example &ec) {
  base.predict(ec, 0);
  ...
  if (is_learn) {
    ...
    float threshold =
        get_threshold((float)all.sd->sum_loss, t, a.active_c0, a.alpha);
    ...
    if (in_dis) {
      cost = r * (fmax(importance, 0.f)) *
             ((float)(sign(prediction) != sign(ec_input_label)));
    } else {
      cost = 0.f;
      cost_delta = -r;
    }
    ...
    for (size_t i = 0; i < a.cover_size; i++) {
      ...
      a.lambda_n[i] += 2.f *
                       ((float)(sign(ec.pred.scalar) != sign(prediction))) *
                       cost_delta;
      a.lambda_n[i] = fmax(a.lambda_n[i], 0.f);

      // Update denominator of lambda
      a.lambda_d[i] +=
          ((float)(sign(ec.pred.scalar) != sign(prediction) && in_dis)) /
          (float)pow(q2, 1.5);
...
base_learner *active_cover_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("active_cover", active_cover_option)
               .keep()
      ...
      .add(make_option("oracular", data->oracular)
  ...
  // Create new learner
  learner<active_cover, example> &l =
      init_learner(data, base, predict_or_learn_active_cover<true>,
                   predict_or_learn_active_cover<false>, data->cover_size + 1);
