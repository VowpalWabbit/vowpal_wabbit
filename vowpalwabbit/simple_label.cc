...
char *bufread_simple_label(shared_data *sd, label_data *ld, char *c) {
  ...
  //  std::cout << ld->label << " " << sd->is_more_than_two_labels_observed << "
  //  " << sd->first_observed_label << std::endl;
  c += sizeof(ld->label);
...
void cache_simple_label(void *v, io_buf &cache) {
  ...
  label_data *ld = (label_data *)v;
  cache.buf_write(c,
                  sizeof(ld->label) + sizeof(ld->weight) + sizeof(ld->initial));
...
void parse_simple_label(parser *, shared_data *sd, void *v,
...
label_parser simple_label = {default_simple_label,
                             parse_simple_label,
                             cache_simple_label,
...
void print_update(vw &all, example &ec) {
  if (all.sd->weighted_labeled_examples + all.sd->weighted_unlabeled_examples >=
          all.sd->dump_interval &&
      !all.quiet && !all.bfgs) {
    all.sd->print_update(all.holdout_set_off, all.current_pass,
                         ec.l.simple.label, ec.pred.scalar, ec.num_features,
...
bool summarize_holdout_set(vw &all, size_t &no_win_counter) {
  float thisLoss =
      (all.sd->weighted_holdout_examples_since_last_pass > 0)
          ? (float)(all.sd->holdout_sum_loss_since_last_pass /
  ...
  if ((thisLoss != FLT_MAX) ||
      (std::isfinite(all.sd->holdout_best_loss))) // it's only a loss if we're
                                                  // not infinite when the
                                                  // previous one wasn't
                                                  // infinite
    no_win_counter++;
  return false;
