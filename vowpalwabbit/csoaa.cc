...
inline void inner_loop(single_learner &base, example &ec, uint32_t i,
                       float cost, uint32_t &prediction, float &score,
  ...
  if (ec.partial_prediction < score ||
      (ec.partial_prediction == score && i < prediction)) {
...
void predict_or_learn(csoaa &c, single_learner &base, example &ec) {
  ...
  if (ld.costs.size() > 0) {
    for (auto &cl : ld.costs)
      inner_loop<is_learn>(base, ec, cl.class_index, cl.x, prediction, score,
                           cl.partial_prediction);
...
base_learner *csoaa_setup(options_i &options, vw &all) {
  ...
  new_options.add(make_option("csoaa", c->num_classes)
                      .keep()
  ...
  learner<csoaa, example> &l = init_learner(
      c, as_singleline(setup_base(*all.options, all)), predict_or_learn<true>,
...
bool ec_is_label_definition(
    example &ec) // label defs look like "0:___" or just "label:___"
{
...
inline bool cmp_wclass_ptr(const COST_SENSITIVE::wclass *a,
                           const COST_SENSITIVE::wclass *b) {
  return a->x < b->x;
}
...
void compute_wap_values(vector<COST_SENSITIVE::wclass *> costs) {
  ...
  for (size_t i = 1; i < costs.size(); i++)
    costs[i]->wap_value =
        costs[i - 1]->wap_value + (costs[i]->x - costs[i - 1]->x) / (float)i;
}

// Substract a given feature from example ec.
// Rather than finding the corresponding namespace and feature in ec,
// add a new feature with opposite value (but same index) to ec to a special
// wap_ldf_namespace.
// This is faster and allows fast undo in unsubtract_example().
void subtract_feature(example &ec, float feature_value_x,
                      uint64_t weight_index) {
  ec.feature_space[wap_ldf_namespace].push_back(-feature_value_x, weight_index);
}

// Iterate over all features of ecsub including quadratic and cubic features and
// subtract them from ec.
...
void unsubtract_example(example *ec) {
  if (ec->indices.size() == 0) {
    cerr << "internal error (bug): trying to unsubtract_example, but there are "
            "no namespaces!"
         << endl;
    return;
  }
  ...
  if (ec->indices.last() != wap_ldf_namespace) {
    cerr << "internal error (bug): trying to unsubtract_example, but either it "
            "wasn't added, or something was added "
...
void make_single_prediction(ldf &data, single_learner &base, example &ec) {
  ...
  LabelDict::add_example_namespace_from_memory(data.label_features, ec,
                                               ld.costs[0].class_index);
  ...
  LabelDict::del_example_namespace_from_memory(data.label_features, ec,
                                               ld.costs[0].class_index);
  ec.l.cs = ld;
}
...
void do_actual_learning_wap(ldf &data, single_learner &base, multi_ex &ec_seq) {
  ...
  for (const auto &example : ec_seq)
    all_costs.push_back(&example->l.cs.costs[0]);
  ...
  for (size_t k1 = 0; k1 < K; k1++) {
    ...
    LabelDict::add_example_namespace_from_memory(data.label_features, *ec1,
                                                 costs1[0].class_index);
    ...
    for (size_t k2 = k1 + 1; k2 < K; k2++) {
      ...
      LabelDict::add_example_namespace_from_memory(data.label_features, *ec2,
                                                   costs2[0].class_index);
      ...
      LabelDict::del_example_namespace_from_memory(data.label_features, *ec2,
                                                   costs2[0].class_index);
    }
    LabelDict::del_example_namespace_from_memory(data.label_features, *ec1,
                                                 costs1[0].class_index);
...
void do_actual_learning_oaa(ldf &data, single_learner &base, multi_ex &ec_seq) {
  ...
  for (const auto &ec : ec_seq) {
    ...
    // restore original cost-sensitive label, sum of importance weights and
    // partial_prediction
...
void do_actual_learning(ldf &data, single_learner &base, multi_ex &ec_seq_all) {
  ...
  if (data.rank) {
    ...
    qsort((void *)data.a_s.begin(), data.a_s.size(), sizeof(action_score),
          score_comp);
  } else {
  ...
  if (data.is_probabilities) {
    ...
    for (const auto &example : ec_seq) {
      ...
      // for better classes,
      // but partial_prediction is lower for better classes (we are predicting
      // the cost),
...
void output_example(vw &all, example &ec, bool &hit_loss, multi_ex *ec_seq,
  ...
  if (data.is_probabilities) {
    ...
    // but we cannot store it in ec.pred union because we store ec.pred.prob
    // there.
  ...
  for (int sink : all.final_prediction_sink)
    all.print(sink,
              data.is_probabilities ? ec.pred.prob : (float)ec.pred.multiclass,
              0, ec.tag);
  ...
  COST_SENSITIVE::print_update(all, COST_SENSITIVE::cs_label.test_label(&ec.l),
                               ec, ec_seq, false, predicted_class);
}

void output_rank_example(vw &all, example &head_ec, bool &hit_loss,
                         multi_ex *ec_seq) {
  ...
  COST_SENSITIVE::print_update(all,
                               COST_SENSITIVE::cs_label.test_label(&head_ec.l),
                               head_ec, ec_seq, true, 0);
}
...
void output_example_seq(vw &all, ldf &data, multi_ex &ec_seq) {
  ...
  if ((K > 0) && !ec_seq_is_label_definition(ec_seq)) {
    ...
    if (data.is_probabilities) {
      ...
      // Based on parse_args.cc (where "average multiclass log loss") is
      // printed,
...
base_learner *csldf_setup(options_i &options, vw &all) {
  ...
  csldf_outer_options.add(
      make_option("csoaa_ldf", csoaa_ldf).keep().help("Use one-against-all "
                                                      "multiclass learning "
                                                      "with label dependent "
                                                      "features."));
  csldf_outer_options.add(
      make_option("ldf_override", ldf_override).help("Override singleline or "
                                                     "multiline from csoaa_ldf "
                                                     "or wap_ldf, eg if stored "
                                                     "in file"));
  csldf_outer_options.add(make_option("csoaa_rank", ld->rank)
                              .keep()
                              .help("Return actions sorted by score order"));
  csldf_outer_options.add(make_option("probabilities", ld->is_probabilities)
                              .keep()
  ...
  csldf_inner_options.add(make_option("wap_ldf", wap_ldf)
                              .keep()
                              .help("Use weighted all-pairs multiclass "
  ...
  if (ldf_arg.compare("multiline") == 0 || ldf_arg.compare("m") == 0)
    ld->treat_as_classifier = false;
  else if (ldf_arg.compare("multiline-classifier") == 0 ||
           ldf_arg.compare("mc") == 0)
    ...
    if ((ldf_arg.compare("singleline") == 0 || ldf_arg.compare("s") == 0) ||
        (ldf_arg.compare("singleline-classifier") == 0 ||
         ldf_arg.compare("sc") == 0))
      THROW("ldf requires either m/multiline or mc/multiline-classifier.  "
            "s/sc/singleline/singleline-classifier is no "
            "longer supported");
  }
  ...
  if (ld->is_probabilities) {
    ...
    if (loss_function_type != "logistic")
      all.trace_message << "WARNING: --probabilities should be used only with "
  ...
  all.p->emptylines_separate_examples =
  ...
  learner<ldf, multi_ex> &l = init_learner(
      ld, as_singleline(setup_base(*all.options, all)),
