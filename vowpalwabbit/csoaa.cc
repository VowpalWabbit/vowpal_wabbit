...
base_learner* csldf_setup(options_i& options, vw& all)
{
  ...
  csldf_outer_options.add(make_typed_option("csoaa_rank", ld->rank).help("Return actions sorted by score order"));
  csldf_outer_options.add(make_typed_option("probabilities", ld->is_probabilities).help("predict probabilites of all classes"));
  ...
  learner<ldf,multi_ex>& l = init_learner(ld, as_singleline(setup_base(*all.options, all)), do_actual_learning<true>, do_actual_learning<false>, 1, pred_type);
