...
VW::LEARNER::base_learner* baseline_challenger_cb_setup(VW::setup_base_i& stack_builder)
{
  ...
  option_group_definition new_options(
      "[Reduction] Baseline challenger");
  new_options.add(make_option("baseline_challenger_cb", is_enabled).necessary().keep().help("Build a CI around the baseline action and use it instead of the model if it's "
      "perfoming better"))
