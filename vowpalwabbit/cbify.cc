...
float loss_cs(cbify &data, v_array<COST_SENSITIVE::wclass> &costs,
...
void predict_or_learn(cbify &data, single_learner &base, example &ec) {
  ...
  if (sample_after_normalizing(data.app_seed + data.example_counter++,
                               begin_scores(ec.pred.a_s),
                               end_scores(ec.pred.a_s), chosen_action))
...
void do_actual_learning_ldf(cbify &data, multi_learner &base,
                            multi_ex &ec_seq) {
...
base_learner *cbify_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("cbify", num_actions).keep().help("Convert multiclass "
                                                         "on <k> classes into "
                                                         "a contextual bandit "
                                                         "problem"))
      .add(make_option("cbify_cs", use_cs).help("consume cost-sensitive "
  ...
  if (data->use_adf) {
    ...
    if (use_cs)
      l = &init_cost_sensitive_learner(
          ...
          predict_or_learn_adf<false, true>, all.p, 1);
    else
      l = &init_multiclass_learner(
          data, base, predict_or_learn_adf<true, false>,
          predict_or_learn_adf<false, false>, all.p, 1);
  } else {
...
base_learner *cbifyldf_setup(options_i &options, vw &all) {
  ...
  learner<cbify, multi_ex> &l = init_learner(
      data, base, do_actual_learning_ldf<true>, do_actual_learning_ldf<false>,
      1, prediction_type::multiclass);
