  ...
  if (data->use_adf)
  {
    ...
    if (use_cs)
      l = &init_cost_sensitive_learner(
          data, base, learn_adf<true>, predict_adf<true>, all.example_parser, 1, all.get_setupfn_name(cbify_setup) + "-adf-cs");
    else
      l = &init_multiclass_learner(
          data, base, learn_adf<false>, predict_adf<false>, all.example_parser, 1, all.get_setupfn_name(cbify_setup) + "-adf");
  }
  else
  {
    ...
    if (use_reg)
    {
      ...
      if (use_discrete)
      {
        l = &init_learner(data, base, predict_or_learn_regression_discrete<true>,
            predict_or_learn_regression_discrete<false>, 1, prediction_type_t::scalar, all.get_setupfn_name(cbify_setup) + "-reg-discrete", true);
...
base_learner* cbifyldf_setup(options_i& options, vw& all)
{
  ...
  learner<cbify, multi_ex>& l = init_learner(
      data, base, do_actual_learning_ldf, do_actual_predict_ldf, 1, prediction_type_t::multiclass, all.get_setupfn_name(cbifyldf_setup));
