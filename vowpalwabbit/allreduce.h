...
public:
  ...
  AllReduce(size_t ptotal, const size_t pnode, bool pquiet = false)
      : total(ptotal), node(pnode), quiet(pquiet) {
    assert(node < total);
  }
...
public:
  AllReduceThreads(AllReduceThreads *root, const size_t ptotal,
                   const size_t pnode, bool quiet = false);
  ...
  template <class T, void (*f)(T &, const T &)>
  void all_reduce(T *buffer, const size_t n) { // register buffer
    ...
    if (blockSize == 0) {
      ...
      } else { // more threads than bytes --> don't do any work
        index = end = 0;
      }
    } else {
    ...
    for (; index < end;
         index++) { // Perform transposed AllReduce to help data locallity
...
private:
  ...
  template <class T>
  void pass_up(char *buffer, size_t left_read_pos, size_t right_read_pos,
               size_t &parent_sent_pos) {
    size_t my_bufsize =
        std::min(ar_buf_size, std::min(left_read_pos, right_read_pos) /
                                      sizeof(T) * sizeof(T) -
                                  parent_sent_pos);

    if (my_bufsize > 0) { // going to pass up this chunk of data to the parent
      int write_size =
          send(socks.parent, buffer + parent_sent_pos, (int)my_bufsize, 0);
      if (write_size < 0)
        THROW("Write to parent failed "
              << my_bufsize << " " << write_size << " " << parent_sent_pos
  ...
  void reduce(char *buffer, const size_t n) {
    ...
    size_t child_read_pos[2] = {
        0, 0}; // First unread float from left and right children
    ...
    while (parent_sent_pos < n || child_read_pos[0] < n ||
           child_read_pos[1] < n) {
      if (socks.parent != -1)
        pass_up<T>(buffer, child_read_pos[0], child_read_pos[1],
                   parent_sent_pos);

      if (parent_sent_pos >= n && child_read_pos[0] >= n &&
          child_read_pos[1] >= n)
        break;

      if (child_read_pos[0] < n || child_read_pos[1] < n) {
        if (max_fd > 0 &&
            select((int)max_fd, &fds, nullptr, nullptr, nullptr) == -1)
          THROWERRNO("select");

        for (int i = 0; i < 2; i++) {
          if (socks.children[i] != -1 &&
              FD_ISSET(socks.children[i],
                       &fds)) { // there is data to be left from left child
            if (child_read_pos[i] == n)
              THROW("I think child has no data to send but he thinks he has "
                    << FD_ISSET(socks.children[0], &fds) << " "
                    << FD_ISSET(socks.children[1], &fds));

            size_t count = std::min(ar_buf_size, n - child_read_pos[i]);
            int read_size =
                recv(socks.children[i],
                     &child_read_buf[i][child_unprocessed[i]], (int)count, 0);
            if (read_size == -1)
              THROWERRNO("recv from child");

            addbufs<T, f>((T *)buffer + child_read_pos[i] / sizeof(T),
                          (T *)child_read_buf[i],
                          (child_read_pos[i] + read_size) / sizeof(T) -
                              child_read_pos[i] / sizeof(T));
            ...
            for (int j = 0; j < child_unprocessed[i]; j++) {
              child_read_buf[i][j] =
                  child_read_buf[i][((old_unprocessed + read_size) /
                                     (int)sizeof(T)) *
                                        sizeof(T) +
                                    j];
            }
      ...
      if (socks.parent == -1 && child_read_pos[0] == n &&
          child_read_pos[1] == n)
        parent_sent_pos = n;
    }
  }

  void pass_down(char *buffer, const size_t parent_read_pos,
...
public:
  AllReduceSockets(std::string pspan_server, const int pport,
                   const size_t punique_id, size_t ptotal, const size_t pnode,
                   bool pquiet)
      : AllReduce(ptotal, pnode, pquiet), span_server(pspan_server),
        port(pport), unique_id(punique_id) {}
