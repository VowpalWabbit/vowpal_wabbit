...
po::typed_value<std::vector<bool>> *options_boost_po::convert_to_boost_value(
  ...
  if (opt->default_value_supplied()) {
    THROW("Using a bool option type acts as a switch, no explicit default "
          "value is allowed.")
  }
...
void options_boost_po::add_to_description(
  ...
  add_to_description_impl<supported_options_types>(std::move(opt),
                                                   options_description);
}
...
void options_boost_po::add_and_parse(const option_group_definition &group) {
  ...
  for (auto opt_ptr : group.m_options) {
    ...
    // The last definition is kept. There was a bug where using .insert at a
    // later pointer changed the command line but
    // the previously defined option's default value was serialized into the
    // model. This resolves that state info.
  ...
  try {
    ...
    for (auto const &option : parsed_options.options) {
      // If the supplied option is interpreted as a number, then ignore it.
      // There are no options like this and it is just a false positive.
      ...
      // If a std::string is later determined to be a value the erase it. This
      // happens for negative numbers "-2"
      for (auto &val : option.value) {
        m_ignore_supplied.insert(val);
      }

      // Parsed options can contain short options in the form -k, we can only
      // check these as the group definitions come
      // in.
    ...
    THROW_EX(VW::vw_argument_invalid_value_exception, ex.what());
      ...
      boost::exception_detail::clone_impl<
          boost::exception_detail::error_info_injector<boost::bad_lexical_cast>>
          &ex) {
    THROW_EX(VW::vw_argument_invalid_value_exception, ex.what());
  } catch (boost::exception_detail::clone_impl<
           boost::exception_detail::error_info_injector<
...
bool options_boost_po::was_supplied(const std::string &key) {
  ...
  auto it = std::find(m_command_line.begin(), m_command_line.end(),
                      std::string("--" + key));
...
std::vector<std::shared_ptr<base_option>> options_boost_po::get_all_options() {
  ...
  std::transform(
      m_options.begin(), m_options.end(), std::back_inserter(output_values),
      [](std::pair<const std::string, std::shared_ptr<base_option>> &kv) {
        return kv.second;
      });
...
std::shared_ptr<base_option>
VW::config::options_boost_po::get_option(const std::string &key) {
...
void options_boost_po::check_unregistered() {
  for (auto const &supplied : m_supplied_options) {
    if (m_defined_options.count(supplied) == 0 &&
        m_ignore_supplied.count(supplied) == 0) {
      THROW_EX(VW::vw_unrecognised_option_exception, "unrecognised option '--"
                                                         << supplied << "'");
    }
  }
}
