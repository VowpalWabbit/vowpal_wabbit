...
po::typed_value<std::vector<bool>> *options_boost_po::convert_to_boost_value(std::shared_ptr<typed_option<bool>> &opt)
{
  ...
  {
    THROW(
        "Using a bool option type acts as a switch, no explicit default "
        "value is allowed.")
  }
...
void options_boost_po::add_and_parse(const option_group_definition &group)
{
  ...
  for (auto opt_ptr : group.m_options)
  {
    ...
    // later pointer changed the command line but the previously defined
    // option's default value was serialized into the model. This resolves that
    // state info.
  ...
  try
  {
    ...
    for (auto const &option : parsed_options.options)
    {
      ...
      // check these as the group definitions come in.
