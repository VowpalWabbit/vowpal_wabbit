...
template <class T> struct beam_element {
  ...
  //  then recomb_friends is everything that's
  //  equivalent to us but worse... NOT USED if we're not doing k-best
  //  predictions
};
...
private:
  ...
  float worst_cost; // what is the cost of the worst (highest cost) item in the
                    // beam
  float
      best_cost; // what is the cost of the best (lowest cost) item in the beam
...
public:
  beam(size_t beam_size, float prune_coeff = FLT_MAX,
       bool (*test_equiv)(T *, T *) = nullptr, bool kbest = false)
      : beam_size(beam_size), pruning_coefficient(prune_coeff), do_kbest(kbest),
  ...
  inline bool might_insert(float cost) {
    return (cost <= prune_if_gt) &&
           ((count < beam_size) || (cost < worst_cost));
  }
              ...
              uint32_t hash) // returns TRUE iff element was actually added
  {
    ...
    if (beam_size <
        BEAM_CONSTANT_SIZE) { // find the worst item and directly replace it
      size_t worst_idx = 0;
      ...
      for (size_t i = start; i < end; i++) {
        ...
        for (size_t j = i + 1; j < end; j++) {
          ...
          if (is_equivalent(A[i].data, A[j].data)) {
            ...
            // std::cerr << "equivalent " << i << "," << j << ": " <<
            // ((size_t)A[i].data) << " and " << ((size_t)A[j].data)
            // << std::endl;
          }
        }
      }
  ...
  void compact(void (*free_data)(T *) = nullptr) {
    ...
    if (is_equivalent) // we might be able to get rid of even more
      while ((count > 1) && !A[count - 1].active)
        count--;
