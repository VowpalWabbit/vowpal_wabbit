...
template <bool is_learn>
void cb_explore_adf_greedy::predict_or_learn_impl(LEARNER::multi_learner &base,
                                                  multi_ex &examples) {
  // Explore uniform random an epsilon fraction of the time.
  LEARNER::multiline_learn_or_predict<is_learn>(base, examples,
  ...
  if (!_first_only) {
    for (size_t i = 0; i < tied_actions; ++i)
      preds[i].score += (1.f - _epsilon) / tied_actions;
  } else
...
LEARNER::base_learner *setup(VW::config::options_i &options, vw &all) {
  ...
  // NOTE: epsilon-greedy is the default explore type. This basically runs if
  // none of the other explore strategies are
  // used
  bool use_greedy =
      !(options.was_supplied("first") || options.was_supplied("bag") ||
        options.was_supplied("cover") || options.was_supplied("regcb") ||
        options.was_supplied("regcbopt") || options.was_supplied("softmax"));
