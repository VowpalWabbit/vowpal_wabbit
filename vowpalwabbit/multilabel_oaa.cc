...
void predict_or_learn(multi_oaa &o, LEARNER::single_learner &base,
                      example &ec) {
  ...
  if (is_learn && multilabel_index < multilabels.label_v.size())
    cout << "label " << multilabels.label_v[multilabel_index]
         << " is not in {0," << o.k - 1 << "} This won't work right." << endl;
...
LEARNER::base_learner *multilabel_oaa_setup(options_i &options, vw &all) {
  ...
  new_options.add(make_option("multilabel_oaa", data->k)
                      .keep()
  ...
  LEARNER::learner<multi_oaa, example> &l = LEARNER::init_learner(
