...
void predict_or_learn_cover(cb_explore &data, single_learner &base,
  ...
  float min_prob =
      min(1.f / num_actions, 1.f / (float)sqrt(counter * num_actions));
  ...
  if (is_learn) {
    ...
    for (size_t i = 0; i < cover_size; i++) {
      ...
      for (uint32_t j = 0; j < num_actions; j++) {
        float pseudo_cost =
            data.cs_label.costs[j].x -
            data.psi * min_prob / (max(probabilities[j], min_prob) / norm) + 1;
      ...
      if (probabilities[predictions[i] - 1] < min_prob)
        norm += max(0, additive_probability -
...
void output_example(vw &all, cb_explore &data, example &ec, CB::label &ld) {
  ...
  if ((c.known_cost = get_observed_cost(ld)) != nullptr)
    for (uint32_t i = 0; i < ec.pred.a_s.size(); i++)
      loss += get_unbiased_cost(c.known_cost, c.pred_scores, i + 1) *
              ec.pred.a_s[i].score;
