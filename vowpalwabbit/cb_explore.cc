...
// All exploration algorithms return a vector of probabilities, to be used by
// GenericExplorer downstream
...
void predict_or_learn_first(cb_explore &data, single_learner &base,
                            example &ec) {
...
template <bool is_learn>
void predict_or_learn_greedy(cb_explore &data, single_learner &base,
                             example &ec) {
  // Explore uniform random an epsilon fraction of the time.
  // TODO: pointers are copied here. What happens if base.learn/base.predict
  // re-allocs?
  ...
  for (uint32_t i = 0; i < data.cbcs.num_actions; i++)
    probs.push_back({i, 0});
  generate_epsilon_greedy(data.epsilon, ec.pred.multiclass - 1,
...
void get_cover_probabilities(cb_explore &data, single_learner & /* base */,
                             example &ec, v_array<action_score> &probs) {
  ...
  float min_prob = std::min(1.f / num_actions,
                            1.f / (float)std::sqrt(data.counter * num_actions));

  enforce_minimum_probability(min_prob * num_actions, false,
                              begin_scores(probs), end_scores(probs));
...
template <bool is_learn>
void predict_or_learn_cover(cb_explore &data, single_learner &base,
                            example &ec) {
  // Randomize over predictions from a base set of predictors
  ...
  float min_prob = std::min(1.f / num_actions,
                            1.f / (float)std::sqrt(counter * num_actions));
  ...
  if (is_learn) {
    ...
    for (size_t i = 0; i < cover_size; i++) {
      ...
      for (uint32_t j = 0; j < num_actions; j++) {
        float pseudo_cost = data.cs_label.costs[j].x -
                            data.psi * min_prob /
                                (std::max(probabilities[j], min_prob) / norm) +
                            1;
      ...
      if (probabilities[predictions[i] - 1] < min_prob)
        norm +=
            std::max(0.f, additive_probability -
                              (min_prob - probabilities[predictions[i] - 1]));
      else
...
void print_update_cb_explore(vw &all, bool is_test, example &ec,
  ...
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.quiet &&
    ...
    all.sd->print_update(all.holdout_set_off, all.current_pass,
                         label_string.str(), pred_string.str(), ec.num_features,
...
void output_example(vw &all, cb_explore &data, example &ec, CB::label &ld) {
  ...
  if ((c.known_cost = get_observed_cost(ld)) != nullptr)
    for (uint32_t i = 0; i < ec.pred.a_s.size(); i++)
      loss += get_cost_estimate(c.known_cost, c.pred_scores, i + 1) *
              ec.pred.a_s[i].score;

  all.sd->update(ec.test_only, get_observed_cost(ld) != nullptr, loss, 1.f,
                 ec.num_features);
...
base_learner *cb_explore_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("cb_explore", data->cbcs.num_actions)
               .keep()
               .help("Online explore-exploit for a <k> action contextual "
                     "bandit problem"))
  ...
  if (options.was_supplied("cover")) {
    data->cs =
        (learner<cb_explore, example> *)(as_singleline(all.cost_sensitive));
    data->second_cs_label.costs.resize(num_actions);
    data->second_cs_label.costs.end() =
        data->second_cs_label.costs.begin() + num_actions;
