...
float safe_probability(float prob) {
  if (prob <= 0.) {
    std::cout << "Probability " << prob
              << " is not possible, replacing with 1e-3.  Fix your dataset. "
              << std::endl;
    return 1e-3f;
  } else
    return prob;
  ...
  for (uint32_t i = 0; i < examples.size(); i++) {
    ...
    if (ld.costs.size() == 1 && ld.costs[0].cost != FLT_MAX)
      wc.x = ld.costs[0].cost /
             safe_probability(std::max(ld.costs[0].probability, clip_p));
...
void gen_cs_example_ips(cb_to_cs &c, CB::label &ld,
                        COST_SENSITIVE::label &cs_ld, float clip_p) {
  // this implements the inverse propensity score method, where cost are
  // importance weighted by the probability of the
    ...
    for (uint32_t i = 1; i <= c.num_actions; i++) {
      ...
      if (c.known_cost != nullptr && i == c.known_cost->action) {
        ...
        wc.x = c.known_cost->cost /
               safe_probability(std::max(c.known_cost->probability, clip_p));

        // ips can be thought as the doubly robust method with a fixed regressor
        // that predicts 0 costs for everything
        // update the loss of this regressor
        c.nb_ex_regressors++;
        c.avg_loss_regressors += (1.0f / c.nb_ex_regressors) *
                                 ((c.known_cost->cost) * (c.known_cost->cost) -
...
void gen_cs_example_sm(multi_ex &, uint32_t chosen_action, float sign_offset,
  ...
  for (uint32_t i = 0; i < action_vals.size(); i++) {
    ...
    // TODO: This clipping is conceptually unnecessary because the example
    // weight for this instance should be close to
    // 0.
    if (wc.x > 100.)
      wc.x = 100.0;
    if (wc.x < -100.)
