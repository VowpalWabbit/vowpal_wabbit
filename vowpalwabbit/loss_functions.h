...
class loss_function {
public:
  // Identifies the type of the implementing loss function, matches the name
  // used in getLossFunction.
   ...
   */
  virtual float getUpdate(float prediction, float label, float update_scale,
  ...
  //      * getUpdate(prediction, opposite,
  //      pred_per_update*getRevertingWeight(), pred_per_update)
  virtual float getRevertingWeight(shared_data *, float prediction,
                                   float eta_t) = 0;
  virtual float getSquareGrad(float prediction, float label) = 0;
  virtual float first_derivative(shared_data *, float prediction,
                                 float label) = 0;
  virtual float second_derivative(shared_data *, float prediction,
                                  float label) = 0;
  virtual ~loss_function(){};
};

loss_function *getLossFunction(vw &, std::string funcName,
