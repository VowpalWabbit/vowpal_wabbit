...
struct task_data {
  ...
  size_t numN; // number of neighbor predictions (equals K+1 for undirected, or
               // 2*(K+1) for directed)
  bool use_structure;
  ...
  std::vector<std::vector<size_t>>
      adj; // adj[n] is a vector of *edge example ids* that contain n
  ...
  float *neighbor_predictions; // prediction on this neighbor for
                               // add_edge_features_fn
...
void initialize(Search::search &sch, size_t &num_actions, options_i &options) {
  ...
  new_options
      .add(make_option("search_graph_num_loops", D->num_loops)
               .default_value(2)
               .help("how many loops to run [def: 2]"))
      .add(make_option("search_graph_no_structure", D->use_structure)
               .help("turn off edge features"))
...
void setup(Search::search &sch, multi_ex &ec) {
  ...
  if ((D.N == 0) && (D.E > 0))
    THROW("error: got edges without any nodes (perhaps ring_size is too "
          "small?)!");
  ...
  for (size_t i = D.N; i < ec.size(); i++) {
    for (size_t n = 0; n < ec[i]->l.cs.costs.size(); n++) {
      if (ec[i]->l.cs.costs[n].class_index > D.N)
        THROW("error: edge source points to too large of a node id: "
              << (ec[i]->l.cs.costs[n].class_index) << " > " << D.N);
    }
    for (size_t n = 0; n < ec[i]->l.cs.costs.size(); n++) {
      size_t nn = ec[i]->l.cs.costs[n].class_index;
      if ((nn > 0) && (((D.adj[nn - 1].size() == 0) ||
                        (D.adj[nn - 1][D.adj[nn - 1].size() - 1] !=
                         i)))) // don't allow dups
...
void add_edge_features_single_fn(task_data &D, float fv, uint64_t fx) {
  ...
  fs.push_back(fv, (uint32_t)((fx2 + 348919043 * k) * D.multiplier) &
                       (uint64_t)D.mask);
}

void add_edge_features(Search::search &sch, task_data &D, size_t n,
                       multi_ex &ec) {
  D.cur_node = ec[n];

  for (size_t i : D.adj[n]) {
    for (size_t k = 0; k < D.numN; k++)
      D.neighbor_predictions[k] = 0.;
    ...
    if (D.use_structure) {
      ...
      for (size_t j = 0; j < ec[i]->l.cs.costs.size(); j++) {
        ...
        if (m == n)
          continue;
        size_t other_side =
            (D.directed && (n_in_sink != m_in_sink)) ? (D.K + 1) : 0;
    ...
    if (pred_total == 0.)
      continue;
    // std::cerr << n << ':' << i << " -> ["; for (size_t k=0; k<D.numN; k++)
    // std::cerr << ' ' << D.neighbor_predictions[k]; std::cerr
    // << " ]" << std::endl;
    for (size_t k = 0; k < D.numN; k++)
      D.neighbor_predictions[k] /= pred_total;
    example &edge = *ec[i];

    if (pred_total <= 1.) // single edge
    {
      D.neighbor_predictions[0] = (float)last_pred;
      GD::foreach_feature<task_data, uint64_t, add_edge_features_single_fn>(
          sch.get_vw_pointer_unsafe(), edge, D);
    } else // lots of edges
      GD::foreach_feature<task_data, uint64_t, add_edge_features_group_fn>(
          sch.get_vw_pointer_unsafe(), edge, D);
  }
  ec[n]->indices.push_back(neighbor_namespace);
  ec[n]->total_sum_feat_sq +=
      ec[n]->feature_space[neighbor_namespace].sum_feat_sq;
  ...
  for (std::string &i : all.pairs) {
    ...
    if ((i0 == (int)neighbor_namespace) || (i1 == (int)neighbor_namespace)) {
      ec[n]->num_features +=
          ec[n]->feature_space[i0].size() * ec[n]->feature_space[i1].size();
      ec[n]->total_sum_feat_sq += ec[n]->feature_space[i0].sum_feat_sq *
                                  ec[n]->feature_space[i1].sum_feat_sq;
    }
  }
}
...
void run(Search::search &sch, multi_ex &ec) {
  ...
  for (size_t loop = 0; loop < D.num_loops; loop++) {
    ...
    for (int n_id = start; n_id != end; n_id += step) {
      ...
      uint32_t k =
          (ec[n]->l.cs.costs.size() > 0) ? ec[n]->l.cs.costs[0].class_index : 0;
      ...
      if (false && (k > 0)) {
        ...
        for (size_t k2 = 1; k2 <= D.K; k2++)
          min_count = std::min(min_count, D.true_counts[k2]);
        float w = min_count / D.true_counts[k];
        // float w = D.true_counts_total / D.true_counts[k] / (float)(D.K);
        P.set_weight(w);
        // std::cerr << "w = " << D.true_counts_total / D.true_counts[k] /
        // (float)(D.K) << std::endl;
      ...
      if (ec[n]->l.cs.costs.size() > 0) // for test examples
        sch.loss((ec[n]->l.cs.costs[0].class_index == D.pred[n])
                     ? 0.f
                     : (last_loop ? 0.5f : loss_val));
