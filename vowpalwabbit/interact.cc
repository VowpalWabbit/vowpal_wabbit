...
bool contains_valid_namespaces(features& f_src1, features& f_src2, interact& in, VW::io::logger& logger)
{
  ...
  if (f_src1.values[0] != 1)
  {
    ...
    logger.error("Namespace '{}' misses anchor feature with value 1", static_cast<char>(in.n1));
    return false;
  }

  if (f_src2.values[0] != 1)
  {
    logger.error("Namespace '{}' misses anchor feature with value 1", static_cast<char>(in.n2));
    return false;
  }
