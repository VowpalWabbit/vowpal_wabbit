...
bool contains_valid_namespaces(vw &all, features &f_src1, features &f_src2,
                               interact &in) {
  ...
  if (f_src1.values[0] != 1) {
    all.trace_message << "Namespace '" << (char)in.n1
                      << "' misses anchor feature with value 1";
    return false;
  }

  if (f_src2.values[0] != 1) {
    all.trace_message << "Namespace '" << (char)in.n2
                      << "' misses anchor feature with value 1";
    return false;
  }
...
void multiply(features &f_dest, features &f_src2, interact &in) {
  ...
  for (size_t i1 = 1, i2 = 1; i1 < f_src1.size() && i2 < f_src2.size();) {
    // calculating the relative offset from the namespace offset used to match
    // features
    uint64_t cur_id1 = (uint64_t)(
        ((f_src1.indicies[i1] & weight_mask) - base_id1) & weight_mask);
    uint64_t cur_id2 = (uint64_t)(
        ((f_src2.indicies[i2] & weight_mask) - base_id2) & weight_mask);

    // checking for sorting requirement
    if (cur_id1 < prev_id1) {
      std::cout << "interact features are out of order: " << cur_id1 << " < "
                << prev_id1 << ". Skipping features." << std::endl;
      return;
    }

    if (cur_id2 < prev_id2) {
      std::cout << "interact features are out of order: " << cur_id2 << " < "
                << prev_id2 << ". Skipping features." << std::endl;
      return;
    }

    if (cur_id1 == cur_id2) {
      f_dest.push_back(f_src1.values[i1] * f_src2.values[i2],
                       f_src1.indicies[i1]);
      i1++;
      i2++;
...
void predict_or_learn(interact &in, LEARNER::single_learner &base,
                      example &ec) {
  ...
  for (size_t i = 0; i < ec.indices.size(); i++) {
    if (ec.indices[i] == in.n2) {
      n2_i = (int)i;
      memmove(&ec.indices[n2_i], &ec.indices[n2_i + 1],
              sizeof(unsigned char) * (ec.indices.size() - n2_i - 1));
      ec.indices.decr();
      break;
    }
  }
  ...
  ec.indices.incr();
  memmove(&ec.indices[n2_i + 1], &ec.indices[n2_i],
          sizeof(unsigned char) * (ec.indices.size() - n2_i - 1));
...
LEARNER::base_learner *interact_setup(options_i &options, vw &all) {
  ...
  if (s.length() != 2) {
    std::cerr << "Need two namespace arguments to interact: " << s
              << " won't do EXITING\n";
    return nullptr;
  }
  ...
  if (!all.quiet)
    std::cerr << "Interacting namespaces " << data->n1 << " and " << data->n2
              << std::endl;
  data->all = &all;

  LEARNER::learner<interact, example> *l;
  l = &LEARNER::init_learner(data, as_singleline(setup_base(options, all)),
                             predict_or_learn<true, true>,
                             predict_or_learn<false, true>, 1);
