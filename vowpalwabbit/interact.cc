...
void multiply(features &f_dest, features &f_src2, interact &in) {
  ...
  for (size_t i1 = 1, i2 = 1; i1 < f_src1.size() && i2 < f_src2.size();) {
    ...
    if (cur_id1 < prev_id1) {
      cout << "interact features are out of order: " << cur_id1 << " < "
           << prev_id1 << ". Skipping features." << endl;
      return;
    }

    if (cur_id2 < prev_id2) {
      cout << "interact features are out of order: " << cur_id2 << " < "
           << prev_id2 << ". Skipping features." << endl;
      return;
    }
...
LEARNER::base_learner *interact_setup(options_i &options, vw &all) {
  ...
  if (s.length() != 2) {
    cerr << "Need two namespace arguments to interact: " << s
         << " won't do EXITING\n";
    return nullptr;
  }
  ...
  if (!all.quiet)
    cerr << "Interacting namespaces " << data->n1 << " and " << data->n2
         << endl;
  data->all = &all;
