...
void parse_label(parser *p, shared_data *, void *v, v_array<substring> &words) {
  ...
  default:
    ...
    for (size_t i = 0; i < words.size(); i++)
      std::cerr << words[i].begin << " ";
    std::cerr << std::endl;
  }
}

label_parser multilabel = {default_label,     parse_label,  cache_label,
                           read_cached_label, delete_label, weight,
                           copy_label,        test_label,   sizeof(labels)};
...
void output_example(vw &all, example &ec) {
  ...
  if (!test_label(&ld)) {
    ...
    while (preds_index < preds.label_v.size() &&
           given_index < given.label_v.size()) {
