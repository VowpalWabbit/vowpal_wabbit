...
namespace MWT {
  ...
  void value_policy(mwt& c, float val, uint64_t index)//estimate the value of a single feature.
  {
    ...
    uint64_t new_index = ((index & c.all->wv.getMask()) >> c.all->wv.getStride());
	...
	for (unsigned char ns : ec.indices)
	  if (c.namespaces[ns])
	      ...
	      if (learn)
		{
		  ...
		  uint32_t stride_shift = c.all->wv.getStride();
		  for ( features::iterator& f : ec.feature_space[ns])
		    {
		      uint64_t new_index=((f.index()& c.all->wv.getMask()) >> stride_shift)*c.num_classes +f.value();
