...
inline bool observed_cost(CB::cb_class *cl) {
  // cost observed for this action if it has non zero probability and cost !=
  // FLT_MAX
  if (cl != nullptr)
...
void value_policy(mwt &c, float val,
                  uint64_t index) // estimate the value of a single feature.
{
  ...
  uint64_t new_index =
      (index & c.all->weights.mask()) >> c.all->weights.stride_shift();
...
void predict_or_learn(mwt &c, single_learner &base, example &ec) {
  ...
  if (c.observation != nullptr) {
    ...
    for (uint64_t policy : c.policies) {
      c.evals[policy].cost +=
          get_cost_estimate(c.observation, c.evals[policy].action);
      ...
      if (c.namespaces[ns]) {
        ...
        if (learn) {
          ...
          for (features::iterator &f : ec.feature_space[ns]) {
            uint64_t new_index =
                ((f.index() & weight_mask) >> stride_shift) * c.num_classes +
                (uint64_t)f.value();
  ...
  // modify the predictions to use a vector with a score for each evaluated
  // feature.
...
void finish_example(vw &all, mwt &c, example &ec) {
  ...
  all.sd->update(ec.test_only, c.observation != nullptr, loss, 1.f,
                 ec.num_features);
...
void save_load(mwt &c, io_buf &model_file, bool read, bool text) {
  ...
  // total
  msg << "total: " << c.total;
  bin_text_read_write_fixed_validated(model_file, (char *)&c.total,
                                      sizeof(c.total), "", read, msg, text);

  // policies
  size_t policies_size = c.policies.size();
  bin_text_read_write_fixed_validated(model_file, (char *)&policies_size,
                                      sizeof(policies_size), "", read, msg,
                                      text);
  ...
  bin_text_read_write_fixed_validated(model_file, (char *)c.policies.begin(),
                                      policies_size * sizeof(feature_index), "",
                                      read, msg, text);
  ...
  for (feature_index &policy : c.policies) {
    ...
    bin_text_read_write_fixed_validated(model_file, (char *)&c.evals[policy],
                                        sizeof(policy_data), "", read, msg,
                                        text);
  }
}
} // namespace MWT
...
base_learner *mwt_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("multiworld_test", s)
               .keep()
               .help("Evaluate features as a policies"))
      .add(make_option("learn", c->num_classes)
               .help("Do Contextual Bandit learning on <n> classes."))
      .add(make_option("exclude_eval", exclude_eval)
  ...
  for (char i : s)
    c->namespaces[(unsigned char)i] = true;
  c->all = &all;
  ...
  if (c->learn)
    if (exclude_eval)
      l = &init_learner(c, as_singleline(setup_base(options, all)),
                        predict_or_learn<true, true, true>,
                        predict_or_learn<true, true, false>, 1,
                        prediction_type::scalars);
    else
      l = &init_learner(c, as_singleline(setup_base(options, all)),
                        predict_or_learn<true, false, true>,
                        predict_or_learn<true, false, false>, 1,
                        prediction_type::scalars);
  else
    l = &init_learner(c, as_singleline(setup_base(options, all)),
                      predict_or_learn<false, false, true>,
                      predict_or_learn<false, false, false>, 1,
                      prediction_type::scalars);
