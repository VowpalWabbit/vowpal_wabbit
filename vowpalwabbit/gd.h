...
void save_load_online_state(vw &all, io_buf &model_file, bool read, bool text,
                            double &total_weight, GD::gd *g = nullptr,
                            uint32_t ftrl_size = 0);

template <class T> struct multipredict_info {
  size_t count;
  size_t step;
  polyprediction *pred;
...
template <class T>
inline void vec_add_multipredict(multipredict_info<T> &mp, const float fx,
                                 uint64_t fi) {
...
inline void foreach_feature(vw &all, features &fs, R &dat, uint64_t offset = 0,
                            float mult = 1.) {
...
inline void foreach_feature(vw &all, example &ec, R &dat) {
  return all.weights.sparse ? foreach_feature<R, S, T, sparse_parameters>(
                                  all.weights.sparse_weights,
                                  all.ignore_some_linear, all.ignore_linear,
                                  all.interactions, all.permutations, ec, dat)
                            : foreach_feature<R, S, T, dense_parameters>(
...
// function T(some_data_R, feature_value_x,
// feature_weight)
...
inline float inline_predict(vw &all, example &ec) {
  return all.weights.sparse
             ? inline_predict<sparse_parameters>(
                   all.weights.sparse_weights, all.ignore_some_linear,
                   all.ignore_linear, all.interactions, all.permutations, ec,
                   ec.l.simple.initial)
             : inline_predict<dense_parameters>(
                   all.weights.dense_weights, all.ignore_some_linear,
                   all.ignore_linear, all.interactions, all.permutations, ec,
                   ec.l.simple.initial);
}
