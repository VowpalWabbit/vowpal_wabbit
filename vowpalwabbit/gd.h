...
void output_and_account_example(example* ec);
 ...
 template <void (*T)(void*, float, uint32_t)>
   void foreach_feature(void* dat, feature* begin, feature* end, uint32_t offset=0, float mult=1.)
   {
 ...
 template <void (*T)(void*, float, uint32_t)>
   void foreach_feature(vw& all, void* dat, example* ec)
   {
     uint32_t offset = ec->ft_offset;

     for (unsigned char* i = ec->indices.begin; i != ec->indices.end; i++) 
       foreach_feature<T>(dat, ec->atomics[*i].begin, ec->atomics[*i].end, offset);
	 ...
	 for (; temp.begin != temp.end; temp.begin++)
	   {
	     ...
	     foreach_feature<T>(dat, ec->atomics[(int)(*i)[1]].begin, ec->atomics[(int)(*i)[1]].end, 
	 ...
	 for (; temp2.begin != temp2.end; temp2.begin++) {
	   ...
	   return foreach_feature<T>(dat, ec->atomics[(int)(*i)[2]].begin, ec->atomics[(int)(*i)[2]].end, halfhash + offset, mult);
	 }
       }
     }
   }
