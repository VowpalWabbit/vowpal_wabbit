...
inline uint64_t constant_feat(const stagewise_poly &poly) { return stride_shift(poly, constant * poly.all->gs.wpp); }
...
void sort_data_update_support(stagewise_poly &poly)
{
  ...
  for (uint64_t i = 0; i != poly.all->length(); ++i)
  {
    ...
    {
      float weightsal = (fabsf(poly.all->weights[wid]) * poly.all->weights[poly.all->gs.normalized_idx + (wid)]);
      /*
...
void synthetic_reset(stagewise_poly &poly, example &ec)
{
  ...
  poly.synth_ec.interactions = &poly.all->gs.interactions;
