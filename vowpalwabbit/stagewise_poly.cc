...
inline uint64_t do_ft_offset(const stagewise_poly &poly, uint64_t idx) {
  ...
  assert(!poly.original_ec ||
         poly.synth_ec.ft_offset == poly.original_ec->ft_offset);
...
inline bool parent_get(const stagewise_poly &poly, uint64_t wid) {
  ...
  return poly.depthsbits[wid_mask_un_shifted(poly, do_ft_offset(poly, wid)) *
                             2 +
                         1] &
         parent_bit;
}
...
inline void parent_toggle(stagewise_poly &poly, uint64_t wid) {
  assert(wid % stride_shift(poly, 1) == 0);
  assert(do_ft_offset(poly, wid) % stride_shift(poly, 1) == 0);
  poly.depthsbits[wid_mask_un_shifted(poly, do_ft_offset(poly, wid)) * 2 + 1] ^=
      parent_bit;
}
...
void sanity_check_state(stagewise_poly &poly) {
  for (uint64_t i = 0; i != poly.all->length(); ++i) {
    ...
    assert(
        !(min_depths_get(poly, wid) == default_depth && parent_get(poly, wid)));

    if (poly.all->weights.sparse)
      assert(!(min_depths_get(poly, wid) == default_depth &&
               fabsf(poly.all->weights.sparse_weights[wid]) > 0));
    else
      assert(!(min_depths_get(poly, wid) == default_depth &&
               fabsf(poly.all->weights.dense_weights[wid]) > 0));
    // assert( min_depths_get(poly, wid) != default_depth &&
    // fabsf(poly.all->weights[wid]) < tolerance );

    assert(!(poly.depthsbits[wid_mask_un_shifted(poly, wid) * 2 + 1] &
             ~(parent_bit + cycle_bit + indicator_bit)));
  }
}
#endif // NDEBUG
...
inline uint64_t child_wid(const stagewise_poly &poly, uint64_t wi_atomic,
                          uint64_t wi_general) {
  ...
  if (wi_atomic == constant_feat_masked(poly))
    ...
    return wid_mask(
        poly,
        stride_shift(poly, stride_un_shift(poly, wi_atomic) ^
...
#endif // DEBUG
    poly.sd_len = (len_candidate > poly.all->length()) ? poly.all->length()
                                                       : len_candidate;
#ifdef DEBUG
...
int sort_data_compar(const void *a_v, const void *b_v) {
  return 2 * (((sort_data *)a_v)->weightsal < ((sort_data *)b_v)->weightsal) -
         1;
}
#endif // DEBUG
...
void sort_data_update_support(stagewise_poly &poly) {
  ...
  size_t num_new_features = (size_t)pow(
      poly.sum_input_sparsity * 1.0f / poly.num_examples, poly.sched_exponent);
  num_new_features = (num_new_features > poly.all->length())
                         ? (uint64_t)poly.all->length()
                         : num_new_features;
...
#ifdef DEBUG
    cout << "Adding feature " << pos << "/" << num_new_features << " || wid "
         << poly.sd[pos].wid << " || sort value " << poly.sd[pos].weightsal
         << endl;
#endif // DEBUG
  }
...
#ifdef DEBUG
  cout << "depths:";
  for (uint64_t depth = 0; depth <= poly.max_depth &&
                           depth < sizeof(poly.depths) / sizeof(*poly.depths);
       ++depth)
...
void synthetic_create_rec(stagewise_poly &poly, float v, uint64_t findex) {
...
#ifdef DEBUG
      cout << "FOUND A TRANSPLANT!!! moving [" << wid_cur << "] from depth "
           << (uint64_t)min_depths_get(poly, wid_cur) << " to depth "
           << poly.cur_depth << endl;
#endif // DEBUG
  ...
  if (!cycle_get(poly, wid_cur) &&
      ((poly.cur_depth > default_depth ? default_depth : poly.cur_depth) ==
       min_depths_get(poly, wid_cur))) {
    ...
    poly.synth_ec.feature_space[tree_atomics].push_back(temp.x,
                                                        temp.weight_index);
...
#ifdef DEBUG
      poly.max_depth =
          (poly.max_depth > poly.cur_depth) ? poly.max_depth : poly.cur_depth;
#endif // DEBUG
      GD::foreach_feature<stagewise_poly, uint64_t, synthetic_create_rec>(
...
void synthetic_create(stagewise_poly &poly, example &ec, bool training) {
  ...
  poly.synth_rec_f.weight_index =
      constant_feat_masked(poly); // note: not ft_offset'd
  ...
  poly.synth_ec.total_sum_feat_sq =
      poly.synth_ec.feature_space[tree_atomics].sum_feat_sq;
...
void learn(stagewise_poly &poly, single_learner &base, example &ec) {
  ...
  if (training) {
    ...
    if (ec.example_counter
      ...
      poly.update_support =
          (poly.all->all_reduce == nullptr || poly.numpasses == 1);
    }
...
void end_pass(stagewise_poly &poly) {
  ...
  if (all.all_reduce != nullptr) {
     ...
     * case...
     */
    all_reduce<uint8_t, reduce_min_max>(all, poly.depthsbits,
                                        depthsbits_sizeof(poly));

    sum_input_sparsity_inc =
        (uint64_t)accumulate_scalar(all, (float)sum_input_sparsity_inc);
    sum_sparsity_inc =
        (uint64_t)accumulate_scalar(all, (float)sum_sparsity_inc);
    num_examples_inc =
        (uint64_t)accumulate_scalar(all, (float)num_examples_inc);
  }

  poly.sum_input_sparsity_sync =
      poly.sum_input_sparsity_sync + sum_input_sparsity_inc;
  poly.sum_input_sparsity = poly.sum_input_sparsity_sync;
...
#ifdef DEBUG
  cout << "total feature number (after poly expansion!) = " << poly.sum_sparsity
       << endl;
#endif // DEBUG
...
void save_load(stagewise_poly &poly, io_buf &model_file, bool read, bool text) {
  if (model_file.files.size() > 0) {
    stringstream msg;
    bin_text_read_write_fixed(model_file, (char *)poly.depthsbits,
                              (uint32_t)depthsbits_sizeof(poly), "", read, msg,
                              text);
  }
...
base_learner *stagewise_poly_setup(options_i &options, vw &all) {
...
#ifdef MAGIC_ARGUMENT
  new_options.add(make_typed_option("magic_argument", poly->magic_argument)
                      .default_value(0.)
                      .help("magical feature flag"));
#endif // MAGIC_ARGUMENT
  ...
  learner<stagewise_poly, example> &l = init_learner(
      poly, as_singleline(setup_base(options, all)), learn, predict);
  l.set_finish(finish);
