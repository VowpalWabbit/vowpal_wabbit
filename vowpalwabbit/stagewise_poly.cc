...
struct stagewise_poly {
  ...
  ~stagewise_poly() {
#ifdef DEBUG
    cout << "total feature number (after poly expansion!) = " << sum_sparsity
         << std::endl;
#endif // DEBUG
...
inline uint64_t do_ft_offset(const stagewise_poly &poly, uint64_t idx) {
  // std::cout << poly.synth_ec.ft_offset << "  " << poly.original_ec->ft_offset
  // << std::endl;
...
#ifdef DEBUG
    std::cout << "Adding feature " << pos << "/" << num_new_features
              << " || wid " << poly.sd[pos].wid << " || sort value "
...
#ifdef DEBUG
      std::cout << "FOUND A TRANSPLANT!!! moving [" << wid_cur
