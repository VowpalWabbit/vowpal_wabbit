...
const uint64_t valid_ns_size =
    printable_end - printable_start -
    1; // -1 to skip characters ':' and '|' excluded in is_valid_ns()

// exand all wildcard namespaces in vector<string>
// req_length must be 0 if interactions of any length are allowed, otherwise
// contains required length
// err_msg will be printed plus exception will be thrown if req_length != 0 and
// mismatch interaction length.
...
// function estimates how many new features will be generated for example and
// ther sum(value^2).
...
// this code is for C++98/03 complience as I unable to pass null
// function-pointer as template argument in g++-4.6
