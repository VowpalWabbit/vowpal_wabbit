...
inline void inner_kernel(R& dat, features::iterator_all& begin, features::iterator_all& end, const uint64_t offset, weight_vector& wv, feature_value ft_value, feature_index halfhash)
{
  if (audit)
  {
    for (; begin != end; ++begin)
    {
      ...
      call_T<R, T>(dat, wv,  INTERACTION_VALUE(ft_value, begin.value()), (begin.index() ^ halfhash) + offset);
            ...
            if (second.nonempty())
              {
                ...
                for(size_t i = 0; i < first.indicies.size(); ++i)
                  { feature_index halfhash = FNV_prime * (uint64_t)first.indicies[i];
                    ...
                    inner_kernel<R, S, T, audit, audit_func>(dat, begin, end, offset, wv, ft_value, halfhash);
