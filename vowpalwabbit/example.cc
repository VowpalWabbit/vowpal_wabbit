...
{ features_and_source fs;
  fs.stride_shift = all.wv.stride_shift();
  fs.mask = (uint64_t)all.wv.mask() >> all.wv.stride_shift();
...
{ flat_example& fec = calloc_or_throw<flat_example>();
  ...
  ffs.stride_shift = all.wv.stride_shift();
  ffs.mask = (uint64_t)all.wv.mask() >> all.wv.stride_shift();
