...
namespace VW {
void copy_example_label(example *dst, example *src, size_t,
                        void (*copy_label)(void *, void *)) {
  if (copy_label)
    copy_label(&dst->l,
               &src->l); // TODO: we really need to delete_label on dst :(
  else
    dst->l = src->l;
}
...
void copy_example_data(bool audit, example *dst, example *src,
                       size_t label_size, void (*copy_label)(void *, void *)) {
  copy_example_data(audit, dst, src);
  copy_example_label(dst, src, label_size, copy_label);
}

void move_feature_namespace(example *dst, example *src, namespace_index c) {
  if (std::find(src->indices.begin(), src->indices.end(), c) ==
      src->indices.end())
    return; // index not present in src
  if (std::find(dst->indices.begin(), dst->indices.end(), c) ==
      dst->indices.end())
...
void vec_store(features_and_source &p, float fx, uint64_t fi) {
  p.feature_map.push_back(
      feature(fx, (uint64_t)(fi >> p.stride_shift) & p.mask));
}
...
flat_example *flatten_example(vw &all, example *ec) {
  ...
  if (all.weights.not_null()) // TODO:temporary fix. all.weights is not
                              // initialized at this point in some cases.
    ffs.mask = (uint64_t)all.weights.mask() >> all.weights.stride_shift();
  else
    ffs.mask = (uint64_t)LONG_MAX >> all.weights.stride_shift();
  GD::foreach_feature<full_features_and_source, uint64_t, vec_ffs_store>(
      all, *ec, ffs);
...
example *alloc_examples(size_t, size_t count = 1) {
  ...
  for (size_t i = 0; i < count; i++) {
    ec[i].in_use = true;
    ec[i].ft_offset = 0;
    //  std::cerr << "  alloc_example.indices.begin()=" << ec->indices.begin()
    //  << " end=" << ec->indices.end() << " //
