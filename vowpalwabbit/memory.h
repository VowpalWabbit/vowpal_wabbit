...
template <class T> T *calloc_or_throw(size_t nmemb) {
  if (nmemb == 0)
    return nullptr;
...
template <class T> T &calloc_or_throw() { return *calloc_or_throw<T>(1); }

typedef void (*free_fn)(void *);
template <class T> using free_ptr = std::unique_ptr<T, free_fn>;
template <class T> void destroy_free(void *temp) {
  ((T *)temp)->~T();
  free(temp);
}
...
template <class T> T *calloc_mergable_or_throw(size_t nmemb) {
  if (nmemb == 0)
    return nullptr;
