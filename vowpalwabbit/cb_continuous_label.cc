...
void parse_pdf(
    const std::vector<VW::string_view>& words, std::vector<VW::string_view>& parse_name,
    size_t words_index, reduction_features& red_features)
{
  ...
  for (size_t i = words_index; i < words.size(); i++)
  {
    ...
    tokenize(':', words[i], parse_name);
    if (parse_name.empty() || parse_name.size() < 3) { continue; }
    VW::continuous_actions::pdf_segment seg;
    seg.left = float_of_string(parse_name[0]);
    seg.right = float_of_string(parse_name[1]);
    seg.pdf_value = float_of_string(parse_name[2]);
    ...
    size_t words_index, reduction_features& red_features)
{
  ...
  {
    tokenize(':', words[i], parse_name);
    if (parse_name.empty() || parse_name.size() < 1) { continue; }
    cats_reduction_features.chosen_action = float_of_string(parse_name[0]);
...
void parse_label(parser*, shared_data*, continuous_label& ld, std::vector<VW::string_view>& words,
    std::vector<VW::string_view>& parse_name, reduction_features& red_features)
{
  ld.costs.clear();
  ...
  {
    if (words[i] == PDF) { parse_pdf(words, parse_name, i + 1, red_features); }
    else if (words[i] == CHOSEN_ACTION)
    {
      parse_chosen_action(words, parse_name, i + 1, red_features);
    }
    else if (words[i - 1] == CA_LABEL)
    {
      ...
      if (parse_name.empty() || parse_name.size() > 4)
        THROW("malformed cost specification: "
            << "parse_name");

      f.action = float_of_string(parse_name[0]);

      if (parse_name.size() > 1) f.cost = float_of_string(parse_name[1]);

      if (std::isnan(f.cost)) THROW("error NaN cost (" << parse_name[1] << " for action: " << parse_name[0]);

      f.pdf_value = .0;
      if (parse_name.size() > 2) f.pdf_value = float_of_string(parse_name[2]);

      if (std::isnan(f.pdf_value))
        THROW("error NaN pdf_value (" << parse_name[2] << " for action: " << parse_name[0]);
...
label_parser the_label_parser = {
  ...
  [](parser* p, shared_data* sd, polylabel* v, std::vector<VW::string_view>& words, std::vector<VW::string_view>& parse_name, reduction_features& red_features) {
    parse_label(p, sd, v->cb_cont, words, parse_name, red_features);
  },
  // cache_label
