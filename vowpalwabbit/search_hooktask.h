...
struct task_data {
  ...
  void *run_object;   // for python this will really be a (py::object*), but we
                      // don't want basic VW to have to know about
                      // hook
  void *setup_object; // for python this will really be a (py::object*), but we
                      // don't want basic VW to have to know
                      // about hook
  void *takedown_object; // for python this will really be a (py::object*), but
                         // we don't want basic VW to have to know
                         // about hook
  void (*delete_run_object)(void *); // we can't delete run_object on our own
                                     // because we don't know its size, so
                                     // provide a hook
  void (*delete_extra_data)(
      task_data &);           // ditto for extra_data and extra_data2
