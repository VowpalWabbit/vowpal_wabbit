...
/*    Caveats:
   ...
   */
vw *initialize(config::options_i &options, io_buf *model = nullptr,
...
vw *initialize_escaped(std::string const &s, io_buf *model = nullptr,
                       bool skipModelLoad = false,
                       trace_message_t trace_listener = nullptr,
                       void *trace_context = nullptr);
...
VW_DEPRECATED("By value version is deprecated, pass std::string by const ref "
              "instead using `to_argv`")
...
/*
  Call finish() after you are done with the vw instance.  This cleans up memory
  usage.
 */
...
// The next commands deal with creating examples.  Caution: VW does not all
// allow creation of many examples at once by
// default.  You can adjust the exact number by tweaking ring_size.

/* The simplest of two ways to create an example.  An example_line is the
 * literal line in a VW-format datafile.
 */
...
// after you create and fill feature_spaces, get an example with everything
// filled in.
...
// this interface must be used with care as finish_example is a no-op for these
// examples.
// thus any delay introduced when freeing examples must be at least as long as
// the one
// introduced by all.l->finish_example implementations.
// e.g. multiline examples as used by cb_adf must not be released before the
// finishing newline example.
...
void copy_example_data(bool audit, example *, example *, size_t,
                       void (*copy_label)(void *, void *));
void copy_example_metadata(bool audit, example *, example *);
void copy_example_data(bool audit, example *,
                       example *);  // metadata + features, don't copy the label
...
inline uint64_t hash_space_static(const std::string &s,
...
inline uint64_t hash_feature_static(const std::string &s, uint64_t u,
...
inline float get_weight(vw &all, uint32_t index, uint32_t offset) {
  return (
      &all.weights[((uint64_t)index) << all.weights.stride_shift()])[offset];
}
...
inline void set_feature(primitive_feature_space &fs, size_t index,
                        uint64_t feature_hash, float value) {
