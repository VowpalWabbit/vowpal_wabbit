...
{
  return all.p->hasher(s.data(), s.length(), u) & all.gs.parse_mask;
}
...
{
  return all.p->hasher(fstr, strlen(fstr), u) & all.gs.parse_mask;
}
...
inline uint64_t chain_hash(vw& all, const std::string& name, const std::string& value, uint64_t u)
{
  ...
  return all.p->hasher(value.data(), value.length(), all.p->hasher(name.data(), name.length(), u)) & all.gs.parse_mask;
}
