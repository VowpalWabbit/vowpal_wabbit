...
struct cb_sample_data {
  explicit cb_sample_data(std::shared_ptr<rand_state> &random_state)
  ...
  inline void learn_or_predict(multi_learner &base, multi_ex &examples) {
    multiline_learn_or_predict<is_learn>(base, examples,
    ...
    auto it = std::find_if(examples.begin(), examples.end(), [](example *item) {
    ...
    // If we are learning and have a label, then take that action as the chosen
    // action. Otherwise sample the
    // distribution.
    if (is_learn && labelled_action != -1) {
      // Find where the labelled action is in the final prediction to determine
      // if swapping needs to occur.
      // This only matters if the prediction decided to explore, but the same
      // output should happen for the learn case.
      ...
      if (!examples[0]->tag.empty()) {
        ...
        if (strncmp(examples[0]->tag.begin(), SEED_IDENTIFIER.c_str(),
                    SEED_IDENTIFIER.size()) == 0 &&
            examples[0]->tag.size() > SEED_IDENTIFIER.size()) {
          substring tag_seed{examples[0]->tag.begin() + 5,
                             examples[0]->tag.begin() +
                                 examples[0]->tag.size()};
      ...
      auto result = exploration::sample_after_normalizing(
          seed, ACTION_SCORE::begin_scores(action_scores),
          ACTION_SCORE::end_scores(action_scores), chosen_action);
    ...
    auto result = exploration::swap_chosen(action_scores.begin(),
                                           action_scores.end(), chosen_action);
    assert(result == S_EXPLORATION_OK);
...
template <bool is_learn>
void learn_or_predict(cb_sample_data &data, multi_learner &base,
                      multi_ex &examples) {
...
base_learner *cb_sample_setup(options_i &options, vw &all) {
  ...
  new_options.add(make_option("cb_sample", cb_sample_option)
                      .keep()
  ...
  if (options.was_supplied("no_predict")) {
    THROW("cb_sample cannot be used with no_predict, as there would be no "
          "predictions to sample.");
  }

  auto data = scoped_calloc_or_throw<cb_sample_data>(all.get_random_state());
  return make_base(init_learner(
      data, as_multiline(setup_base(options, all)), learn_or_predict<true>,
