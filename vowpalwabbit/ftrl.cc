...
inline void predict_with_confidence(uncertainty &d, const float fx, float &fw) {
  ...
  float uncertain = ((d.b.data.ftrl_beta + sqrtf_ng2) / d.b.data.ftrl_alpha +
                     d.b.data.l2_lambda);
  d.score += (1 / uncertain) * sign(fx);
}
float sensitivity(ftrl &b, base_learner & /* base */, example &ec) {
  uncertainty uncetain(b);
  GD::foreach_feature<uncertainty, predict_with_confidence>(*(b.all), ec,
                                                            uncetain);
  return uncetain.score;
}

template <bool audit> void predict(ftrl &b, single_learner &, example &ec) {
...
template <bool audit>
void multipredict(ftrl &b, base_learner &, example &ec, size_t count,
                  size_t step, polyprediction *pred,
  ...
  if (b.all->weights.sparse) {
    ...
    GD::foreach_feature<GD::multipredict_info<sparse_parameters>, uint64_t,
                        GD::vec_add_multipredict>(all, ec, mp);
  } else {
    GD::multipredict_info<dense_parameters> mp = {
        count, step, pred, all.weights.dense_weights, (float)all.sd->gravity};
    GD::foreach_feature<GD::multipredict_info<dense_parameters>, uint64_t,
                        GD::vec_add_multipredict>(all, ec, mp);
  }
...
void inner_update_pistol_state_and_predict(update_data &d, float x,
                                           float &wref) {
  float *w = &wref;
  ...
  w[W_XT] = std::sqrt(w[W_G2]) * d.ftrl_beta * w[W_ZT] *
            correctedExp(squared_theta / 2.f * tmp) * tmp;
...
void inner_update_cb_state_and_predict(update_data &d, float x, float &wref) {
  ...
  if (w[W_MG] * w_mx > 0)
    w_xt = (d.ftrl_alpha + w[W_WE]) * w[W_ZT] /
           (w[W_MG] * w_mx * (w[W_MG] * w_mx + w[W_G2]));
...
void inner_update_cb_post(update_data &d, float x, float &wref) {
  ...
  if (w[W_MG] * w[W_MX] > 0)
    w[W_XT] = (d.ftrl_alpha + w[W_WE]) * w[W_ZT] /
              (w[W_MG] * w[W_MX] * (w[W_MG] * w[W_MX] + w[W_G2]));
  else
    w[W_XT] = 0;
...
void update_state_and_predict_cb(ftrl &b, single_learner &, example &ec) {
  ...
  GD::foreach_feature<update_data, inner_update_cb_state_and_predict>(
      *b.all, ec, b.data);

  b.all->normalized_sum_norm_x +=
      ((double)ec.weight) * b.data.normalized_squared_norm_x;
  b.total_weight += ec.weight;

  ec.partial_prediction =
      b.data.predict /
      ((float)((b.all->normalized_sum_norm_x + 1e-6) / b.total_weight));
...
void update_state_and_predict_pistol(ftrl &b, single_learner &, example &ec) {
  ...
  GD::foreach_feature<update_data, inner_update_pistol_state_and_predict>(
      *b.all, ec, b.data);
...
void update_after_prediction_proximal(ftrl &b, example &ec) {
  b.data.update = b.all->loss->first_derivative(b.all->sd, ec.pred.scalar,
                                                ec.l.simple.label) *
                  ec.weight;
...
void update_after_prediction_pistol(ftrl &b, example &ec) {
  ...
  GD::foreach_feature<update_data, inner_update_pistol_post>(*b.all, ec,
                                                             b.data);
}
...
void save_load(ftrl &b, io_buf &model_file, bool read, bool text) {
  ...
  if (!model_file.files.empty()) {
    ...
    if (resume)
      GD::save_load_online_state(*all, model_file, read, text, b.total_weight,
                                 nullptr, b.ftrl_size);
    else
...
base_learner *ftrl_setup(options_i &options, vw &all) {
  ...
  if (all.audit || all.hash_inv)
    l = &init_learner(b, learn_ptr, predict<true>,
                      UINT64_ONE << all.weights.stride_shift());
  else
    l = &init_learner(b, learn_ptr, predict<false>,
                      UINT64_ONE << all.weights.stride_shift());
