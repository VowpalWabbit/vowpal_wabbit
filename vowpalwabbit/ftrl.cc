...
void update_after_prediction_proximal(ftrl& b, example& ec)
{
  ...
  if(b.all->weights.sparse)
    {
      b.all->weights.sparse_weights.set_tag(hashall(ec.tag.begin(),ec.tag.size(),b.all->hash_seed)%b.all->feature_bitset_num_bits); // find the 5-bit hash of the tag for sparse weights
