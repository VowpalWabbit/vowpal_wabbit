...
void update_after_prediction_proximal(ftrl& b, example& ec)
{
  ...
  if(b.all->weights.sparse)
    {
      b.all->weights.sparse_weights.set_tag(hashall(ec.tag.begin(),ec.tag.size(),b.all->hash_seed)%32); // find the 5-bit hash of the tag for sparse weights
    ...
    {
      b.all->weights.dense_weights.set_tag(hashall(ec.tag.begin(),ec.tag.size(),b.all->hash_seed)%32); // find the 5-bit hash of the tag for dense weights
      GD::foreach_feature<ftrl_update_data, inner_update_proximal>(*b.all, ec, b.data);
