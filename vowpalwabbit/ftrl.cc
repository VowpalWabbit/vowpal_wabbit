...
base_learner* ftrl_setup(options_i& options, vw& all)
{
  ...
  if (all.audit || all.hash_inv)
    l = &init_learner(
        b, learn_ptr, predict<true>, UINT64_ONE << all.weights.stride_shift(), all.get_setup_name(ftrl_setup) +"-"+ algorithm_name + "-audit");
  else
    l = &init_learner(b, learn_ptr, predict<false>, UINT64_ONE << all.weights.stride_shift(), all.get_setup_name(ftrl_setup) + "-" + algorithm_name);
