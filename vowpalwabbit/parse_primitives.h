...
void tokenize(char delim, substring s, ContainerT &ret,
...
// Note this will destructively parse the passed in substring as it replaces
// delimiters with '\0'
std::vector<substring> escaped_tokenize(char delim, substring s,
                                        bool allow_empty = false);
...
inline float parseFloat(char *p, char **end, char *endLine = nullptr) {
  ...
  int s = 1;
  while ((*p == ' ') && (endLine_is_null || p < endLine))
    p++;
...
inline int int_of_substring(substring s) {
  ...
  if (endptr == s.begin && s.begin != s.end) {
    std::cout << "warning: " << std::string(s.begin, s.end - s.begin).c_str()
              << " is not a good int, replacing with 0" << std::endl;
    i = 0;
  }
