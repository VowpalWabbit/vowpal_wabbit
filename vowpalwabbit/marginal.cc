...
struct data {
  ...
  float alg_loss;           // temporary storage for the loss of the current
                            // marginal-based predictor
  unordered_map<uint64_t, expert_pair> expert_state; // pair of weights on
                                                     // marginal and feature
                                                     // based predictors, one
                                                     // per marginal feature
...
template <bool is_learn> void make_marginal(data &sm, example &ec) {
  ...
  for (example::iterator i = ec.begin(); i != ec.end(); ++i) {
    ...
    if (sm.id_features[n]) {
      ...
      features &f = *i;
      f.clear();
      for (features::iterator j = sm.temp[n].begin(); j != sm.temp[n].end();
           ++j) {
        float first_value = j.value();
        uint64_t first_index = j.index() & mask;
        if (++j == sm.temp[n].end()) {
          cout << "warning: id feature namespace has " << sm.temp[n].size()
               << " features. Should be a multiple of 2" << endl;
          break;
        }
        ...
        if (sm.marginals.find(key) ==
            sm.marginals.end()) // need to initialize things.
        {
          sm.marginals.insert(make_pair(
              key, make_pair(sm.initial_numerator, sm.initial_denominator)));
          if (sm.compete) {
            expert e = {0, 0, 1.};
        ...
        float marginal_pred =
            (float)(sm.marginals[key].first / sm.marginals[key].second);
        ...
        if (sm.compete) // compute the prediction from the marginals using the
                        // weights
        {
          ...
          if (is_learn)
            sm.alg_loss +=
                weight * all.loss->getLoss(all.sd, marginal_pred, label);
        }
      }
    }
  }
...
template <bool is_learn> void compute_expert_loss(data &sm, example &ec) {
  vw &all = *sm.all;
  ...
  if (is_learn) {
    sm.alg_loss += sm.net_feature_weight *
                   all.loss->getLoss(all.sd, sm.feature_pred, label);
    ...
    if (sm.id_features[n])
      for (features::iterator j = sm.temp[n].begin(); j != sm.temp[n].end();
        ...
        if (sm.compete) // now update weights, before updating marginals
        {
          ...
          float regret1 =
              sm.alg_loss -
              all.loss->getLoss(all.sd, (float)(m.first / m.second), label);
          float regret2 =
              sm.alg_loss - all.loss->getLoss(all.sd, sm.feature_pred, label);

          e.first.regret += regret1 * weight;
          e.first.abs_regret += regret1 * regret1 * weight; // fabs(regret1);
          e.first.weight =
              get_adanormalhedge_weights(e.first.regret, e.first.abs_regret);
          e.second.regret += regret2 * weight;
          e.second.abs_regret += regret2 * regret2 * weight; // fabs(regret2);
          e.second.weight =
              get_adanormalhedge_weights(e.second.regret, e.second.abs_regret);
        }
...
void save_load(data &sm, io_buf &io, bool read, bool text) {
  ...
  bin_text_read_write_fixed_validated(io, (char *)&total_size,
                                      sizeof(total_size), "", read, msg, text);
  ...
  for (size_t i = 0; i < total_size; ++i) {
    ...
    bin_text_read_write_fixed(io, (char *)&index, sizeof(index), "", read, msg,
                              text);
    double numerator;
    if (!read) {
    ...
    bin_text_read_write_fixed(io, (char *)&numerator, sizeof(numerator), "",
                              read, msg, text);
    double denominator;
    if (!read) {
    ...
    bin_text_read_write_fixed(io, (char *)&denominator, sizeof(denominator), "",
                              read, msg, text);
    if (read)
      sm.marginals.insert(
          make_pair(index << stride_shift, make_pair(numerator, denominator)));
    else
      ++iter;
  }
  ...
  if (sm.compete) {
    ...
    bin_text_read_write_fixed_validated(
        io, (char *)&total_size, sizeof(total_size), "", read, msg, text);
    ...
    for (size_t i = 0; i < total_size; ++i) {
      ...
      bin_text_read_write_fixed(io, (char *)&index, sizeof(index), "", read,
                                msg, text);
      ...
      bin_text_read_write_fixed(io, (char *)&r1, sizeof(r1), "", read, msg,
                                text);
      if (!read)
        msg << c1 << ":";
      bin_text_read_write_fixed(io, (char *)&c1, sizeof(c1), "", read, msg,
                                text);
      if (!read)
        msg << w1 << ":";
      bin_text_read_write_fixed(io, (char *)&w1, sizeof(w1), "", read, msg,
                                text);
      if (!read)
        msg << r2 << ":";
      bin_text_read_write_fixed(io, (char *)&r2, sizeof(r2), "", read, msg,
                                text);
      if (!read)
        msg << c2 << ":";
      bin_text_read_write_fixed(io, (char *)&c2, sizeof(c2), "", read, msg,
                                text);
      if (!read)
        msg << w2 << ":";
      bin_text_read_write_fixed(io, (char *)&w2, sizeof(w2), "", read, msg,
                                text);

      if (read) {
        expert e1 = {r1, c1, w1};
        expert e2 = {r2, c2, w2};
        sm.expert_state.insert(
            make_pair(index << stride_shift, make_pair(e1, e2)));
      } else
        ++exp_iter;
    }
  }
...
LEARNER::base_learner *marginal_setup(options_i &options, vw &all) {
  ...
  marginal_options.add(
      make_option("marginal", marginal)
          .keep()
          .help("substitute marginal label estimates for ids"));
  marginal_options.add(
      make_option("initial_denominator", d->initial_denominator)
          .default_value(1.f)
          .help("initial denominator"));
  marginal_options.add(make_option("initial_numerator", d->initial_numerator)
                           .default_value(0.5f)
                           .help("initial numerator"));
  marginal_options.add(make_option("compete", d->compete)
                           .help("enable competition with marginal features"));
  marginal_options.add(
      make_option("update_before_learn", d->update_before_learn)
          .help("update marginal values before learning"));
  marginal_options.add(
      make_option("unweighted_marginals", d->unweighted_marginals)
          .help("ignore importance weights when computing marginals"));
  marginal_options.add(
      make_option("decay", d->decay)
          .default_value(0.f)
  ...
  LEARNER::learner<MARGINAL::data, example> &ret =
      init_learner(d, as_singleline(setup_base(options, all)),
