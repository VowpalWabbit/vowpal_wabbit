...
LEARNER::base_learner* marginal_setup(options_i& options, vw& all)
{
  ...
  marginal_options.add(make_typed_option("update_before_learn", d->update_before_learn).default_value(false).help("update marginal values before learning"));
  marginal_options.add(make_typed_option("unweighted_marginals", d->unweighted_marginals).default_value(false).help("ignore importance weights when computing marginals"));
  marginal_options.add(make_typed_option("decay", d->decay).default_value(0.f).help("decay multiplier per event (1e-3 for example)"));
