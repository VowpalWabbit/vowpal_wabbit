...
struct data {
  ...
  std::unordered_map<uint64_t, expert_pair> expert_state; // pair of weights on
                                                          // marginal and
                                                          // feature based
                                                          // predictors, one per
                                                          // marginal feature
    ...
    if (sm.id_features[n]) {
      ...
      for (features::iterator j = sm.temp[n].begin(); j != sm.temp[n].end();
        ...
        if (first_value != 1. || second_value != 1.) {
          std::cout << "warning: bad id features, must have value 1."
                    << std::endl;
          continue;
        }
        ...
        {
          sm.marginals.insert(
              std::make_pair(key, std::make_pair(sm.initial_numerator,
                                                 sm.initial_denominator)));
          if (sm.compete) {
            expert e = {0, 0, 1.};
...
void save_load(data &sm, io_buf &io, bool read, bool text) {
  ...
  for (size_t i = 0; i < total_size; ++i) {
    ...
    bin_text_read_write_fixed(io, (char *)&denominator, sizeof(denominator), "",
                              read, msg, text);
    if (read)
      sm.marginals.insert(std::make_pair(
          index << stride_shift, std::make_pair(numerator, denominator)));
    else
      ++iter;
  }
    ...
    for (size_t i = 0; i < total_size; ++i) {
      ...
      if (read) {
        ...
        sm.expert_state.insert(
            std::make_pair(index << stride_shift, std::make_pair(e1, e2)));
      } else
        ++exp_iter;
    }
  }
