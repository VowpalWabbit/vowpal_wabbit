...
static const std::vector<std::string> option_strings = {
    "csoaa_ldf", "wap_ldf",        "cb_adf", "explore_eval",
    "cbify_ldf", "cb_explore_adf", "warm_cb"};
...
void predict_or_learn(sfm_data &, LEARNER::multi_learner &base,
                      multi_ex &ec_seq) {
  if (ec_seq.size() == 0)
...
LEARNER::base_learner *shared_feature_merger_setup(config::options_i &options,
                                                   vw &all) {
  ...
  auto &learner = LEARNER::init_learner(data, base, predict_or_learn<true>,
                                        predict_or_learn<false>);

  // TODO: Incorrect feature numbers will be reported without merging the
  // example namespaces from the
  //       shared example in a finish_example function. However, its too
  //       expensive to perform the full operation.
