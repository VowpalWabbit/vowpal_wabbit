...
inline void audit_regressor_feature(audit_regressor_data& dat, const float /*ft_weight*/, const uint64_t ft_idx)
{
	...
	tempstream << ':' << ((ft_idx & weights.mask()) >> dat.all->stride_shift) << ':' << weights[ft_idx];
...
{       
    ...
    if (all.lda > 0)
    {
        ...
        for (unsigned char* i = ec.indices.begin(); i != ec.indices.end(); i++)
        {
            ...
            { tempstream << '\t' << fs.space_names[j].get()->first << '^' << fs.space_names[j].get()->second << ':' << ((fs.indicies[j] >> all.stride_shift) & all.parse_mask);
