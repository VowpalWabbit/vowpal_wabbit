...
inline void audit_regressor_interaction(audit_regressor_data &dat,
...
inline void audit_regressor_feature(audit_regressor_data &dat, const float,
  ...
  string ns_pre;
  for (vector<string>::const_iterator s = dat.ns_pre->begin();
       s != dat.ns_pre->end(); ++s)
    ns_pre += *s;

  ostringstream tempstream;
  tempstream << ':' << ((ft_idx & weights.mask()) >> weights.stride_shift())
             << ':' << weights[ft_idx];
...
void audit_regressor_lda(audit_regressor_data &rd,
                         LEARNER::single_learner & /* base */, example &ec) {
  vw &all = *rd.all;
  ...
  for (unsigned char *i = ec.indices.begin(); i != ec.indices.end(); i++) {
    ...
    for (size_t j = 0; j < fs.size(); ++j) {
      tempstream << '\t' << fs.space_names[j].get()->first << '^'
                 << fs.space_names[j].get()->second << ':'
                 << ((fs.indicies[j] >> weights.stride_shift()) &
                     all.parse_mask);
  ...
  rd.out_file->bin_write_fixed(tempstream.str().c_str(),
                               (uint32_t)tempstream.str().size());
}

// This is a learner which does nothing with examples.
// void learn(audit_regressor_data&, LEARNER::base_learner&, example&) {}

void audit_regressor(audit_regressor_data &rd, LEARNER::single_learner &base,
                     example &ec) {
  vw &all = *rd.all;
    ...
    while (rd.cur_class < rd.total_class_cnt) {
        ...
        if (fs.space_names.size() > 0)
          for (size_t j = 0; j < fs.size(); ++j) {
            ...
            audit_regressor_feature(rd, fs.values[j],
                                    (uint32_t)fs.indicies[j] + ec.ft_offset);
      ...
      if (rd.all->weights.sparse)
        INTERACTIONS::generate_interactions<
            audit_regressor_data, const uint64_t, audit_regressor_feature, true,
            audit_regressor_interaction, sparse_parameters>(
            rd.all->interactions, rd.all->permutations, ec, rd,
            rd.all->weights.sparse_weights);
      else
        INTERACTIONS::generate_interactions<
            audit_regressor_data, const uint64_t, audit_regressor_feature, true,
            audit_regressor_interaction, dense_parameters>(
            rd.all->interactions, rd.all->permutations, ec, rd,
            rd.all->weights.dense_weights);
...
inline void print_ex(vw &all, size_t ex_processed, size_t vals_found,
                     size_t progress) {
  all.trace_message << std::left << std::setw(shared_data::col_example_counter)
                    << ex_processed << " " << std::right << std::setw(9)
                    << vals_found << " " << std::right << std::setw(12)
                    << progress << '%' << std::endl;
}
...
void finish_example(vw &all, audit_regressor_data &dd, example &ec) {
  ...
  if (ec.example_counter + 1 >= all.sd->dump_interval && !all.quiet) {
    print_ex(all, ec.example_counter + 1, dd.values_audited,
             dd.values_audited * 100 / dd.loaded_regressor_values);
    all.sd->weighted_unlabeled_examples =
        (double)(ec.example_counter + 1); // used in update_dump_interval
...
void finish(audit_regressor_data &dat) {
  if (dat.values_audited < dat.loaded_regressor_values)
    dat.all->trace_message << "Note: for some reason audit couldn't find all "
                           ...
                           << dat.values_audited << " of "
                           << dat.loaded_regressor_values << " found)." << endl;
}

template <class T> void regressor_values(audit_regressor_data &dat, T &w) {
...
void init_driver(audit_regressor_data &dat) {
  ...
  // is called
  if ((dat.all->options->was_supplied("cache_file") ||
       dat.all->options->was_supplied("cache")) &&
      !dat.all->options->was_supplied("kill_cache")) {
    THROW("audit_regressor is incompatible with a cache file.  Use it in "
          "single pass mode only.");
  }

  dat.all->sd->dump_interval =
      1.; // regressor could initialize these if saved with --save_resume
  ...
  if (!dat.all->quiet) {
    dat.all->trace_message << "Regressor contains "
                           << dat.loaded_regressor_values << " values\n";
    dat.all->trace_message << std::left
                           << std::setw(shared_data::col_example_counter)
                           << "example"
                           << " " << std::setw(shared_data::col_example_weight)
                           << "values"
                           << " " << std::setw(shared_data::col_current_label)
                           << "total" << std::endl;
    dat.all->trace_message << std::left
                           << std::setw(shared_data::col_example_counter)
                           << "counter"
...
LEARNER::base_learner *audit_regressor_setup(options_i &options, vw &all) {
  ...
  new_options.add(make_option("audit_regressor", out_file)
                      .keep()
                      .help("stores feature names and their regressor values. "
                            "Same dataset must be used for both "
  ...
  dat->all = &all;
  dat->ns_pre =
      new vector<string>(); // explicitly invoking vector's constructor
  ...
  LEARNER::learner<audit_regressor_data, example> &ret =
      LEARNER::init_learner(dat, as_singleline(setup_base(options, all)),
                            audit_regressor, audit_regressor, 1);
