    ...
    while (rd.cur_class < rd.total_class_cnt)
    {
      ...
      if (rd.all->weights.sparse)
        INTERACTIONS::generate_interactions<audit_regressor_data, const uint64_t, audit_regressor_feature, true,
            ...
            rd.all->gs.interactions, rd.all->fc.permutations, ec, rd, rd.all->weights.sparse_weights);
      else
        INTERACTIONS::generate_interactions<audit_regressor_data, const uint64_t, audit_regressor_feature, true,
            audit_regressor_interaction, dense_parameters>(
            rd.all->gs.interactions, rd.all->fc.permutations, ec, rd, rd.all->weights.dense_weights);
...
{
  all.oc.trace_message << std::left << std::setw(shared_data::col_example_counter) << ex_processed << " " << std::right
...
void finish(audit_regressor_data& dat)
{
  if (dat.values_audited < dat.loaded_regressor_values)
    dat.all->oc.trace_message << "Note: for some reason audit couldn't find all regressor values in dataset ("
...
void init_driver(audit_regressor_data& dat)
{
  ...
  {
    dat.all->oc.trace_message << "Regressor contains " << dat.loaded_regressor_values << " values\n";
    dat.all->oc.trace_message << std::left << std::setw(shared_data::col_example_counter) << "example"
                           << " " << std::setw(shared_data::col_example_weight) << "values"
                           << " " << std::setw(shared_data::col_current_label) << "total" << std::endl;
    dat.all->oc.trace_message << std::left << std::setw(shared_data::col_example_counter) << "counter"
