...
struct bfgs {
  ...
  weight *regularizers;
  // the below needs to be included when resetting, in addition to
  // preconditioner and derivative
  int lastj, origin;
...
float predict_and_gradient(vw &all, example &ec) {
  ...
  float loss_grad =
      all.loss->first_derivative(all.sd, fp, ld.label) * ec.weight;
...
void update_preconditioner(vw &all, example &ec) {
  float curvature =
      all.loss->second_derivative(all.sd, ec.pred.scalar, ec.l.simple.label) *
      ec.weight;
...
template <class T>
double regularizer_direction_magnitude(vw & /* all */, bfgs &b,
                                       double regularizer, T &weights) {
  double ret = 0.;
  if (b.regularizers == nullptr)
    for (typename T::iterator iter = weights.begin(); iter != weights.end();
         ++iter)
    ...
    for (typename T::iterator iter = weights.begin(); iter != weights.end();
      ...
      ret +=
          ((double)
               b.regularizers[2 * (iter.index() >> weights.stride_shift())]) *
          (&(*iter))[W_DIR] * (&(*iter))[W_DIR];
  }
  return ret;
}
...
double regularizer_direction_magnitude(vw &all, bfgs &b, float regularizer) {
  ...
  if (all.weights.sparse)
    return regularizer_direction_magnitude(all, b, regularizer,
...
template <class T> float direction_magnitude(vw & /* all */, T &weights) {
  // compute direction magnitude
  double ret = 0.;
  for (typename T::iterator iter = weights.begin(); iter != weights.end();
       ++iter)
...
template <class T>
void bfgs_iter_start(vw &all, bfgs &b, float *mem, int &lastj,
                     double importance_weight_sum, int &origin, T &weights) {
  double g1_Hg1 = 0.;
  double g1_g1 = 0.;
...
void bfgs_iter_start(vw &all, bfgs &b, float *mem, int &lastj,
                     double importance_weight_sum, int &origin) {
  if (all.weights.sparse)
    bfgs_iter_start(all, b, mem, lastj, importance_weight_sum, origin,
                    all.weights.sparse_weights);
  else
    bfgs_iter_start(all, b, mem, lastj, importance_weight_sum, origin,
                    all.weights.dense_weights);
...
void bfgs_iter_middle(vw &all, bfgs &b, float *mem, double *rho, double *alpha,
                      int &lastj, int &origin, T &weights) {
  float *mem0 = mem;
  ...
  if (b.m == 0) {
    ...
    for (typename T::iterator w = weights.begin(); w != weights.end(); ++w) {
      ...
      g_Hg += ((double)mem[(MEM_GT + origin) % b.mem_stride]) *
              ((&(*w))[W_COND]) * mem[(MEM_GT + origin) % b.mem_stride];
    }
  ...
  for (typename T::iterator w = weights.begin(); w != weights.end(); ++w) {
    float *mem1 = mem + (w.index() >> weights.stride_shift()) * b.mem_stride;
    mem1[(MEM_YT + origin) % b.mem_stride] =
        (&(*w))[W_GT] - mem1[(MEM_GT + origin) % b.mem_stride];
    mem1[(MEM_ST + origin) % b.mem_stride] =
        (&(*w))[W_XT] - mem1[(MEM_XT + origin) % b.mem_stride];
    (&(*w))[W_DIR] = (&(*w))[W_GT];
    y_s += ((double)mem1[(MEM_YT + origin) % b.mem_stride]) *
           mem1[(MEM_ST + origin) % b.mem_stride];
    y_Hy += ((double)mem1[(MEM_YT + origin) % b.mem_stride]) *
            mem1[(MEM_YT + origin) % b.mem_stride] * ((&(*w))[W_COND]);
  ...
  for (int j = 0; j < lastj; j++) {
    ...
    for (typename T::iterator w = weights.begin(); w != weights.end(); ++w) {
      mem = mem0 + (w.index() >> weights.stride_shift()) * b.mem_stride;
      (&(*w))[W_DIR] -=
          (float)alpha[j] * mem[(2 * j + MEM_YT + origin) % b.mem_stride];
      s_q += ((double)mem[(2 * j + 2 + MEM_ST + origin) % b.mem_stride]) *
             ((&(*w))[W_DIR]);
    }
  }
  ...
  for (typename T::iterator w = weights.begin(); w != weights.end(); ++w) {
    mem = mem0 + (w.index() >> weights.stride_shift()) * b.mem_stride;
    (&(*w))[W_DIR] -=
        (float)alpha[lastj] * mem[(2 * lastj + MEM_YT + origin) % b.mem_stride];
    (&(*w))[W_DIR] *= gamma * ((&(*w))[W_COND]);
    y_r += ((double)mem[(2 * lastj + MEM_YT + origin) % b.mem_stride]) *
           ((&(*w))[W_DIR]);
  }
  ...
  for (int j = lastj; j > 0; j--) {
    ...
    for (typename T::iterator w = weights.begin(); w != weights.end(); ++w) {
      mem = mem0 + (w.index() >> weights.stride_shift()) * b.mem_stride;
      (&(*w))[W_DIR] +=
          (float)coef_j * mem[(2 * j + MEM_ST + origin) % b.mem_stride];
      y_r += ((double)mem[(2 * j - 2 + MEM_YT + origin) % b.mem_stride]) *
             ((&(*w))[W_DIR]);
    }
  }
...
void bfgs_iter_middle(vw &all, bfgs &b, float *mem, double *rho, double *alpha,
  ...
  if (all.weights.sparse)
    bfgs_iter_middle(all, b, mem, rho, alpha, lastj, origin,
                     all.weights.sparse_weights);
  else
    bfgs_iter_middle(all, b, mem, rho, alpha, lastj, origin,
                     all.weights.dense_weights);
...
template <class T>
double wolfe_eval(vw &all, bfgs &b, float *mem, double loss_sum,
                  double previous_loss_sum, double step_size,
  ...
  if (!all.quiet)
    fprintf(stderr, "%-10.5f\t%-10.5f\t%s%-10f\t%-10f\t",
            g1_g1 / (importance_weight_sum * importance_weight_sum),
...
double wolfe_eval(vw &all, bfgs &b, float *mem, double loss_sum,
                  ...
                  double importance_weight_sum, int &origin, double &wolfe1) {
  if (all.weights.sparse)
    return wolfe_eval(all, b, mem, loss_sum, previous_loss_sum, step_size,
                      importance_weight_sum, origin, wolfe1,
                      all.weights.sparse_weights);
  else
    return wolfe_eval(all, b, mem, loss_sum, previous_loss_sum, step_size,
                      importance_weight_sum, origin, wolfe1,
...
double add_regularization(vw &all, bfgs &b, float regularization, T &weights) {
  ...
  // if we're not regularizing the intercept term, then subtract it off from the
  // result above
  // when accessing weights[constant], always use
  // weights.strided_index(constant)
  if (all.no_bias) {
    if (b.regularizers == nullptr) {
      (&weights.strided_index(constant))[W_GT] -=
          regularization * (weights.strided_index(constant));
      ret -= 0.5 * regularization * (weights.strided_index(constant)) *
             (weights.strided_index(constant));
    } else {
      uint64_t i = constant >> weights.stride_shift();
      weight delta_weight =
          (weights.strided_index(constant)) - b.regularizers[2 * i + 1];
      (&weights.strided_index(constant))[W_GT] -=
          b.regularizers[2 * i] * delta_weight;
      ret -= 0.5 * b.regularizers[2 * i] * delta_weight * delta_weight;
    }
  }
...
double add_regularization(vw &all, bfgs &b, float regularization) {
  if (all.weights.sparse)
    return add_regularization(all, b, regularization,
                              all.weights.sparse_weights);
  else
    return add_regularization(all, b, regularization,
                              all.weights.dense_weights);
...
void finalize_preconditioner(vw & /* all */, bfgs &b, float regularization,
  ...
  if (b.regularizers == nullptr)
    ...
    for (typename T::iterator w = weights.begin(); w != weights.end(); ++w) {
      (&(*w))[W_COND] +=
          b.regularizers[2 * (w.index() >> weights.stride_shift())];
      if ((&(*w))[W_COND] > max_hessian)
        max_hessian = (&(*w))[W_COND];
      if ((&(*w))[W_COND] > 0)
        (&(*w))[W_COND] = 1.f / (&(*w))[W_COND];
  ...
  float max_precond =
      (max_hessian == 0.f) ? 0.f : max_precond_ratio / max_hessian;
...
void preconditioner_to_regularizer(vw &all, bfgs &b, float regularization) {
  if (all.weights.sparse)
    preconditioner_to_regularizer(all, b, regularization,
                                  all.weights.sparse_weights);
  else
    preconditioner_to_regularizer(all, b, regularization,
                                  all.weights.dense_weights);
...
double derivative_in_direction(vw &all, bfgs &b, float *mem, int &origin) {
  if (all.weights.sparse)
    return derivative_in_direction(all, b, mem, origin,
                                   all.weights.sparse_weights);
  else
    return derivative_in_direction(all, b, mem, origin,
                                   all.weights.dense_weights);
...
template <class T> void update_weight(vw & /* all */, float step_size, T &w) {
...
int process_pass(vw &all, bfgs &b) {
  ...
  if (b.first_pass) {
    ...
    if (!all.quiet)
      fprintf(stderr, "%2lu %-10.5f\t", (long unsigned int)b.current_pass + 1,
    ...
    if (b.first_hessian_on) {
      ...
      b.net_time =
          (int)(1000.0 * (b.t_end_global.time - b.t_start_global.time) +
      ...
      if (b.gradient_pass) // We just finished computing all gradients
  {
    ...
    if (!all.quiet) {
      if (!all.holdout_set_off && b.current_pass >= 1) {
        if (all.sd->holdout_sum_loss_since_last_pass == 0. &&
          ...
          fprintf(stderr, "%2lu h%-10.5f\t",
                  (long unsigned int)b.current_pass + 1,
                  all.sd->holdout_sum_loss_since_last_pass /
                      all.sd->weighted_holdout_examples_since_last_pass);
      } else
        fprintf(stderr, "%2lu %-10.5f\t", (long unsigned int)b.current_pass + 1,
                b.loss_sum / b.importance_weight_sum);
    }
    double wolfe1;
    double new_step =
        wolfe_eval(all, b, b.mem, b.loss_sum, b.previous_loss_sum, b.step_size,
    ...
    /********************************************************************/
    else if (b.backstep_on &&
             (wolfe1 < b.wolfe1_bound || b.loss_sum > b.previous_loss_sum)) {
      ...
      b.net_time =
          (int)(1000.0 * (b.t_end_global.time - b.t_start_global.time) +
                (b.t_end_global.millitm - b.t_start_global.millitm));
      float ratio =
          (b.step_size == 0.f) ? 0.f : (float)new_step / (float)b.step_size;
      if (!all.quiet)
    ...
    /********************************************************************/
      ...
      double rel_decrease =
          (b.previous_loss_sum - b.loss_sum) / b.previous_loss_sum;
      if (!nanpattern((float)rel_decrease) && b.backstep_on &&
          fabs(rel_decrease) < b.rel_threshold) {
        fprintf(stdout, "\nTermination condition reached in pass %ld: decrease "
                        "in loss less than %.3f%%.\n"
                        "If you want to optimize further, decrease termination "
                        "threshold.\n",
...
void process_example(vw &all, bfgs &b, example &ec) {
  ...
  else // computing curvature
  {
    ...
    if (b.example_number >=
        b.predictions
            .size()) // Make things safe in case example source is strange.
    ...
    ec.loss = all.loss->getLoss(all.sd, ec.pred.scalar, ld.label) * ec.weight;
    float sd = all.loss->second_derivative(
        all.sd, b.predictions[b.example_number++], ld.label);
    ...
    if (b.current_pass < b.final_pass) {
      ...
      if (b.final_pass == b.current_pass) {
        ...
        if (!b.output_regularizer)
          b.all->trace_message << "If you want to optimize further, increase "
                                  "the number of passes\n";
        if (b.output_regularizer) {
          b.all->trace_message << "\nRegular model file has been created. ";
          b.all->trace_message
              << "Output feature regularizer file is created only when the "
      ...
      if (status != LEARN_OK && b.final_pass > b.current_pass) {
        ...
        // Not converged yet.
        // Reset preconditioner to zero so that it is correctly recomputed in
        // the next pass
        zero_preconditioner(*all);
      }
...
// placeholder
template <bool audit> void predict(bfgs &b, base_learner &, example &ec) {
  vw *all = b.all;
...
template <bool audit> void learn(bfgs &b, base_learner &base, example &ec) {
  vw *all = b.all;
  assert(ec.in_use);
...
void save_load_regularizer(vw &all, bfgs &b, io_buf &model_file, bool read,
                           bool text) {
  int c = 0;
  ...
  do {
    ...
    } else // write binary or text
    {
      ...
      if (*v != 0.) {
        ...
        msg << i;
        brw =
            bin_text_write_fixed(model_file, (char *)&i, sizeof(i), msg, text);

        msg << ":" << *v << "\n";
        brw +=
            bin_text_write_fixed(model_file, (char *)v, sizeof(*v), msg, text);
      }
    }
    if (!read)
      i++;
...
void save_load(bfgs &b, io_buf &model_file, bool read, bool text) {
  ...
  if (read) {
    ...
    if (!all->quiet)
      cerr << "m = " << m << endl
           << "Allocated " << ((long unsigned int)all->length() *
                                   (sizeof(float) * (b.mem_stride) +
                                    (sizeof(weight) << stride_shift)) >>
                               20)
    ...
    if (!all->quiet) {
      ...
      fprintf(stderr, header_fmt, "##", "avg. loss", "der. mag.", "d. m. cond.",
              "wolfe1", "wolfe2", "mix fraction", "curvature", "dir. magnitude",
              "step size");
      cerr.precision(5);
    }
    ...
    b.output_regularizer = (all->per_feature_regularizer_output != "" ||
                            all->per_feature_regularizer_text != "");
    reset_state(*all, b, false);
  }

  // bool reg_vector = b.output_regularizer ||
  // all->per_feature_regularizer_input.length() > 0;
  bool reg_vector = (b.output_regularizer && !read) ||
                    (all->per_feature_regularizer_input.length() > 0 && read);

  if (model_file.files.size() > 0) {
    stringstream msg;
    msg << ":" << reg_vector << "\n";
    bin_text_read_write_fixed(model_file, (char *)&reg_vector,
                              sizeof(reg_vector), "", read, msg, text);
...
base_learner *bfgs_setup(options_i &options, vw &all) {
  ...
  bfgs_outer_options.add(
      make_option("conjugate_gradient", conjugate_gradient)
          .keep()
  ...
  bfgs_inner_options.add(
      make_option("bfgs", bfgs_option)
          .keep()
          .help("use conjugate gradient based optimization"));
  bfgs_inner_options.add(make_option("hessian_on", all.hessian_on)
                             .help("use second derivative in line search"));
  bfgs_inner_options.add(
      make_option("mem", b->m).default_value(15).help("memory in bfgs"));
  bfgs_inner_options.add(make_option("termination", b->rel_threshold)
