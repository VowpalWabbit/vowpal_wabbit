...
struct label_parser {
  ...
  void (*copy_label)(void *, void *); // copy_label(dst,src) performs a DEEP
                                      // copy of src into dst (dst is allocated
  // correctly).  if this function is nullptr, then we assume that a memcpy of
  // size
  // label_size is sufficient, so you need only specify this function if your
  // label
