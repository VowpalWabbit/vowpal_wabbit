...
{
  return all.parser_runtime.example_parser->hasher(s.data(), s.length(), static_cast<uint32_t>(u)) &
...
{
  return all.parser_runtime.example_parser->hasher(fstr, strlen(fstr), static_cast<uint32_t>(u)) &
      all.runtime_state.parse_mask;
}
...
inline uint64_t chain_hash(VW::workspace& all, const std::string& name, const std::string& value, uint64_t u)
{
  ...
  return all.parser_runtime.example_parser->hasher(value.data(), value.length(),
             all.parser_runtime.example_parser->hasher(name.data(), name.length(), static_cast<uint32_t>(u))) &
      all.runtime_state.parse_mask;
}
...
{
  // chain hash is hash(feature_value, hash(feature_name, namespace_hash)) & parse_mask
  return hash_func(value.data(), value.length(), hash_func(name.data(), name.length(), static_cast<uint32_t>(u))) &
      parse_mask;
}
