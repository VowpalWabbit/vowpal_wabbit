...
inline void read_line(VW::workspace& all, example* ex, VW::string_view line) { VW::parsers::text::read_line(all, ex, line); }

VW_DEPRECATED("read_line moved to VW::parsers::text::read_line and should be used with a string_view")
inline void read_line(VW::workspace& all, example* ex, const char* line) { VW::parsers::text::read_line(all, ex, VW::string_view(line)); }
