...
bool weights_offset_test(cb_sim&, VW::workspace& all, VW::multi_ex&)
{
  ...
  VW::reductions::multi_model::clear_innermost_offset(
      weights, offset_to_clear, all.reduction_state.wpp, all.reduction_state.wpp);
  ...
  VW::reductions::multi_model::move_innermost_offsets(
      weights, offset_to_clear + 1, offset_to_clear, all.reduction_state.wpp, all.reduction_state.wpp);
...
bool all_weights_equal_test(cb_sim&, VW::workspace& all, VW::multi_ex&)
{
  ...
  for (auto iter = weights.begin(); iter != weights.end(); ++iter)
  {
    ...
    size_t current_offset = (iter.index() >> weights.stride_shift()) & (all.reduction_state.wpp - 1);
    if (current_offset == 0)
    {
      float* first_weight = &weights.first()[(prestride_index + 0) << weights.stride_shift()];
      uint32_t till = 1;  // instead of all.reduction_state.wpp, champdupe only uses 3 configs
