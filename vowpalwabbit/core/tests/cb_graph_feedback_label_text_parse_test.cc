...
void parse_cb_label(VW::string_view label, CB::label& l, VW::reduction_features& red_features)
{
  ...
  CB::parse_label(l, red_features, mem, words, null_logger);
}
...
TEST(cb_gf_label_tests, cb_graph_parse_label)
{
  ...
  parse_cb_label("shared graph 0,1,2 1,2,3 5,5,5 | s_1 s_2", label, red_features);
...
TEST(cb_gf_label_tests, throws_w_graph_in_label)
{
  ...
  VW::reduction_features red_features;
  EXPECT_THROW(
      parse_cb_label("0:1.0:0.5 graph 0,1,2 0,1,2 5,5,5  | a_1 b_1 c_1", label, red_features), VW::vw_exception);
}
