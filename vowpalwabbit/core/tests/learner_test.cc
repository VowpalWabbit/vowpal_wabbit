...
TEST(LearnerEdgeCases, SinglelineLearnerWithSinglelineExample)
{
  // Test singleline learner with singleline example works
  auto vw = VW::initialize(vwtest::make_args("--quiet"));
  auto* ex = VW::read_example(*vw, "1 | a b c");
  vw->learn(*ex);
  VW::finish_example(*vw, *ex);
...
TEST(LearnerEdgeCases, MultilineLearnerWithMultilineExample)
{
  // Test multiline learner with multiline example works
  auto vw = VW::initialize(vwtest::make_args("--cb_explore_adf", "--quiet"));

  VW::multi_ex examples;
  examples.push_back(VW::read_example(*vw, "shared | s"));
  examples.push_back(VW::read_example(*vw, "0:1:0.5 | a"));
  examples.push_back(VW::read_example(*vw, "| b"));

  vw->learn(examples);
  vw->finish_example(examples);
}

TEST(LearnerEdgeCases, LearnerHasName)
{
  auto vw = VW::initialize(vwtest::make_args("--quiet"));
  // All learners should have a name
  EXPECT_FALSE(vw->l->get_name().empty());
}

TEST(LearnerEdgeCases, LearnerMergeUnsupported)
{
  // Most learners don't support merge
  auto vw = VW::initialize(vwtest::make_args("--quiet"));
  // merge() might throw if not supported - that's the expected behavior
}

TEST(LearnerEdgeCases, LearnerSaveLoad)
{
  // Test save/load through model file
  auto vw = VW::initialize(vwtest::make_args("--quiet"));
  ...
  // Save to file
  VW::save_predictor(*vw, "test_learner_saveload.model");

  // Load from file
  auto vw2 = VW::initialize(vwtest::make_args("--quiet", "-i", "test_learner_saveload.model"));
  ...
  // Cleanup
  std::remove("test_learner_saveload.model");
}

TEST(LearnerEdgeCases, LearnerIsMultiline)
{
  // Test singleline learner
  {
    auto vw = VW::initialize(vwtest::make_args("--quiet"));
    EXPECT_FALSE(vw->l->is_multiline());
  }

  // Test multiline learner
  {
    auto vw = VW::initialize(vwtest::make_args("--cb_explore_adf", "--quiet"));
    EXPECT_TRUE(vw->l->is_multiline());
  }
...
TEST(LearnerEdgeCases, LearnerPredictionType)
{
  // Different learners have different prediction types
  {
    auto vw = VW::initialize(vwtest::make_args("--quiet"));
    EXPECT_EQ(vw->l->get_output_prediction_type(), VW::prediction_type_t::SCALAR);
  }

  {
    auto vw = VW::initialize(vwtest::make_args("--oaa", "3", "--quiet"));
    EXPECT_EQ(vw->l->get_output_prediction_type(), VW::prediction_type_t::MULTICLASS);
  }
}

TEST(LearnerEdgeCases, LearnerLabelType)
{
  // Different learners have different label types
  {
    auto vw = VW::initialize(vwtest::make_args("--quiet"));
    EXPECT_EQ(vw->l->get_input_label_type(), VW::label_type_t::SIMPLE);
  }

  {
    auto vw = VW::initialize(vwtest::make_args("--oaa", "3", "--quiet"));
    EXPECT_EQ(vw->l->get_input_label_type(), VW::label_type_t::MULTICLASS);
  }

  {
    auto vw = VW::initialize(vwtest::make_args("--cb", "3", "--quiet"));
    EXPECT_EQ(vw->l->get_input_label_type(), VW::label_type_t::CB);
  }
}

TEST(LearnerEdgeCases, MultiplePassesAllowed)
{
  auto vw = VW::initialize(vwtest::make_args("--quiet", "--passes", "2", "--holdout_off", "--cache_file", "test_cache.cache"));
  ...
  // Cleanup
  std::remove("test_cache.cache");
}

TEST(LearnerEdgeCases, EmptyExample)
{
  auto vw = VW::initialize(vwtest::make_args("--quiet"));

  // Example with no features
  auto* ex = VW::read_example(*vw, "1 |");
  vw->learn(*ex);
  VW::finish_example(*vw, *ex);
}

TEST(LearnerEdgeCases, LargeNumberOfFeatures)
{
  auto vw = VW::initialize(vwtest::make_args("--quiet"));

  // Example with many features
  std::string example_str = "1 |";
  for (int i = 0; i < 100; i++) { example_str += " f" + std::to_string(i); }
