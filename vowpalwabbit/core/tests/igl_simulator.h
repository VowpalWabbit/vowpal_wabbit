...
public:
  ...
  std::vector<float> run_simulation_hook(VW::workspace* igl_vw, size_t num_iterations,
      simulator::callback_map& callbacks, bool do_learn = true);
  ...
  std::string to_dsjson_format(const std::map<std::string, std::string>& context,
      const std::string& chosen_action="", float prob=0.f, std::string feedback="", VW::action_scores a_s={});
};
}