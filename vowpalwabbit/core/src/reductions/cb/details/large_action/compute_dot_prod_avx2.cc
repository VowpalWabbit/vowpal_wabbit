...
float compute_dot_prod_avx2(uint64_t column_index, VW::workspace* _all, uint64_t seed, VW::example* ex)
{
  ...
  const bool ignore_some_linear = _all->fc.ignore_some_linear;
  const auto& ignore_linear = _all->fc.ignore_linear;
  ...
  for (const auto& ns : interactions)
  {
    ...
    const bool same_namespace = (!_all->fc.permutations && (ns[0] == ns[1]));
