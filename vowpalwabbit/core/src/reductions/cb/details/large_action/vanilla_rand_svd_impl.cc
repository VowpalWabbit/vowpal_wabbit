...
bool vanilla_rand_svd_impl::generate_Y(const multi_ex& examples, const std::vector<float>& shrink_factors)
{
  ...
  for (auto* ex : examples)
  {
    ...
    for (uint64_t col = 0; col < _d; col++)
    {
      ...
      {
        Y_triplet_constructor tc(
            _all->weights.sparse_weights.mask(), row_index, col, _seed, _triplets, max_non_zero_col, shrink_factors);
      ...
      {
        Y_triplet_constructor tc(
            _all->weights.dense_weights.mask(), row_index, col, _seed, _triplets, max_non_zero_col, shrink_factors);
  ...
  return static_cast<uint64_t>(Y.rows()) >= _d;
}
