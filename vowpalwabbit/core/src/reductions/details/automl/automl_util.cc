...
bool count_namespaces(const multi_ex& ecs, std::map<namespace_index, uint64_t>& ns_counter, std::set<uint64_t>& index_set)
{
  ...
  for (const example* ex : ecs)
  {
    ...
    {
      for (auto index : ex->feature_space[ns].indices)
      {
