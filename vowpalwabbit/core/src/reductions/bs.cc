...
void output_example_prediction_bs(
    VW::workspace& all, const bootstrap_data& data, const VW::example& ec, VW::io::logger& logger)
{
...
base_learner* VW::reductions::bs_setup(VW::setup_base_i& stack_builder)
{
  ...
  auto* l = make_reduction_learner(std::move(bs_data), as_singleline(stack_builder.setup_base_learner()),
      predict_or_learn<true>, predict_or_learn<false>, stack_builder.get_setupfn_name(bs_setup))
                ...
                .set_update_stats(VW::details::update_stats_simple_label<bootstrap_data>)
                .set_print_update(VW::details::print_update_simple_label<bootstrap_data>)
