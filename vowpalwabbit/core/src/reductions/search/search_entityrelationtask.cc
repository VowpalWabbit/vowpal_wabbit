...
void initialize(Search::search& sch, size_t& /*num_actions*/, options_i& options)
{
  ...
  else
  {
    ...
    for (size_t a = 0; a < NUM_LDF_ENTITY_EXAMPLES; a++)
    {
      ...
      my_task_data->ldf_entity[a].interactions = &sch.get_vw_pointer_unsafe().fc.interactions;
      my_task_data->ldf_entity[a].extent_interactions = &sch.get_vw_pointer_unsafe().fc.extent_interactions;
    }
