...
base_learner* VW::reductions::gd_setup(VW::setup_base_i& stack_builder)
{
  ...
  else { stride = GD::set_learn<false>(all, feature_mask_off, *gd_data.get()); }

  all.weights.stride_shift(static_cast<uint32_t>(GD::ceil_log_2(stride - 1)));

  auto* bare = gd_data.get();
  learner<GD::gd, VW::example>* l =
      make_base_learner(std::move(gd_data), gd_data->learn, bare->predict, stack_builder.get_setupfn_name(gd_setup),
