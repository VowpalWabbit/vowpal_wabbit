...
void coin_betting_predict(ftrl& b, base_learner&, VW::example& ec)
{
  ...
  b.data.normalized_sum_norm_x += (static_cast<double>(ec.weight)) * b.data.normalized_squared_norm_x;
  b.total_weight += ec.weight;
  b.data.average_squared_norm_x = (static_cast<float>((b.data.normalized_sum_norm_x + 1e-6) / b.total_weight));
  ...
  if (model_file.num_files() != 0)
  {
    ...
    if (resume)
    {
      GD::save_load_online_state(
          *all, model_file, read, text, b.total_weight, b.data.normalized_sum_norm_x, nullptr, b.ftrl_size);
    }
    else
    {
...
base_learner* VW::reductions::ftrl_setup(VW::setup_base_i& stack_builder)
{
  ...
  b->all = &all;
  b->no_win_counter = 0;
  b->data.normalized_sum_norm_x = 0;
  b->total_weight = 0;
