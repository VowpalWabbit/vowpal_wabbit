...
epsilon_decay_data::epsilon_decay_data(uint64_t model_count, uint64_t min_scope,
    ...
    float initial_epsilon, uint64_t shift_model_bounds, bool reward_as_cost, double tol_x)
    ...
    , _reward_as_cost(reward_as_cost)
{
  ...
  for (uint64_t i = 0; i < model_count; ++i)
  {
    ...
    for (uint64_t j = 0; j < i + 1; ++j) { conf_seq_estimators.back().emplace_back(epsilon_decay_significance_level, tol_x); }
  }
}
