...
void print_label_pred(VW::workspace& all, const VW::example& ec, uint32_t prediction)
{
  ...
  all.sd->print_update(*all.trace_message, all.pc.holdout_set_off, all.pc.current_pass,
...
void print_probability(VW::workspace& all, const VW::example& ec, uint32_t prediction)
{
  ...
  all.sd->print_update(*all.trace_message, all.pc.holdout_set_off, all.pc.current_pass, label_ss.str(), pred_ss.str(),
      ec.get_num_features());
}
...
{
  all.sd->print_update(*all.trace_message, all.pc.holdout_set_off, all.pc.current_pass, ec.l.multi.label, prediction,
      ec.get_num_features());
}
