...
double countable_discrete_base::get_log_weight(double) { THROW("Unimplemented"); }

double countable_discrete_base::get_log_remaining_weight(double) { THROW("Unimplemented"); }
...
double countable_discrete_base::log_sum_exp(std::vector<double>)
{
    // TODO: Find cpp equivalent
    return 0.0;
}
...
robust_mixture::robust_mixture(double eta, double r, double k) : gt(k)
{
    ...
    this->eta = eta;
    this->r = r;
    //this->scale_fac = 1/2 * (1 + mpmath.fp.polylog(r, eta) / (eta * mpmath.fp.zeta(r)));
