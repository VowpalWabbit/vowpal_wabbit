...
namespace VW
{
void lower_cs_base::add_obs(double) { THROW("Unimplemented"); }

double lower_cs_base::get_ci(double) { THROW("Unimplemented"); }
...
double countable_discrete_base::get_log_weight(double)
{
    THROW("Unimplemented");
}

double countable_discrete_base::get_log_remaining_weight(double)
{
    THROW("Unimplemented");
}
...
double countable_discrete_base::get_v_impl(std::map<uint64_t, double>& memo, uint64_t j)
{
    ...
    {
        memo[j] = get_v(get_lam_sqrt_tp1(j));
    }
    return memo[j];
}
...
double countable_discrete_base::log_wealth_mix(double mu, double s, double thres, std::map<uint64_t, double>& memo)
{
    ...
    while(true)
    {
        ...
        for (size_t i = 0; i < log_es.size() - 1; ++i)
        {
            max_of_begin = std::max(max_of_begin, log_es[i]);
        }
...
double countable_discrete_base::get_s()
{
    THROW("Unimplemented");
}

double countable_discrete_base::get_v(double)
{
    THROW("Unimplemented");
}
...
double g_tilde::histo_variance(std::map<std::pair<uint64_t, bool>, double>& hist, double lam_sqrt_tp1)
{
    ...
    for (const std::pair<std::pair<uint64_t, bool>, double>& hist_p : hist)
    {
        ...
        if (strong_term)
        {
            curr_val = std::pow(lam_sqrt_tp1 * (k - 1) * x_ds / (1 + lam_sqrt_tp1 * k * x_ds), 2);
        }
        else
        {
...
void g_tilde::histo_insert(std::map<std::pair<uint64_t, bool>, double>& hist, double x)
{
    ...
    hist[std::make_pair(n, false)] = hist.count(std::make_pair(n, false)) ? hist[std::make_pair(n, false)] + alpha : alpha;
    hist[std::make_pair(n + 1, false)] = hist.count(std::make_pair(n + 1, false)) ? hist[std::make_pair(n + 1, false)] + 1 - alpha : 1 - alpha;
    hist[std::make_pair(n, true)] = hist.count(std::make_pair(n, true)) ? hist[std::make_pair(n, true)] + -0.5 * alpha * (1 - alpha) : -0.5 * alpha * (1 - alpha);
}
...
robust_mixture::robust_mixture(double eta, double r, double k) : gt(k)
{
...
double robust_mixture::polylog(double r, double eta)
{
    double ret_val = 0.0;
...
double robust_mixture::get_log_weight(double j)
{
    return log_scale_fac + log_xi_m1 - (1 + j) * log_xi;
}

double robust_mixture::get_log_remaining_weight(double j)
{
    return log_scale_fac - j * log_xi;
}

double robust_mixture::get_s()
{
    return gt.get_s();
}

double robust_mixture::get_v(double lam_sqrt_tp1)
{
    return gt.get_v(lam_sqrt_tp1);
}

void robust_mixture::add_obs(double x)
{
    t += 1;
    add_obs(x);
}
