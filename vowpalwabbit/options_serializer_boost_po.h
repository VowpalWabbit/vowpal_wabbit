...
private:
  template <typename T> bool serialize_if_t(base_option &base_option) {
  ...
  template <typename T> void serialize(typed_option<T> &typed_option) {
  ...
  template <typename TTypes> void add_impl(base_option &options) {
...
template <>
void options_serializer_boost_po::serialize<bool>(
    typed_option<bool> &typed_argument);
