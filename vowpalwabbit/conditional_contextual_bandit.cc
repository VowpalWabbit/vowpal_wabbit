...
void clear_all(ccb &data) {
  // data.include_list and data.exclude_list aren't cleared here but are
  // assigned in the predict/learn function
  data.shared = nullptr;
  data.actions.clear();
  data.slots.clear();
...
// split the slots, the actions and the shared example from the multiline
// example
bool split_multi_example_and_stash_labels(const multi_ex &examples, ccb &data) {
  for (auto ex : examples) {
    ...
    data.stored_labels.push_back(
        {ex->l.conditional_contextual_bandit.type,
         ex->l.conditional_contextual_bandit.outcome,
  ...
  if (is_learn) {
    for (auto slot : data.slots) {
      if (slot->l.conditional_contextual_bandit.outcome != nullptr &&
          slot->l.conditional_contextual_bandit.outcome->probabilities.size() ==
              0) {
        std::cerr << "ccb_adf_explore: badly formatted example - missing label "
                     "probability";
        return false;
      }
    }
...
void attach_label_to_example(uint32_t action_index_one_based, example *example,
...
void clear_pred_and_label(ccb &data) {
  // Don't need to return to pool, as that will be done when the example is
  // output.
...
// This function intentionally does not handle increasing the num_features of
// the example because
// the output_example function has special logic to ensure the number of feaures
...
void inject_slot_features(example *shared, example *slot) {
  for (auto index : slot->indices) {
    // constant namespace should be ignored, as it already exists and we don't
    // want to double it up.
    if (index == constant_namespace) {
      continue;
    } else if (index == default_namespace) // slot default namespace has a
                                           // special namespace in shared
    {
      LabelDict::add_example_namespace(*shared, ccb_slot_namespace,
                                       slot->feature_space[default_namespace]);
    } else {
      LabelDict::add_example_namespace(*shared, index,
                                       slot->feature_space[index]);
    }
  }
}
...
void inject_slot_id(ccb &data, example *shared, size_t id) {
  ...
  if (data.slot_id_hashes[id] == 0) {
    ...
    index =
        VW::hash_feature(*data.all, current_index_str, data.id_namespace_hash);

    // To maintain indicies consistent with what the parser does we must scale.
    index *= static_cast<uint64_t>(data.all->wpp)
             << data.base_learner_stride_shift;
  ...
  if (audit) {
    ...
    shared->feature_space[ccb_id_namespace].space_names.push_back(
        audit_strings_ptr(
            new audit_strings(data.id_namespace_str, current_index_str)));
  }
}

// Since the slot id is the only thing in this namespace, the popping the value
// off will work correctly.
template <bool audit> void remove_slot_id(example *shared) {
...
void remove_slot_features(example *shared, example *slot) {
  for (auto index : slot->indices) {
    ...
    else if (index == default_namespace) // slot default namespace has a special
                                         // namespace in shared
    {
      LabelDict::del_example_namespace(*shared, ccb_slot_namespace,
                                       slot->feature_space[32]);
    } else {
      LabelDict::del_example_namespace(*shared, index,
                                       slot->feature_space[index]);
    }
  }
}

// Generates quadratics between each namespace and the slot id as well as
// appends slot id to every existing interaction.
void calculate_and_insert_interactions(
    example *shared, std::vector<example *> actions,
    std::vector<std::string> &generated_interactions) {
  ...
  for (const auto &action : actions) {
    for (const auto &action_index : action->indices) {
      if (INTERACTIONS::is_printable_namespace(action_index) &&
        ...
        generated_interactions.push_back(
            {(char)action_index, (char)ccb_id_namespace});
      }
    }
  }
  ...
  for (const auto &shared_index : shared->indices) {
    if (INTERACTIONS::is_printable_namespace(shared_index) &&
      ...
      generated_interactions.push_back(
          {(char)shared_index, (char)ccb_id_namespace});
    }
  }
}
...
void build_cb_example(multi_ex &cb_ex, example *slot, ccb &data) {
  bool slot_has_label =
      slot->l.conditional_contextual_bandit.outcome != nullptr;

  // Merge the slot features with the shared example and set it in the cb
  // multi-example
  // TODO is it imporant for total_sum_feat_sq and num_features to be correct at
  // this point?
  inject_slot_features(data.shared, slot);
  cb_ex.push_back(data.shared);

  // Retrieve the action index whitelist (if the list is empty, then all actions
  // are white-listed)
  auto &explicit_includes =
      slot->l.conditional_contextual_bandit.explicit_included_actions;
  if (explicit_includes.size() != 0) {
    // First time seeing this, initialize the vector with falses so we can start
    // setting each included action.
  ...
  for (size_t i = 0; i < data.actions.size(); i++) {
    // Filter actions that are not explicitly included. If the list is empty
    // though, everything is included.
    ...
    if (is_learn && slot_has_label &&
        i ==
            slot->l.conditional_contextual_bandit.outcome->probabilities[0]
                .action) {
      // This is used to remove the label later.
      data.action_with_label = (uint32_t)i;
      attach_label_to_example(index, data.actions[i],
                              slot->l.conditional_contextual_bandit.outcome,
                              data);
    }
  }

  // Must provide a prediction that cb can write into, this will be saved into
  // the decision scores object later.
  data.shared->pred.a_s = data.action_score_pool.get_object();

  // Tag can be used for specifying the sampling seed per slot. For it to be
  // used it must be inserted into the shared
  // example.
  std::swap(data.shared->tag, slot->tag);
}

// iterate over slots contained in the multi-example, and for each slot, build a
// cb example and perform a
// cb_explore_adf call.
template <bool is_learn>
void learn_or_predict(ccb &data, multi_learner &base, multi_ex &examples) {
  clear_all(data);
  if (!split_multi_example_and_stash_labels(
          examples, data)) // split shared, actions and slots
    return;
  ...
  for (example *slot : data.slots) {
    ...
    std::copy(data.original_interactions->begin(),
              data.original_interactions->end(),
              std::back_inserter(data.generated_interactions));
    calculate_and_insert_interactions(data.shared, data.actions,
    ...
    if (has_action(data.cb_ex)) {
      ...
      multiline_learn_or_predict<is_learn>(base, data.cb_ex,
  ...
  for (size_t i = 0; i < examples.size(); i++) {
    examples[i]->l.conditional_contextual_bandit = {
        data.stored_labels[i].type, data.stored_labels[i].outcome,
...
void print_update(vw &all, std::vector<example *> &slots,
                  decision_scores_t &decision_scores, size_t num_features) {
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.quiet &&
    ...
    label_buf << std::setw(all.sd->col_current_label) << std::right
              << std::setfill(' ') << label_str;
    ...
    pred_buf << std::setw(all.sd->col_current_predict) << std::right
             << std::setfill(' ') << pred_str;

    all.sd->print_update(all.holdout_set_off, all.current_pass, label_buf.str(),
                         pred_buf.str(), num_features, all.progress_add,
                         all.progress_arg);
  }
}
...
void output_example(vw &all, ccb & /*c*/, multi_ex &ec_seq) {
  ...
  for (size_t i = 0; i < slots.size(); i++) {
    ...
    if (outcome != nullptr) {
      ...
      float l = CB_ALGS::get_cost_estimate(
          outcome->probabilities[TOP_ACTION_INDEX], outcome->cost,
          preds[i][TOP_ACTION_INDEX].action);
  ...
  all.sd->update(holdout_example, num_labelled > 0, loss,
                 ec_seq[SHARED_EX_INDEX]->weight, num_features);
...
base_learner *ccb_explore_adf_setup(options_i &options, vw &all) {
  ...
  bool slate = false;
  option_group_definition new_options("EXPERIMENTAL: Conditional Contextual "
                                      "Bandit Exploration with Action "
                                      "Dependent Features");
  new_options.add(
      make_option("ccb_explore_adf", ccb_explore_adf_option)
          .keep()
          .help("EXPERIMENTAL: Do Conditional Contextual Bandit learning with "
                "multiline action dependent features."));
  new_options.add(
      make_option("slate", slate)
          .keep()
          .help("EXPERIMENTAL - MAY CHANGE: Enable slate mode in CCB."));
  ...
  learner<ccb, multi_ex> &l =
      init_learner(data, base, learn_or_predict<true>, learn_or_predict<false>,
                   1, prediction_type::decision_probs);
