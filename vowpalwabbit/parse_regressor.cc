...
template <class T> double calculate_sd(vw & /* all */, T &weights) {
  ...
  transform(weights.begin(), weights.end(), diff.begin(),
    ...
    if (model_file.files.size() > 0) {
      ...
      for (size_t i = 0; i < ngram_len; i++) {
        ...
        if (!read) {
          ...
          memcpy(ngram, all.ngram_strings[i].c_str(),
                 min(3, all.ngram_strings[i].size()));
        }
        bytes_read_write += bin_text_read_write_fixed_validated(
      ...
      if (read) {
        ...
        if (len > 0) {
          ...
          //
          // In some cases we end up with a string like: "--bit_precision 18
          // <something_not_an_int>", which will
...
void dump_regressor(vw &all, string reg_name, bool as_text) {
  ...
  if (0 != rename(start_name.c_str(), reg_name.c_str()))
    THROW("WARN: dump_regressor(vw& all, string reg_name, bool as_text): "
          "cannot rename: "
