...
public:
  static void func(weight &w, float &initial, uint64_t /* index */) {
    w = initial;
  }
};
...
template <class T> void initialize_regressor(vw &all, T &weights) {
  ...
  try {
    ...
    THROW(" Failed to allocate weight array with "
          << all.num_bits << " bits: try decreasing -b <bits>");
  }
  if (weights.mask() == 0) {
    THROW(" Failed to allocate weight array with "
          << all.num_bits << " bits: try decreasing -b <bits>");
  } else if (all.initial_weight != 0.)
    weights.template set_default<float, set_initial_wrapper<T>>(
        all.initial_weight);
...
bool resize_buf_if_needed(char *&__dest, size_t &__dest_size,
                          const size_t __n) {
  char *new_dest;
  if (__dest_size < __n) {
...
inline void safe_memcpy(char *&__dest, size_t &__dest_size, const void *__src,
                        size_t __n) {
...
void save_load_header(vw &all, io_buf &model_file, bool read, bool text,
  ...
  try {
    if (model_file.files.size() > 0) {
      ...
      memcpy(buff2, VW::version.to_string().c_str(),
             std::min(v_length, buf2_size));
      if (read) {
        v_length = buf2_size;
        buff2[std::min(v_length, default_buf_size) - 1] = '\0';
      }
      bytes_read_write +=
          bin_text_read_write(model_file, buff2, v_length, "", read, msg, text);
      all.model_file_ver =
          buff2; // stored in all to check save_resume fix in gd
      ...
      bytes_read_write += bin_text_read_write_fixed_validated(
          model_file, (char *)&all.sd->min_label, sizeof(all.sd->min_label), "",
          read, msg, text);

      msg << "Max label:" << all.sd->max_label << "\n";
      bytes_read_write += bin_text_read_write_fixed_validated(
          model_file, (char *)&all.sd->max_label, sizeof(all.sd->max_label), "",
          read, msg, text);

      msg << "bits:" << all.num_bits << "\n";
      uint32_t local_num_bits = all.num_bits;
      bytes_read_write += bin_text_read_write_fixed_validated(
          model_file, (char *)&local_num_bits, sizeof(local_num_bits), "", read,
          msg, text);
      ...
      if (all.model_file_ver < VERSION_FILE_WITH_INTERACTIONS_IN_FO) {
        ...
        msg << pair_len << " pairs: ";
        bytes_read_write += bin_text_read_write_fixed_validated(
            model_file, (char *)&pair_len, sizeof(pair_len), "", read, msg,
            text);
        ...
        for (size_t i = 0; i < pair_len; i++) {
          ...
          bytes_read_write += bin_text_read_write_fixed_validated(
              model_file, pair, 2, "", read, msg, text);
          if (read) {
        ...
        msg << "\n";
        bytes_read_write += bin_text_read_write_fixed_validated(
            model_file, nullptr, 0, "", read, msg, text);

        uint32_t triple_len = (uint32_t)all.triples.size();

        msg << triple_len << " triples: ";
        bytes_read_write += bin_text_read_write_fixed_validated(
            model_file, (char *)&triple_len, sizeof(triple_len), "", read, msg,
            text);
        ...
        for (size_t i = 0; i < triple_len; i++) {
          ...
          bytes_read_write += bin_text_read_write_fixed_validated(
              model_file, triple, 3, "", read, msg, text);
          if (read) {
                                            ...
                                            // (previous if)
        {
          ...
          bytes_read_write += bin_text_read_write_fixed_validated(
              model_file, (char *)&len, sizeof(len), "", read, msg, text);
          ...
          for (size_t i = 0; i < len; i++) {
            ...
            bytes_read_write += bin_text_read_write_fixed_validated(
                model_file, (char *)&inter_len, sizeof(inter_len), "", read,
                msg, text);
            if (!read) {
            ...
            bytes_read_write += bin_text_read_write_fixed_validated(
                model_file, buff2, inter_len, "", read, msg, text);
        ...
        } else // < VERSION_FILE_WITH_INTERACTIONS
        {
          ...
          all.interactions.insert(std::end(all.interactions),
                                  std::begin(all.pairs), std::end(all.pairs));
          all.interactions.insert(std::end(all.interactions),
                                  std::begin(all.triples),
                                  std::end(all.triples));
        }
      }
      ...
      if (all.model_file_ver <= VERSION_FILE_WITH_RANK_IN_HEADER) {
        ...
        msg << "rank:" << rank << "\n";
        bytes_read_write += bin_text_read_write_fixed_validated(
            model_file, (char *)&rank, sizeof(rank), "", read, msg, text);
        if (rank != 0) {
          if (!options.was_supplied("rank")) {
            ...
            all.trace_message
                ...
                << "' value but it will be ignored as another value specified "
      ...
      msg << "lda:" << all.lda << "\n";
      bytes_read_write += bin_text_read_write_fixed_validated(
          model_file, (char *)&all.lda, sizeof(all.lda), "", read, msg, text);

      // TODO: validate ngram_len?
      uint32_t ngram_len = (uint32_t)all.ngram_strings.size();
      msg << ngram_len << " ngram:";
      bytes_read_write += bin_text_read_write_fixed_validated(
          model_file, (char *)&ngram_len, sizeof(ngram_len), "", read, msg,
          text);
      for (size_t i = 0; i < ngram_len; i++) {
        ...
        if (!read) {
          ...
          memcpy(ngram, all.ngram_strings[i].c_str(),
                 std::min(static_cast<size_t>(3), all.ngram_strings[i].size()));
        }
        bytes_read_write += bin_text_read_write_fixed_validated(
            model_file, ngram, 3, "", read, msg, text);
        if (read) {
      ...
      msg << skip_len << " skip:";
      bytes_read_write += bin_text_read_write_fixed_validated(
          model_file, (char *)&skip_len, sizeof(skip_len), "", read, msg, text);

      for (size_t i = 0; i < skip_len; i++) {
        char skip[4] = {0, 0, 0, 0};
        if (!read) {
          msg << all.skip_strings[i] << " ";
          memcpy(skip, all.skip_strings[i].c_str(),
                 std::min(static_cast<size_t>(3), all.skip_strings[i].size()));
        }

        bytes_read_write += bin_text_read_write_fixed_validated(
            model_file, skip, 3, "", read, msg, text);
        if (read) {
      ...
      if (read) {
        ...
        if (len > 0) {
          // There is a potential bug here if len is read out to be zero (e.g.
          // corrupted file). If we naively
          // append buff2 into file_options it might contain old information and
          // thus be invalid. Before, what
          // probably happened is boost::program_options did the right thing,
          // but now we have to construct the
          // input to it where we do not know whether a particular option key
          // can have multiple values or not.
          //
          // In some cases we end up with a std::string like: "--bit_precision
          // 18 <something_not_an_int>", which will
          // cause a "bad program options value" exception, rather than the true
          // "file is corrupted" issue. Only
          // pushing the contents of buff2 into file_options when it is valid
          // will prevent this false error.
        ...
        if (all.save_resume &&
            all.get_random_state()->get_current_state() != 0) {
          serialized_keep_options += " --random_seed";
          serialized_keep_options +=
              " " + std::to_string(all.get_random_state()->get_current_state());
        }
        ...
        if (len > 0)
          safe_memcpy(buff2, buf2_size, serialized_keep_options.c_str(),
                      len + 1);
        *(buff2 + len) = 0;
        bytes_read_write += bin_text_read_write(model_file, buff2,
                                                len + 1, // len+1 to write a \0
                                                "", read, msg, text);
      }
      ...
      if (all.model_file_ver >= VERSION_FILE_WITH_HEADER_HASH) {
        uint32_t check_sum =
            (all.model_file_ver >= VERSION_FILE_WITH_HEADER_CHAINED_HASH)
                ? model_file.hash()
                : (uint32_t)uniform_hash(model_file.space.begin(),
                                         bytes_read_write, 0);
...
void dump_regressor(vw &all, std::string reg_name, bool as_text) {
  ...
  if (0 != rename(start_name.c_str(), reg_name.c_str()))
    THROW("WARN: dump_regressor(vw& all, std::string reg_name, bool as_text): "
          "cannot rename: "
...
void read_regressor_file(vw &all, std::vector<std::string> all_intial,
                         io_buf &io_temp) {
  if (all_intial.size() > 0) {
    ...
    if (!all.quiet) {
      ...
      if (all_intial.size() > 1) {
        all.trace_message << "warning: ignoring remaining "
                          << (all_intial.size() - 1) << " initial regressors"
                          << std::endl;
      }
    }
  }
...
void parse_mask_regressor_args(vw &all, std::string feature_mask,
  ...
  // TODO does this extra check need to be used? I think it is duplicated but
  // there may be some logic I am missing.
  std::string file_options;
  if (!feature_mask.empty()) {
    if (initial_regressors.size() > 0) {
      if (feature_mask == initial_regressors[0]) //-i and -mask are from same
                                                 //file, just generate mask
      {
        return;
      }
    }

    // all other cases, including from different file, or -i does not exist,
    // need to read in the mask file
    io_buf io_temp_mask;
    io_temp_mask.open_file(feature_mask.c_str(), false, io_buf::READ);
    save_load_header(all, io_temp_mask, true, false, file_options,
                     *all.options);
    ...
    if (initial_regressors.size() > 0) {
      ...
      // Re-zero the weights, in case weights of initial regressor use different
      // indices
      all.weights.set_zero(0);
    } else {
      // If no initial regressor, just clear out the options loaded from the
      // header.
