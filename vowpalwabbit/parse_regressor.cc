    ...
    vw& all, io_buf& model_file, bool read, bool text, std::string& file_options, VW::config::options_i& options)
{
  ...
  try
  {
    if (model_file.num_files() > 0)
    {
      ...
      size_t ngram_len = (g_transformer != nullptr) ? g_transformer->get_initial_ngram_definitions().size() : 0;
      msg << ngram_len << " ngram:";
      bytes_read_write +=
      ...
      // TODO: validate skips?
      size_t skip_len = (g_transformer != nullptr) ? g_transformer->get_initial_skip_definitions().size() : 0;
      msg << skip_len << " skip:";
      bytes_read_write +=
