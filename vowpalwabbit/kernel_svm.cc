...
void free_svm_model(svm_model *model) {
  for (size_t i = 0; i < model->num_support; i++) {
    ...
    // deallocated using (b) 'operator delete
...
struct svm_params {
  ...
  ~svm_params() {
    ...
    if (all) {
      ...
      all->trace_message << "Number of kernel evaluations = "
                         << num_kernel_evals << " "
...
float kernel_function(const flat_example *fec1, const flat_example *fec2,
...
int svm_example::compute_kernels(svm_params &params) {
  ...
  if (krow.size() < n) {
    ...
    for (size_t i = krow.size(); i < n; i++) {
      ...
      float kv = kernel_function(&ex, &(sec->ex), params.kernel_params,
                                 params.kernel_type);
      krow.push_back(kv);
      alloc += 1;
      // std::cerr<<kv<<" ";
...
static int make_hot_sv(svm_params &params, size_t svi) {
  ...
  if (svi >= model->num_support)
    params.all->trace_message << "Internal error at " << __FILE__ << ":"
                              << __LINE__ << endl;
  ...
  if (read) {
    ...
    if (brw > 0) {
      if (fec->tag_len > 0) {
        ...
        brw = model_file.bin_read_fixed((char *)fec->tag,
                                        fec->tag_len * sizeof(char), "");
        if (!brw)
          return 2;
      }
      if (fec->fs.size() > 0) {
        ...
        brw = model_file.bin_read_fixed((char *)fs.values.begin(),
                                        len * sizeof(feature_value), "");
        if (!brw)
          return 3;
        ...
        brw = model_file.bin_read_fixed((char *)fs.indicies.begin(),
                                        len * sizeof(feature_index), "");
        if (!brw)
          return 3;
    ...
    if (brw > 0) {
      if (fec->tag_len > 0) {
        brw = model_file.bin_write_fixed((char *)fec->tag,
                                         (uint32_t)fec->tag_len * sizeof(char));
        if (!brw) {
      ...
      if (fec->fs.size() > 0) {
        brw = model_file.bin_write_fixed((char *)fec->fs.values.begin(),
                                         (uint32_t)fec->fs.size() *
                                             sizeof(feature_value));
        if (!brw)
          return 3;
        brw = model_file.bin_write_fixed((char *)fec->fs.indicies.begin(),
                                         (uint32_t)fec->fs.indicies.size() *
                                             sizeof(feature_index));
        if (!brw)
          return 3;
      }
    } else
...
void save_load_svm_model(svm_params &params, io_buf &model_file, bool read,
                         bool text) {
  svm_model *model = params.model;
  // TODO: check about initialization

  // params.all->opts_n_args.trace_message<<"Save load svm "<<read<<" "<<text<<
  // endl;
  if (model_file.files.size() == 0)
    return;
  std::stringstream msg;
  bin_text_read_write_fixed(model_file, (char *)&(model->num_support),
                            sizeof(model->num_support), "", read, msg, text);
  ...
  bin_text_read_write_fixed(model_file, (char *)model->alpha.begin(),
                            (uint32_t)model->num_support * sizeof(float), "",
                            read, msg, text);
  if (read)
    model->delta.resize(model->num_support);
  bin_text_read_write_fixed(model_file, (char *)model->delta.begin(),
                            (uint32_t)model->num_support * sizeof(float), "",
                            read, msg, text);
}
...
float poly_kernel(const flat_example *fec1, const flat_example *fec2,
                  int power) {
...
float rbf_kernel(const flat_example *fec1, const flat_example *fec2,
                 float bandwidth) {
  float dotprod = linear_kernel(fec1, fec2);
  // std::cerr<<"Bandwidth = "<<bandwidth<< endl;
  return expf(
      -(fec1->total_sum_feat_sq + fec2->total_sum_feat_sq - 2 * dotprod) *
      bandwidth);
}
...
void predict(svm_params &params, svm_example **ec_arr, float *scores,
             size_t n) {
  svm_model *model = params.model;
  for (size_t i = 0; i < n; i++) {
    ...
    if (ec_arr[i]->krow.size() > 0)
      scores[i] =
          dense_dot(ec_arr[i]->krow.begin(), model->alpha, model->num_support) /
          params.lambda;
    else
      scores[i] = 0;
  }
...
size_t suboptimality(svm_model *model, double *subopt) {
  ...
  for (size_t i = 0; i < model->num_support; i++) {
    ...
    if ((tmp < model->support_vec[i]->ex.l.simple.weight &&
         model->delta[i] < 0) ||
        (tmp > 0 && model->delta[i] > 0))
...
bool update(svm_params &params, size_t pos) {
  ...
  // std::cout<<model->num_support<<" "<<pos<<" "<<proj<<" "<<alphaKi<<"
  // "<<alpha_old<<" "<<ld.label<<"
...
void train(svm_params &params) {
  // params.all->opts_n_args.trace_message<<"In train "<<params.all->training<<
  // endl;
  ...
  if (params.active) {
    if (params.active_pool_greedy) {
      ...
      for (size_t i = 0; i < params.pool_pos; i++)
        scoremap.insert(
            std::pair<const double, const size_t>(fabs(scores[i]), i));
      ...
      for (size_t train_size = 1;
           iter != scoremap.end() && train_size <= params.subsample;
           train_size++) {
      ...
      for (size_t i = 0; i < params.pool_pos; i++) {
        float queryp =
            2.0f /
            (1.0f + expf((float)(params.active_c * fabs(scores[i])) *
  ...
  if (params.all->training) {
    ...
    for (size_t i = 0; i < params.pool_pos; i++) {
      ...
      // "<<model->support_vec[model_pos]->example_counter<<
      // endl;std::cout<<"After adding in train
      // "<<model->num_support<< endl;
      ...
      if (model_pos >= 0) {
        ...
        for (size_t j = 0; j < params.reprocess; j++) {
          ...
          if (randi) {
            ...
            if (subopt[max_pos] > 0) {
              if (!overshoot && max_pos == (size_t)model_pos && max_pos > 0 &&
                  j == 0)
                params.all->trace_message
                    << "Shouldn't reprocess right after process!!!" << endl;
              // std::cout<<max_pos<<" "<<subopt[max_pos]<< endl;
              // std::cout<<params.model->support_vec[0]->example_counter<<
              // endl;
            ...
            size_t rand_pos =
                (size_t)floorf(params._random_state->get_and_update_random() *
                               model->num_support);
  ...
  // params.all->opts_n_args.trace_message<<params.model->support_vec[0]->example_counter<<
  // endl;
...
void learn(svm_params &params, single_learner &, example &ec) {
  ...
  if (fec) {
    ...
    if (params.all->training && ec.example_counter % 1000 == 0 &&
        ec.example_counter >= 2) {
      params.all->trace_message
          << "Number of support vectors = " << params.model->num_support
          << endl;
      params.all->trace_message
          << "Number of kernel evaluations = " << num_kernel_evals << " "
          << "Number of cache queries = " << num_cache_evals
...
LEARNER::base_learner *kernel_svm_setup(options_i &options, vw &all) {
  ...
  // only be true if --active was supplied and
  // NOT --simulation
  if (all.active)
    params->active = true;
  if (params->active)
