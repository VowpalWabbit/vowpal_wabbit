...
struct svm_params
{
  ...
  ~svm_params()
  {
    ...
    if (all)
    {
      all->trace_message << "Done freeing model" << std::endl;
    }

    free(kernel_params);
    if (all)
    {
      all->trace_message << "Done freeing kernel params" << std::endl;
      all->trace_message << "Done with finish " << std::endl;
    }
  }
};
