...
void train(svm_params &params) {
  ...
  if (params.all->training) {
    ...
    for (size_t i = 0; i < params.pool_pos; i++) {
      ...
      // "<<model->support_vec[model_pos]->example_counter<<endl; cout<<"After
      // adding in train
      // "<<model->num_support<<endl;
      ...
      if (model_pos >= 0) {
        ...
        for (size_t j = 0; j < params.reprocess; j++) {
          ...
          if (randi) {
            ...
            size_t rand_pos = (size_t)floorf(
                merand48(params.all->random_state) * model->num_support);
...
void finish(svm_params &params) {
  ...
  params.all->trace_message << "Num support = " << params.model->num_support
                            << endl;
  params.all->trace_message
      << "Number of kernel evaluations = " << num_kernel_evals << " "
