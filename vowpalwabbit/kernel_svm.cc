...
struct svm_params
{
  ...
  ~svm_params()
  {
    ...
    if (all)
    {
      all->oc.trace_message << "Num support = " << model->num_support << endl;
      all->oc.trace_message << "Number of kernel evaluations = " << num_kernel_evals << " "
                         << "Number of cache queries = " << num_cache_evals << endl;
      all->oc.trace_message << "Total loss = " << loss_sum << endl;
    }
    if (model)
    {
    ...
    if (all)
    {
      all->oc.trace_message << "Done freeing model" << endl;
    }

    free(kernel_params);
    if (all)
    {
      all->oc.trace_message << "Done freeing kernel params" << endl;
      all->oc.trace_message << "Done with finish " << endl;
    }
  }
};
...
static int make_hot_sv(svm_params& params, size_t svi)
{
  ...
  if (svi >= model->num_support)
    params.all->oc.trace_message << "Internal error at " << __FILE__ << ":" << __LINE__ << endl;
...
void save_load(svm_params& params, io_buf& model_file, bool read, bool text)
{
  if (text)
  {
    params.all->oc.trace_message << "Not supporting readable model for kernel svm currently" << endl;
    return;
  }
          ...
          if (randi)
          {
            ...
            if (subopt[max_pos] > 0)
            {
              if (!overshoot && max_pos == (size_t)model_pos && max_pos > 0 && j == 0)
                params.all->oc.trace_message << "Shouldn't reprocess right after process!!!" << endl;
...
VW::LEARNER::base_learner* kernel_svm_setup(options_i& options, vw& all)
{
  ...
  params->all->oc.trace_message << "Kernel = " << kernel_type << endl;

  if (kernel_type.compare("rbf") == 0)
  {
    params->kernel_type = SVM_KER_RBF;
    params->all->oc.trace_message << "bandwidth = " << bandwidth << endl;
  ...
  else if (kernel_type.compare("poly") == 0)
  {
    ...
    params->all->oc.trace_message << "degree = " << degree << endl;
