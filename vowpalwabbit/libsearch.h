...
public:
	SearchTask(vw& vw_obj) : vw_obj(vw_obj), sch(*(Search::search*)vw_obj.searchstr) {
		bogus_example = alloc_examples(sizeof(uint32_t), 1);
		...
		blank_line = alloc_examples(sizeof(uint32_t), 1);
	...
	~SearchTask() {
		VW::finish_example(vw_obj, bogus_example);
		VW::finish_example(vw_obj, blank_line);
		//dealloc_example(NULL, *bogus_example); 
		//free(bogus_example);
		//dealloc_example(NULL, *blank_line);    
		//free(blank_line);
	}
