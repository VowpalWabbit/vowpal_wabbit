...
private:
  shared_data *_sd;
  // model_file_ver is only used to conditionally run save_load(). In the setup
  // function
  ...
  v_array<uint32_t> _backup_nf;   // temporary storage for sm; backup for
                                  // numFeatures in examples
  v_array<float> _backup_weights; // temporary storage for sm; backup for
                                  // weights in examples
...
public:
  ...
  cb_adf(shared_data *sd, size_t cb_type, VW::version_struct *model_file_ver,
         bool rank_all, float clip_p, bool no_predict)
      : _sd(sd), _model_file_ver(model_file_ver), _no_predict(no_predict),
        _rank_all(rank_all), _clip_p(clip_p) {
  ...
  ~cb_adf() {
    _cb_labels.delete_v();
    for (auto &prepped_cs_label : _prepped_cs_labels)
...
CB::cb_class get_observed_cost(multi_ex &examples) {
  ...
  if (index == -1) {
    ...
    return known_cost;
    // std::cerr << "None of the examples has known cost. Exiting." <<
    // std::endl;
    // throw exception();
  }
...
void cb_adf::learn_IPS(multi_learner &base, multi_ex &examples) {
  ...
  call_cs_ldf<true>(base, examples, _cb_labels, _cs_labels, _prepped_cs_labels,
                    _offset);
}

void cb_adf::learn_SM(multi_learner &base, multi_ex &examples) {
  gen_cs_test_example(examples, _cs_labels); // create test labels.
  call_cs_ldf<false>(base, examples, _cb_labels, _cs_labels, _prepped_cs_labels,
                     _offset);
  ...
  for (uint32_t i = 0; i < examples[0]->pred.a_s.size(); i++) {
    _a_s.push_back(
        {examples[0]->pred.a_s[i].action, examples[0]->pred.a_s[i].score});
  ...
  for (uint32_t i = 0; i < examples.size(); i++) {
    ...
    if (ld.costs.size() == 1 && ld.costs[0].cost != FLT_MAX) {
      chosen_action = i;
      example_weight =
          ld.costs[0].cost / safe_probability(ld.costs[0].probability);

      // Importance weights of examples cannot be negative.
      // So we use a trick: set |w| as weight, and use sign(w) as an offset in
  ...
  // Lambda is -1 in the call to generate_softmax because in vw, lower score is
  // better; for softmax higher score is
  // better.
  generate_softmax(-1.0, begin_scores(_a_s), end_scores(_a_s),
                   begin_scores(_prob_s), end_scores(_prob_s));

  // TODO: Check Marco's example that causes VW to report prob > 1.

  for (auto const &action_score :
       _prob_s) // Scale example_wt by prob of chosen action
  {
  ...
  for (auto const &action_score : _prob_s) {
    ...
    if (current_action == chosen_action)
      examples[current_action]->weight =
          example_weight * (1.0f - action_score.score);
    else
...
void cb_adf::learn_MTR(multi_learner &base, multi_ex &examples) {
  ...
  if (predict) // first get the prediction to return
  {
    ...
    call_cs_ldf<false>(base, examples, _cb_labels, _cs_labels,
                       _prepped_cs_labels, _offset);
  ...
  const float clipped_p = std::max(
      examples[_gen_cs.mtr_example]->l.cb.costs[0].probability, _clip_p);
  examples[_gen_cs.mtr_example]->weight *=
      1.f / clipped_p * ((float)_gen_cs.event_sum / (float)_gen_cs.action_sum);

  std::swap(_gen_cs.mtr_ec_seq[0]->pred.a_s, _a_s_mtr_cs);
  // TODO!!! cb_labels are not getting properly restored (empty costs are
  // dropped)
  GEN_CS::call_cs_ldf<true>(base, _gen_cs.mtr_ec_seq, _cb_labels, _cs_labels,
                            _prepped_cs_labels, _offset);
...
example *test_adf_sequence(multi_ex &ec_seq) {
  if (ec_seq.empty())
    THROW("cb_adf: At least one action must be provided for an example to be "
          "valid.");
...
void cb_adf::do_actual_learning(multi_learner &base, multi_ex &ec_seq) {
  ...
  if (is_learn && test_adf_sequence(ec_seq) != nullptr) {
    ...
    case CB_TYPE_SM:
      ...
      break;
    default:
      THROW("Unknown cb_type specified for contextual bandit learning: "
            << _gen_cs.cb_type);
    }

    /*      for (size_t i = 0; i < temp_scores.size(); i++)
    if (temp_scores[i] != data.ec_seq[0]->pred.a_s[i].score)
     std::cout << "problem! " << temp_scores[i] << " != " <<
    ...
    call_cs_ldf<false>(base, ec_seq, _cb_labels, _cs_labels, _prepped_cs_labels,
                       _offset);
  }
}
...
bool cb_adf::update_statistics(example &ec, multi_ex *ec_seq) {
  ...
  if (_gen_cs.known_cost.probability > 0)
    loss =
        get_cost_estimate(&(_gen_cs.known_cost), _gen_cs.pred_scores, action);
  else
    labeled_example = false;

  bool holdout_example = labeled_example;
  for (auto const &i : *ec_seq)
    holdout_example &= i->test_only;
...
void output_example(vw &all, cb_adf &c, example &ec, multi_ex *ec_seq) {
  ...
  if (all.raw_prediction > 0) {
    ...
    for (size_t i = 0; i < costs.size(); i++) {
      if (i > 0)
        outputStringStream << ' ';
      outputStringStream << costs[i].action << ':'
...
void save_load(cb_adf &c, io_buf &model_file, bool read, bool text) {
  if (c.get_model_file_ver() != nullptr &&
      *c.get_model_file_ver() < VERSION_FILE_WITH_CB_ADF_SAVE)
    return;
  std::stringstream msg;
  msg << "event_sum " << c.get_gen_cs().event_sum << "\n";
  bin_text_read_write_fixed(model_file, (char *)&c.get_gen_cs().event_sum,
                            sizeof(c.get_gen_cs().event_sum), "", read, msg,
                            text);

  msg << "action_sum " << c.get_gen_cs().action_sum << "\n";
  bin_text_read_write_fixed(model_file, (char *)&c.get_gen_cs().action_sum,
                            sizeof(c.get_gen_cs().action_sum), "", read, msg,
                            text);
}

void learn(cb_adf &c, multi_learner &base, multi_ex &ec_seq) {
  c.do_actual_learning<true>(base, ec_seq);
}

void predict(cb_adf &c, multi_learner &base, multi_ex &ec_seq) {
  c.do_actual_learning<false>(base, ec_seq);
}
...
base_learner *cb_adf_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("cb_adf", cb_adf_option).keep().help("Do Contextual "
                                                            "Bandit learning "
                                                            "with multiline "
                                                            "action dependent "
                                                            "features."))
      .add(make_option("rank_all", rank_all)
               .keep()
               .help("Return actions sorted by score order"))
      .add(make_option("no_predict", no_predict)
               .help("Do not do a prediction when training"))
      .add(make_option("clip_p", clip_p)
               .keep()
               .default_value(0.f)
               .help("Clipping probability in importance weight. Default: 0.f "
                     "(no clipping)."))
      .add(
          make_option("cb_type", type_string).keep().help("contextual bandit "
                                                          "method to use in "
                                                          "{ips, dm, dr, mtr, "
                                                          "sm}. Default: mtr"));
  ...
  if (type_string == "dr") {
    ...
    all.trace_message << "warning: cb_type must be in "
                         "{'ips','dr','mtr','dm','sm'}; resetting to mtr."
                      << std::endl;
    cb_type = CB_TYPE_MTR;
  }

  if (clip_p > 0.f && cb_type == CB_TYPE_SM)
    all.trace_message << "warning: clipping probability not yet implemented "
                         "for cb_type sm; p will not be clipped."
                      << std::endl;
  ...
  if ((!options.was_supplied("csoaa_ldf") &&
      ...
      rank_all || !options.was_supplied("csoaa_rank")) {
    if (!options.was_supplied("csoaa_ldf")) {
  ...
  auto ld = scoped_calloc_or_throw<cb_adf>(all.sd, cb_type, &all.model_file_ver,
                                           rank_all, clip_p, no_predict);
  ...
  learner<cb_adf, multi_ex> &l =
      init_learner(ld, base, learn, predict, problem_multiplier,
