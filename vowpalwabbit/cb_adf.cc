...
struct cb_adf {
  ...
  v_array<uint32_t>
      backup_nf; // temporary storage for sm; backup for numFeatures in examples
  v_array<float> backup_weights; // temporary storage for sm; backup for weights
                                 // in examples
...
void learn_IPS(cb_adf &mydata, multi_learner &base, multi_ex &examples) {
  ...
  call_cs_ldf<true>(base, examples, mydata.cb_labels, mydata.cs_labels,
                    mydata.prepped_cs_labels, mydata.offset);
}
...
void learn_SM(cb_adf &mydata, multi_learner &base, multi_ex &examples) {
  ...
  call_cs_ldf<false>(base, examples, mydata.cb_labels, mydata.cs_labels,
                     mydata.prepped_cs_labels, mydata.offset);
  ...
  gen_cs_example_sm(examples, chosen_action, sign_offset, mydata.a_s,
                    mydata.cs_labels);

  // Lambda is -1 in the call to generate_softmax because in vw, lower score is
  // better; for softmax higher score is
  // better.
  generate_softmax(-1.0, begin_scores(mydata.a_s), end_scores(mydata.a_s),
                   begin_scores(mydata.prob_s), end_scores(mydata.prob_s));

  // TODO: Check Marco's example that causes VW to report prob > 1.

  for (uint32_t i = 0; i < mydata.prob_s.size();
       i++) // Scale example_wt by prob of chosen action
  {
  ...
  for (uint32_t i = 0; i < mydata.prob_s.size(); i++) {
    ...
    mydata.backup_nf.push_back(
        (uint32_t)examples[current_action]->num_features);

    if (current_action == chosen_action)
      examples[current_action]->weight =
          example_weight * (1.0f - mydata.prob_s[i].score);
    else
      examples[current_action]->weight =
          example_weight * mydata.prob_s[i].score;
...
void learn_MTR(cb_adf &mydata, multi_learner &base, multi_ex &examples) {
  ...
  examples[mydata.gen_cs.mtr_example]->weight *=
      1.f / examples[mydata.gen_cs.mtr_example]->l.cb.costs[0].probability *
      ((float)mydata.gen_cs.event_sum / (float)mydata.gen_cs.action_sum);

  // TODO!!! mydata.cb_labels are not getting properly restored (empty costs are
  // dropped)
  GEN_CS::call_cs_ldf<true>(base, mydata.gen_cs.mtr_ec_seq, mydata.cb_labels,
                            mydata.cs_labels, mydata.prepped_cs_labels,
                            mydata.offset);
...
void do_actual_learning(cb_adf &data, multi_learner &base, multi_ex &ec_seq) {
  ...
  data.gen_cs.known_cost =
      get_observed_cost(ec_seq); // need to set for test case
  if (is_learn && !test_adf_sequence(ec_seq)) {
    ...
    call_cs_ldf<false>(base, ec_seq, data.cb_labels, data.cs_labels,
                       data.prepped_cs_labels, data.offset);
  }
}
...
bool update_statistics(vw &all, cb_adf &c, example &ec, multi_ex *ec_seq) {
  ...
  if (c.gen_cs.known_cost.probability > 0)
    loss =
        get_unbiased_cost(&(c.gen_cs.known_cost), c.gen_cs.pred_scores, action);
  else
...
void save_load(cb_adf &c, io_buf &model_file, bool read, bool text) {
  ...
  bin_text_read_write_fixed(model_file, (char *)&c.gen_cs.event_sum,
                            sizeof(c.gen_cs.event_sum), "", read, msg, text);

  msg << "action_sum " << c.gen_cs.action_sum << "\n";
  bin_text_read_write_fixed(model_file, (char *)&c.gen_cs.action_sum,
                            sizeof(c.gen_cs.action_sum), "", read, msg, text);
}
...
base_learner *cb_adf_setup(options_i &options, vw &all) {
  ...
  new_options
      .add(make_option("cb_adf", cb_adf_option).keep().help("Do Contextual "
                                                            ...
                                                            "action dependent "
                                                            "features."))
      .add(make_option("rank_all", ld->rank_all)
               .keep()
               .help("Return actions sorted by score order"))
      .add(make_option("no_predict", ld->no_predict)
  ...
  if ((!options.was_supplied("csoaa_ldf") &&
      ...
      ld->rank_all || !options.was_supplied("csoaa_rank")) {
    if (!options.was_supplied("csoaa_ldf")) {
  ...
  learner<cb_adf, multi_ex> &l =
      init_learner(ld, base, CB_ADF::do_actual_learning<true>,
                   CB_ADF::do_actual_learning<false>, problem_multiplier,
