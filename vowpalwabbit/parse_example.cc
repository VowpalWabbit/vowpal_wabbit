...
public:
  ...
  inline FORCE_INLINE void parserWarning(const char* message, VW::string_view var_msg, const char* message2, size_t example_number)
  {
  ...
  inline FORCE_INLINE bool isFeatureValueFloat(float& float_feature_value)
  {
    ...
    else if (_line[_read_idx] == ':')
    {
      ...
      if (std::isnan(_v))
      {
        ...
        parserWarning(
            "warning: invalid feature value:\"", _line.substr(_read_idx), "\" read as NaN. Replacing with 0.", _ae->example_counter);
      }
      _read_idx += end_read;
      return true;
    }
    else
    {
      _v = float_feature_value = 0.f;
      // syntax error
      parserWarning("malformed example! '|', ':', space, or EOL expected after : \"", _line.substr(0, _read_idx), "\"", _ae->example_counter);
      return true;
    }
  }
  ...
  inline FORCE_INLINE void nameSpaceInfoValue()
  {
    ...
    else if (_line[_read_idx] == ':')
    {
      ...
      { parserWarning("malformed example! Float expected after : \"", _line.substr(0, _read_idx), "\"", _ae->example_counter); }
      if (std::isnan(_cur_channel_v))
      {
        _cur_channel_v = 1.f;
        parserWarning(
            "warning: invalid namespace value:\"", _line.substr(_read_idx), "\" read as NaN. Replacing with 1.", _ae->example_counter);
      }
      _read_idx += end_read;
    }
    else
  ...
  inline FORCE_INLINE void nameSpaceInfo()
  {
    ...
    {
      // syntax error
      parserWarning("malformed example! String expected after : \"", _line.substr(0, _read_idx), "\"", _ae->example_counter);
    }
    else
    {
  ...
  inline FORCE_INLINE void nameSpace()
  {
    ...
    else
    {
      // syntax error
      parserWarning(
          "malformed example! '|',String,space, or EOL expected after : \"", _line.substr(0, _read_idx), "\"", _ae->example_counter);
    }
  ...
  inline FORCE_INLINE void listNameSpace()
  {
    ...
    {
      // syntax error
      parserWarning("malformed example! '|' or EOL expected after : \"", _line.substr(0, _read_idx), "\"", _ae->example_counter);
    }
  }
