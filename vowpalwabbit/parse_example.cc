...
size_t read_features(vw *all, char *&line, size_t &num_chars) {
  ...
  if (line[0] == '\xef' && num_chars >= 3 && line[1] == '\xbb' &&
      line[2] == '\xbf') {
    line += 3;
    num_chars -= 3;
  }
...
template <bool audit> class TC_parser {
public:
  char *beginLine;
  char *reading_head;
  char *endLine;
  ...
  inline void parserWarning(const char *message, char *begin, char *pos,
                            const char *message2) {
    std::stringstream ss;
    ss << message << std::string(begin, pos - begin).c_str() << message2
       << "in Example #" << this->p->end_parsed_examples << ": \""
       << std::string(this->beginLine, this->endLine).c_str() << "\"" << endl;
    if (p->strict_parse) {
  ...
  inline float featureValue() {
    if (*reading_head == ' ' || *reading_head == '\t' || *reading_head == '|' ||
      ...
      if (end_read == reading_head) {
        parserWarning("malformed example! Float expected after : \"", beginLine,
                      reading_head, "\"");
      }
      if (nanpattern(v)) {
        v = 0.f;
        parserWarning("warning: invalid feature value:\"", reading_head,
                      end_read, "\" read as NaN. Replacing with 0.");
      }
      ...
      // syntax error
      parserWarning(
          "malformed example! '|', ':', space, or EOL expected after : \"",
          beginLine, reading_head, "\"");
      return 0.f;
    }
  }
  ...
  inline void maybeFeature() {
    if (*reading_head == ' ' || *reading_head == '\t' || *reading_head == '|' ||
      ...
      if (audit) {
        ...
        push_many(feature_v, feature_name.begin,
                  feature_name.end - feature_name.begin);
        feature_v.push_back('\0');
        fs.space_names.push_back(
            audit_strings_ptr(new audit_strings(base, feature_v.begin())));
        feature_v.delete_v();
      }
      if ((affix_features[index] > 0) &&
          (feature_name.end != feature_name.begin)) {
        ...
        while (affix > 0) {
          ...
          word_hash = p->hasher(affix_name, (uint64_t)channel_hash) *
          ...
          if (audit) {
            ...
            affix_v.push_back('=');
            push_many(affix_v, affix_name.begin,
                      affix_name.end - affix_name.begin);
            affix_v.push_back('\0');
            affix_fs.space_names.push_back(
                audit_strings_ptr(new audit_strings("affix", affix_v.begin())));
          }
          affix >>= 4;
        }
      }
      if (spelling_features[index]) {
        ...
        if (audit) {
          ...
          push_many(spelling_v, spelling_ss.begin,
                    spelling_ss.end - spelling_ss.begin);
          spelling_v.push_back('\0');
          spell_fs.space_names.push_back(audit_strings_ptr(
              new audit_strings("spelling", spelling_v.begin())));
        }
      }
      if (namespace_dictionaries[index].size() > 0) {
        for (size_t dict = 0; dict < namespace_dictionaries[index].size();
             dict++) {
          feature_dict *map = namespace_dictionaries[index][dict];
          uint64_t hash = uniform_hash(feature_name.begin,
                                       feature_name.end - feature_name.begin,
                                       quadratic_constant);
          ...
          if ((feats != nullptr) && (feats->values.size() > 0)) {
            ...
            push_many(dict_fs.values, feats->values.begin(),
                      feats->values.size());
            push_many(dict_fs.indicies, feats->indicies.begin(),
                      feats->indicies.size());
            ...
            if (audit)
              for (size_t i = 0; i < feats->indicies.size(); ++i) {
                ...
                stringstream ss;
                ss << index << '_';
                for (char *fc = feature_name.begin; fc != feature_name.end;
                     ++fc)
                  ss << *fc;
                ss << '=' << id;
                dict_fs.space_names.push_back(audit_strings_ptr(
                    new audit_strings("dictionary", ss.str())));
              }
          }
        }
      }
  ...
  inline void nameSpaceInfoValue() {
    if (*reading_head == ' ' || *reading_head == '\t' ||
      ...
      if (nanpattern(cur_channel_v)) {
        cur_channel_v = 1.f;
        parserWarning("warning: invalid namespace value:\"", reading_head,
                      end_read, "\" read as NaN. Replacing with 1.");
      }
      reading_head = end_read;
    } else {
      // syntax error
  ...
  inline void nameSpaceInfo() {
    if (reading_head == endLine || *reading_head == '|' ||
        *reading_head == ' ' || *reading_head == '\t' || *reading_head == ':' ||
        *reading_head == '\r') {
      // syntax error
      parserWarning("malformed example! String expected after : \"", beginLine,
                    reading_head, "\"");
    } else {
  ...
  inline void listFeatures() {
    ...
    if (!(*reading_head == '|' || reading_head == endLine ||
          *reading_head == '\r')) {
      // syntax error
      parserWarning("malformed example! '|',space, or EOL expected after : \"",
                    beginLine, reading_head, "\"");
    }
  }
  ...
  inline void nameSpace() {
    ...
    if (*reading_head == ' ' || *reading_head == '\t' ||
      ...
      channel_hash =
          this->hash_seed == 0 ? 0 : uniform_hash("", 0, this->hash_seed);
      listFeatures();
    } else if (*reading_head != ':') {
      ...
      // syntax error
      parserWarning(
          "malformed example! '|',String,space, or EOL expected after : \"",
          beginLine, reading_head, "\"");
    }
  ...
  inline void listNameSpace() {
    while ((*reading_head == '|') &&
           (reading_head <
            endLine)) // ListNameSpace --> '|' NameSpace ListNameSpace
    {
      ++reading_head;
      nameSpace();
    }
    if (reading_head != endLine && *reading_head != '\r') {
      // syntax error
      parserWarning("malformed example! '|' or EOL expected after : \"",
                    beginLine, reading_head, "\"");
    }
  }
