...
inline func_data tuple_dbf(void *data, base_learner *base,
                           void (*func)(void *)) {
  func_data foo;
  foo.data = data;
  foo.base = base;
...
struct learn_data {
  ...
  using multi_fn = void (*)(void *data, base_learner &base, void *ex,
                            size_t count, size_t step, polyprediction *pred,
...
inline void increment_offset(example &ex, const size_t increment,
                             const size_t i) {
  ex.ft_offset += static_cast<uint32_t>(increment * i);
}

inline void increment_offset(multi_ex &ec_seq, const size_t increment,
                             const size_t i) {
  for (auto ec : ec_seq)
    ec->ft_offset += static_cast<uint32_t>(increment * i);
}

inline void decrement_offset(example &ex, const size_t increment,
                             const size_t i) {
  assert(ex.ft_offset >= increment * i);
  ex.ft_offset -= static_cast<uint32_t>(increment * i);
}

inline void decrement_offset(multi_ex &ec_seq, const size_t increment,
                             const size_t i) {
  for (auto ec : ec_seq) {
...
public:
  ...
  size_t weights; // this stores the number of "weight vectors" required by the
                  // learner.
  size_t increment;
  ...
  inline void learn(E &ec, size_t i = 0) {
    assert(
        (is_multiline && std::is_same<multi_ex, E>::value) ||
        (!is_multiline &&
         std::is_same<example, E>::value)); // sanity check under debug compile
  ...
  inline void multipredict(E &ec, size_t lo, size_t count, polyprediction *pred,
                           bool finalize_predictions) {
    assert(
        (is_multiline && std::is_same<multi_ex, E>::value) ||
        (!is_multiline &&
    ...
    if (learn_fd.multipredict_f == NULL) {
      ...
      for (size_t c = 0; c < count; c++) {
        ...
        if (finalize_predictions)
          pred[c] = ec.pred; // TODO: this breaks for complex labels because =
                             // doesn't do deep copy!
        else
          pred[c].scalar = ec.partial_prediction;
        // pred[c].scalar = finalize_prediction ec.partial_prediction; // TODO:
        // this breaks for complex labels because =
        // doesn't do deep copy! // note works if ec.partial_prediction, but
        // only if finalize_prediction is run????
        increment_offset(ec, increment, 1);
      }
      decrement_offset(ec, increment, lo + count);
    } else {
      increment_offset(ec, increment, lo);
      learn_fd.multipredict_f(learn_fd.data, *learn_fd.base, (void *)&ec, count,
                              increment, pred, finalize_predictions);
      decrement_offset(ec, increment, lo);
    }
  }

  template <class L> inline void set_predict(void (*u)(T &, L &, E &)) {
    learn_fd.predict_f = (learn_data::fn)u;
  }
  template <class L> inline void set_learn(void (*u)(T &, L &, E &)) {
    learn_fd.learn_f = (learn_data::fn)u;
  }
  template <class L>
  inline void set_multipredict(void (*u)(T &, L &, E &, size_t, size_t,
                                         polyprediction *, bool)) {
  ...
  template <class L> inline void set_update(void (*u)(T &data, L &base, E &)) {
    learn_fd.update_f = (learn_data::fn)u;
  }

  // used for active learning and confidence to determine how easily predictions
  // are changed
  inline void set_sensitivity(float (*u)(T &data, base_learner &base,
                                         example &)) {
  ...
  inline float sensitivity(example &ec, size_t i = 0) {
    increment_offset(ec, increment, i);
    const float ret =
        sensitivity_fd.sensitivity_f(sensitivity_fd.data, *learn_fd.base, ec);
    decrement_offset(ec, increment, i);
    return ret;
  }
  ...
  void set_finish(void (*f)(T &)) {
    finisher_fd =
        tuple_dbf(learn_fd.data, learn_fd.base, (finish_fptr_type)(f));
  }
  inline void finish() {
    if (finisher_fd.data) {
  ...
  inline void finish_example(vw &all, E &ec) {
    finish_example_fd.finish_example_f(all, finish_example_fd.data,
                                       (void *)&ec);
  }
  // called after learn example for each example.  Explicitly not recursive.
  ...
  template <class L>
  static learner<T, E> &
  init_learner(T *dat, L *base, void (*learn)(T &, L &, E &),
               void (*predict)(T &, L &, E &), size_t ws,
               prediction_type::prediction_type_t pred_type) {
    learner<T, E> &ret = calloc_or_throw<learner<T, E>>();

    if (base != nullptr) { // a reduction

      // This is a copy assignment into the current object. The purpose is to
      // copy all of the
      // function data objects so that if this reduction does not define a
      // function such as
      // save_load then calling save_load on this object will essentially result
      // in forwarding the
      // call the next reduction that actually implements it.
      ret = *(learner<T, E> *)(base);

      ret.learn_fd.base = make_base(*base);
      ret.sensitivity_fd.sensitivity_f =
          (sensitivity_data::fn)recur_sensitivity;
    ...
    } else // a base learner
    {
      ...
      ret.finish_example_fd.finish_example_f =
          (finish_example_data::fn)return_simple_example;
    }
...
learner<T, E> &init_learner(free_ptr<T> &dat, L *base,
                            void (*learn)(T &, L &, E &),
                            void (*predict)(T &, L &, E &), size_t ws,
                            prediction_type::prediction_type_t pred_type) {
  auto ret = &learner<T, E>::init_learner(dat.get(), base, learn, predict, ws,
                                          pred_type);
...
template <class T, class E, class L>
learner<T, E> &init_learner(free_ptr<T> &dat, void (*learn)(T &, L &, E &),
                            void (*predict)(T &, L &, E &),
                            size_t params_per_weight) {
  auto ret =
      &learner<T, E>::init_learner(dat.get(), (L *)nullptr, learn, predict,
...
// base predictor only
template <class T, class E, class L>
learner<T, E> &init_learner(void (*predict)(T &, L &, E &),
                            size_t params_per_weight) {
  return learner<T, E>::init_learner(nullptr, (L *)nullptr, predict, predict,
                                     params_per_weight,
...
learner<T, E> &init_learner(free_ptr<T> &dat, void (*learn)(T &, L &, E &),
  ...
  auto ret = &learner<T, E>::init_learner(
      dat.get(), (L *)nullptr, learn, predict, params_per_weight, pred_type);
  dat.release();
  return *ret;
}
...
learner<T, E> &init_learner(free_ptr<T> &dat, L *base,
                            void (*learn)(T &, L &, E &),
                            void (*predict)(T &, L &, E &)) {
  auto ret = &learner<T, E>::init_learner(dat.get(), base, learn, predict, 1,
                                          base->pred_type);
...
template <class T, class E, class L>
learner<T, E> &init_learner(L *base, void (*learn)(T &, L &, E &),
                            void (*predict)(T &, L &, E &)) {
  return learner<T, E>::init_learner(nullptr, base, learn, predict, 1,
                                     base->pred_type);
}
...
template <class T, class E, class L>
learner<T, E> &
init_multiclass_learner(free_ptr<T> &dat, L *base, void (*learn)(T &, L &, E &),
...
template <class T, class E, class L>
learner<T, E> &init_cost_sensitive_learner(
    free_ptr<T> &dat, L *base, void (*learn)(T &, L &, E &),
    void (*predict)(T &, L &, E &), parser *p, size_t ws,
    prediction_type::prediction_type_t pred_type =
        prediction_type::multiclass) {
  learner<T, E> &l = learner<T, E>::init_learner(dat.get(), base, learn,
...
template <class T, class E> base_learner *make_base(learner<T, E> &base) {
  return (base_learner *)(&base);
}

template <class T, class E> multi_learner *as_multiline(learner<T, E> *l) {
  if (l->is_multiline) // Tried to use a singleline reduction as a multiline
                       // reduction
    return (multi_learner *)(l);
  THROW("Tried to use a singleline reduction as a multiline reduction");
}

template <class T, class E> single_learner *as_singleline(learner<T, E> *l) {
  if (!l->is_multiline) // Tried to use a multiline reduction as a singleline
                        // reduction
...
void multiline_learn_or_predict(multi_learner &base, multi_ex &examples,
                                const uint64_t offset, const uint32_t id = 0) {
