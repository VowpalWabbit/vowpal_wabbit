...
void cb_explore_adf_base::output_example(vw& all, multi_ex& ec_seq)
{
  ...
  if (m_gen_cs.known_cost.probability > 0)
  {
    for (uint32_t i = 0; i < preds.size(); i++)
    {
      float l = CB_ALGS::get_cost_estimate(&m_gen_cs.known_cost, preds[i].action);
  ...
  for (int sink : all.final_prediction_sink) ACTION_SCORE::print_action_score(sink, ec.pred.a_s, ec.tag);
