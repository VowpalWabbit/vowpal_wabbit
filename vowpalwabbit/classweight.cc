...
static void predict_or_learn(classweights &cweights,
...
LEARNER::base_learner *classweight_setup(options_i &options, vw &all) {
  ...
  new_options.add(make_option("classweight", classweight_array)
  ...
  if (!all.quiet)
    all.trace_message << "parsed " << cweights->weights.size()
                      << " class weights" << endl;
  ...
  if (base->pred_type == prediction_type::scalar)
    ret = &LEARNER::init_learner<classweights>(
        cweights, base, predict_or_learn<true, prediction_type::scalar>,
        predict_or_learn<false, prediction_type::scalar>);
  else if (base->pred_type == prediction_type::multiclass)
    ret = &LEARNER::init_learner<classweights>(
        cweights, base, predict_or_learn<true, prediction_type::multiclass>,
