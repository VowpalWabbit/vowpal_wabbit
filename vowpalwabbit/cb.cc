...
void parse_label(parser *p, shared_data *, void *v, v_array<substring> &words) {
  ...
  for (size_t i = 0; i < words.size(); i++) {
    ...
    if (nanpattern(f.cost))
      THROW("error NaN cost (" << p->parse_name[1]
                               << " for action: " << p->parse_name[0]);
    ...
    if (nanpattern(f.probability))
      THROW("error NaN probability (" << p->parse_name[2]
                                      << " for action: " << p->parse_name[0]);

    if (f.probability > 1.0) {
      cerr << "invalid probability > 1 specified for an action, resetting to 1."
           << endl;
      f.probability = 1.0;
    }
    if (f.probability < 0.0) {
      cerr << "invalid probability < 0 specified for an action, resetting to 0."
           << endl;
      f.probability = .0;
    }
...
label_parser cb_label = {default_label,     parse_label,  cache_label,
...
void print_update(vw &all, bool is_test, example &ec, multi_ex *ec_seq,
  ...
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.quiet &&
    ...
    if (action_scores) {
      ...
      all.sd->print_update(all.holdout_set_off, all.current_pass, label_buf,
                           (uint32_t)pred, num_features, all.progress_add,
                           all.progress_arg);
  }
}
} // namespace CB
...
label_parser cb_eval = {
    default_label,     parse_label,  cache_label,
    read_cached_label, delete_label, CB::weight,
