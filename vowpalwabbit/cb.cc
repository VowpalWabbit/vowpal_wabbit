  ...
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.logger.quiet && !all.bfgs)
  {
    ...
    if (action_scores)
    {
      ...
      if (!ec.pred.a_s.empty())
        ...
        pred_buf << "no action";
      all.sd->print_update(all.ec.holdout_set_off, all.gs.current_pass, label_buf, pred_buf.str(), num_features,
          all.progress_add, all.progress_arg);
    }
    else
      all.sd->print_update(all.ec.holdout_set_off, all.gs.current_pass, label_buf, (uint32_t)pred, num_features,
          all.progress_add, all.progress_arg);
  }
}
}  // namespace CB
