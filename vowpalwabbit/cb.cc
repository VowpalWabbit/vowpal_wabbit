  ...
  for (auto const &word : words) {
    ...
    if (substring_equal(p->parse_name[0], "shared")) {
      if (p->parse_name.size() == 1) {
        ...
        std::cerr << "shared feature vectors should not have costs"
                  << std::endl;
    }
...
bool ec_is_example_header(
    example const &ec) // example headers just have "shared"
{
...
void print_update(vw &all, bool is_test, example &ec, multi_ex *ec_seq,
  ...
  if (all.sd->weighted_examples() >= all.sd->dump_interval && !all.quiet &&
    ...
    if (action_scores) {
      ...
      pred_buf << std::setw(shared_data::col_current_predict) << std::right
               << std::setfill(' ');
