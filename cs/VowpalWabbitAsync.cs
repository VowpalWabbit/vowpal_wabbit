...
namespace VW
{
    ...
    public class VowpalWabbitAsync<TExample, TActionDependentFeature> : IDisposable
    {
        ...
        public Task<Tuple<int, TActionDependentFeature>[]> LearnAndPredict(TExample example, IReadOnlyCollection<TActionDependentFeature> actionDependentFeatures, int index, ILabel label)
        {
            Contract.Requires(example != null);
            Contract.Requires(actionDependentFeatures != null);
            Contract.Requires(index >= 0);
