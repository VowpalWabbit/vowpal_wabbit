...
public
ref class VowpalWabbit : VowpalWabbitBase, IVowpalWabbitExamplePool
{
private:
  /// <summary>
  ...
  /// </summary>
  /// <remarks>
  /// Avoiding if-else for hash function selection. Delegates outperform function pointers according to
  /// http://stackoverflow.com/questions/13443250/performance-of-c-cli-function-pointers-versus-net-delegates
  /// </remarks>
...
public:
  ...
  /// </summary>
  property VowpalWabbitPerformanceStatistics ^
      PerformanceStatistics { VowpalWabbitPerformanceStatistics ^ get(); }

      /// <summary>
      /// Parses <paramref name="line"/> using the C++ parser.
      /// </summary>
      /// <returns>
      /// Returns a <see cref="VowpalWabbitExample"/> ready to be used for <see cref="Learn(VowpalWabbitExample^)"/> or
      /// <see cref="Predict(VowpalWabbitExample^)"/>.
      /// </returns>
      VowpalWabbitExample ^
  ...
  /// <summary>
  /// Parses <paramref name="line"/> using the C++ parser.
  /// TODO: this should return VowpalWabbitExampleCollection, but that would require moving
  /// VowpalWaabitExampleCollection to C++/CLI
  /// </summary>
  /// <returns>
  /// Returns a <see cref="VowpalWabbitExample"/> ready to be used for <see cref="Learn(VowpalWabbitExample^)"/> or <see
  /// cref="Predict(VowpalWabbitExample^)"/>.
  /// </returns>
  List<VowpalWabbitExample ^> ^ ParseJson(String ^ line);

  /// <summary>
  /// Parses <paramref name="json"/> using the C++ parser and supports the extra wrapping introduced by Decision
  /// Service.
  /// TODO: this should return VowpalWabbitExampleCollection, but that would require moving
  /// VowpalWaabitExampleCollection to C++/CLI
  /// TODO: the header should be passed along with the List of VowpalWabbit examples, but that requires additional care
  /// wrt disposing items.
  /// </summary>
  ...
  /// cref="Predict(VowpalWabbitExample^)"/>.
  /// </returns>
  List<VowpalWabbitExample ^> ^
      VowpalWabbit::ParseDecisionServiceJson(cli::array<Byte> ^ json, int offset, int length,
          bool copyJson, [Out] VowpalWabbitDecisionServiceInteractionHeader ^ % header);
  ...
  /// <remarks>The hash code depends on the vowpal wabbit instance as different has functions can be
  /// configured.</remarks>
  ...
  /// <summary>
  /// The associated <see cref="VowpalWabbitBase"/> instance learns from this example and returns the prediction result
  /// for this example.
  /// </summary>
  ...
  /// <param name="predictionFactory">The prediction factory to be used. See <see
  /// cref="VowpalWabbitPredictionType"/>.</param> <returns>The prediction for the given <paramref
  ...
  virtual property VowpalWabbit ^
      Native { virtual VowpalWabbit ^ get() sealed; }
      ...
      virtual VowpalWabbitExample ^
      GetOrCreateNativeExample() sealed;
