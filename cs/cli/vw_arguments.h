...
/// </summary>
public
ref class VowpalWabbitArguments
{
private:
      ...
      , m_passes((int)vw->numpasses)
  {
    ...
    if (vw->initial_regressors.size() > 0)
    {
      ...
      for (auto& r : vw->initial_regressors) m_regressors->Add(gcnew String(r.c_str()));
    }
    ...
    if (options->was_supplied("cb")) { m_numberOfActions = (int)options->get_typed_option<uint32_t>("cb").value(); }
...
public:
  ...
  /// </summary>
  property String ^
      FinalRegressor { String ^ get() { return m_finalRegressor; } }

      /// <summary>
      /// The list of input model filenames.
      /// </summary>
      property List<String ^> ^
      InitialRegressors { List<String ^> ^ get() { return m_regressors; } }

      property String ^
      CommandLine { String ^ get() { return m_commandLine; } }

      property int ContextualBanditNumberOfActions
  {
    int get() { return m_numberOfActions; }
  }

  property float LearningRate
  {
    float get() { return m_learning_rate; }
  }

  property float PowerT
  {
    float get() { return m_power_t; }
  }
};
}  // namespace VW
