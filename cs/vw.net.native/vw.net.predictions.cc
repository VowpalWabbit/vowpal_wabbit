...
namespace vw_net_native {
  ...
  inline vw_net_native::dotnet_size_t v_copy_to_managed(const v_array<T>& source, T* destination, vw_net_native::dotnet_size_t count)
  {
  ...
  template<typename T> /* TODO: ideally we should enable_if this only on .NET Blittable types */
  inline vw_net_native::dotnet_size_t v_copy_n_to_managed(const v_array<T>& source, T* destination, vw_net_native::dotnet_size_t limit)
  {
...
{
  return static_cast<size_t>(vw->lda);
}
...
API vw_net_native::dotnet_size_t GetPredictionActiveMulticlassMoreInfoRequiredClasses(VW::workspace* vw, example* ex, int32_t* values, vw_net_native::dotnet_size_t count)
{
  ...
  return vw_net_native::v_copy_to_managed(ex->pred.active_multiclass.more_info_required_for_classes, unsigned_values, count);
}
