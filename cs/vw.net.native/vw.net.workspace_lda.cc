...
template<typename T>
int64_t fill_topic_allocation(vw_net_native::workspace_context*  workspace, T& weights, float** topic_weight_buffers, size_t buffer_size, size_t buffers_count)
{
  ...
  {
    weight* wp = &(*iter);
    for (uint64_t k = 0; k < topic_count; k++)
      topic_weight_buffers[(int)k][(int)iter.index()] = wp[k] + lda_rho;
  }

  return returned;
}

API int64_t WorkspaceFillTopicAllocation(vw_net_native::workspace_context* workspace, float** topic_weight_buffers, vw_net_native::dotnet_size_t buffer_size, vw_net_native::dotnet_size_t buffer_count)
{

  if (workspace->vw->weights.sparse)
  {
    return fill_topic_allocation(workspace, workspace->vw->weights.sparse_weights, topic_weight_buffers, buffer_size, buffer_count);
  }
  else
  {
    return fill_topic_allocation(workspace, workspace->vw->weights.dense_weights, topic_weight_buffers, buffer_size, buffer_count);
  }
}
