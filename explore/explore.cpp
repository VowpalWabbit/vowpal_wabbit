...
int main(int argc, char* argv[])
{
  ...
  //	return 0;
	...
	u32 num_bags = 2;
	StatefulFunctionWrapper<int>::Policy_Func* bags[] = { Stateful_Default_Policy_1, Stateful_Default_Policy_2 };
	StatelessFunctionWrapper::Policy_Func* stateless_bags[] = { Stateless_Default_Policy_1, Stateless_Default_Policy_2 };
	...
	if (strcmp(argv[1],"greedy") == 0)
	  { 
	    ...
	    if (stateful) //Initialize Epsilon-Greedy explore algorithm using a default policy function that accepts parameters 
	      ...
	      mwt.Initialize_Epsilon_Greedy(epsilon, Stateless_Default_Policy_1, NUM_ACTIONS);
	  }
	else if (strcmp(argv[1],"tau-first") == 0)
	  {
	    ...
	    if (stateful) //Initialize Tau-First explore algorithm using a default policy function that accepts parameters 
	      mwt.Initialize_Tau_First<int>(tau, Stateful_Default_Policy_1, &policy_params, NUM_ACTIONS);
	    else // Initialize Tau-First explore algorithm using a stateless default policy function 
	      mwt.Initialize_Tau_First(tau, Stateless_Default_Policy_1, NUM_ACTIONS);
	  }
	...
	else if (strcmp(argv[1],"softmax") == 0)
	  {
	    ...
	    if (stateful) //Initialize Softmax explore algorithm using a default scorer function that accepts parameters
	      mwt.Initialize_Softmax<int>(lambda, Stateful_Default_Scorer_1, &policy_params, NUM_ACTIONS);
	    else 	    // Initialize Softmax explore algorithm using a stateless default scorer function 
	      mwt.Initialize_Softmax(lambda, Stateless_Default_Scorer_1, NUM_ACTIONS);
	  }
	else
	  {
