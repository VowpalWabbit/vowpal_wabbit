 ...
 template<typename It>
  ...
  int sample_after_normalizing(const char* seed, It pdf_first, It pdf_last, uint32_t& chosen_index)
  {
		...
		OutputIt ranking_begin, OutputIt ranking_last, std::random_access_iterator_tag ranking_category)
	{
		...
		typename std::vector<PdfVal> sorted_pdf(pdf_size);
		int idx = 0;
		...
		int ret = sample_after_normalizing(seed, pdf_first, pdf_last, chosen_action_idx);
		if (ret)
			return ret;
		...
		{
			std::iter_swap(ranking_begin, ranking_begin + chosen_action_idx);
			std::iter_swap(pdf_first, pdf_first + chosen_action_idx);
		}
