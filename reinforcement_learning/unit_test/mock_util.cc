...
std::unique_ptr<fakeit::Mock<r::i_sender>> get_mock_sender(std::vector<unsigned char>& recorded_messages) {
  auto mock = std::unique_ptr<fakeit::Mock<r::i_sender>>(
    new fakeit::Mock<r::i_sender>());
...
std::unique_ptr<r::sender_factory_t> get_mock_sender_factory(fakeit::Mock<r::i_sender>* mock_observation_sender, fakeit::Mock<r::i_sender>* mock_interaction_sender) {
  ...
  factory->register_type(r::value::OBSERVATION_EH_SENDER,
    [mock_observation_sender](r::i_sender** retval, const u::configuration&, r::i_trace*, r::api_status*) { *retval = &mock_observation_sender->get(); return r::error_code::success; });
  factory->register_type(r::value::INTERACTION_EH_SENDER,
    [mock_interaction_sender](r::i_sender** retval, const u::configuration&, r::i_trace*, r::api_status*) { *retval = &mock_interaction_sender->get(); return r::error_code::success; });
  return factory;
}
