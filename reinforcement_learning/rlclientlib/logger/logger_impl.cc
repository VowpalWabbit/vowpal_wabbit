...
namespace reinforcement_learning {
  event_hub_logger_impl::event_hub_logger_impl(
  ...
  int event_hub_logger_impl::init(api_status* status) {
    RETURN_IF_FAIL(_batcher.init(status));
    RETURN_IF_FAIL(_client.init(status));
    return error_code::success;
  }

  int event_hub_logger_impl::append(std::string& item, api_status* status) {
