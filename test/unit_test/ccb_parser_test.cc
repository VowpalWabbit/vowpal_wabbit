...
BOOST_AUTO_TEST_CASE(ccb_parse_label)
{
  ...
  label = parse_label(&p, "ccb slot");
  BOOST_CHECK_EQUAL(label.explicit_included_actions.size(), 0);
  BOOST_CHECK(label.outcome == nullptr);
  BOOST_CHECK_EQUAL(label.type, CCB::example_type::slot);

  label = parse_label(&p, "ccb slot 1,3,4");
  ...
  label = parse_label(&p, "ccb slot 1:1.0:0.5 3");
  ...
  label = parse_label(&p, "ccb slot 1:-2.0:0.5,2:0.25,3:0.25 3,4");
  ...
  BOOST_REQUIRE_THROW(parse_label(&p, "ccb slot 1:1.0:0.5,4:0.7"), VW::vw_exception);
}
...
BOOST_AUTO_TEST_CASE(ccb_cache_label)
{
  ...
  auto lp = CCB::ccb_label_parser;
  auto label = parse_label(&p, "ccb slot 1:-2.0:0.5,2:0.25,3:0.25 3,4");
