...
public:
  ...
  float get_reaction(const std::map<std::string, std::string>& context, const std::string& action, bool add_noise = false, bool swap_reward = false);
  std::vector<std::string> to_vw_example_format(
      const std::map<std::string, std::string>& context, const std::string& chosen_action, float cost = 0.f, float prob = 0.f);
  ...
  std::vector<float> run_simulation_hook(VW::workspace* vw, size_t num_iterations, callback_map& callbacks, bool do_learn = true, size_t shift = 1, bool add_noise = false,
      uint64_t num_useless_features = 0, const std::vector<uint64_t>& swap_after = std::vector<uint64_t>());
...
std::vector<float> _test_helper_hook(
    const std::string& vw_arg, callback_map& hooks, size_t num_iterations = 3000, int seed = 10, const std::vector<uint64_t>& swap_after = std::vector<uint64_t>());
}  // namespace simulator
