...
public:
  ...
  cb_sim(uint64_t = 0);
  float get_reaction(const std::map<std::string, std::string>&, const std::string&, bool = false, bool = false);
  ...
  std::vector<float> run_simulation_hook(VW::workspace*, size_t, callback_map&, bool = true, size_t = 1, bool = false,
      uint64_t = 0, const std::vector<uint64_t>& = std::vector<uint64_t>());
