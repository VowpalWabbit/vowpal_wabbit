...
public:
    Simulator(std::string debug_logfile = "", int seed = 0) : debug_logfile(debug_logfile), seed(seed)
    {
        srand(seed);
    }
    ...
    float get_cost(std::map<std::string, std::string> context, std::string action)
    {
        if (context["user"] == "Tom")
        {
            if (context["time_of_day"] == "morning" && action == "politics")
            {
        ...
        {
            if (context["time_of_day"] == "morning" && action == "sports")
            {
                return USER_LIKED_ARTICLE;
            }
    ...
    std::vector<std::string> to_vw_example_format(std::map<std::string, std::string> context, std::string chosen_action = "", float cost = 0.f, float prob = 0.f)
    {
    ...
    std::pair<int, float> sample_custom_pmf(std::vector<float> pmf)
    {
        ...
        for (int index = 0; index < pmf.size(); ++index)
        {
            ...
            if (sum_prob > draw)
            {
                return std::pair<int, float>(index, pmf[index]);
            }
        }
    ...
    std::pair<std::string, float> get_action(vw* vw, std::map<std::string, std::string> context)
    {
        ...
        multi_ex examples;
        for (const std::string& ex : multi_ex_str)
        {
            examples.push_back(VW::read_example(*vw, ex));
        }
        vw -> predict(examples);
