...
BOOST_AUTO_TEST_CASE(one_diff_impl_unittest)
{
  ...
  test_hooks.emplace(1, [&](cb_sim& sim, VW::workspace& all, VW::multi_ex&) {
    ...
    std::vector<std::pair<aml_estimator<VW::confidence_sequence>,
        VW::confidence_sequence>>
        estimators;
    ...
    {
      interaction_config_manager<config_oracle<one_diff_impl>,
          VW::confidence_sequence>::apply_config_at_slot(estimators, oracle.configs, i,
    ...
    // excl_2 is now champ
    interaction_config_manager<config_oracle<one_diff_impl>,
        VW::confidence_sequence>::apply_new_champ(oracle, 2, estimators, 0, false, ns_counter);
    ...
    // excl_4 is now champ
    interaction_config_manager<config_oracle<one_diff_impl>,
        VW::confidence_sequence>::apply_new_champ(oracle, 3, estimators, 0, false, ns_counter);
