...
void parse_slates_label(VW::string_view label, VW::SlatesLabel& l)
{
  std::vector<VW::string_view> words;
  VW::tokenize(' ', label, words);
  VW::slates_default_label(l);
  VW::reduction_features red_fts;
  VW::label_parser_reuse_mem mem;
  auto null_logger = VW::io::create_null_logger();
  VW::slates_parse_label(l, mem, words, null_logger);
}

BOOST_AUTO_TEST_CASE(slates_parse_label)
{
  {
    VW::SlatesLabel label;
    parse_slates_label("slates shared", label);
    BOOST_CHECK_EQUAL(label.type, VW::SlatesExampleType::SHARED);
...

  {
    ...
    BOOST_CHECK_EQUAL(label.type, VW::SlatesExampleType::ACTION);
...

  {
    ...
    BOOST_CHECK_EQUAL(label.type, VW::SlatesExampleType::SLOT);
    BOOST_CHECK_EQUAL(label.labeled, false);
  }
...
BOOST_AUTO_TEST_CASE(slates_cache_shared_label)
{
  ...
  VW::SlatesLabel uncached_label;
  VW::slates_default_label(uncached_label);
  VW::model_utils::read_model_field(io_reader, uncached_label);

  BOOST_CHECK_EQUAL(uncached_label.type, VW::SlatesExampleType::SHARED);
...
BOOST_AUTO_TEST_CASE(slates_cache_action_label)
{
  ...
  BOOST_CHECK_EQUAL(uncached_label.type, VW::SlatesExampleType::ACTION);
...
BOOST_AUTO_TEST_CASE(slates_cache_slot_label)
{
  ...
  BOOST_CHECK_EQUAL(uncached_label.type, VW::SlatesExampleType::SLOT);
  BOOST_CHECK_EQUAL(uncached_label.labeled, true);
  check_collections_with_float_tolerance(
...
BOOST_AUTO_TEST_CASE(slates_copy_label)
{
  ...
  VW::SlatesLabel copied_to;
  VW::slates_default_label(copied_to);
  copied_to = label;
  BOOST_CHECK_EQUAL(copied_to.type, VW::SlatesExampleType::SLOT);
