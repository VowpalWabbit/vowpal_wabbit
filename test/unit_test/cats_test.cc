...
BOOST_AUTO_TEST_CASE(cats_test_get_loss_not_zero_for_bad_prediction_and_large_b)
{
  ...
  // bandwidth_range = min(max_value, action + bandwidth) - max(min_value, action - bandwidth)
  // bandiwdth_range = min(32, 40) - max(0, -20) <=> bandwidth_range = 32 - 0 <=> bandwidth_range = 32
...
BOOST_AUTO_TEST_CASE(cats_test_get_loss_for_good_prediction_and_small_b_not_close_to_range_edges)
{
  ...
  // bandwidth_range = min(max_value, action + bandwidth) - max(min_value, action - bandwidth)
  // bandiwdth_range = min(34, 21) - max(1, 13) <=> bandwidth_range = 21 - 13 <=> bandwidth_range = 8
...
BOOST_AUTO_TEST_CASE(cats_test_get_loss_for_good_prediction_and_small_b_close_to_range_edges)
{
  ...
  // label/action is 33.0f and bandiwdth is 2.0f
  // so if: action - bandwidth <= centre <= action + bandwidth then we calculate the loss (29 <= 29.875 <= 37)
  // bandwidth_range = min(max_value, action + bandwidth) - max(min_value, action - bandwidth)
  // bandiwdth_range = min(34, 37) - max(1, 29) <=> bandwidth_range = 34 - 29 <=> bandwidth_range = 5
  // loss = cost / (pdf_value * bandwidth_range) <=> loss = 1.0f / (0.125f * 5) <=> loss = 1.6
  auto loss = data->get_loss(cont_label, predicted_action);

  BOOST_CHECK_CLOSE(loss, 1.6f, FLOAT_TOL);
