...
static void string_view_by_ref(benchmark::State& state)
{
  string_view<by_ref>(state);
}

static void string_view_by_val(benchmark::State& state)
{
  string_view<by_val>(state);
}
