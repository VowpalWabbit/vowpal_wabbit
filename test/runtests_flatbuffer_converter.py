...
class FlatbufferTest:
    def __init__(self, test, working_dir, depends_on=None):
        self.test = test
        ...
        self.depends_on_cmd = depends_on
    ...
    def replace_vw_command(self):
        ...
        self.test["vw_command"] = self.remove_arguments(
            self.test["vw_command"], json_args, flags=True
        )

        # replace data files with flatbuffer ones in vw_command
        for i, from_file, to_file in self.files_to_be_converted:
            self.test["vw_command"] = self.test["vw_command"].replace(
                from_file, to_file
            )
