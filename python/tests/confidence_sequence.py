...
class ConfidenceSequence(object):
    ...
    def getci(self, alpha):
        ...
        sumXlow = (float(self.sumwr) - float(self.sumw) * self.rmin) / (self.rmax - self.rmin)
        l = self.lblogwealth(t=self.t, sumXt=sumXlow, v=sumvlow, eta=self.eta, s=self.s, alpha=alpha/2)
        ...
        sumXhigh = (float(self.sumw) * self.rmax - float(self.sumwr)) / (self.rmax - self.rmin)
        u = 1 - self.lblogwealth(t=self.t, sumXt=sumXhigh, v=sumvhigh, eta=self.eta, s=self.s, alpha=alpha/2)
    ...
    def lblogwealth(self, *, t, sumXt, v, eta, s, alpha):
        ...
        return max(0, (sumXt - sqrt(gamma1**2 * ll * v + gamma2**2 * ll**2) - gamma2 * ll)/t)
