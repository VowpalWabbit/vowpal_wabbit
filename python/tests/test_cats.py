...
def test_cats():
    ...
    vw_example = vw.parse(
        "ca 15:0.657567:6.20426e-05 | f1 f2 f3 f4", pyvw.LabelType.CONTINUOUS
    )
    vw.learn(vw_example)
    vw.finish_example(vw_example)

    assert (
        vw.get_prediction_type() == pyvw.PredictionType.ACTION_PDF_VALUE
...
def test_cats_pdf():
    ...
    assert (
        vw.get_prediction_type() == pyvw.PredictionType.PDF
