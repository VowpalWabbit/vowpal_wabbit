...
start = time.time()
os.system(
    "vw {} -c --plt {} --loss_function logistic --kary_tree {} -l {} --passes {} -b {} -f {} --holdout_off".format(
...
start = time.time()
os.system("vw {} -i {} --loss_function logistic --threshold 0.5 -t".format(test_data, output_model))
thr_test_time = time.time() - start

print("\nTesting with top-5 prediction\n")
start = time.time()
os.system("vw {} -i {} --loss_function logistic --top_k 5 -t".format(test_data, output_model))
