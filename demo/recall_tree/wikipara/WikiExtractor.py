...
class Extractor(object):
    ...
    def __init__(self, id, title, page):
        ...
        self.recursion_exceeded_1_errs = (
            0  # template recursion within expandTemplates()
        )
...
def load_templates(file, output_file=None):
    ...
    for line in file:
        ...
        if tag == "page":
            ...
            if output_file and (
                title.startswith(templatePrefix) or title.startswith(modulePrefix)
            ):
                output.write("<page>\n")
...
def process_dump(
    ...
    if expand_templates:
        ...
        logging.info(
            "Loaded %d templates in %.1fs", len(templates), template_load_elapsed
        )
    ...
    while True:
        if next_ordinal in ordering_buffer:
            ...
            if next_ordinal % period == 0:
                ...
                logging.info(
                    "Extracted %d articles (%.1f art/s)", next_ordinal, interval_rate
                )
                interval_start = default_timer()
        else:
