...
let vw_js_loc = '../vw.js';
const { initializeVWModule } = require(vw_js_loc);
...
describe('Call WASM VWModule', () => {
    it('invalid positional argument', () => {
        const vw = require(vw_js_loc);
        assert.throws(
            () => new vw.VW({ args_str: "test" }));
    });
    it('Pass argument which is invalid', () => {
        const vw = require(vw_js_loc);
        assert.throws(
            () => new vw.VW({ args_str: "--data data.txt" }));
    });
    ...
    it('Make scalars prediction', () => {
        const vw = require(vw_js_loc);
        let model = new vw.VW({ args_str: "--oaa 2 --probabilities" });
    ...
    it('Make action_scores prediction with generic VW model', () => {
        const vw = require(vw_js_loc);
        let model = new vw.VW({ args_str: "--cb_explore_adf" });
    ...
    it('Make action_scores prediction with CBVW', () => {
        const vw = require(vw_js_loc);
        let model = new vw.VWCB({ args_str: "--cb_explore_adf" });
        assert.equal(model.predictionType(), vw.Prediction.Type.ActionProbs);
    ...
    it('Make decision_scores prediction', () => {
        const vw = require(vw_js_loc);
        let model = new vw.VW({ args_str: "--ccb_explore_adf" });
    ...
    it('Load model', () => {
        const vw = require(vw_js_loc);
        let model = new vw.VW({ args_str: "", model_file: path.join(__dirname, "model.vw") });
        assert.equal(model.predictionType(), vw.Prediction.Type.Scalar);
    ...
    it('Compare save and load model with continuous learning', () => {
        ...
        let model1 = new vw.VWCB({ args_str: "--cb_explore_adf" });
        ...
        let model2 = new vw.VWCB({ args_str: "--cb_explore_adf" });
