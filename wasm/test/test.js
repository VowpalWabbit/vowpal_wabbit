...
const { VW, VWCB } = require('../vw.js');

// Delay test execution until the WASM VWModule is ready
VWWasmModule['onRuntimeInitialized'] = () => { mocha.run(); }
...
describe('Call WASM VWModule', () => {
    ...
    it('Make scalar prediction', () => {
        let model = new VW({ args_str: "" });
        assert.equal(model.predictionType(), VWWasmModule.PredictionType.scalar);
    ...
    it('Make scalars prediction', () => {
        ...
        assert.equal(model.predictionType(), VWWasmModule.PredictionType.scalars);
        let example = model.parse("|a ab");
        let prediction = model.predict(example);
    ...
    it('Make action_scores prediction with generic VW model', () => {
        ...
        assert.equal(model.predictionType(), VWWasmModule.PredictionType.action_probs);
    ...
    it('Make action_scores prediction with CBVW', () => {
        ...
        try {
            ...
            console.error('Error caught from C++ during learn:', VWWasmModule.getExceptionMessage(e));
        }
    ...
    it('Make decision_scores prediction', () => {
        ...
        assert.equal(model.predictionType(), VWWasmModule.PredictionType.decision_probs);
        let example = model.parse(`
            ccb shared |User b
            ccb action |Action d
            ccb action |Action e
    ...
    it('Compare save and load model with continuous learning', () => {
        ...
        assert.equal(model1.predictionType(), VWWasmModule.PredictionType.action_probs);
        ...
        assert.equal(model2.predictionType(), VWWasmModule.PredictionType.action_probs);
        ...
        try {
            ...
            console.error('Error caught while loading saved file:', VWWasmModule.getExceptionMessage(e));
        }
